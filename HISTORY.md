# BizNavi 작업 이력

프로젝트 개요·현재 구조·작업 규칙·남은 이슈는 [CLAUDE.md](CLAUDE.md)에 있다.
이 파일은 **날짜별 작업 이력 보관소**이며 최신이 위에 온다.
특정 결정의 배경(왜 그렇게 설계했는지 / 어떤 사고가 있었는지)을 확인할 때만 열어보라.

⚠ 아래 내용은 **작성 시점의 기록**이다. 현재 구조와 다를 수 있으므로
   현재 상태는 반드시 CLAUDE.md와 코드에서 확인할 것.

---

## 작업 이력 (2026-09-22) — sme PPT 3단계: 분량 확대·넘침 처리·배치

### 왜
사용자 요구: "글자는 오히려 많을수록 좋다. PDF 수준의 내용이 최대한 들어가면 좋다."
2단계까지 sme PPT는 `LIMIT`(줄 110 / 문단 260 / 제목 42)로 자르고 `…`을 붙였고, 칸별 3개·KPI 5장·
90일 3개월·핵심 전략 4개·취약 시스템 3개로 잘랐다. 유형별 특화 분석·린 캔버스는 아예 빼고
"리포트에서 확인할 내용" 장으로 안내만 했다. 화면(8섹션)보다 내용이 확연히 적었다.

### 무엇을
- **sme는 `_clip`을 쓰지 않는다.** 칸을 넘는 내용은 다음 장에 이어 싣고 제목에 `(계속)`,
  앞 장 하단에 `다음 장에 계속 →`을 붙인다. 표지 회사명·핵심 결론도 자르지 않고 글자 크기로 맞춘다.
- **화면 필드를 전부 싣는다** — 새로 들어간 것: 유형별 특화 분석(요약+블록 전부), 린 캔버스 9블록,
  SWOT 근거(`evidence`), 핵심 전략 `description`·`priority`·`owner`·`timeline`, 6시스템 전부의
  `issue`·`actions` 전부·`resource`, KPI `progress`(막대)·`owner`·`method`, 로드맵 `period`·`budget`·
  `framework`·과제 전부, 90일 `theme`·`goal`·`expectedResult`·`govSupport`, 정부지원 전부(`summary`·`url` 링크).
- **배치 엔진 2개** — `_smeCards`(카드 흐름: 행 단위로 쌓고, 긴 행은 남은 높이에서 쪼개 다음 장 같은 칸에
  잇는다) / `_smeGrid`(병렬 칸: SWOT·STP·4P·90일처럼 칸 위치가 의미인 것 — 모든 장에서 칸 배치를 유지).
- **큰 숫자 띠** — 한눈에 보기(종합·최고·최저 역량, `fd.domainScores` 그대로), 6시스템(상태별 개수,
  AI 상태값 그대로). KPI는 현재·목표를 큰 글씨 두 줄로.
- **슬라이드 제목 = 결론 한 문장** — 출처: 한눈에 보기 = `executiveSummary` 첫 문장 / 특화 분석 =
  `specializedAnalysis.summary` 첫 문장 / STP = `positioning` 첫 문장 / 린 캔버스 = `uniqueValueProposition`
  첫 문장 / 핵심 전략 = `'최우선 전략: '` + priority `high` 첫 전략의 `title` / 6시스템 = 첫 취약 시스템
  `name` + `issue` 첫 문장. **SWOT·4P·KPI·로드맵·90일·정부지원은 결론 필드가 없어 섹션 이름 그대로.**
  결론이 15pt 두 줄에 안 들어가면 섹션 이름으로 폴백한다(제목을 자르지 않는다).

### 넘침 판정 — 실측으로 계수를 잡았다
PptxGenJS는 글자 크기를 재지 않으므로 `_wrap`이 줄 수를 추정한다. PowerPoint COM의
`TextRange.BoundHeight`로 실측한 결과:
- **줄 높이는 정확히 1.2 × pt**(20pt→24.0, 10.5pt 2줄→25.2). `LH = 1.2`는 여유가 없다 — 줄이지 마라.
  처음 1.3으로 잡았다가 칸 아래 ~15%가 비어 1.2로 내렸다.
- 여유는 **폭 쪽에만** 있다. 한글 1.0em으로 시작했더니 4줄로 추정한 문단이 실제 3줄이라 0.95로 조였다.
- **Bold는 한글이 더 넓다.** 0.95로 조인 직후 굵은 제목 `… 병목이다. (계속)`이 실제 두 줄로 넘쳐
  +20pt 넘침이 났다 → `_em(ch, bold)`로 굵은 글씨는 1.0em.
- 검증 덱 3종(대표 분량 19장 / 2~3배 긴 분량 37장 / 영문·숫자 혼합 22장) 전부 넘침 0건.

### 채택하지 않은 대안
- **글자 크기 자동 축소(`fit: 'shrink'`)** — PowerPoint가 파일을 열 때만 적용되고 생성 시점엔 안 줄어든다.
  칸마다 글자 크기가 들쭉날쭉해지고, 사용자가 원한 것은 "더 많이"지 "더 작게"가 아니다.
- **줄마다 `\n`을 박아 줄바꿈을 고정** — 받은 사람이 문구를 고치면 줄이 엉킨다. 추정은 쪼갤 위치를
  정하는 데만 쓰고 줄바꿈은 PowerPoint에 맡긴다(추정이 보수적이므로 실제 줄 수 ≤ 추정).
- **LIMIT 값만 올리기** — 올린 만큼 넘침이 생길 뿐 잘림은 남는다.
- **짧은 문단도 줄 단위로 쪼개기** — SWOT 항목의 마지막 낱말("보유")만 다음 장으로 넘어가는 것을 실제로 봤다.
  → 4줄 이상 문단만 쪼개고 양쪽에 2줄 이상 남긴다. 소제목·항목명은 `keep`으로 다음 문단과 붙인다.
- **작은 카드도 남은 높이에 맞춰 쪼개기** — 6시스템에서 진단 문장만 이 장, 실행 액션은 다음 장으로 갈라졌다.
  → 새 장 절반 이하 높이의 행은 통째로 넘긴다. 6시스템은 3열로 바꿔 장 수를 줄였다.
- **KPI 현재→목표 한 줄** — `4,000만 / 원`처럼 낱말 중간에서 꺾였다 → 현재·목표 두 줄.
- **SWOT 칸별 3개 상한 유지** — 사용자 요구와 정면으로 어긋난다. 칸 배치는 유지하고 칸마다 다음 장에 잇는다.

### 유지한 것
- LEGACY(micro·사회적경제·정책자금)는 `LIMIT`·`_clip` 그대로 — HEAD 대비 슬라이드 XML 동일 확인.
- 2단계 5대 역량 차트(`_smeCompetencySlide`)는 손대지 않았다 — 차트 XML 동일 확인.
- 현재·목표가 둘 다 없는 KPI는 카드를 만들지 않는다(2단계 규칙). 진행률은 화면처럼 막대로만 그린다.
- 정부지원 목록 조회를 `_govList()`로 뺐고 `_govSlide`(micro)는 이를 호출만 한다 — 출력 불변.

### 남은 것
- sme 화면 `#sec-diag`의 **취약 영역 배너**(공통 4영역+업종특화 점수, `AIEngine.calcDiagScores`)는
  PPT에 없다. PPT의 역량 슬라이드는 2단계부터 `fd.domainScores` 5대 역량 기준이라 점수 체계가 다르다.
  넣으려면 두 점수 체계 중 어느 것을 PPT의 기준으로 할지 먼저 정해야 한다.
- 추정 기반이라 실제 AI 출력에서 넘침이 날 가능성은 0이 아니다 — 검증 덱은 합성 데이터다.

---

## 작업 이력 (2026-09-22) — sme PPT 2단계: 차트와 시각화

**범위**: `js/ppt-export.js`의 sme 빌더(`_buildSme`)만. micro·사회적경제·정책자금(LEGACY)은 불변.

### 무엇을 바꿨나
| 슬라이드 | 이전 | 이후 |
|---|---|---|
| 5대 역량 | 캔버스 PNG + 점수 표 | **네이티브 레이더 + 가로 막대**(`addChart`) 나란히 |
| SWOT | 글자 4묶음 | 2×2 매트릭스 — 칸마다 신호색 머리띠 + 옅은 면 |
| KPI | 점수 표 재사용 | 카드(지표명 / 현재 → 목표 / 기간) 최대 5장 |
| 90일 | 3열 텍스트 | 화살표 도형(`homePlate`·`chevron`) 가로 타임라인 |

### 왜 이렇게 했나
- **네이티브 차트**: 캔버스 이미지는 흐리고 수신자가 숫자·색을 고칠 수 없다. `addChart`는 차트 XML +
  내장 워크북으로 들어가 PowerPoint '데이터 편집'이 된다. sme는 `_radarDataUrl`을 더 이상 쓰지 않는다
  (micro·사회적경제는 여전히 사용 — LEGACY 불변 조건).
- **값은 `fd.domainScores[k].avg` 그대로**: 화면 막대·레이더·표지와 같은 숫자여야 한다. 재계산·재반올림 없음.
- **라벨도 `domainScores[k].label`을 쓴다**: 1단계까지 PPT가 `재무건전성`·`조직·인력`을 하드코딩해
  화면(`경영재무역량`·`인적자원역량`)과 달랐다(CLAUDE.md 남은 이슈). 창업초기 라벨도 자동으로 따라간다.
- **미입력(0) 영역은 차트에서 제외**하고 하단에 명시 — 0을 그리면 '0점'으로 읽힌다.
  축이 3개 미만이면 레이더가 성립하지 않아 막대만, 점수가 전무하면 슬라이드 자체를 생략(표지가 이미 '미입력' 표시).
- **막대 색 = `_levelOf` 신호색**, 범례를 막대 아래에 둔다. PptxGenJS는 단일 계열 + `chartColors` 2개 이상일 때
  점마다 `<c:dPt>`를 쓴다(3.12.0 소스 확인).
- **KPI 필드는 화면(`#kpiGrid`)과 같은 `metric`·`current`·`target`·`timeline`**. 이전 PPT는 점수 열에 `target`,
  수준 열에 `current || owner`를 넣고 있었다. 현재·목표가 둘 다 없는 KPI는 카드를 만들지 않는다(이름만 있는 빈 카드).
- **SWOT 한 칸만 비면 칸을 유지**하고 '항목 없음'을 적는다 — 칸을 빼면 2×2가 무너진다. 네 칸 전부 비면 슬라이드 생략.
  이전 코드는 `d.swot`이 있기만 하면 `—`만 4개인 슬라이드를 만들었다.
- **90일 타임라인은 6가지 시스템과 같은 장에 둔다** — 장을 나누면 분량 확대이고 이번 범위가 아니다(3단계).
  달 수가 3보다 적어도 3등분 폭을 유지한다(시간 축 눈금 불변).
- **팔레트 키 4개 추가**(`okBg`·`criticalBg`·`highBg`·`titleBg`) — THEME에 넣고 LEGACY에도 같은 키를 둔다
  (키 이름 일치 규칙). LEGACY 빌더는 참조하지 않으므로 출력 불변.

### 레이더 축 라벨 줄바꿈 — 시도한 것
PowerPoint가 레이더 라벨 폭을 좁게 잡아 `차별화·경쟁우 / 위역량`처럼 낱말 중간에서 자른다.
1. `layout`으로 플롯 영역 축소 → **맨 위 라벨이 차트 밖으로 잘림**. 기각.
2. 라벨 9pt → 여전히 한 글자가 넘어감. 기각.
3. **라벨에 직접 `\n`** — 가운뎃점 뒤, 없으면 7자 이상일 때 `역량` 앞. 채택. 막대 차트는 원래 이름 그대로.
   (내장 워크북의 레이더 범주 셀에도 줄바꿈이 들어간다 — 감수)

### 검증 (PptxGenJS 3.12.0, Node 하네스 + PowerPoint COM 렌더)
- 레이더·막대가 `ppt/charts/chart*.xml`로 존재, `ppt/media/` 파일 0개, 내장 워크북 2개
- 차트 값·라벨 = `domainScores` avg·label, 두 축 0~5, 막대 `dPt` 5개
- 빈 데이터: 점수 전무 → 차트 0 / KPI·SWOT·90일 빈 필드 → 슬라이드 미생성 / 2영역 → 막대만 / 0점 영역 제외·명시
- LEGACY micro·social·funding: **HEAD 버전과 슬라이드·차트 XML 완전 동일**
- PowerPoint로 4장(5대 역량·SWOT·KPI·90일) PNG 렌더 육안 확인

### 남긴 것
- 한 칸 3개·KPI 5장·90일 3개월 상한과 넘침 처리는 **3단계**.
- `_radarSlide`·`_radarDataUrl`은 micro·사회적경제가 아직 쓴다 — LEGACY를 THEME으로 옮길 때 같이 정리.

---

## 작업 이력 (2026-09-21) — sme PPT 디자인 1단계: 색 체계·표지·본문 띠

### 문제
사용자 확인 결과 "워드 문서를 가로로 만든 것 같다. PPT 느낌이 없다. 색이 밋밋하고 표지가 허전하다."
실제로 표지는 흰 배경에 텍스트 4줄뿐이었고, 본문 슬라이드는 제목 아래 1px 회색 선이 전부였다.

### 왜 이렇게 했나

**① 팔레트를 하나로 합치지 않고 `LEGACY` / `THEME` 둘로 나눴다.**
이번 범위는 sme뿐이고 micro·사회적경제·정책자금은 출력이 바뀌면 안 된다.
기존 `C`를 그 자리에서 고치면 네 유형이 전부 바뀐다. 그래서 기존 값을 `LEGACY`로
이름만 바꿔 그대로 두고, 새 색 체계를 `THEME`으로 신설했다.
**두 팔레트는 키 이름이 같다** — 그래야 슬라이드 헬퍼가 분기 없이 양쪽을 다 그린다.
2·3단계에서 나머지 유형을 옮기면 `LEGACY`는 삭제한다.

**② 헬퍼가 팔레트를 직접 참조하지 않고 활성 팔레트 `TH`만 본다.**
`C.title` 90곳을 `TH.title`로 치환하고 `TH`를 `download()`가 유형에 따라 갈아끼운다.
헬퍼 하나라도 `LEGACY`를 직접 붙잡으면 **그 헬퍼가 그린 슬라이드만 옛 색으로 남는다** —
한 문서 안에서 색이 섞이는데 예외가 나지 않아 발견이 늦다.
같은 이유로 `LEVEL_COLOR` **상수를 `_levelColor()` 함수로 바꿨다.** 상수는 모듈 로드
시점의 팔레트를 박아버려 팔레트를 갈아끼워도 경고 배지만 옛 색으로 남는다.

**③ 팔레트 선택 조건을 `kind === 'sme'`로 쓰지 않았다.**
`_buildSme`는 분기의 `else` 폴백이라 예상 못한 kind도 받는다. 조건을 좁게 쓰면
**팔레트는 LEGACY인데 빌더는 sme**가 되어 흰 표지에 남색 띠 본문이 붙는다.
빌더 분기와 같은 조건(`funding`·`micro`·`social`·`venture`·`coop` → LEGACY, 나머지 → THEME)을 썼다.
또 `_useTheme`은 **빌더 밖, 분기 직전 한 곳**에서만 호출한다 — `TH`가 모듈 변수라
빌더 안에서 갈아끼우면 다음 다운로드에 이전 유형의 팔레트가 남는다.

**④ 본문 상단 띠는 빌더가 아니라 팔레트의 `band` 플래그로 갈린다.**
빌더에서 유형을 보고 띠를 그리면 슬라이드를 추가할 때 띠를 빠뜨린 장이 섞인다.
`_newSlide` 한 곳에서만 처리하므로 sme 본문 10장 전부에 자동으로 적용된다(검증함).

**⑤ 표지 종합 점수는 5점 만점이다 — 100점이 아니다.**
지시는 "62 / 100" 예시를 들었으나, **sme 화면에는 100점 만점 종합 점수가 존재하지 않는다.**
확인 결과: `fd.scaleScores`는 sme에서 항상 `{}`다(`DiagSme`가 렌더링되지 않아
`wizard.js`의 `diag-sme-container_` 접두어 가드에 걸린다). sme 화면이 쓰는 유일한
점수 소스는 `fd.domainScores`의 5대 역량 `avg`이고, 진단 결과 화면 막대(`#drScoreList`)·
레이더차트·대시보드 점수 pill이 **전부 5점 만점으로** 표시한다.
100점으로 환산하면 **화면 어디에도 없는 숫자가 표지에 대문짝만하게 실린다** —
"화면과 PPT가 다른 숫자를 보이면 안 된다"는 제약에 정면으로 걸린다. 그래서
같은 값의 평균을 `3.2 / 5.0`으로 표시했다. 100점 표기가 필요하면 **화면에 먼저
100점 종합 점수를 만들고 PPT가 그것을 읽는 순서**여야 한다.
⚠ 0점(미응답) 영역은 평균에서 뺐다 — `classifyConsultingType`의 규칙과 같게 맞춘 것이다.
다르게 잡으면 표지 점수와 컨설팅 유형이 서로 다른 모집단을 근거로 하게 된다.
⚠ 전체 미입력일 때 `0.0`을 크게 띄우지 않는다("진단 결과가 0점"으로 읽힌다) — '진단 점수 미입력'으로 바꾼다.

**⑥ 핵심 결론 한 줄에서 `[레이블]` 머리표를 지운다.**
`executiveSummary`는 `[진단 요약] …` 형식이라 그대로 쓰면 표지에 대괄호가 남는다.
문장 경계는 마침표 뒤에 공백·문자열 끝을 **전방탐색으로 요구**했다 — 소수점(3.4)에
걸리지 않게 하려는 것이며, 한글 뒤에서는 `\b`가 절대 매치되지 않는다(주의사항 ①).

### 검토했으나 채택하지 않은 것
- **`C`를 직접 새 색으로 덮기** — 네 유형이 전부 바뀐다. 이번 범위는 sme뿐이다.
- **sme 전용 슬라이드 헬퍼(`_smeSlide`) 신설** — `_newSlide`를 쓰는 곳이 빌더·`_radarSlide`·
  `_govSlide`·`_plan90Slide`로 흩어져 있어 호출부를 전부 고쳐야 하고, 하나라도 빠뜨리면
  띠 없는 슬라이드가 섞인다. 팔레트 플래그 방식이 호출부를 하나도 건드리지 않는다.
- **표지 점수를 100점으로 환산** — 위 ⑤. 화면에 없는 숫자다.
- **`_smeRows`의 역량 라벨 불일치 수정** — `_smeRows`는 `재무건전성`·`조직·인력` 등을
  하드코딩하는데 화면(`calcDomainScores`)은 `경영재무역량`·`인적자원역량`이다.
  **실재하는 불일치지만 슬라이드 내용 변경이라 1단계 범위 밖**이다. 2단계에서 처리한다.

### 검증
`out_head`(HEAD) / `out_new`(수정본)로 4개 유형 .pptx를 실제 생성해 슬라이드 XML을 해시 비교했다.
- micro 7장 · 사회적기업 8장 · 정책자금 6장 — **차이 0장** (바이트 크기도 동일)
- sme 11장 — 11장 전부 변경 (의도대로)
- 4개 파일 모두 zip 시그니처 + `[Content_Types].xml` + `ppt/presentation.xml` 존재
- sme 표지: 회사명·유형·진단일·`3.2`·`/ 5.0`·결론 한 줄 전부 출력, `[레이블]` 잔존 0건
- 표지 점수 `3.2` = 화면 막대값 2.4·3.6·3.0·4.2·2.8의 평균 (같은 출처)
- 경계: 0점 영역 제외(4.0·2.0·0 → 3.0) / 전체 미입력·`domainScores` 없음 → '진단 점수 미입력'
- sme 본문 10장 전부에 남색(`16233F`)+금색(`E0BC5E`) 띠 존재 — 누락 0장
- 제어문자(0x00~0x1F, 개행·탭 제외) 0건 — `perl -pi` 치환 경유이므로 확인함(주의사항 ⑥)

### 남은 작업 (2·3단계)
슬라이드 내용·장수, `_smeRows` 라벨 불일치, 나머지 유형의 THEME 전환.

---

## 작업 이력 (2026-09-21) — 잔존 diagScores가 AI·교차경고로 새는 문제 수정

### 문제
`step1`에서 직원 수를 고치고 `analyzeBiz()`를 다시 돌리는 흐름은 `reset()`을 거치지 않는다.
그래서 경로가 바뀌어도(사회적기업 → 일반 기업 등) `diagScores`에 **이전 경로 점수가 그대로 남는다.**
`collectAllScores()`는 접두어 구분 없이 전 키를 반환했고, `CrossContext.buildScoreMap`은
접두어 무관으로 전 키를 훑으므로 **응답한 적 없는 문항이 AI 프롬프트와 교차 경고에 섞일 수 있었다.**
예외가 나지 않아 발견이 늦는 부류다.

### 조치 — `js/wizard.js`
`diagScores`를 **지우지 않는다.** 경로 전환이 오조작일 수 있어 지우면 응답이 사라진다.
대신 **읽는 쪽에서 좁혔다.**

- `_isActiveScoreKey(k)` 신설 — `_activeContainers` 접두어 기준 판정 한 곳.
- `collectAllScores()`가 이 판정을 통과한 키만 담는다.
- `collect()`의 `allMemos` 수집도 같은 판정을 쓴다 — 점수만 막고 메모를 두면
  응답한 적 없는 문항의 메모가 프롬프트에 남는다.

### 왜 `keyPrefix`가 아니라 `_activeContainers`인가
한 경로는 **주 진단 + 업종 특화 두 컨테이너**를 쓴다(`diag-industry-container_` 16문항).
`_orgMod.KEY_PREFIX` 하나로 필터하면 **업종 특화 점수 16개가 통째로 AI에 안 간다.**
진행률 분자(`_countDoneScores`)가 이미 `_activeContainers`를 기준으로 쓰고 있으므로
같은 기준을 공유시켰다 — 기준이 갈리면 "100% 응답인데 AI에는 일부만 가는" 어긋남이 생긴다.

### 채택하지 않은 대안
| 대안 | 기각 이유 |
|---|---|
| 경로 변경 시 `diagScores` 초기화 | 응답 손실. 오조작으로 경로를 바꿨다 되돌리면 전부 날아간다 |
| `analyzeBiz()` 재실행 시 `reset()` 호출 | step1 입력(회사명·매출 등)까지 날아간다 |
| `_orgMod.KEY_PREFIX` 단일 필터 | 업종 특화 16문항이 탈락한다(위 항목) |
| `CrossContext` 쪽에서 접두어 검사 | 소비처가 늘 때마다 같은 검사를 복제하게 된다. 공급원 한 곳에서 막는다 |

### 안전장치
`_activeContainers`가 비어 있으면(`loadDiagnosisUI` 미실행) **좁히지 않고 전부 통과**시킨다.
여기서 `{}`를 반환하면 점수가 통째로 사라지는데, 그것이 잔존 키가 섞이는 것보다 나쁘다.

### 검증 (앵커 방식 하네스 — 줄 번호 하드코딩 없음, 9/9 PASS)
중괄호 균형으로 `_isActiveScoreKey`·`collectAllScores` 블록을 잘라 **실제 코드를 그대로 실행**했다.
① 사회적기업 잔존 키(`diag-social-container_`) 제외 확인 ② micro·sme·사회적경제 정상 경로 결과 불변
③ 업종 특화 16문항 포함 + 주 진단 키 동시 포함 ④ 활성 목록 공백 시 전부 통과
⑤ 평면 숫자 맵 계약 유지(`typeof === 'number'`, 메모 미혼입)

### 남은 것
`dashboard.js`도 `diagScores`를 자체적으로 평면 맵으로 만든다(같은 파일 내 별도 함수).
이번에는 지시 범위 밖이라 건드리지 않았다 — **같은 잔존 키 노출 경로가 남아 있다.**

---

## 작업 이력 (2026-09-18) — sme AI 출력 스펙 정리 (B단계: 중복 생성 제거)

A단계(`dcd897b`)로 화면은 8섹션이 됐는데 AI는 여전히 13섹션분을 생성하고 있었다.
**필드를 없애는 작업이 아니다** — 화면이 11개 필드를 전부 쓴다. 중복 생성만 걷어냈다.

### ① 가장 큰 문제 — 1차 SYSTEM이 자기 자신과 모순돼 있었다

유저 프롬프트(`buildPrompt1` sme 분기)는 이렇게 말한다:
> `kpi, roadmap, sixSystems, plan90days, leanCanvas는 포함하지 마세요.`

그런데 같은 요청에 실리는 `SYSTEM` 상수에는 **그 셋의 JSON 템플릿이 그대로 남아 있었다** —
`kpi` 10개(883토큰) · `roadmap` 3단계×6태스크(676토큰) · `leanCanvas` 9블록(405토큰).

**micro 92초 사고(2026-09-08)와 같은 원인이 sme에 그대로 있었다.**
한 요청에 상반된 두 명세가 들어가면 모델은 큰 쪽을 따라간다 — 작은 쪽을 적어 둬도 소용없다.

그리고 만들어도 **전부 버려진다.** 근거 두 가지:
- `FIRST_PASS_KEYS`(sme)에 이 셋이 없어 `Object.assign({}, result1, r2Clean)`에서
  **2차 값이 1차 값을 덮는다**
- `buildPrompt2`는 1차 결과 중 `keyStrategies`(제목+설명 160자)와
  `executiveSummary`(300자)만 읽는다. 나머지는 입력으로도 쓰지 않는다

즉 **만들고 버리는 데 시간을 쓰고 있었다.**

⚠ **2026-09-08 주석 "SYSTEM은 sme가 공유하므로 절대 건드리지 마라"의 해석.**
그것은 micro 작업 중 sme 회귀를 막으려던 것이지 **sme 쪽 명세가 옳다는 뜻이 아니었다.**
`SYSTEM`의 소비처는 grep 전수 확인 결과 **sme 1차 한 곳뿐**이다
(`_isMicro ? _SYSTEM_MICRO_1 : SYSTEM`).

→ 템플릿 3,142자(≈1,964토큰)를 앵커 방식으로 제거했다.
   `[사업 규모별 모드 분기]`의 `sme: … KPI·roadmap 풍부하게 작성` 문장도 함께 정정했다 —
   이것도 1차에 없는 필드를 만들라고 말하는 모순이었다.

⚠ **산문 지시는 건드리지 않았다.** `[필수 반영 원칙] 2번(TAM/SAM/SOM → KPI)`처럼
2차 필드를 겨냥하는 문장이 몇 줄 남아 있으나, **2차로 전달되는 경로가 없어**
지우면 그 지시가 통째로 사라진다. 충돌의 핵심(큰 JSON 명세)은 제거됐고
산문은 "반영하라"이지 "출력하라"가 아니므로 모순이 남지 않는다.

### ② 우연한 폴백을 명시적으로 바꿨다

지금까지 1차가 `kpi`를 만들어 두는 바람에 **2차가 그 키를 빠뜨리면 1차 값이 우연히 살아남고**
있었다(1차 유저 프롬프트는 만들지 말라고 지시하는데도). 템플릿을 걷어내면 그 폴백이 사라진다.

→ 병합부에 명시적 폴백을 넣었다. ⚠ `undefined`만 보면 부족하다 —
2차가 `"kpi": null`이나 `[]`를 주면 `Object.assign`이 1차 값을 그것으로 덮는다.
**내용이 실제로 있는지**(`_hasContent`)를 기준으로 판정한다.
둘 다 없으면 `undefined`로 남긴다 — 빈 값을 지어내지 않는다(주의사항 ⑪).

### ③ 같은 90일을 세 번 쓰고 있었다 — 역할 분리

`roadmap` 1단계 `period`가 `"1~3개월"`이고 `plan90days`는 `"1~3개월차"`다. **같은 90일이다.**
`sixSystems.actions`도 "이번 주 / 이번 달 / 3개월 내"로 또 같은 90일이다.
2차 한 번에 **실행 항목 45개**(roadmap 18 + plan90days 9 + sixSystems 18)가 나왔다.

→ `_SYSTEM_EXEC`에 `[★ 필드별 역할 분리 ★]` 블록을 넣고 셋의 역할을 갈랐다:

| 필드 | 역할 |
|---|---|
| `plan90days` | 90일 **안**에서 무엇을 언제 — 주 단위 실행 |
| `roadmap` 1단계 | 90일 **이후**로 넘어가는 마일스톤만 (검증 완료 기준·의사결정 분기점) |
| `sixSystems` | 체질 진단(`issue`)과 **그 시스템 고유** 액션 |

`roadmap` 1단계 템플릿도 태스크 6개 → 3개로 다시 썼고 `phase` 이름을
`"1단계: MVP 검증·기반 구축"` → `"1단계: 검증 완료·2단계 진입 조건"`으로 바꿨다.

⚠ **지시문을 `tasks` 배열에 넣었다가 되돌렸다.** 그 배열은 화면에서 태스크 칩으로
그대로 렌더링되므로 "이건 ~하는 자리가 아니다" 같은 메타 지시가 사용자에게 보인다.
역할 설명은 `[★ 필드별 역할 분리 ★]` 산문 블록에만 둔다.

### ④ 재방문율·ROAS 중복 — leanCanvas 쪽을 풀었다

`SYSTEM`의 `kpi` 템플릿은 지표2·3을 **재방문율·ROAS로 못박고**, `leanCanvas.keyMetrics`도
"반드시 재방문율과 ROAS 포함"을 요구했다. **한 응답 안에서 같은 두 지표를 두 번** 쓴다.

→ **`leanCanvas.keyMetrics` 쪽을 풀었다.** 근거:
- `kpi`는 `current`·`target`·`timeline`·`method`·`owner`를 가진 **측정 전용 그릇**이다.
  재방문율·ROAS를 추적하기에 구조가 맞다
- 인과사슬 3축 추적 지시가 `SYSTEM`·`buildCausalChain` 여러 곳에서 **KPI를 겨냥**한다.
  KPI 쪽을 풀면 그 지시들이 갈 곳을 잃는다
- `leanCanvas`는 "사업 모델 한 장 요약"이라 지표 3개에 현재값·목표값·측정주기를
  서술하라는 요구 자체가 그 칸의 성격과 맞지 않았다

→ `keyMetrics`를 **단위경제**(고객 1명당 수익·손익분기 고객 수 등)로 바꾸고
`kpi`와 겹치지 말라고 명시했다. `kpi` = 무엇을 추적할 것인가 / `keyMetrics` = 이 모델이
수익을 내는 구조인가.

### ⑤ kpi 10 → 8

`leanCanvas.keyMetrics`와 지표가 겹치도록 못박혀 있어 손실 없이 줄일 수 있었다.
`지표10 (업종 벤치마크 기준)`은 의미가 있어 `지표8`로 옮겨 살렸다.
⚠ **SWOT 4분면 × 6개는 건드리지 않았다.** 8섹션에서 "강점과 약점"이 독립 섹션이라
분량이 곧 그 섹션의 내용이다.

### 검증

| 항목 | 결과 |
|---|---|
| 1차 `SYSTEM`에 `kpi`·`roadmap`·`leanCanvas` 템플릿 | **0건** |
| 1차 `SYSTEM`에 남겨야 할 6필드 | 전부 유지 |
| micro·사회적경제·정책자금 프롬프트 (`git show HEAD:` 원문 비교) | 상수 5개 + 함수 4개 **전부 불변** |
| `buildPrompt1` 전체 | **바이트 단위 불변** |
| 최종 병합 필드 구성 11개 | 불변 (2차 템플릿 5필드 유지 확인) |
| 폴백 7종 (정상·키없음·null·[]·{}·둘다없음·1차보호) | 전부 통과 |
| **입력 토큰 절감** | 1차 **−1,899** / 2차 **+239** → **합계 −1,660토큰** |
| `node --check` · 제어문자 · 조사 파손 | 통과 / 0 / 0건 |

⚠ **2차가 +239토큰 늘어난 것은 의도한 것이다.** 역할 분리 지시를 넣은 대가이며,
그 대신 **출력**에서 중복 실행 항목이 줄어든다. 입력 1토큰과 출력 1토큰은 지연 기여도가
다르다 — 지연은 거의 전적으로 출력 토큰 수에 비례한다.

⚠ **출력 절감은 실측하지 않았다.** 실제 API 호출이 필요하다.
1차가 만들어 버리던 `kpi` 10 + `roadmap` 18태스크 + `leanCanvas` 9블록이
**추정 1,900~2,500 출력 토큰**이고 이것이 통째로 사라지는 것이 이번 변경의 핵심 이득이다.
⚠ **폴백 검증은 병합부 로직을 하네스에 복제해 돌린 것**이다(`ai-engine.js`는 IIFE + DOM 의존).
로직 자체는 동일하나 실제 코드 실행이 아니다.

### 별건 — Vercel 플랜 기록 정정

CLAUDE.md에 **Pro 플랜**으로 적혀 있었으나 사용자가 대시보드 배지로 확인한 실제 값은
**Hobby(함수 상한 60초)** 다. 정정했다.
⚠ **기록이 틀리면 "300초까지 여유 있다"고 오판한다.** 과거 92초·141초 초과 사고가
전부 이 60초 상한이었다는 사실과도 맞아떨어진다.

### 검사기가 틀렸던 건 — 기록해 둔다

micro 분기 불변을 정규식으로 떼어내 비교했더니 **거짓 실패**가 났다.
앵커 어구가 파일 앞쪽에도 있어 매치가 38,000자를 건너뛰어 시작했다.
`buildPrompt1` 함수 전체를 비교하니 **바이트 단위로 동일**이었다.
→ 주의사항 ①·⑦의 재확인: **검사 결과가 이상하면 코드가 아니라 검사기를 먼저 의심하라.**
   그리고 단정문은 **실제 불변식**(함수 전체)으로 적어야 한다. 좁게 적으면 정당한 변경이
   회귀로 잡히고, 넓게 적으면 회귀를 놓친다.

---

## 작업 이력 (2026-09-18) — sme 리포트 13섹션 → 8섹션 (A단계: 화면 구조만)

### 문제 — 같은 결론을 네 번 다르게 읽힌다

sme 리포트 13섹션이 같은 질문을 프레임워크만 바꿔 반복하고 있었다.

| 사장님이 묻는 것 | 답하던 섹션 |
|---|---|
| 무엇을 할 것인가 | SWOT · 핵심전략 · 린캔버스 |
| 누구에게 무엇을 | STP · 4P · 린캔버스 |
| 어떻게 실행할 것인가 | KPI · 실행로드맵 · 6시스템 · 90일플랜 |

목차 라벨도 전부 프레임워크 이름(`SWOT 분석`·`STP 분석`·`4P 마케팅`·`KPI 지표`)이라
**읽는 사람이 자기 질문을 목차에서 찾을 수 없었다.** micro(7섹션)·사회적경제(9섹션)는
이미 사장님이 읽는 말로 바꿔 놨는데 sme만 남아 있었다.

### 확정안 — 8섹션

| # | 목차 라벨 | 카드 id | 흡수한 것 |
|---|---|---|---|
| 1 | 한눈에 보기 | `sec-summary` | — |
| 2 | 우리 회사 지금 상태 | `sec-diag` | **유형별 특화 분석** |
| 3 | 강점과 약점 | `sec-swot` | — |
| 4 | 누구에게 무엇을 팔 것인가 | `sec-stp` | **린 캔버스** |
| 5 | 어떻게 알릴 것인가 | `sec-4p` | — |
| 6 | 무엇부터 할 것인가 | `sec-six-systems` | **핵심 전략 6가지** |
| 7 | 90일 실행 계획 | `sec-plan90` | **KPI · 실행 로드맵** |
| 8 | 정부지원사업 | `sec-gov` | — |

### 왜 이렇게 합쳤나 — 카드 id 선택 근거

**흡수되는 쪽이 아니라 `micro`가 이미 목차 href로 쓰는 쪽을 카드로 남겼다.**
`sec-six-systems`·`sec-plan90`은 micro 목차(`영역별 처방`·`90일 실행 계획`)의 앵커다.
이것들을 하위 블록으로 내리면 **micro 목차 링크가 죽는다.** 그래서 sme에서
"핵심 전략 → 6시스템", "KPI → 로드맵 → 90일" 순서가 되도록 **sme 전용 섹션을
micro 섹션 안으로** 넣었다. 반대 방향(6시스템을 sec-strategy 안에 넣기)은
micro를 깨뜨리므로 검토 후 버렸다.

린 캔버스를 STP에 넣은 이유는 **두 섹션이 같은 질문에 답하기 때문**이다 —
린 캔버스 9블록 중 `customerSegments`·`uniqueValueProposition`·`channels`가
STP의 세분화·포지셔닝·유통과 정확히 겹친다. 독립 섹션으로 둘 근거가 없다.

### 구조 — `.section-card` 안의 `.sub-block`

흡수된 5개(`sec-consulting`·`sec-lean-canvas`·`sec-strategy`·`sec-kpi`·`sec-roadmap`)는
**id를 그대로 둔 채** 최상위 섹션 카드에서 하위 블록으로 내려갔다.

⚠ **id를 바꾸지 않은 것은 의도적이다.** `render()`의 `smeOnly` 배열이 이 id들로
micro 숨김을 처리하고 있어, id를 바꾸면 그 배열과 두 곳이 각자 판정하게 된다.
`display:none`은 하위 블록에도 똑같이 먹으므로 기존 코드가 그대로 작동한다.

### 하위 블록 렌더 함수는 카드를 숨기면 안 된다 — 이번 작업의 핵심 위험

기존 `renderSixSystems`·`renderPlan90`·`renderDiagSection`·`renderSpecializedSection`은
**데이터가 없으면 자기 섹션 카드를 통째로 `display:none`** 했다. 통합 후 그대로 두면:

- `sixSystems`가 비면 → 같은 카드의 **핵심 전략까지 사라진다**
- `plan90days`가 비면 → 같은 카드의 **KPI·로드맵까지 사라진다**
- 진단 점수가 없으면 → 같은 카드의 **유형별 특화 분석까지 사라진다**

**예외가 나지 않아 조용히 비는** 전형적인 패턴(CLAUDE.md 작업 규칙의 경로 판정 사고와 같은 성질)이라
네 함수를 전부 **"자기 블록만 토글하고 내용 유무를 boolean으로 반환"** 하도록 바꾸고,
**카드 표시 여부는 `render()`가 블록 결과를 OR로 합쳐 한 곳에서** 정하게 했다.
빈 카드도 안 나오고, 내용 있는 카드가 사라지지도 않는다.

### 곁가지 수정 — `sec-summary` 복구 누락 (HEAD에도 있던 결함)

`renderSocial`·`renderFunding`은 keep 화이트리스트로 **모든 `.section-card`를 숨긴다.**
그런데 `render()`의 `smeOnly`에 `sec-summary`가 없어 **사회적경제·정책자금 리포트를 본 뒤
경영진단으로 돌아오면 요약 카드가 숨은 채 남았다.** 카드 표시 판정을 한 곳으로 모으는
작업 중에 드러나 함께 고쳤다.

### CSS

- `dashboard.css`에 `.sub-block`·`.sub-title` 추가. **카드-속-카드로 보이지 않게**
  배경 없이 구분선만 쓴다. `.sec-title + .sub-block`은 구분선이 겹쳐 제거한다
- `print.css`의 `break-before: page`에서 `#sec-strategy`·`#sec-kpi`·`#sec-roadmap` 제거 —
  **하위 블록에 page break가 걸려 있으면 통합 카드가 한가운데서 쪼개진다.**
  대신 최상위가 된 `#sec-plan90`을 추가
- `dashboard.css` 인쇄 블록의 `#sec-strategy` → `#sec-4p`로 교체 (같은 이유)

### 의도한 중간 상태

**AI는 여전히 13섹션분을 생성하고 화면은 8개만 쓴다.** 출력 스펙 정리는 B단계다.
⚠ 지금 필드를 빼면 안 된다 — 화면에서 사라졌다고 버려지는 것이 아니다(주의사항 ⑨).
`leanCanvas`는 화면에서 하위 블록으로 내려갔을 뿐 여전히 렌더링되고,
`keyStrategies`·`kpi`·`roadmap`도 마찬가지다.

### 검증

| 항목 | 결과 |
|---|---|
| sme 목차 8개 · 라벨 확정안 일치 | ✓ |
| 목차 href가 전부 실제 최상위 `.section-card` | ✓ (연결 실패 0건) |
| 목차 순서 = DOM 순서 (스크롤 방향 일치) | ✓ |
| micro 목차 블록 원문 불변 (`git show HEAD:` 기준, 앵커 추출) | ✓ |
| 사회적경제 목차 블록 원문 불변 | ✓ |
| 정책자금 목차 블록 원문 불변 | ✓ |
| 최상위 카드 22개 순서 (micro 5개 순서 보존) | ✓ |
| id 중복 | 0건 |
| `dashboard.js`가 찾는 id 중 `index.html`에 없는 것 | 0건 |
| `node --check js/dashboard.js` | 통과 |
| `?v=` 53곳 → `20260918h` | ✓ (잔존 `g` 0건) |

⚠ **브라우저 실렌더 확인은 하지 않았다** (jsdom 미설치, 정적 검증만).
스크롤 스파이 `allSecIds`에서 하위 블록 5개를 제거했으므로 목차 활성 표시는
8개 카드 기준으로만 움직인다.

---

## 작업 이력 (2026-09-18) — sme 경로 AI 반영 강화: 응답자 메모·점수 분포 주입

### 문제 — 36문항을 답아도 AI가 받는 것은 "영역 점수 + 2점 이하 label"뿐이었다
micro 경로는 2026-09-17에 이미 `buildPromptSummary(scores, group, memos)`로 확장돼
응답자 메모와 점수 분포를 AI에 넘기고 있었다. **sme 경로(`DiagCommon`)만 빠져 있었다.**

`DiagCommon.buildPromptSummary(scores, industryKey)`가 만들던 것:
종합 점수 / 5영역 점수 / 복합 경고 / 2점 이하 label 나열. 그게 전부다.
- **응답자가 적은 메모가 통째로 버려졌다.** "원자재 값이 올랐는데 납품 단가는 그대로"처럼
  점수 1점만으로는 절대 알 수 없는 원인이 `diagScores[k].memo`에 들어 있는데
  `ai-engine.js`의 주입부가 `score`만 뽑아 `flat`을 만들고 메모는 버렸다.
- **5점(우수) 항목이 한 번도 전달되지 않았다.** 처방만 있고 "이건 이미 잘하니 그대로 살려라"가
  없으니 보고서가 강점을 못 짚는다.
- **미응답(0점)과 실제 0점의 구분은 이미 되어 있었다**(주입부 `if (n > 0)` 가드) — 유지했다.

### 한 것 — `common.js`에 `_memoBlock` · `_scoreDistBlock` 추가

| 블록 | 내용 | 상한 |
|---|---|---|
| 응답자 메모 | **요약 맨 앞**. 낮은 점수 문항 우선 정렬 | 문항당 200자 · 총 400자 · 초과 시 `외 N건 생략` |
| 점수 분포 | 취약(1~2점) **전체** + 우수(5점) **전체** + 중간은 개수만 | 취약 12개 노출 · 초과 시 `외 N개` |

시그니처: `buildPromptSummary(scores, industryKey, memos)`.

**왜 메모가 맨 앞인가** — 요약 앞부분만 잘라 쓰는 후속 호출 경로가 있다
(`_buildPrompt2Micro`·`_buildPrompt3Micro`의 `microPrompt.substring(0, 500)`).
뒤에 두면 통째로 사라진다. 검증에서 메모 4건이 전부 앞 500자 안에 들어오는 것을 확인했다.
⚠ 다만 **sme의 2차 호출(`buildPrompt2`)은 진단 요약을 아예 싣지 않는다** — 자르는 게 아니라
없다. 이번 변경은 1차 호출에만 닿는다. 2차에도 넣을지는 별도 판단이 필요해 손대지 않았다.

**왜 지시문을 메모 블록 안에 뒀는가** — "메모를 우선하라"는 지시는 메모가 있을 때만 의미가 있다.
블록 밖에 두면 메모가 없는 사용자에게도 "메모와 점수가 어긋나면" 같은 문장이 나가
모델이 없는 메모를 근거로 서술하려 든다. 메모 0건이면 지시문도 함께 사라진다.
**시스템 프롬프트(`SYSTEM`)는 건드리지 않았다** — sme 경로가 공유하는 공용 상수이고,
CLAUDE.md 규칙상 출력 명세·지시를 두 곳에 두면 모델이 큰 쪽을 따라간다.

### 함께 고친 회귀 — `ITEMS` 직접 참조
기존 `buildPromptSummary`가 `Object.entries(ITEMS)`로 label을 읽고 있었다.
`DiagCommon`은 `INDUSTRY_WORDING`이 그룹별로 label을 덮으므로 **화면은 제조업 문구인데
AI 프롬프트는 기본(service) 문구가 나갔다** — 제조업 사용자가 `매출 대비 재료비+노무비 통제력`을
보고 답했는데 AI는 `매출 대비 재료비+인건비 통제력`을 받았다.
DiagMicro의 `buildPromptSummary`에서 2026-09-16에 터진 사고와 **같은 유형이고 같은 원인**이다
(CLAUDE.md 작업 규칙에 이미 적혀 있던 것을 DiagCommon이 어기고 있었다).
→ `getSchema(industryKey).items`로 교체. 신규 두 블록도 이 `items`를 인자로 받는다.

### 채택하지 않은 대안
- **`buildPromptSummary`를 4벌 → 공용 모듈로 먼저 통합**: 5벌째 복제를 감수했다.
  4개 모듈이 독립 IIFE이고 시그니처(`(scores, memos)` vs `(scores, group, memos)`)와
  label 소스가 서로 달라 **공용화에는 `index.html` 로드 순서 조정이 선행**되어야 한다.
  이번 변경보다 범위가 크고 회귀 위험도 다른 축이다 — CLAUDE.md 남은 이슈로 그대로 둔다.
- **`wizard.js collect()`에서 `allMemos`를 `data`에 실어 보내기**: 하지 않았다.
  `ai-engine.js`의 주입부가 이미 `d.diagScores`(`{key:{score,memo}}` 원본)를 들고 있어
  거기서 뽑으면 된다. 필드를 하나 더 만들면 초기화 누락 시 이전 회사 메모가 섞인다.
- **메모를 `flat`(평면 숫자 맵)에 함께 담기**: 절대 불가. `flat`은 `Number()`로 소비되므로
  전 항목 `NaN`이 된다 — 2026-09-17에 실제로 터진 사고와 같은 형태다.

### 추가 토큰 (Vercel Hobby 60초 상한)
`buildPromptSummary` 출력 길이 기준(한국어 ≈ 1.4자/토큰):

| 상황 | 길이 | 추정 토큰 | Δ |
|---|---|---|---|
| 직전 커밋 기준선 | 445자 | ≈ 318 | — |
| 메모 없음(우수 블록만 추가) | 636자 | ≈ 454 | **+136** |
| 메모 4건(전형) | 1,017자 | ≈ 726 | **+408** |
| 최악(20문항 전부 200자 메모 + 취약 20개) | 1,364자 | ≈ 974 | **+656** |

**전부 input 토큰이며 output은 변하지 않는다.** 60초 상한을 밀어올리는 것은 output 생성 시간이고
(2026-09-08 micro 1차 사고가 output 8,964토큰 / 92초였다), input 656토큰은 prefill 수백 ms 수준이다.
1차 호출 전체 프롬프트가 이미 7,000토큰대이므로 **최악에도 10% 미만 증가**다.

### 검증 — `verify-common-memo.js` 45항목 전부 통과
줄 번호를 하드코딩하지 않고 공개 API로만 검사했다(주의사항 ②-2).
기준선은 `git show HEAD:js/diagnosis/common.js`(주의사항 ⑦).

1. **`calcScores` 불변** — 업종 키 7종(빈 문자열·오타·undefined 포함) 전부 HEAD와 직렬화 일치.
   `detectCrossWarnings`·`getSchema`·`getDomains`·`ITEMS` 키 구성·`ai_trigger` 전부 불변
2. **메모 없음 → 블록 생략** — `undefined`·`{}`·공백뿐인 메모 3종 전부 생략, 맨 앞은 기존 헤더.
   지시문도 함께 사라지는 것을 별도 확인
3. **메모 맨 앞 + 앞 500자 안** — 4건 전부 포함, 1점 → 2점 → 5점 → 미응답 순 정렬 확인
4. **미응답(0점) 메모** — `(미응답)`으로 표기해 전달하되 취약 항목으로 세지 않음
5. **상한** — 문항 200자 말줄임, 총량 초과 시 `외 N건 생략`, 취약 20개 → 12개 + `외 8개`
6. **그룹 오버라이드 label** — `mfg_parts`에서 `재료비+노무비`가 나오고 기본 `재료비+인건비`는
   혼입 0건. **HEAD는 같은 입력에서 기본 label을 내보내던 것까지 대조로 확인**(회귀 재발 감지용)
7. **제어문자 0건**(주의사항 ⑥) · 조사 파손 패턴 0건(주의사항 ②)

⚠ 검증 중 하네스 오류 1건 — "미응답 표기" 실패가 코드가 아니라 **시나리오가 20문항 전부에
점수를 채워 미응답 케이스가 존재하지 않았던 것**이었다. 주의사항 ①·②의 교훈대로
검사기를 먼저 의심해 확인했다.

### 변경 파일
- `js/diagnosis/common.js` — `KEY_PREFIX`·`MEMO_ITEM_MAX`·`MEMO_TOTAL_MAX`·`WEAK_MAX` 상수,
  `_memoBlock`·`_scoreDistBlock` 추가, `buildPromptSummary` 3번째 인자 + label 소스 교체,
  `KEY_PREFIX` 공개(다른 조직 모듈과 동일 규약)
- `js/ai-engine.js` — 주입부에서 `memos` 추출·전달. 하드코딩 접두어를 `DiagCommon.KEY_PREFIX`로
- `index.html` — `?v=20260918f` → `20260918g` (53곳)

---

## 작업 이력 (2026-09-18) — DiagCommon 2-3 guide 단독 분기 검증 + 3건 보강

### 착수 시점 상태 — 2-3은 이미 들어와 있었다
`3_1`·`3_4`·`4_3` × 3그룹 = **9개 guide 단독 오버라이드가 이미 존재했다.**
2-2 그룹별 커밋(`c9e209d`·`13ec39c`·`6ba7a25`)이 14건을 쓸 때 이 3건을 함께 집필했기 때문이다.
**그래서 새로 쓰지 않고 먼저 검증했다** — 있는 것을 덮어쓰면 2-2에서 맞춰 둔 그룹 축이 흐트러진다.

검증 결과 9/9 전부 **guide 단독 · 실효**(기본과 다름 = 죽은 오버라이드 0건, 주의사항 ⑧),
`label`·`question`·`scale`·`ai_trigger` 36개 필드 전부 기본 `ITEMS`에서 상속 불변,
`service`는 오버라이드 블록 자체가 없어 `getSchema().items === ITEMS` 원본 참조.

### 보강한 3건 — 지시서가 명시한 축이 guide에 없었다
검증은 통과했지만 지시서의 문항별 방향과 대조하니 **명시된 축 3개가 문구에 빠져 있었다.**
guide 3건만 고쳤고 나머지 6건은 손대지 않았다.

| 위치 | 빠져 있던 축 | 보강 내용 |
|---|---|---|
| `manufacturing 3_1` | 불량률 목표 | `생산 목표`→`생산 계획`, "생산 수량만 걸면 속도만 올라간다 — 불량을 어디까지 줄일지도 함께" 추가 |
| `trade_retail 3_1` | 매입 목표 · 재고 **회전** | `재고를 줄이는 목표`→`매입 물량과 재고가 몇 번 도는지`, "파는 목표만 걸고 사들이는 목표를 걸지 않으면 매출이 늘어도 자금은 더 말라간다" 추가 |
| `manufacturing 3_4` | 공정 표준서 | 업무 매뉴얼 정의 뒤에 "제조에서는 공정별 표준 작업서가 그 자리다" 추가 |

**왜 `재고를 줄이는 목표`가 아니라 `몇 번 도는지`인가**: 재고는 줄이는 것이 목적이 아니라
**같은 자금으로 몇 번 더 파는지**가 목적이다. 줄이라고만 쓰면 품절로 기회를 잃는 쪽으로 읽힌다.
이 그룹의 `1_2`가 이미 "재고가 한 번 더 돌면 같은 자금으로 한 번 더 번다"로 서술하므로
**같은 모듈 안에서 두 문항이 같은 것을 말하게** 맞췄다.

**왜 불량률에 수치를 넣지 않았는가**: 업종별 불량률 기준치는 근거가 없다.
`small_biz_cost_guide.csv`에 불량률 항목이 없으므로 "어디까지 줄일지"라고만 쓰고
목표치는 사용자가 정하게 남겼다(주의사항 ⑬ — 확인되지 않은 것은 넣지 않는다).

### 채택하지 않은 대안
- **`service`에도 3건을 채운다** — 기각. 기준 그룹이므로 기본 `ITEMS`와 글자까지 같아야 하고,
  채우면 죽은 오버라이드 3개가 생긴다(주의사항 ⑧).
- **`3_1`·`3_4`·`4_3`도 네 필드를 전부 덮는다** — 기각. 기본 `label`·`question`·`scale`이
  이미 업종 중립임을 확인했다(주의사항 ③의 조건부 예외). 덮으면 죽은 필드만 늘어난다.
- **`4_3`도 함께 보강한다** — 불필요. 세 그룹 모두 지시서의 축(생산·재고 연계 /
  현장 보고·배차 / 주문·재고·정산)이 이미 문구에 들어 있었다.

### 검증 하네스 — 단정문을 실제 규칙 폭에 맞춰야 한다 (주의사항 ⑦)
"본문에 영어 약자 0건" 검사가 `3_4`의 `QR`·`4_3`의 `DX`를 잡았으나 **전부 오탐**이었다.
둘은 **기본 `ITEMS` 본문에 원래 있던 것**이고(이번 작업은 guide만 건드렸다)
`디지털 전환(DX)`은 프로젝트가 채택한 표기다. **검사기를 실제 규칙보다 좁게 적어** 정당한
기존 문구가 회귀로 잡힌 사례다 — 주의사항 ①·⑦과 같은 교훈이다.
기준선은 `git show HEAD:`가 아니라 **같은 모듈의 `ITEMS`**로 잡았다(영구 불변식).

셸 경유로 정규식을 넘기다 백슬래시가 한 층 소실돼 `SyntaxError`가 났다(주의사항 ⑥).
→ 검사 스크립트는 heredoc으로 **파일에 쓴 뒤 실행**했다.

### 검증 결과
1. guide 단독 실효 오버라이드 **9/9**
2. `label`·`question`·`scale`·`ai_trigger` 상속 불변 **36/36**
3. `service`·`etc`·미전달·`getSchema()` 무인자 → 기본 `ITEMS` **원본 참조 반환**
4. `calcScores` 출력 · `detectCrossWarnings` 출력 **HEAD와 동일**
5. HEAD 대비 변경 필드 **정확히 3건** (전부 `.guide`), 기본 `ITEMS` 불변
6. 제어문자 0개 / 조사 파손 패턴 0건 / guide 내 출처 없는 수치 0건
7. guide 약자 `SOP`·`ERP`·`MES`·`TMS`·`WMS` 전부 풀네임 병기 확인

---

## 작업 이력 (2026-09-18) — 프라임코스트 기준을 CSV 근거로 교체 (2/2 DiagCommon)

1/2에서 DiagMicro 10그룹을 CSV 근거로 바꿨다. 이제 sme 경로를 맞춰 **모순을 닫는다.**

### ① 기본 `1_1` guide의 출처 없는 백분율 제거

`업종 기준치: 외식 60% / 제조 75% / IT 30% / 유통 80% 이하` → 삭제.

**이 기본값을 받는 것은 `service` 기준 그룹뿐이고, 그 그룹은 외식·미용·의원·학원·IT·금융·
미디어를 한꺼번에 담는다.** CSV 근거로 계산해도 교육 30~45%와 외식 50~60%가 같은 그룹에
들어오므로 단일 수치가 성립하지 않는다. 대신 "적정 비율은 업종에 따라 크게 다르다
(교육·미용처럼 30%대부터 유통·수출처럼 90%까지) · 같은 업종·업태의 기준을 확인해 비교하라"로
바꿨다. **수치는 그룹 오버라이드에만 둔다**는 원칙을 코드 주석으로 못박았다.

### ② 3그룹 오버라이드에 CSV 범위 반영

| 그룹 | 범위 | 비고 |
|---|---|---|
| `manufacturing` | 60~80% | 제조업-소규모 |
| `field_service` | 70~90% | 건설업-소내. **⚠ 물류·시설관리 제외 명시** |
| `trade_retail` | 60~75% ~ 75~90% | 소매·도매 / 수출·무역 **범위 병기** |

`field_service`는 건설·에너지·물류·시설관리를 담는데 CSV의 건설 수치만 있다.
그래서 범위를 적고 **곧바로 제외 조항을 붙였다** — 물류는 사 오는 물건이 없어 재료비 자리가
비고, 시설관리는 인건비가 사실상 전부여서 계약 단가 대비 실질 인건비 비율을 자체 기준으로 삼는다.
`trade_retail`은 소매와 수출이 15%포인트 벌어지므로 **두 범위를 병기**하고
"같은 그룹 안에서도 이만큼 벌어지므로 자기 업태의 범위를 골라 쓴다"를 덧붙였다.

### ③ 모순 해소 확인 — 검사기가 두 모듈을 교차 대조한다

| 업종 | micro | sme | |
|---|---|---|---|
| `mfg_parts`·`food_mfg`·`agri_food` | 60~80 | 60~80 | ✓ |
| `construction`·`energy` | 70~90 | 70~90 | ✓ |
| `wholesale`·`fashion` | 60~75 | 60~75 / 75~90 | ✓ |
| `export_sme` | 75~90 | 60~75 / 75~90 | ✓ |
| `logistics`·`facility_service` | 적용 범위 없음 | 적용 범위 없음 | ✓ |
| `restaurant`·`education` 등 | 50~60 / 30~45 | 침묵 | 모순 아님 |

**한쪽이 침묵하는 것은 모순이 아니다.** sme 경로의 `service` 그룹이 수치를 주지 않는 것은
설계이고, micro가 그 업종에 대해 CSV 수치를 주는 것과 충돌하지 않는다.

### ⚠ 검사기 오탐 하나와 하네스 전제 붕괴 셋

**오탐 — `logistics` 모순 경고.** 검사기가 guide에서 범위만 긁어
micro `75~90`(수출) vs sme `70~90`(건설)을 모순으로 보고했다. 그런데 **양쪽 guide 모두
물류를 명시적으로 제외**하고 있었다 — 물류에 적용되는 수치는 어디에도 없다.
산문으로 쓴 제외 조항을 정규식이 읽지 못한 것이다(주의사항 ①).
→ 범위가 **어느 업종을 지배하는가**(`RANGE_SCOPE`)를 하네스에 명시하고,
**제외 문구가 guide에 실제로 있는지도 함께 검증**했다. 표에만 적고 코드에서 빠지면
그 표가 거짓말이 되기 때문이다.

**전제 붕괴 — 이전 하네스 셋이 거짓 실패를 냈다.** `verify-2-1`·`verify-2-2`가
`기본 ITEMS == HEAD`를 영구 불변식으로 박아 뒀는데, 그것은 **각 커밋 범위에서만 참인 조건**이었다.
이번에 기본 `1_1` guide를 의도적으로 고치자 전부 빨간불이 됐다.

| 하네스 | 붕괴한 전제 | 고친 불변식 |
|---|---|---|
| `verify-2-1` | `INDUSTRY_WORDING = {}` | 오버라이드 **없는 그룹**에서만 `items === ITEMS` 검사 |
| `verify-2-1` | `getSchema == HEAD` | 기준선을 HEAD가 아니라 **같은 모듈의 `ITEMS`**로 |
| `verify-2-2` | `ITEMS == HEAD` | **키 구성·`ai_trigger`·scale 단계 수** 불변 |
| `verify-2-2` | 오버라이드에 백분율 0건 | **본문에 수치 0건 + guide 수치는 출처와 함께** |

**이것이 주의사항 ⑦("회귀 기준선은 직전 커밋")의 연장선이다.** 기준선을 직전 커밋으로
잡는 것만으로는 부족하다 — **단정문이 "지금 이 커밋에서만 참인 것"인지 "언제나 참인 것"인지
구분해 써야 한다.** 전자를 영구 불변식처럼 적어 두면 다음 정당한 변경에서 거짓 실패가 난다.
수치 검사의 교훈이 특히 그렇다: 진짜 규칙은 "수치 금지"가 아니라 **"출처 없는 수치 금지"**였다.

### 검증 — 세 하네스 전부 통과

| 하네스 | 결과 |
|---|---|
| `verify-prime` (이번 작업) | **1,505건** |
| `verify-2-2` (문항 오버라이드 회귀) | **4,262건** |
| `verify-2-1` (골격 회귀) | **1,331건** |

`verify-prime`이 보는 것: CSV를 파싱해 `인건비비중 + 재료비/원가`를 **다시 합산**해 대조 /
단정 표현 6패턴 0건 / micro 10그룹·common 4그룹 전수 / 수치 제외 그룹은 범위 부재와
안내 문구 / **두 모듈 18업종 교차 대조** / `calcScores` 불변 /
`ai_trigger.threshold`·`warning` 불변 / 교차 경고 `code`·`level` 불변 / 제어문자 0건.

---

## 작업 이력 (2026-09-18) — 프라임코스트 기준을 CSV 근거로 교체 (1/2 DiagMicro)

### 문제 — 출처 없는 단일 수치 + 두 모듈의 모순

전 그룹에 `60% 이하가 목표`가 박혀 있었고 **출처가 없었다.** 더 나쁜 것은 모순이다:

| | 외식 | 제조 | IT | 유통 |
|---|---|---|---|---|
| `common.js` 기본 `1_1` guide | 60% | 75% | 30% | 80% |
| `diagnosis-micro.js` `1_3` | **전 그룹 일괄 60%** | 60% | 60% | 60% |

**같은 회사가 직원 수만 넘어가면 기준이 두 배로 바뀐다.** 경로 판정(`_diagPathOf`)이
micro에서 sme로 넘어가는 순간 제조업 사장님이 보는 기준이 60%에서 75%가 된다.

### 근거 — `knowledge_base/small_biz_cost_guide.csv`

프라임코스트 = `인건비비중(%)` + `재료비/원가(%)`. **CSV에서 직접 계산했다**:

| CSV 분류 | 인건비 | 재료비 | 프라임코스트 |
|---|---|---|---|
| 외식업-일반 | 20-25 | 30-35 | **50~60%** |
| 외식업-카페 | 15-20 | 25-30 | 40~50% |
| 서비스업-미용 | 30-40 | 5-10 | **35~50%** |
| 소매업-오프라인 | 10-15 | 50-60 | **60~75%** |
| 소매업-온라인 | 10-20 | 50-70 | 60~90% |
| 교육업 | 30-40 | 0-5 | **30~45%** |
| 제조업-소규모 | 25-35 | 35-45 | **60~80%** |
| 건설업-소내 | 30-40 | 40-50 | **70~90%** |
| 수출형-소기업 | 15-20 | 60-70 | **75~90%** |

⚠ **검사기도 CSV에서 다시 계산해 대조한다.** 코드에 적힌 수치를 코드에서 읽어 비교하면
아무것도 검증하지 못한다 — 하네스가 CSV를 파싱해 합산한 값과 guide 문자열을 맞춘다.

### 적용 — DiagMicro 10그룹

| 그룹 | 처리 | 근거 |
|---|---|---|
| `food` | 50~60% | 외식업-일반. 카페는 더 낮다고 병기 |
| `beauty` | 35~50% | 서비스업-미용 |
| `retail` | 60~75% | 소매업-오프라인. 온라인은 더 높다고 병기 |
| `edu_service` | 30~45% | 교육업 |
| `manufacturing` | 60~80% | 제조업-소규모 |
| `construction` | 70~90% | 건설업-소내. **수치가 없던 곳에 추가** |
| `trade_logistics` | **분기 병기** | 수출 75~90%(매입가 기준) / 물류는 재료비 자리가 비어 유류비+기사 인건비로 대체 |
| `pro_service` | **수치 제외** | 의원·약국(약제 재료비 있음)과 IT·금융(재료비 없음)이 한 그룹 — 단일 수치를 고르면 절반이 오진 |
| `facility_service` | **수치 제외** | 인건비가 원가의 대부분. 재료비를 전제한 CSV 분류와 계산 구조 자체가 다르다 |
| `etc` | **수치 제외** | 업종 미판별. "같은 업종·업태 기준을 확인해 비교하라" |

`construction`·`trade_logistics`·`facility_service`는 이미 `목표치 이하`로 중립이었다 —
수치가 박혀 있던 곳은 **기본 ITEMS + 6그룹 = 7곳**이었다.

### 표현 원칙 — 범위로 쓰고 단정하지 않는다

`60% 이하가 목표` → `참고 범위: 일반 음식점 50~60%(소상공인 업종별 비용구조 기준) ·
업태·입지·인력 구성에 따라 달라지므로 단정 기준이 아니라 비교 출발점으로 쓴다`.

scale 서술도 같이 바꿨다 — `상시 60%를 초과함` → `참고 범위를 상시 넘고 있음`,
`60% 이하로 엄격히 관리` → `참고 범위 안에서 엄격히 관리`.
**question·scale에 수치를 남기면 guide를 고쳐도 화면에는 옛 기준이 그대로 뜬다**(주의사항 ③).

### 교차 경고에서도 임계 수치를 걷어냈다

`sell_more_lose_more`가 `${w.cost} 60% 초과 + 실제 남는 돈 미산출`이었다.
그런데 이 경고의 발동 조건은 `1_3 <= 2 && 1_4 <= 2`이고, **2점의 뜻은 "기준을 넘었다"가
아니라 "비율을 관리하지 않는다"**(scale 2단계 서술 그대로)다. 수치를 지우는 김에
**조건과 문구의 어긋남도 함께 고쳤다** → `${w.cost} 비율을 관리하지 않고 실제 남는 돈도
산출하지 않습니다`.

### 육안 확인에서 잡은 것 — 내가 만든 모순

검사기 1,487건이 전부 통과한 상태에서 덤프를 읽다가 발견했다.
기본 `ai_trigger.warning_msg`를 `동종업계 기준과 비교해…`로 바꿨는데,
이 필드를 **`construction`·`trade_logistics`·`facility_service`·`etc` 네 그룹이 상속**한다.
그중 `facility_service` guide에는 방금 내가 `업종별 참고 범위를 제시하지 않는다`라고 썼다.
**한 문항 안에서 guide와 경고가 정반대를 말하는 상태**가 됐다.

→ 기본 warning_msg를 네 그룹 모두에 맞는 말로 되돌렸다:
`직접 원가와 인건비를 합친 비율을 관리하지 않고 있습니다`.
**상속받는 쪽을 확인하지 않고 기본값을 고치면 이런 일이 난다**(주의사항 ③·⑨).

### 검증 1,487건 통과 (DiagMicro)

기준선은 직전 커밋. CSV 재계산 대조 / 단정 표현 6패턴 0건 / 10그룹 전수 —
수치 그룹은 CSV 범위와 출처 표기 확인, 수치 제외 4그룹은 범위 부재와 안내 문구 확인 /
`calcScores` 10그룹 × 시드 25종 불변 / 문항 수 불변 /
**`ai_trigger.threshold`·`warning` 전 문항 불변**(`warning_msg` 텍스트만 변경) /
교차 경고 `code`·`level` 불변 + 경고 문구에 단정 수치 0건 / 제어문자 0건.

⚠ DiagCommon 8건은 의도적으로 실패 상태로 남겼다 — 2/2 커밋에서 처리한다.

---

## 작업 이력 (2026-09-18) — 2-2 DiagCommon 문항 오버라이드 집필

2-1에서 깔아둔 `INDUSTRY_WORDING` 골격에 본문을 채운다.
**그룹 단위 3커밋**으로 나눴다 — 본문 약 2만 6천 자를 한 번에 넣으면 회귀가 났을 때
어느 그룹이 원인인지 가릴 수 없고, 육안 확인(주의사항 ②-1)이 사실상 불가능하다.

### 설계 결정 ① — label을 덮을 것인가 (주의사항 ③ vs ⑧의 충돌)

③은 "`label`·`question`·`guide`·`scale` 네 필드를 함께 덮어라"(일부만 덮으면 나머지가
기본에서 상속돼 질문과 척도가 서로 다른 것을 말한다)이고, ⑧은 "기본과 글자까지 같은
오버라이드는 실효가 0인 죽은 것"이다. **`label`에서 둘이 정면으로 부딪힌다** —
네 필드를 채우라는 규칙을 따르면 바꿀 이유가 없는 label에 죽은 오버라이드가 생긴다.

→ **"네 필드 전부 or 아예 없음"으로 갈랐다.** 본문을 고치는 문항은 label까지 다시 쓰고,
고칠 이유가 없는 문항은 오버라이드 자체를 만들지 않는다.
`guide` 하나만 다른 3문항(`3_1`·`3_4`·`4_3`)은 예외로 guide만 덮되,
**상속되는 label·question·scale이 업종 중립인지 검사기로 확인**했다 — 그게 ③의 실제 사고다.

검사기도 그렇게 짰다: 오버라이드에 들어 있는 필드는 전부 기본과 달라야 하고(⑧),
동시에 **20문항 전부**의 최종 `label`·`question`·`scale`에 타 그룹 전용어가 없어야 한다(③).
오버라이드한 문항만 훑으면 상속 필드에 닿지 않는다.

### 설계 결정 ② — 오버라이드하지 않은 6문항

`1_3`(손익분기점) · `1_4`(자금 투명성) · `4_1`(AI 문서 작성) · `4_2`(백오피스 자동화) ·
`4_4`(전사 AI 리터러시) · `5_3`(표준근로계약)은 **기본 문구가 이미 업종 중립**이다.
억지로 채우면 죽은 오버라이드만 늘어난다. 검사기가 이 6문항에 대해
`items[k] === ITEMS[k]` **원본 참조**임을 확인한다 — 불필요한 사본조차 만들지 않는다.

### 설계 결정 ③ — 기본 1_1 guide의 미검증 수치를 옮겨 적지 않았다

기본 `1_1` guide에 `외식 60% / 제조 75% / IT 30% / 유통 80% 이하`가 있다. **출처가 없다.**
이번 오버라이드가 그 자리를 덮으므로 그룹 guide에는 **백분율을 일절 쓰지 않았다**.
검사기가 오버라이드 텍스트 전체에서 `N%`·`N배` 패턴을 0건으로 확인한다.
(기본값에 남아 있는 것은 `service` 경로가 계속 받는다 — 별건 A 유형 작업이다.)

### ① manufacturing — mfg_parts · food_mfg · agri_food (14건)

축: 원가 재료비+노무비 / 재고 3단계 / 가시성 설비·인증 / 통제 공장·생산관리시스템 /
ESG 배출·자투리, 원청 있음.

| 문항 | 기본 → manufacturing |
|---|---|
| `1_1` | 재료비+**인건비** → 재료비+**노무비** (제조원가 용어) |
| `1_2` | 재고일수 → **원자재·재공품·완제품 3단계** |
| `2_1` | 상호·주소·연락처 → + **보유 설비·인증 유효기간** |
| `2_2` | 온라인 정보 → **제품 사양서·설비 현황·인증 갱신일** |
| `2_3` | 고객 리뷰·평점 → **거래처 평가·품질 클레임 이력** |
| `2_4` | 판로별 마케팅 → **수주 경로별 영업**(전시회·조달 등록·중개상사) |
| `3_2` | 업무 시작·종료 점검 → **시업 점검·공정 검사·출하 검사** |
| `3_3` | 사업장 부재 → **공장 부재**(생산 실적·설비 가동·불량) |
| `5_1` | 탄소 배출 → **연료 연소 배출과 전력 사용 배출 구분** |
| `5_2` | 폐기물·자원 순환 → **불량품·자투리 자재**(재료비+노무비를 두 번 쓰는 신호) |
| `5_4` | 원청·플랫폼 → **원청**(도면·금형 반환 약정, 사급 자재 정산) |

`1_1` label을 바꾼 이유가 대표적이다. 제조원가 용어는 `인건비`가 아니라 **`노무비`**다.
기본 label을 그대로 두면 **질문과 척도는 노무비를 말하는데 제목만 인건비**가 된다 —
2026-09-17에 실제로 있었던 상태(주의사항 ③)의 재발이다.

`5_1`의 `Scope 1·2`는 쓰지 않았다. 본문에는 `연료를 태워 나오는 배출`·`전기를 써서 생기는 배출`로
풀어 쓰고, guide에 `직접 배출`·`간접 배출` 정의를 뒀다. 영어 약자는 전부 같은 방식으로 처리했다
(`생산관리시스템(MES, …)`·`전사자원관리(ERP, …)`는 guide에만).

**세 업종을 모두 포괄해야 한다.** 한쪽 전용어(절삭·도정)를 쓰지 않고, guide마다
`부품가공 / 식품제조 / 농식품` 세 예시를 병기했다.

#### 검증 1,503건 통과 (manufacturing)

기준선은 직전 커밋(`git show HEAD:`). 검사 항목:
`calcScores` 불변(시드 25종+빈 맵) / `service` 12케이스가 기준선과 `JSON` 동일하고
`items === ITEMS` 원본 참조 / 오버라이드 문항 구성이 설계와 일치 /
본문 11문항은 네 필드 전부·guide 3문항은 guide만 / **죽은 오버라이드 0건** /
`key`·`weight`·`id`·`ai_trigger` 분기 0건 / scale 5단계·score 1~5 /
**20문항 전부**의 본문에 타 그룹 전용어 0건 / 본문 영어 약자 0건 /
출처 없는 수치 0건 / 조사 파손 실패턴 17종 0건 / 제어문자 0건 / 2-1 골격 회귀 없음.

⚠ 전용어 검사는 `label`·`question`·`scale`만 대상으로 한다 — `guide`에는 업종 예시가
의도적으로 들어간다(CLAUDE.md 작업 규칙).
⚠ 낱말을 완성형으로만 적었다. `발주`로 검사하면 `발주서`·`구두 발주`(정상)가 걸린다 —
주의사항 ①의 부분 문자열 오탐이다. `발주처`로 적었다.
⚠ **검사기 통과로 끝내지 않고 14건 전문을 파일로 덤프해 읽었다**(주의사항 ②-1).

### ② field_service — construction · energy · logistics · facility_service (14건)

축: 원가 노무비+외주비·유류비 / 재고 거의 없음 / 가시성 실적·면허 /
통제 현장·배차 / ESG 유류 배출·폐기물 처리비, 원청 있음.

**네 업종의 편차가 가장 큰 그룹이다.** 건설의 `시공`, 물류의 `배차`, 시설관리의 `순찰`은
서로에게 전용어다. → **본문에는 `현장`·`계약`·`실적`만 쓰고 업종별 어휘는 전부 guide의
예시로 내렸다.** 이것이 이 그룹의 집필 원칙이었다.

| 문항 | 기본 → field_service |
|---|---|
| `1_1` | 재료비+인건비 → **현장 투입 원가**(노무비·외주비·유류비) |
| `1_2` | 재고일수 포함 → **재고가 거의 없다.** 먼저 치른 인건비가 묶이는 돈의 대부분 |
| `2_1` | 상호·주소·연락처 → + **보유 면허·등록 정보**(표기가 곧 입찰 자격) |
| `2_2` | 온라인 정보 → **수행 실적**(광고가 아니라 수행 능력의 증명) |
| `2_3` | 고객 리뷰·평점 → **발주처 평가·재계약 여부** |
| `2_4` | 판로별 마케팅 → **수주·계약 경로별**(입찰·소개·재계약) |
| `3_2` | 업무 시작·종료 점검 → **현장 안전 점검·인수인계** |
| `3_3` | 사업장 부재 → **흩어진 현장**(진행 상황·인력 배치) |
| `5_1` | 탄소 배출 → **유류가 배출의 대부분**(주유·충전 기록) |
| `5_2` | 폐기물·자원 순환 → **폐기물 처리비**(실행 예산에 잘 안 잡히는 원가) |
| `5_4` | 원청·플랫폼 → **원청·발주처**(추가 작업 미정산·설계 변경 합의) |

`1_1` label을 바꾼 이유는 지시에 명시된 대로다 — 기본 label이 `재료비+인건비`인데
이 그룹은 **재료비가 거의 없다.** 그대로 두면 질문은 노무비·외주비·유류비를 묻는데
제목만 재료비를 말한다.

`5_2`에 행정 처분을 언급하면서 **과태료 금액을 쓰지 않고 `처분 기준은 관할 기관에
확인할 것`으로 남겼다**(주의사항 ⑫).

#### ⚠ 주의사항 ①이 5번째로 재발했다 — `수율` → `이수율`

전용어 검사기에 `수율`(제조 전용어)을 넣었더니 **기본 `3_4` scale의 `이수율`**
(교육 이수율 자동 추적)에 걸렸다. 지금까지 기록된 `포스`→`포스터`, `네일`→`썸네일`,
`환율`→`전환율`, `자문`→`전자문서`와 **완전히 같은 패턴**이다.

→ `공정 수율`로 늘려 잡았다. **한 낱말이 다른 낱말의 꼬리가 될 수 있으면 앞말을 붙인다.**
검사 결과가 이상하면 코드가 아니라 검사기를 먼저 의심하라는 교훈이 또 맞았다 —
실제로 field_service 본문에 `수율`은 없었고, 걸린 것은 손대지도 않은 기본 문항이었다.

#### ⚠ 검사기의 죽은 패턴 하나를 함께 고쳤다

수치 검사에 쓰던 `/\d+\s*배\b/`가 **한 번도 매치될 수 없는 죽은 패턴**이었다.
JS의 `\b`는 ASCII `\w` 기준이라 한글 뒤에서는 경계가 성립하지 않는다.
→ `/\d+\s*배(?=[\s,.·)]|$)/`로 바꿔 `2배`는 잡고 `배차`는 거르게 했다.
**"0건"이라는 결론은 검사기가 살아 있을 때만 의미가 있다.**

#### 검증 2,806건 통과 (manufacturing + field_service 누적)

항목은 ①과 같다. 두 그룹을 함께 돌려 **그룹 간 간섭이 없는지**도 확인했다 —
`getSchema('mfg_parts')`와 `getSchema('construction')`이 서로의 오버라이드를 받지 않는다.
14건 전문을 덤프해 육안 확인했다.

### ③ trade_retail — wholesale · fashion · export_sme (14건)

축: 원가 매입원가 / 재고 회전이 핵심 / 가시성 채널·거래처 /
통제 창고·물류 / ESG 파손·반품, **원청 약함**.

| 문항 | 기본 → trade_retail |
|---|---|
| `1_1` | 재료비+인건비 → **매입원가**(+운반비·관세·반품 손실) |
| `1_2` | 재고일수 → **재고 회전이 이 그룹의 전부** |
| `2_1` | 상호·주소·연락처 → + **취급 품목 정보**(채널마다 다르면 다른 회사로 읽힌다) |
| `2_2` | 온라인 정보 → **품목 정보·공급 가능 여부·거래 조건** |
| `2_3` | 고객 리뷰·평점 → **반품·클레임 처리 속도가 곧 재주문 여부** |
| `2_4` | 판로별 마케팅 → **판매 채널별 남는 금액**(수수료·물류비·반품 손실 차감) |
| `3_2` | 업무 시작·종료 점검 → **입고 검수·출고 대조·재고 실사** |
| `3_3` | 사업장 부재 → **창고**(장부와 실물이 맞는지가 원격 통제의 전부) |
| `5_1` | 탄소 배출 → **직접 배출이 적다.** 창고 전기 + 운송 유류 |
| `5_2` | 폐기물·자원 순환 → **파손·반품**(매입원가를 날리고 처리비까지 드는 이중 손실) |
| `5_4` | 원청·플랫폼 → **대형 구매처·판매 채널·해외 거래처** |

**`5_4`가 이 그룹의 핵심 판단이다.** 세 그룹 중 **원청이 뚜렷하지 않은 유일한 그룹**이라
`원청`을 그대로 쓰면 문항이 성립하지 않는다. 기본 guide가 이미 같은 취지를 적어 두었다 —
"원청이 없는 업종은 힘의 우위에 있는 거래 상대로 바꿔 보면 된다". 그 자리를
**대형 구매처·판매 채널·해외 거래처**로 명시하고, 남길 증빙도 발주서가 아니라
**반품·정산 조건과 판촉 비용 분담 근거**로 바꿨다.

`1_2`에 "안 팔리는 재고는 창고에 있는 한 비용이 아니라 **자산으로 잡혀 손실이 늦게
드러난다**"를 넣었다. 이 그룹에서 가장 자주 놓치는 지점이고, 재고 회전을 왜 재야 하는지의
근거이기도 하다.

#### 육안 확인에서 잡은 것 — 검사기는 통과했다

`3_2` guide의 `재고 실사 주기와, 차이가 났을 때의 처리 절차까지`에서 쉼표 위치가
부자연스러웠다(`주기와 차이`가 붙어 읽히는 것을 막으려다 생긴 군더더기).
→ `재고 실사를 얼마나 자주 할지, 장부와 차이가 났을 때 어떻게 처리할지까지`로 고쳤다.
**검사기 4,168건이 전부 통과한 상태에서 읽어서 잡았다** — 문장이 어색한 것은
기계가 잡아주지 않는다. 육안 확인을 생략하면 안 되는 이유다.

#### 검증 4,168건 통과 (3그룹 전체)

`calcScores` 불변 / `service` 12케이스가 기준선과 `JSON` 동일 + `items === ITEMS` 원본 참조 /
그룹당 오버라이드 14건 구성 일치 / 본문 11문항 네 필드·guide 3문항 guide만 /
죽은 오버라이드 0건 / 분기 금지 필드 0건 / scale 5단계·score 1~5 /
**3그룹 × 20문항 = 60문항 전부**의 본문에 타 그룹 전용어 0건 /
본문 영어 약자 0건 / 출처 없는 수치 0건 / 조사 파손 실패턴 0건 / 제어문자 0건 /
2-1 골격 회귀 없음 / **그룹 간 간섭 없음**.

### 2-2 완료 후 남은 것

- `service` 기준 그룹의 기본 `1_1` guide에 **출처 없는 업종 기준치 백분율**이 남아 있다
  (`외식 60% / 제조 75% / IT 30% / 유통 80% 이하`). 3그룹은 오버라이드가 덮었으므로
  이제 이 수치를 보는 것은 `service` 경로뿐이다. **A 유형 정리 대상**이다.
- 기본 `2_1` scale의 `API 기반`·`구조화 데이터(Schema)`, `1_3` label의 `BEP`도
  `service` 경로에만 남는다. 같은 정리에서 함께 다루면 된다.

---

## 작업 이력 (2026-09-17) — 2-1 DiagCommon 업종 분화 골격 (문구 집필 없음)

`DiagCommon`을 업종 그룹별로 분화하기 위한 **뼈대만** 깔았다.
문항 문구(`INDUSTRY_WORDING` 33개 오버라이드) 집필은 2-2에서 한다.

### 이번 작업의 범위 선 긋기 — 왜 골격과 집필을 나눴나

`04aafbe`에서 `INDUSTRY_GROUP_MAP`·`getGroup()`·`getSchema(industryKey)`를 이미 깔았고
`e72203e`에서 기본 `ITEMS` 20문항을 업종 중립화했다. 남은 것은 두 종류인데 성격이 다르다.

| | 성격 | 회귀 위험 | 이번 처리 |
|---|---|---|---|
| 골격 (배선·분기 지점) | 구조 변경 | **높다** — 키 접두어·문항 수·점수 계산에 닿는다 | 2-1 (지금) |
| 문구 (오버라이드 본문) | 텍스트 집필 | 낮다 — 데이터만 늘어난다 | 2-2 |

**구조와 텍스트를 한 커밋에 섞으면 회귀가 났을 때 원인을 가릴 수 없다.**
`INDUSTRY_WORDING`을 `{}`로 둔 채 골격만 넣으면, `getSchema`의 `items` 반환값이
**수정 전과 완전히 동일한 객체 참조**여야 한다는 강한 검증 기준이 생긴다. 실제로 그렇게 검증했다.

### ① `GROUP_MAP` → `INDUSTRY_GROUP_MAP` 개명 + 공개

`DiagMicro`가 같은 역할을 `INDUSTRY_GROUP_MAP`으로 부른다. 두 모듈의 표면을 맞춰
"내가 지금 어느 모듈에 있는가"를 헷갈리지 않게 했다.

⚠ **이름은 같지만 내용이 다르다** — `logistics`가 DiagCommon에서는 `field_service`,
DiagMicro에서는 `trade_logistics`다. 그래서 **호출부가 그룹을 계산해 넘기면 안 된다**는
규칙이 더 중요해졌다. 공개 API는 전부 **업종 키**만 받는다.

`etc`는 DiagMicro와 달리 전용 그룹을 두지 않고 기준 그룹 `service`로 **명시적으로** 보냈다.
기본 `ITEMS`가 이미 업종 중립이라 `etc` 전용 문구를 따로 쓸 이유가 없다.

### ② `DOMAIN_DESC_BY_GROUP` — D1·D2 × 3그룹

영역 설명(`desc`)은 **`guide`가 없는 자리**이고 영역 제목 바로 아래 첫 화면에 보인다.
기본 D1은 "사업에 직접 들어가는 비용", D2는 "고객이 우리를 찾는 온라인 접점"인데,
B2B 제조·건설·도매에서는 그 주체가 고객이 아니라 **거래처·발주처·구매처**다.

- `manufacturing` — 제조원가(재료비·노무비) / 거래처가 확인하는 기업 정보·제품 자료
- `field_service` — 현장 노무비·외주비·유류비 / 발주처가 확인하는 실적·보유 역량
- `trade_retail` — 매입원가와 재고에 묶이는 자금 / 구매처·고객이 확인하는 상품 정보·거래 조건
- `service` (기준) — **항목 없음.** `DOMAINS` 원본을 그대로 반환한다

**D3·D4·D5는 분기하지 않았다.** 시스템화·AI/DX·ESG의 기본 `desc`는 이미 업종 중립이라
분기해 봐야 기본과 글자까지 같은 **죽은 오버라이드**가 된다(주의사항 ⑧).

⚠ `desc`만 덮는다. `label`·`key`·`weight`·`id`·`icon`은 분기 금지 —
`calcScores`가 `domain.label`을 반환값에 실어 레이더차트·PPT·대시보드로 흘려보낸다.
구조적으로도 `_domainsFor`가 `desc`만 읽으므로 `label` 키를 넣어도 무시된다.

### ③ `WARN_WORDING` — 5규칙 × 4그룹

교차 경고 5개 문구에 업종어가 박혀 있었다.

| 규칙 | 박혀 있던 전제 | 문제 |
|---|---|---|
| `digital_blind` | `로컬 검색 노출` | 지역 상권 업종 전제. B2B 제조·도매·수출에 성립하지 않는다 |
| `legal_risk` | `하도급법` | 원청-하도급 구조 전제. 소매·전문서비스에는 원청이 없다 |

⚠ **어절 토큰이 아니라 완성된 문장으로 저장했다.** DiagMicro는 `{cost, subj, dog, …}`
토큰 방식인데, 2026-09-04에 어절만 갈아끼웠다가 **받침이 바뀌며 한국어 조사가 파손**된 전례가 있다
(`저수익 서비스을`). 5×4 = 20문장이면 통째로 쓰는 편이 안전하고, 검증도 육안으로 끝난다.

⚠ `level`·`code`는 분기 금지 — 소비처가 `code`로 식별한다. `msg`만 덮는다.

**`service` 기준값에서도 업종어를 걷어냈다.** DiagMicro의 `food`처럼 "원문과 글자까지 동일"을
지킬 이유가 없다 — `service` 그룹에는 `knowledge_it`·`finance`·`media`가 함께 들어 있어
`로컬 검색 노출`·`하도급법`이 애초에 맞지 않는다. `manufacturing`·`field_service`의
`legal_risk`에는 `하도급법`을 **남겼다**. 그쪽은 실제로 하도급법 적용 대상이다.

### ④ 호출부 2곳

- `wizard.js _diagCommonToAreas` — 이미 `industryKey`를 넘기고 `schema.domains`를 읽고 있어
  **코드 변경 없음**. `DOMAINS` 원본을 직접 읽으면 그룹 분기가 통째로 무시된다는 주석만 달았다
- `ai-engine.js buildPromptSummary(flat)` → `buildPromptSummary(flat, d.industryKey || d.industry || '')`

`buildPromptSummary`는 `calcScores`에 그룹을 넘기지 **않는다.** `DOMAIN_DESC_BY_GROUP`은
`desc`만 덮고 `label`은 분기하지 않으므로 `calcScores` 시그니처가 바뀔 이유가 없다.
(DiagMicro는 D2 label을 분기하므로 `calcScores(scores, group)`이다 — **두 모듈의 계약이 다르다.**)

### ⑤ `module.exports` 추가

`diagnosis/` 다른 모듈과 같은 `if (typeof module !== 'undefined')` 패턴.
Node 검증 하네스가 `git show HEAD:...` 기준선과 나란히 `require`할 수 있어야 했다.

### 검증 — 1,328건 통과

기준선은 **직전 커밋(`git show HEAD:js/diagnosis/common.js`)**이다(주의사항 ⑦).
줄 번호를 하드코딩하지 않고 소스를 통째로 `vm`에 올려 비교했다(주의사항 ②-2).

1. **`INDUSTRY_WORDING={}`에서 `getSchema` 불변** — 18개 업종 키 + 폴백 9종 전부
   `schema.items === ITEMS` **원본 참조**이고 `id`·`label`·`version` 동일.
   `service` 그룹은 반환값 전체가 기준선과 `JSON` 수준에서 동일
2. **17업종 → 4그룹** 18건 + 폴백(`undefined`·`''`·`null`·`0`·대문자·타 모듈 그룹명·오타·공백) 9건
3. **`calcScores` 불변** — 시드 25종 + 빈 맵 + 부분 응답에서 기준선과 `JSON` 동일, 인자 수도 동일
4. **`detectCrossWarnings`** — 발동 조건·`code` 목록·`level` 전부 불변, `msg`만 그룹별로 갈림.
   전항목 5점에서 경고 0건, 인자 미전달 = `service` 폴백
5. **죽은 오버라이드 0건** — `DOMAIN_DESC_BY_GROUP` 6개가 전부 기본 `desc`와 다르고,
   `WARN_WORDING` 5규칙이 전부 그룹 간 2종 이상
6. **조사 파손** — 받침 코드포인트 검사는 쓰지 않았다(주의사항 ②-1: 28건 전부 오탐이었다).
   실제 파손 패턴 16종을 직접 나열해 검사하고, 20문장 + 6개 `desc`를 육안 확인했다
7. **제어문자 검사** 3개 파일 0건(주의사항 ⑥)
8. **음성 대조군** — `ITEMS` 변조 / `service`에 `하도급법` 재삽입 / 죽은 오버라이드 주입을
   각각 심어 하네스가 잡는지 확인했다. **검사기가 통과만 하는지 검증하지 않으면 "0건"이 틀린 결론이 된다**

### 채택하지 않은 대안

- **`GROUP_MAP` 이름 유지** — DiagMicro와 표면이 어긋난 채로 두면 모듈 혼동이 계속된다.
  이름이 같고 내용이 다른 위험은 "공개 API가 업종 키만 받는다"로 막는 편이 낫다
- **`etc` 전용 그룹 신설** — 기본 `ITEMS`가 이미 중립이라 `service`와 같은 내용이 된다.
  죽은 분기가 하나 늘 뿐이다
- **`WARN_WORDING`을 DiagMicro식 어절 토큰으로** — 조사 파손 위험. 20문장이면 통째로 쓴다
- **`calcScores(scores, group)`로 시그니처 확장** — label을 분기하지 않으므로 불필요하다.
  범위에도 "`calcScores`를 건드리지 마라"가 명시돼 있었다

---

## 작업 이력 (2026-09-17) — 출처 없는 수치 전수 조사 + B 유형 7곳 제거

1단계에서 `90% 절감`·`70% 단축` 두 건을 지운 뒤, **같은 유형이 더 있는지 전수 조사**했다.
숫자가 든 문자열 212건을 전부 덤프해 직접 읽었다(정규식 분류기는 보조로만 — 조사 검사기 오탐 교훈).

**효과를 숫자로 단정하는 문장(`N% 절감/단축/향상/증대/개선`)은 5개 모듈 전체에서 0건**이었다.
1단계에서 지운 2건이 그 유형의 전부였다.

| 모듈 | 숫자 문자열 | 출처 없는 수치 주장 |
|---|---|---|
| `common.js` | 15 | 1건 (A) |
| `diagnosis-micro.js` | 187 | 4종 / 22곳 (A·B·C·D) |
| `diagnosis-social.js` · `diagnosis-venture.js` · `diagnosis-coop.js` | 10 | **0건** |

사회적경제 3유형은 `1인 1표`(협동조합기본법)·`12개월 자금 계획`·`최근 3년간 수주 실적`·
`IMP 5개 축` 등 제도 용어와 사실 기술뿐이라 손댈 것이 없었다.

### ① 이번에 처리한 것 — B: `표준치(5% 이하)` 7곳

`diagnosis-micro.js` `4_1.question` 7곳(base · food · beauty · retail · edu_service ·
manufacturing · pro_service)이 지연율을 **`표준치(5% 이하)`라고 단정**했다. 무엇의 표준인지
출처가 없다. 게다가 **외식 주문 접수 지연율과 제조 납기 지연율에 같은 5%를 쓴다** — 제조 납기
지연 5%는 거래 중단 수준이고 외식 5%는 흔하다. 업종을 넘나드는 단일 수치였다.

→ `표준치(5% 이하)로` → `지연이 반복되지 않는 수준으로`. **수치를 지우면 업종 단일화 문제도 함께 해소된다.**

**왜 이것만 지금 했나**: 단정 제거이지 설계 판단이 아니고, `question` 한 줄씩이라 회귀 위험이 낮다.
A는 대체 수치를 정해야 하고, C·D는 성격이 다르다(아래).

⚠ **`4_1.scale5`에 `5% 이하`가 7곳 더 남아 있다.** 조사 당시 덤프에서 `scale`을 의도적으로
제외해(척도 경계는 응답자 자기 상태 구분이지 주장이 아니다) 보고 목록에 오르지 않았던 것을
이번 작업 중 발견했다. **D와 같은 척도 경계 유형이므로 남겨 뒀다.**
질문은 정성적으로 묻고 5점 척도가 구체적 기준을 주는 형태가 되는데, 구체 수치는 원래
척도에 있어야 할 자리이므로 지금 상태가 오히려 낫다고 판단했다.

### ② 기록만 하고 넘어간 것 — 왜 지금 안 고치는가

**A. 프라임코스트 `60%` — 8곳** (`common.js` 1_1.guide 1곳 + `diagnosis-micro.js` 1_3.guide 7곳)

⚠ **출처가 없는 것보다 두 모듈이 서로 모순되는 것이 더 큰 문제다.**

| 업종 | sme 경로 (`common.js`) | micro 경로 (`diagnosis-micro.js`) |
|---|---|---|
| 제조 | 75% | 60% (manufacturing) |
| IT·전문서비스 | 30% | 60% (pro_service) — **두 배 차이** |
| 유통·소매 | 80% | 60% (retail) |
| 외식 | 60% | 60% (food) — 유일하게 일치 |

**같은 회사가 직원 수만 넘어가면 원가 기준이 두 배로 바뀐다.** micro는 7개 그룹 전부에 60%를
적용하는데, 업종마다 원가 구조가 다르다는 것은 `common.js` 자신이 인정하고 있다.

**왜 지금 안 고치나**:
- 어느 쪽으로 맞출지 정하지 않으면 **한쪽만 고쳐서 모순이 남는다**
- `knowledge_base/small_biz_cost_guide.csv`(10개 업종 비용 구조)가 있어
  **출처 있는 수치로 교체할 여지가 있다 — 지우는 것보다 나은 선택지다**
- 문구 수정이 아니라 설계 판단이고, `diagnosis-micro.js` 7개 그룹을 건드린다

**C. 1위 거래처 비중 `50%` — 4곳** (`trade_logistics 3_1.guide` 1곳 + ACTION_PLAN 3곳)
`1위 거래처 비중이 50%를 넘으면 단가 협상력이 **사라진다**`. 거래처 집중도 위험 자체는 널리
인정되지만 **50%라는 경계선과 "사라진다"는 단정**에 근거가 없다.
**왜 지금 안 고치나**: 경계선을 바꿀지 없앨지가 판단 사항이고, ACTION_PLAN 3곳이 함께 움직여야 한다.

**D. 렌탈·리스비 대비 `1.5배`·`2배` — 4곳** (`4_4.scale4/5` base + manufacturing)
**성격이 다르다 — 척도 경계(응답자 자기 상태 구분)이지 세상에 대한 주장이 아니다.**
다만 구분선 자체에 근거가 없어 응답자가 자기 위치를 판정하기 어렵다. **우선순위 최하.**

### ③ 손대지 않기로 확정한 것 — 규범적 목표치

`가지급금 1% 미만` · `증빙 100%` · `평점 4.0` · `48시간 내 대응` · `전사 업무 30% AI`(전부 `common.js`).
**앱이 제시하는 목표이지 "업계가 이렇다"는 주장이 아니다.** 출처를 요구할 성격이 아니므로 유지한다.

### ④ 검증 (17개 단언 통과)

1. `표준치` **0건**(수정 전 7건), 대체 문구 정확히 7곳,
   잔존 `5% 이하` 7곳이 전부 `scale5`인지 확인
2. 7곳 외 변경 0건 — 키 390개 집합 동일, 변경 항목이 전부 `4_1`,
   `4_1`에서도 `label`·`guide`·`scale`·`ai_trigger` 불변(`question`만, 정확히 그 치환만)
3. 문항 키 35개 순서 동일, `calcScores`(10그룹 + 폴백)·`detectCrossWarnings`
   **5개 시드 전부 동일**, `ai_trigger` 전부 불변
4. A 8곳 · C 4곳 · D 4곳 **전부 그대로**(치환 스크립트에 대조군 assert를 걸어 이중 확인)

치환은 줄 번호가 아니라 문자열 앵커로 했고, 발생 횟수(7)·앞뒤 문맥(`지연율을 ` / ` 관리하는가?`)·
필드(`question:`)를 전부 assert한 뒤 기록했다. 제어문자 0건.

### ⑤ 변경 파일

- `js/diagnosis/diagnosis-micro.js` — `4_1.question` 7곳
- `index.html` — 로컬 `?v=` 53곳 `20260917c` → `20260917d`

---

## 작업 이력 (2026-09-17) — 1단계: DiagCommon 기본 ITEMS 업종 중립화 (분화 없이 15문항)

**그룹 분화 없이 기본 ITEMS만 손봐서 "특정 업종에만 성립하는 문항"을 0건으로 만들었다.**
`INDUSTRY_WORDING`은 `{}` 그대로 두었다 — 33개 오버라이드 집필은 2-2 작업이다.

### ① 왜 지금 분화하지 않는가

기본 ITEMS가 중립이어야 **오버라이드가 없는 그룹이 받는 문구가 안전하다.**
DiagMicro에서 기본 ITEMS를 외식업 기준으로 써서 오버라이드 없는 그룹이 그대로 받은 전례가 있다.
순서를 뒤집으면(먼저 분화 → 나중에 기본 정리) 그 사이 기간 동안 폴백 그룹이 편향된 문구를 받는다.

### ② (c) 본문 중립화 2문항 — label·question·scale 전부

**`2_2`** 는 네이버 플레이스 전용이었다. `guide`의 `"요즘뜨는" 필터 노출 = 최신성 가중치 핵심`,
본문의 `고화질 이미지(5장 이상)·FAQ·소식이 주간 단위로`, scale의 `저화질 사진뿐`·`SEO 키워드 조정`이
전부 플레이스·블로그 전제다. B2B 제조·건설·물류에는 성립하지 않는다.
→ label을 `콘텐츠 최신성 및 알고리즘 대응` → `온라인 정보 최신성 및 검색 대응`으로 바꾸고
질문을 "고객이 우리를 확인하는 온라인 접점이 정기적으로 갱신되는가"로 상위화했다.

**`4_1`** 은 `인스타그램 피드·소식·홍보문구`가 B2C 전제였다. `pro_service`는 2026-09-07에
`온라인 콘텐츠 채널`로 상위화했으나 **DiagCommon은 그때 손대지 않아 남아 있었다.**
→ `AI 활용 홍보·영업 문서 작성`으로 상위화. 견적서·제안서·입찰 서류까지 포함된다.

⚠ **scale까지 함께 바꿨다.** label·question만 바꾸고 scale을 두면 5단계 서술이 옛 업종 문구로
남는다(D1·D2에서 실제로 그랬다). `ai_trigger`의 `content_stale`·`ai_literacy_low`는 그대로 뒀다.

### ③ (b) guide 축 보완 13문항 — 신규 28개 축

조사에서 확인된 누락 축을 추가했다. 건설·물류·시설관리가 가장 많이 빠져 있었다.

| 문항 | 추가한 축 |
|---|---|
| `1_1` | 건설(노무비+외주비) · 물류(유류비+지입료) · 시설관리(인건비가 사실상 전부) |
| `1_2` | 재고 없는 업종 — IT·전문서비스(인건비 선지출~대금 회수) · 시설관리 |
| `2_1` | 건설(면허·등록번호) · 물류(운송 허가) · 의료(의료광고 사전심의 대상 여부) |
| `2_3` | 건설(하자 보수 기간·발주처 평가) · 물류(클레임 처리 속도·화주 평가) |
| `2_4` | 건설(수주 경로별 낙찰률) · 물류(운임 단가·공차율) |
| `3_1` | 건설(공정률·수주 물량) · 시설관리(계약 갱신일·인력 충원) |
| `3_2` | 건설(안전 점검·자재 검수) · 물류(배차·상하차 검수) |
| `3_3` | 건설(공정 사진·일일 보고) · 물류(운송관리시스템·배차·위치 추적) |
| `3_4` | 외식(조리 순서·계량 기준) · 건설(공종별 시공 순서·안전 수칙) |
| `4_3` | 건설(공정·원가 관리·전자 계약) · 물류(운송관리·배차·정산) |
| `5_1` | 배출이 거의 없는 지식서비스 — 사무실 전기·업무 차량만 기록해도 요건 충족 |
| `5_2` | 건설(건설폐기물 처리비) · 시설관리(소모품·교체 부품) |
| `5_4` | 원청 없는 업종 — 소매(입점 플랫폼·임대인) · 의료(보험사·공단) · 교육(위탁 기관·가맹 본부) |

⚠ **`1_1`은 guide만 보완하고 label은 건드리지 않았다.** label 자체가
`매출 대비 재료비+인건비 통제력`이라 재료비가 없는 업종에 안 맞지만, **지금 기본 label을 바꾸면
외식·제조가 손해를 본다**(그쪽은 재료비가 실제로 핵심 지표다). 2-2의 그룹 오버라이드에서
`field_service`·`service` 그룹만 바꾸는 것이 옳다.

같은 작업에서 약자·외래어도 정리했다. `ROAS` → `광고비 대비 매출(ROAS = 광고로 올린 매출 ÷ 광고비)`,
`MES`·`CRM`·`TMS`·`WMS`·`POS`·`CCTV` 전부 풀네임 병기, `오퍼레이션 누수` → `운영이 새지 않는다`,
`식재료 로스` → `버려지는 식재료`, `불량·스크랩` → `불량품·자투리 자재`, `응대 스크립트` → `응대 대본`.

⚠ **확인되지 않은 수치 2건을 함께 제거했다**(주의사항 ⑬) — `3_4`의 `대표 교육 시간 90% 절감`,
`4_1`의 `마케팅 실행 주기 70% 단축`. 출처가 없어 그 자체가 오정보다.

### ④ DOMAINS.desc 2건

- `finance`: `재료비+인건비 통제력…` → `사업에 직접 들어가는 비용의 통제력…`
- `digital`: `로컬 SEO·다채널 최적화…` → `고객이 우리를 찾는 온라인 접점의 정확성·최신성·평판…`

⚠ `desc`는 `guide`가 없는 자리이고 점수 바로 아래 첫 화면에 보이므로 약자를 쓰지 않는다.
`id`·`key`·`label`·`weight`는 건드리지 않았다 — 점수 계산이 의존한다.

### ⑤ 검증 (24 + 6 = 30개 단언 통과)

수정 전(`git show HEAD:`)과 수정 후를 같은 프로세스에 로드해 비교했다.

1. `2_2`·`4_1` 본문 B2C 전용어 **0건** (수정 전 2건·4건 → 0건·0건).
   검사 대상은 `label`·`question`·`scale`만 — `guide`에는 업종어가 의도적으로 들어간다
2. 13문항 guide에 지정 축 전부 존재. **죽은 수정 0건**(전부 실제로 바뀜 — 주의사항 ⑧)
3. `DOMAINS.desc` 2건 중립화, 나머지 3건 불변, `id`·`key`·`label`·`weight` 전부 불변
4. (a) 5문항(`1_3`·`1_4`·`4_2`·`4_4`·`5_3`) 직렬화 완전 동일
5. 문항 키 20개 순서까지 동일, `ai_trigger` 20개 불변, 전 문항 scale 5단계 유지,
   `calcScores`·`detectCrossWarnings` **5개 시드 전부 동일**, `INDUSTRY_WORDING={}` 유지 확인
6. `1_1`의 label·question·scale 전부 불변 (2-2로 미룬 것)
- 변경 범위: 문항 정확히 15개, 그중 본문이 바뀐 것은 `2_2`·`4_1` 뿐 — 나머지 13개는 guide만
- 제어문자 0건, 조사 파손 패턴 0건, ㄹ받침+`으로` 0건, guide 약자 전부 병기

⚠ **조사 검사기를 처음에 잘못 만들어 28건이 전부 오탐이었다.** 받침 코드포인트로 조사 일치를
전수 검사했더니 `없는`·`찾는`·`남는`(용언 관형형)과 `평가`·`단가`·`불가`·`상가`(명사 일부)를
조사로 셌다. **형태소 분석 없이 조사와 어미는 구분되지 않는다.** 실제 파손 패턴을 직접 나열하는
방식으로 바꾸고, 변경된 29개 문자열은 전부 육안으로 확인했다.
조사 파손은 *받침이 바뀌는 단어 치환*에서만 생기는데 이번은 문장을 통째로 새로 쓴 것이라
구조적으로 그 경로가 없다 — 그래도 검사는 남겨 둔다.

### ⑥ 변경 파일

- `js/diagnosis/common.js` — ITEMS 15문항(본문 2 + guide 13) · `DOMAINS.desc` 2건
- `index.html` — 로컬 `?v=` 53곳 `20260917b` → `20260917c`

### ⑦ ⚠ 2-2 착수 시 반드시 처리할 것 — `1_1` label 그룹 오버라이드

**`1_1`의 label `매출 대비 재료비+인건비 통제력`은 1단계에서 의도적으로 남겨 뒀다.**
재료비가 없는 업종(건설·물류·시설관리·IT·전문서비스)에는 맞지 않지만,
**기본 label을 바꾸면 외식·제조가 손해를 본다** — 그쪽은 재료비가 실제로 핵심 지표다.
guide에 "재료비가 없는 업종은 그 자리에 해당 비용을 넣어 계산한다"를 넣어 임시로 막아 뒀다.

→ 2-2에서 **`field_service`·`service` 그룹 오버라이드로 처리**한다.
`manufacturing`·`trade_retail`은 기본 label이 정확하므로 오버라이드하지 않는다.
⚠ label을 바꾸면 `question`·`guide`·`scale`도 함께 봐야 한다(네 필드 규칙).

---

## 작업 이력 (2026-09-17) — 결함 1: COMMON_WORDING_MAP 제거 + DiagCommon 4그룹 배선 (2-1 단계)

**당초 지시는 "wizard.js의 COMMON_WORDING_MAP을 DiagCommon의 INDUSTRY_WORDING으로 흡수"였으나,
조사 결과 흡수할 유효 내용이 한 건도 없어 제거 + 배선으로 방향을 바꿨다.**

### ① 왜 흡수가 불가능했나 — 맵이 겨냥한 스키마가 이미 삭제돼 있었다

`COMMON_WORDING_MAP` 11개 업종 문구는 **구 `COMMON_DIAGNOSIS` 스키마를 겨냥한 것**으로,
`08d6dad`(진단 모듈 v2.0)가 **id를 재사용한 채 문항을 전면 교체**하면서 무효화됐다.

| id | 구 COMMON_DIAGNOSIS (맵이 겨냥) | 현 DiagCommon v2.0 (실제로 덮인 것) |
|---|---|---|
| `1_1` | 매출성장률 (`numeric`) | 재료비+인건비 통제력 (bars) |
| `1_2` | 영업이익률 (`numeric`) | 돈이 묶여 있는 기간 (bars) |
| `1_3` | 재구매·재방문율 (`numeric`) | 손익분기점(BEP) 인지 (bars) |
| `3_2` | 차별화 요소 (`mixed`) | 역할·책임 명료화 (bars) |

**id는 맞고 내용이 안 맞는다 — 그래서 조용했다.** id가 어긋났다면 오버라이드가 발동조차 안 해
버그가 없었을 것이다. 맵의 `benchRef: '제조업 매출성장률 평균'`이 구 `1_1`에 정확히 대응하는 것이
겨냥 대상을 확정하는 근거다.

전수 감사 결과 **오버라이드 필드 86개 중 75개가 무효, 11개만 실효**했고,
그 11개(`1_3.anchors`)도 구 재구매율 문항용이라 현 문항과 대상이 다르다.
무효 사유는 현 DiagCommon 20문항의 `type`이 **전부 `null`**이라 `numeric`·`mixed` 분기에
진입하지 않기 때문이다. `NUMERIC_BENCH_REF_DEFAULT`도 같은 이유로 100% 무효였다.

```
무효 75 : text 11 · inputLabel 11 · placeholder 11 · benchRef 20 · choices 11 · noneValue 11
실효 11 : 1_3.anchors × 11개 업종
```

⚠ **원문(11개 업종 재발주율·재방문율 척도와 출처 표기 벤치마크 수치)은 `1c4c259` 시점
`js/wizard.js`에 있다.** 2-2에서 재구매율 문항을 되살릴 경우 원재료로 쓸 것 —
`git show 1c4c259:js/wizard.js`.

### ② 그 사이 사용자에게 실제로 보이던 결함 2개

둘 다 `_applyIndustryWording`이 **구 스키마 키를 화이트리스트로 다시 담는** 방식이라 생겼다.

**버그 A — 영역 제목이 전부 빈 문자열 (sme 경로 17개 업종 전부).**
함수가 `{title, description, insights}` / 영역 `{id, title, description, items}`만 골라 담는데
`_diagCommonToAreas`는 `{id, label, icon, ...}`를 만든다. `label`·`icon`이 탈락해
`renderDiagModule`의 `area.label || area.title`이 빈 문자열이 됐다. 예외가 안 나서 조용했다.

**버그 B — 질문과 5단계 척도가 서로 다른 것을 말함 (11개 업종).**
두 렌더 함수의 우선순위가 반대였다 — `_renderItemHtml`은 `item.question` 우선이라 원본 질문이 이기고,
`_renderBars`는 `item.anchors` 우선이라 오버라이드 척도가 이긴다. `Object.assign`이 둘 다 남겨서
**BEP를 묻고 재발주율로 채점**했다. 이 점수는 `detectCrossWarnings`의 `intuition_mgmt`
(원가·BEP·목표 동시 저점) 판정에 그대로 들어갔다.

⚠ **`_injectDxDetect`도 같은 화이트리스트 결함을 갖고 있었다.** `_applyIndustryWording`만 제거하면
영역 `<h4>`는 살아나지만(`concat`이 객체를 그대로 통과시킨다) 모듈 `<h3>`은 여전히 비었다 —
`_injectDxDetect`가 `label`을 떨어뜨리기 때문이다. `Object.assign({}, diagData, {areas})`로 바꿔
**원본을 통째로 복사한 뒤 areas만 교체**하도록 고쳤다. 화이트리스트로 다시 담는 패턴 자체가 원인이므로
같은 패턴을 남겨두면 스키마가 또 바뀔 때 동일 사고가 재발한다.

### ③ 채택하지 않은 대안

- **기계적 흡수(당초 지시)** — 옮길 유효 내용이 없다. 옮기면 질문·척도 불일치만 `common.js`로 따라간다.
- **11개를 현 20문항에 맞춰 재집필** — 2-2에서 4그룹 33개 오버라이드를 설계·집필할 예정이라
  지금 11개를 따로 쓰면 그때 다시 손봐야 한다. 게다가 살릴 수 있는 11개가 전부 재구매율 문항용인데
  현 DiagCommon에 재구매율 문항 자체가 없어 붙일 자리가 없다.
- **배선 없이 삭제만** — 2-2가 배선부터 시작하게 된다. 4그룹 매핑이 이미 확정됐으므로
  지금 까는 것은 투기가 아니고, `INDUSTRY_WORDING = {}`이면 동작이 동일해 검증으로 증명된다.

### ④ 배선 스캐폴드 — 내용은 한 건도 쓰지 않았다

`common.js`에 `GROUP_MAP`(17업종 → 4그룹) · `getGroup()` · `getSchema(industryKey)` 병합 로직 ·
`INDUSTRY_WORDING = {}`만 추가했다. 오버라이드 0건이므로 `getSchema`는 기본 `ITEMS`를
**동일 참조로** 반환한다(사본조차 만들지 않는다) — 수정 전과 완전히 같다.

⚠ **`getSchema`는 그룹이 아니라 업종 키를 받는다.** DiagCommon의 4그룹 매핑이 DiagMicro의 10그룹과
달라서(`logistics` → DiagCommon `field_service` / DiagMicro `trade_logistics`) **호출부가 그룹을
계산하는 순간 실수가 난다.** 업종→그룹 변환은 DiagCommon의 지식으로 가둔다.
미전달·빈 문자열·`null`·미등록 오타는 전부 기준 그룹 `service`로 폴백한다.

⚠ 2-2에서 오버라이드를 쓸 때 **`label`·`question`·`guide`·`scale` 네 필드를 함께 덮을 것.**
일부만 덮으면 나머지가 기본 `ITEMS`에서 상속돼 이번 버그 B와 같은 상태가 된다.
`key`·`weight`·`id`·`ai_trigger`는 분기 금지 — 점수 계산과 교차 경고가 의존한다.

### ⑤ 변경 파일

- `js/wizard.js` — `COMMON_WORDING_MAP`·`NUMERIC_BENCH_REF_DEFAULT`·`_applyIndustryWording`
  제거(**-299줄**), `_injectDxDetect` 통과 방식으로 수정, `_diagCommonToAreas(diagCommon, industryKey)`
- `js/diagnosis/common.js` — `GROUP_MAP`·`getGroup()`·`INDUSTRY_WORDING`·`getSchema(industryKey)` 추가
- `index.html` — 로컬 `?v=` 53곳 `20260917a` → `20260917b`

### ⑥ 검증 (18개 단언 전부 통과)

수정 전(`git show HEAD:`)과 수정 후 코드를 같은 프로세스에 각각 로드해 렌더 파이프라인을 재현했다.

1. `INDUSTRY_WORDING={}`에서 `getSchema` 반환값이 수정 전과 **직렬화 동일** — 22개 입력
   (17업종 + etc + 미전달·빈문자열·null·오타). `items`가 기본 `ITEMS`와 **동일 참조**
2. 버그 A 해소 — 수정 전 `<h3>`·`<h4>` 모두 `""` 재현 → 수정 후 `"공통 경영 진단"` /
   `"💰 재무·원가 건전성"`. 17업종 × 5영역 = **85개 영역 제목**이 `DOMAINS`와 일치
3. 버그 B 해소 — mfg_parts `1_3` 척도가 `재발주 30% 미만` → `BEP 모름`.
   17업종 전 문항에 오버라이드 `anchors` 잔존 **0건**
4. 17업종 → 4그룹 변환 18건 전부 일치, 폴백 4종 정상, `logistics` → `field_service` 확인
5. `calcScores`·`detectCrossWarnings`·`buildPromptSummary` 직렬화 **전부 동일**
6. `_injectDxDetect` 정상 — 6영역(진단 5 + DX 1), `_signalOnly` 유지,
   점수 대상 문항 수 **20 불변**

제어문자 검사(0x00~0x1F, 개행·탭 제외) 0건 — 줄 범위 splice에 앵커 assert를 걸어
잘못된 위치를 지우는 것을 막았다(실제로 앵커가 2회 불일치해 오삭제를 사전 차단했다).

---

## 작업 이력 (2026-09-17) — ⚠ 공통 진단 결과가 AI에 전달되지 않던 문제 (0-1 단독 수정)

**중소기업(sme) 사장님이 공통 진단 20문항을 답해도 AI 보고서에 반영되지 않고 있었다.**
DiagCommon 업종 분화 조사(0단계~2단계 계획) 중 발견해 **가장 시급하다고 판단, 단독으로 먼저 처리**했다.

### ① 결함 A — 객체 맵을 넘겨 전 영역이 NaN이 됐다
[ai-engine.js:1370](js/ai-engine.js#L1370)이 `DiagCommon.buildPromptSummary(d.diagScores)`를 호출하는데,
`d.diagScores`는 `{key: {score, memo}}` **객체 맵**이고 `buildPromptSummary`는 **평면 숫자 맵**을 기대한다.
`Number({score:3})` = `NaN`이므로:

```
종합 점수: NaN점 / 100점
  - 재무·원가 건전성: NaN점 (위험)     ← 5영역 전부
[복합 경고 신호] - 복합 경고 없음        ← NaN <= 2 가 false라 5개 규칙 전부 미발동
[즉각 처방 필요 항목] - 없음             ← 동일
```

⚠ **점수가 0으로 나간 게 아니라 NaN이면서 등급만 "위험"으로 찍혔다.**
진단 결과는 전달되지 않는데 전 영역이 위험으로 라벨링돼 AI가 그 전제로 보고서를 썼다.

⚠ `collectAllScores()`가 평면 숫자 맵을 만들지만 **`wizard.js` 클로저 내부이고 export되지 않는다.**
호출부에서 `d.diagScores`를 평면화하는 쪽을 택했다 — `collectAllScores()`의 평면 숫자 계약을
건드리지 않고(`DiagMicro.calcScores`·`CrossContext.buildScoreMap`이 의존한다), 스냅샷(`d`)을 쓰므로
Wizard의 현재 DOM 상태에 의존하지 않는다.

### ② ⚠ 조사 중 나온 더 큰 것 — 경로 가드가 없어 micro·창업초기도 이 블록을 받고 있었다
주입 조건이 `window.DiagCommon && d.diagScores`뿐이라 **bizScale·경로와 무관하게 전원**에게 붙었다.
micro는 점수 키 접두어가 `diag-micro-container_`, 창업 초기는 `diag-common-container_s1_1`이라
DiagCommon 문항 키(`1_1`~`5_4`)가 하나도 매칭되지 않는다 → 전 항목 0점.

**전 문항 4점(양호)을 답한 소상공인이 받던 것** (실측):
```
종합 점수: 0점 / 100점 · 5영역 전부 0점 (위험)
⚠ [CRITICAL] 원가·BEP·목표 모두 감에만 의존하는 직감 경영 상태입니다.
⚠ [CRITICAL] 근로기준법·하도급법 위반 리스크가 동시에 높습니다.
⚠ [HIGH] 로컬 검색 노출도 AI 활용도 모두 최하위입니다.   (외 2건)
[즉각 처방 필요 항목] 20문항 전부 (0점)
```
**사용자가 답한 적 없는 내용이다.** 2026-09-08에 `collect()`의 micro 분기에서 고친
*"허위 진단이 AI로 나가고 있었다"* 와 같은 유형이며 **여기만 남아 있었다.**

→ **DiagCommon 문항에 실제 응답(0점 초과)이 하나라도 있을 때만 주입**하도록 고쳤다.
`bizScale`로 판정하지 않고 **점수 키로 판정**한다 — micro·창업초기·사회적경제를 한 번에 거르고,
`_diagPathOf`를 ai-engine으로 끌어오지 않아도 된다(`collect()`가 DiagSme에 쓰는 접두어 가드와 같은 방식).

### ③ 변경 범위
**`js/ai-engine.js` 한 곳뿐이다.** `common.js`·`wizard.js`·진단 문항·점수 로직은 건드리지 않았다.

### ④ 검증 (8케이스)
| 케이스 | 결과 |
|---|---|
| sme 전항목 2점 (객체 맵) | **주입** · 종합 40점 · NaN 0건 ✓ |
| sme 전항목 4점 (객체 맵) | **주입** · 종합 80점 ✓ |
| sme 평면 숫자 맵 | **주입** · 종합 40점 — 두 형태 모두 동작(호환) ✓ |
| **micro 전항목 4점** | **생략** ✓ (수정 전: 0점·CRITICAL 2건 주입) |
| **창업초기(STARTUP) 전항목 4점** | **생략** ✓ |
| 사회적기업 S1~S8 | **생략** ✓ |
| `diagScores` 없음 / 빈 객체 | **생략** ✓ |
| 혼합 점수 + 일부 미응답 | 영역별 50/40/47/53/60 · 경고 1건 정확 ✓ |

⚠ 검증은 **`ai-engine.js`에서 주입 블록 원문을 잘라 실행**했다(코드 복제 검증이 아니다).

### ⑤ ⚠ 드러난 잠재 결함 — 미응답 문항이 "즉각 처방 필요"로 잡힌다
`buildPromptSummary`의 `if (val <= 2)`가 미응답(`0`)도 포함해 `온라인 기업 정보 일치도(0점)`처럼
**답하지 않은 문항을 "2점 이하 즉각 처방 필요"로 AI에 보고**한다.
수정 전에는 NaN이라 이 목록이 아예 비어 있어 드러나지 않았다.

**지금은 도달 불가능하다** — `validate(2)`가 전 문항 응답 전까지 step3 진행을 막는다.
DiagMicro는 `_scoreDistBlock`이 `score > 0`만 세어 이 문제가 없다(2026-09-07).
→ **0-1 범위를 넘으므로 고치지 않았다. 1단계에서 `val > 0 && val <= 2`로 처리할 것.**

### ⑥ 캐시버스팅
`?v=` 53곳 `20260917a`

---

## 최근 수정 이력 (2026-09-16) — D2 영역 제목(label) 그룹 분기 · MICRO_DOMAIN_EXPLAIN 중립화

**문항은 10그룹 전부 중립화했는데 영역 제목과 해설만 소매·외식 전제로 남아 있었다.**
건설·제조·물류·시설관리 사장님이 **진단 화면에서 제일 먼저 보는 것**이 `점포환경·네이버 플레이스`였다.

### ① ⚠ "label은 절대 분기하지 않는다"는 원칙을 이번에 뒤집었다 — 근거
그 원칙은 *"`calcScores`가 `domain.label`을 레이더차트·대시보드로 흘려보내므로 바꾸면 표시가 어긋난다"*였다.
전수 추적 결과 **label이 키로 쓰이는 곳은 한 군데도 없고 전부 표시 문자열**이며,
`HistoryTracker`는 5대 역량(`finance`·`hr`·`bm`·`future`·`differentiation`)을 쓰므로 D1~D7과 무관하다.
→ **분기가 안전하다.** 다만 원칙이 가리키던 위험은 실재했다 — **label 소스가 4곳으로 흩어져 있었다.**

| | 소스 | 표시되는 곳 |
|---|---|---|
| A | `getSchema` | 진단 화면 영역 제목 |
| B | `calcScores` | PPT `_microRows` · `fd.scaleScores` |
| C | `wizard.js _calcMicroDomainScores` | **레이더차트 7축 · 점수 바** |
| D | `wizard.js MICRO_DOMAIN_EXPLAIN` | 해설 카드 |

⚠ **C는 label을 하드코딩하고 있었다.** 한 곳만 고쳤으면 진단 화면과 레이더차트가 다른 이름을 표시한다.
→ **`_domainsFor(group)` 한 함수를 신설해 A·B가 공유**하고, **C는 `DiagMicro.getDomains(group)`에서 파생**시켰다.
D만 성격이 달라(해설 문장) `MICRO_D2_EXPLAIN_BY_GROUP`을 따로 뒀다.

⚠ **하드코딩이 낳은 오타도 함께 드러났다** — C에만 `'D1. 경영진단·손익분析'`(한자 析)이 있었다.
같은 값을 네 곳에 적는 구조였기 때문에 한 곳만 틀려도 아무도 모른다.

### ② 확정된 D2 label (D1·D3~D7은 업종 무관 · 공통)
| 그룹 | D2 label | 그 그룹의 D2가 실제로 재는 것 |
|---|---|---|
| food / beauty | **점포환경·온라인 노출** | 매장 전면 + 온라인 등록 정보 |
| retail | **매장·온라인 노출** | 첫 접점 상품 인지성 + 창고·출고·정산 동선 |
| edu_service | **강의 환경·온라인 노출** | 첫 접점 인지성 + 강의 공간·대기 동선 |
| pro_service | **전문성 노출** | 전문 분야·실적 노출 + 대기·접수 동선 |
| manufacturing | **기업 신뢰도 노출** | 보유 설비·인증 + 공정 동선·로트 편차 |
| construction | **시공 실적·발주처 신뢰** | 시공 사례·면허 + 현장 출입·마감 편차 |
| trade_logistics | **거래처 신뢰도 노출** | 보유 자원·처리 실적 + 거점 동선 |
| facility_service | **관리 실적·현장 운영** | 관리 실적·인력 자격·허가 + 현장 안전 |
| etc | **온라인 노출** | 첫 접점 인지성 + 온라인 정보 일치 |

**D7은 전 그룹 공통 `온라인 홍보·AI 활용`**(기존 `SNS·생성형AI`) — 7_1~7_5가 그룹별로 갈릴 내용이 없다.

### ③ ⚠ 같은 화면의 "솔루션 미리보기" 카드가 label과 모순되고 있었다
`showDiagReveal`의 micro `preview` 6줄이 **`D2 네이버 플레이스·상권 SEO 최적화`·`D3 배달앱·SNS`·`D1 BEP·ACM`**로
하드코딩돼 있었다. 영역 제목만 고치면 **같은 화면에서 D2가 두 이름으로 보인다.**
→ preview의 D2는 `domainScores.d2.label`을 그대로 끌어다 쓰게 하고 나머지 5줄은 중립화했다.

### ④ MICRO_DOMAIN_EXPLAIN — 2026-09-07 전문용어 정리의 사각지대였다
그때 검사 범위가 `diagnosis-*.js`였고 **`wizard.js`가 빠져 있어** 해설 카드에 약자가 그대로 남아 있었다.
| before | after |
|---|---|
| `ACM 관리와 프라임코스트 최적화` | `손이 가는 만큼 남는지 계산하는 습관과 재료비·인건비 비율 관리` |
| `손익분기점(BEP)` | `손익분기점(BEP, 적자와 흑자가 갈리는 매출액)` |
| `네이버 플레이스·로컬SEO 최적화` | 그룹별 D2 해설로 분기 |
| `디지털 전환(DX)` | `디지털 전환(DX, 수작업을 디지털 도구로 바꾸는 것)` |
| `키오스크·POS` | `업무 자동화와 데이터 관리` |
| `소진공` / `ESG·녹색보증` | `소상공인시장진흥공단` / `환경·사회 분야 보증` |
| `SNS 운영·생성형AI` | `온라인 홍보 운영과 생성형 AI 활용` |
| `ChatGPT·클로바X로 메뉴 사진` | `ChatGPT(대화형 AI)·클로바X로 사진과 글` |

⚠ **d3는 분기하지 않고 중립화만 했다.** 문제는 `배달` 하나였고 판로 개념 자체는 모든 업종에 통한다 —
D2처럼 아예 성립하지 않는 경우와 다르다. **`what`만 고치면 반쪽이라 `high`·`low`도 함께 처리했다**
(`배달앱 1개라도 추가 등록` → `판로를 하나라도 더 확보`, `다채널 판로가 잘 구축` → `채널별 수수료를 뺀 실수익 비교`).

### ⑤ 검사 중 추가로 드러난 `DOMAINS.desc` 약자 4건 — 같은 원칙 위반이라 함께 고쳤다
CLAUDE.md에 *"`DOMAINS.desc`에는 약자를 절대 넣으면 안 된다"*가 있는데 `로컬 SEO`가 3그룹에,
`플레이스 CTR`이 기본 desc에 남아 있었다. **desc는 영역 제목 바로 아래 첫 화면에 보인다.**
- `beauty`·`edu_service`·`pro_service` D2 — `로컬 SEO` → `지역 검색 노출`
- 기본 D7 — `플레이스 CTR 최적화` → `플레이스 노출 대비 반응률 관리`
- `retail` D2 — `매장 전면(파사드)·네이버 플레이스` → `첫 접점 상품 인지성`(3단계에서 본문만 중립화하고 desc를 빠뜨렸다)
- `manufacturing`·`construction`·`trade_logistics`·`etc` D7 desc **신설** — 기본 desc가 `플레이스`를 전제해 B2B에 성립하지 않는다(`facility_service`가 이미 같은 이유로 갖고 있던 처리)

⚠ **`food`·`beauty`의 D2 desc는 `매장 전면`을 유지했다.** 점포 업종이라 실제로 성립한다.

### ⑥ 검증 (전 항목 통과)
| 항목 | 결과 |
|---|---|
| **[1] ⚠ 4곳 일치** | 10그룹 전부 **A(getSchema) = B(calcScores) = C(레이더) = D(AI 프롬프트)** ✓ |
| **[2] D2 label** | 10그룹 확정안과 **완전 일치** ✓ |
| **[3] D7** | 10그룹 전부 `온라인 홍보·AI 활용` ✓ |
| **[4] group 미전달 폴백** | `undefined`·`null`·`''`·미등록 키 전부 **공통 label**, `undefined` 0건 ✓ |
| **[5] 점수 불변** | 1·3·5점 × 10그룹 = 30조합 `avg`·`pct`·`weight`·`total` **불일치 0** · `key`·`id`·`icon` 불변 ✓ |
| **[6] 해설 약자** | `ACM`·`프라임코스트`·`로컬SEO`·`D2C`·`CTR`·`SEO`·`POS`·`키오스크` 등 **15종 0건** ✓ |
| **[7] d3 high·low** | `배달`·`오프라인`·`SNS`·`앱` **0건** ✓ |
| 문항 불변 | 10그룹 × 4규모 `items` **JSON 완전 동일**(guide·scale·ai_trigger 포함) · `ACTION_PLAN` 동일 · 35문항 ✓ |
| `DOMAINS.desc` | 전 그룹 약자 **0건** ✓ |
| 레이더차트 | 10그룹 전부 **7축 산출 · 0점 영역 0개** · 한자 혼입 0건 ✓ |
| d2 해설 분기 | 10그룹 전부 **공통 해설과 다름**(실효 분기 확인) ✓ |

### ⑦ wizard.js 전체 전용어·약자 검사 결과 (사용자 노출 문자열 한정)
| 분류 | 판정 |
|---|---|
| `ACM`·`SEO`·`CTR`·`D2C`·`파사드`·`진열`·`프라임코스트` | **0건**(이번에 전부 제거) |
| `BEP`·`DX`·`MVP`·`SOP`·`USP`·`KPI`·`NPS`·`MRR`·`SVI` | 전부 **풀네임·설명 병기 상태** — 유지 |
| `POS`·`CRM`·`배달앱` (`550`~`552`) | **DX 탐지 문항의 scale** — 문항이라 이번 범위 밖. `POS`는 2026-09-07에 "이미 익숙한 용어"로 유지 승인됨 |
| `시술`(`409`·`3633`) · `HACCP`·`OEM`·`ISO` | **해당 업종의 자기 업종어·예시**(의료·식품·제조) — 정상 |
| `B2B`·`B2C`(BM 선택지) · `DART`·`OCR`·`API`(시스템 메시지) | 범위 밖 |

### ⑧ 캐시버스팅
`?v=` **53곳** 전부 `20260916d`

### ⑨ 남은 이슈
- **`DiagCommon`(sme 경로)의 `DOMAINS`에는 여전히 그룹 구조가 없다.** 이번 작업은 `DiagMicro` 전용이다
- `dashboard.js`·`ppt-export.js`가 아직 `_diagPathOf`를 쓰지 않는다(기존 이슈, 변동 없음)

---

## 최근 수정 이력 (2026-09-10) — 5단계: local_service 분리 · facility_service 신설 (5-1 골격)

**⚠ 이 작업은 1~4단계(그룹 배정)와 위험 성격이 다르다. AI 업종 판별 결과가 전 사용자에게 바뀐다.**

### ① 문제 — local_service 하나에 성격이 완전히 다른 업종이 섞여 있었다
`api/analyze-biz.js`의 분류 기준 원문:
> `local_service` : 헤어·네일·피부샵, 세탁소, 수선집, 반려동물샵, 필라테스·요가, **주유소**,
> 사업시설유지관리(FM)·빌딩관리·경비·청소대행·방역·소독 등 생활밀착·**B2B 위탁서비스**

이들이 전부 `beauty` 그룹의 **"시술·예약 밀도·재방문 주기"** 문구를 받았다.

### ② ⚠ 조사에서 나온 더 큰 문제 — local_service가 **폴백 자리를 겸하고 있었다**
| 위치 | 코드 |
|---|---|
| `api/analyze-biz.js:113` | `if (!INDUSTRY_KEYS.includes(...)) result.industry_key = 'local_service'` |
| `js/wizard.js:1412` | `... \|\| INDUSTRY_MAP[industry] \|\| 'local_service'` |
| `js/app.js:114` | `... \|\| 'local_service'` |

**`local_service`는 "생활서비스"이자 "AI 판별 실패 시 기본값" 두 역할을 겸했다.**
1단계에서 `getGroup` 폴백을 `food` → `etc`로 고쳤지만 **그보다 상위 단계인 `industryKey` 폴백이
막고 있어 `etc`가 쓰이지 않았다.** 판별에 실패한 사용자가 계속 미용 문항을 받았다.
→ **세 곳 모두 `etc`로 변경했다.** 이번 조사의 최대 성과다.

### ③ ⚠ facility_service를 pro_service에 넣지 않고 신설했다 — 판단 근거
초안은 `pro_service` 배정이었으나 **문항을 실제 대입하면 성립하지 않는다:**
- `"파일실·상담실·접수대 동선"` — 경비업체에 상담실이 없다
- `"크몽·숨고 입점"` — FM은 그런 플랫폼으로 수주하지 않는다
- `"상담 완료 고객 리뷰 요청"` — 정기 도급에 리뷰 개념이 없다

**결정적인 것은 원가 구조다.** `pro_service`의 `1_3`은 `외주비+인건비`인데
**FM·경비·청소는 인건비가 원가의 대부분**이고, 그것이 이 업종의 핵심 진단 축인데
기존 어느 그룹으로도 잡히지 않는다.
⚠ **기존 그룹에 끼워 넣으면 local_service(미용)에서 겪는 문제를 pro_service(상담업)로
옮기는 것뿐이다.** 이번 5단계의 목적은 부적합한 그룹 배정을 없애는 것이므로 신설했다 — **10번째 그룹.**

### ④ 확정된 분리
| 키 | 대상 | 그룹 |
|---|---|---|
| `local_service` (유지) | 헤어·네일·피부샵, 세탁소, 수선집, 반려동물샵, 필라테스·요가 — **개인 고객 대면** | `beauty` 유지 |
| **`facility_service`** (신설) | FM·빌딩관리·경비·청소대행·방역·소독 — **정기 도급 계약** | **`facility_service`** 신설 |
| `주유소` (이동) | → **`wholesale`** | `retail` |
| `etc` | 업종 미판별 폴백 | `etc` |

⚠ **세탁소·수선집·반려동물샵은 `local_service`에 남겼다.** `beauty` 문항을 실제 대입해 검증했다 —
`1_4`(시술별 재료비+소요 시간) · `2_4`(작업 시간 편차) · `3_5`(단골 재방문) · `7_3`(리뷰)이
접수 → 작업 → 인도 구조에 그대로 성립한다.
⚠ **주유소는 `facility_service`가 아니라 `wholesale`이다.** 불특정 고객 대상 현장 판매·유류 재고
회전·단가 관리는 소매 구조이며, FM·경비의 정기 도급과 정반대다.
같은 그룹에 넣으면 지금 고치려는 문제를 반복하게 된다.

### ⑤ 5-1에서 한 것 (골격)
`api/analyze-biz.js` — `INDUSTRY_KEYS`에 `facility_service`·`etc` 추가 · 분류 기준 3항목 재작성 ·
주의사항 3줄 교체 · `[16개 분류]` → `[18개 분류]` · **폴백 `local_service` → `etc`**
`js/wizard.js` — `INDUSTRY_MAP` · `INDUSTRY_BM_MAP` · `_BIZ_PLACEHOLDERS` ·
`updateRiskPlaceholder` · **폴백 → `etc`** (5곳)
`js/app.js` — **폴백 → `etc`**
`js/diagnosis/diagnosis-micro.js` — `INDUSTRY_GROUP_MAP` · `GROUP_LABELS` · `WARN_WORDING`(8필드) ·
`DOMAIN_DESC_BY_GROUP`(도메인 1~4) · `INDUSTRY_WORDING` 빈 객체(5-2 앵커)
`index.html` — `<option value="facility_service">`

⚠ **`analyze-biz.js`만 바꾸면 위험하다.** 새 키가 반환되는데 매핑이 없으면 `getGroup`이
`etc`로 떨어져 의도하지 않은 중립 문항이 나간다. **매핑·그룹 배정·option까지 한 커밋에 묶었다.**

### ⑥ ⚠ etc 폴백의 부작용 확인 — "51 → 35로 줄어드는" 일은 없다
`industryVarMap`에 `etc`·`facility_service`가 없어 **업종 특화 16문항이 렌더링되지 않는다.**
확인 결과 [wizard.js:1514~1522](js/wizard.js#L1514)가 이미 완결적으로 처리한다:
- `industryData`가 없으면 **탭 버튼 자체를 `display:none`** 처리하고 컨테이너를 비운다
- `_cntSuffix(c)`는 `c > 0`일 때만 숫자를 붙인다(2026-09-01 *"잘못된 숫자를 보여주느니 빼는 편이 낫다"*)

→ 사용자는 **업종 탭이 처음부터 없는 상태**로 `0 / 35`를 보며, 세션 중 분모가 바뀌지 않는다.
**업종을 모르는데 특정 업종 16문항을 주는 것보다 낫다**고 판단해 그대로 두었다
(지금까지는 미판별 사용자가 `local_service` 미용 16문항을 받고 있었다).

### ⑦ 검증 (프롬프트 11/11 · 매핑 12/12 · 구조 전항목)
| 항목 | 결과 |
|---|---|
| **AI 프롬프트 문자열** | 새 키 2개 · 분류 정의 2개 · 주의사항 3줄 · `[18개 분류]` · 폴백 `etc` · **`local_service`에서 FM·경비·주유소 제거 확인** ✓ |
| **매핑 누락 전수** | wizard 5곳 · app 1곳 · micro 5곳 · index 1곳 **12/12** ✓ |
| **⚠ 10그룹 키 개수** | 전 그룹 **35/35**, facility_service 실효 0(의도) — 경계 침범 없음 ✓ |
| 부속 테이블 폴백 | facility_service 경고 = `"인건비+운영비 60% 초과 … 계약 정리"` · D1 desc = `"현장별 배치 인원과 실질 인건비"` — **food 폴백 안 함** ✓ |
| **기존 9그룹 불변** | `items`·`domains`·`ACTION_PLAN` 5키 전부 **JSON 동일** ✓ |
| calcScores·guide | 1·3·5점 동일 · guide **10그룹 × 35 = 350건** ✓ |

### ⑧ 캐시버스팅
`?v=` 52곳 `20260910c`

### ⑩ 5-2 — facility_service 문항 오버라이드 26문항 (2026-09-10)

**이 업종은 인건비가 원가의 대부분이라 다른 어느 그룹과도 원가 구조가 다르다.**
5-1까지는 그룹만 신설되고 오버라이드가 비어 있어 **중립 기본 문항**이 나갔다.

#### ⚠ D1을 인건비 축으로 다시 썼다 — 이 그룹 신설의 근거 그 자체
| 문항 | 내용 |
|---|---|
| **`1_3` 실질 인건비 비율 통제** | 배치 인원 × 시급 × 근무 시간이 **계약 단가 대비 몇 퍼센트**인가. scale3이 **"기본급 기준으로는 보지만 4대보험 사업주 부담분·퇴직충당금·연차수당을 빼고 계산함"** — 실무에서 가장 흔한 상태를 3점에 두었다 |
| **`1_4` 계약별로 실제 남는 금액** | scale5가 **"수익성이 낮은 계약은 갱신 시점에 정리하고 인력을 남는 계약으로 재배치"** |

⚠ **`1_3` guide에 실질 인건비 산식을 명시했다** —
`기본급 + 4대보험 사업주 부담분 + 퇴직충당금 + 연차수당 + 야간·휴일 가산수당`.
**기본급만 넣고 계산하면 남는 줄 알았는데 적자인 현장이 나온다.**
⚠ `1_4` guide 첫 줄은 **"배치 인원이 많은 계약이 반드시 남는 계약은 아니다"** — 도급액이 커도
인원·근무 시간이 그만큼 늘면 남는 금액은 오히려 줄어든다.

#### D2를 "발주처가 우리를 어떻게 평가하는가"로 재해석했다
매장이 없으므로 파사드·보행자 축이 성립하지 않는다. construction·trade_logistics와 같은 처리다.
`2_1` 관리 실적·보유 인력·**자격 현황** / `2_2` 온라인 정보와 **허가·인증** 일치
(경비업 허가·소독업 신고·위생관리용역업 신고) / `2_3` **현장 출입·안전 관리**(배상책임 보장) /
`2_4` 현장별 작업 품질 편차 / `2_5` 자재·장비 보관과 반출 동선.

#### 축별 반영
| 축 | 문항 |
|---|---|
| 인건비 | `1_3` 실질 인건비 비율 · `1_4` 계약별 실수익 |
| 계약 | `3_3` 지급 주기·해지 조항·**최저임금 연동 단가 조정 조항** · `3_2` 표준 도급 패키지(인원·시간·주기·범위) |
| 인력 | `4_1` **결원 대체 투입 + 이직률** · `5_3` 근로계약·가산수당·4대보험·안전보건 교육 · `2_1` 자격 현황 |
| 운영 | `4_5` 근태·점검 기록 · `2_4` 품질 편차 · `4_2` 기록·이동 자동화 |
| 안전 | `2_3` 위험 지점 점검·배상책임 보장·사고 시 책임 범위 |
| 자금 | **`5_2` 급여 지급일과 대금 입금일의 균형** |
| 판로 | `3_1` 계약처 다변화 · `3_4` 견적·입찰 대비 수주율 · `3_5` **재계약·소개**(갱신율) |

⚠ **`5_2`가 이 업종의 최대 자금 위험이다.** 인건비가 원가의 대부분이라 **급여가 먼저 나가고
도급 대금이 나중에 들어온다.** 계약이 늘수록 필요한 운전자금도 함께 늘어나므로
**수주를 늘리다 자금이 막히는 일이 실제로 생긴다** — guide에 명시했다.
⚠ **`3_3`에 최저임금 연동 조항을 넣었다.** 최저임금이 오르면 곧바로 적자로 돌아서는 구조라
계약 기간 중 단가를 조정할 수 있는 조항이 있는지가 생존 문제다.

#### 4업종 포괄 — 한쪽 전용어를 본문에 쓰지 않았다
`순찰`(경비) · `구역별`(청소) · `약품`(방역) · `승강기`(빌딩관리)는 전부 **guide에만** 넣고
본문은 상위 개념(`정해진 주기와 범위`, `소모 자재`, `점검`)으로 썼다.
**guide에 경비·청소·방역·빌딩관리를 모두 병기한 문항이 11개**다.

⚠ **작성 중 `5_4` 본문에 `약품`·`세제`가 남아 검사에서 잡았다.** 경비 업체는 약품·세제를
취급하지 않으므로 `폐기물·자재 처리`로 상위화하고 구체는 guide로 내렸다.

#### ⚠ `DOMAIN_DESC_BY_GROUP`을 3건 고쳤다 — 5-1의 설계를 수정
| 도메인 | 조치 |
|---|---|
| `'2'` | 5-1이 **"현장 근태·점검 기록 관리"**로 써 두었는데 그건 실제로 D4 내용이다. 문항과 어긋나 `발주처가 확인하는 관리 실적·인력 자격과 현장 안전·작업 품질 편차`로 교체 |
| `'5'` | 신설 — `급여 지급일과 대금 입금일의 균형, 근로조건 준수` |
| `'7'` | **신설(필수)** — 기본 desc가 **`플레이스 CTR 최적화`**라 이 업종에 성립하지 않는다 |

⚠ **`desc`는 guide가 없는 자리이고 영역 제목 아래 첫 화면에 바로 보인다.**
`label`·`key`·`weight`·`id`는 **분기하지 않았다**(`calcScores`가 `domain.label`을 반환값에 실어
레이더차트·대시보드로 흘려보낸다).

#### ⚠ 오버라이드하지 않은 9문항 — construction·trade_logistics와 동일
`4_3`·`4_4`(도입 기기 효과) · `5_1`(계좌 분리) · `5_5`(정책자금·보증) ·
`6_1`·`6_2`·`6_3`·`6_5`(폐업 법령·제도) · `7_2`(무료 디자인 도구)
→ **업종과 무관하게 기본 문항이 정확하다.** 억지로 채우지 않았다.

#### ⚠ 법령 수치는 확신한 것만 썼다 (2026-09-07 기준 그대로)
`5_3` guide에 **서면 작성·교부 의무** / **1주 15시간 이상 → 주휴수당** /
**계속 근로 1년 이상 + 4주 평균 주 15시간 → 퇴직금**까지만 적고,
**야간·휴일 가산수당의 요율·적용 사업장 규모는 쓰지 않았다**(5인 기준 등 조건이 갈린다).
`구체적인 적용 범위와 절차는 관할 기관에 확인할 것`으로 남겼다.

#### 검증 (전 항목 통과)
| 항목 | 결과 |
|---|---|
| **[1] 본문 금지어** | 매장·플레이스·계산대·진열·POS·메뉴·시술 등 **19종 0건**(label·question·scale) ✓ |
| **[1-2] 한쪽 전용어 본문 혼입** | 순찰·약품·초소·무전기·승강기·세제·소독·구역별 **0건** ✓ |
| **[1-3] 본문 영문 표기** | **0건** ✓ |
| **[1-4] `DOMAINS.desc` 금지어** | **0건** ✓ |
| **[2] 인건비 관점** | `1_3`에 배치 인원·시급·근무 시간·4대보험·퇴직충당금·연차수당·계약 단가 **전부 포함** · `1_4`에 인원≠수익 관점 포함 ✓ |
| **[3] 기존 9그룹** | `items`·`domains`·`ACTION_PLAN` 전부 **JSON 동일** · 기본 `ITEMS`/`DOMAINS`/`INDUSTRY_GROUP_MAP` 불변 ✓ |
| **[4] calcScores** | 10그룹 × 1·3·5점 **30/30 동일** ✓ |
| **[4] 렌더 파생** | 7영역 × 5문항 = **35** · 레이더 **d1~d7 전부 산출, 0점 영역 0개** ✓ |
| **⚠ 10그룹 키 개수** | 전 그룹 **35/35** · facility_service **실효 26**(construction·trade_logistics와 동일 분포) ✓ |
| scale·guide·ai_trigger | 5단계 35/35 · guide 35/35 · **ai_trigger 35/35 보존** ✓ |
| 경고 | 3점 **0건** / 1점 **6건**(CRITICAL 2) · `WARN_WORDING.facility_service` 반영(`인건비+운영비`·`계약`·`입찰·도급`) ✓ |

#### ⚠ 진행률은 51이 아니라 **35**다
`industryVarMap`에 `facility_service`가 없어 **업종 특화 16문항이 렌더링되지 않는다.**
5-1 ⑥에서 확인한 대로 [wizard.js:1514](js/wizard.js#L1514)가 탭 버튼 자체를 숨기고
`_cntSuffix(0)`이 숫자를 붙이지 않으므로, 사용자는 **업종 탭이 처음부터 없는 상태**로
`0 / 35`를 보며 세션 중 분모가 바뀌지 않는다. **설계대로이며 결함이 아니다.**

#### 캐시버스팅
`?v=` 52곳 `20260910d`

### ⑪ 5-3 — 부속 시스템 연결 (2026-09-10)

#### ⚠ 착수 전 전제 하나가 틀렸다는 것을 실측으로 확인했다
5-1에 *"`PROGRAMS.industry` 태그가 한국어 `'서비스업'`이라 새 라벨을 쓰면 매칭이 0이 된다"*고
적어 두었으나 **매칭 0건은 구조적으로 일어나지 않는다.**
[gov-support.js](js/gov-support.js)의 점수 배분이 `정확 매칭 +3 / 범용('all') +1 / 불일치 0`이고
규모(+1)·BM(+1)이 더해져 `filter(score >= 2)`를 통과한다. `'all'` 태그 사업이 **17개**다.

**진짜 문제는 "업종 정확 매칭 0 → 상위 6개가 전부 범용 사업"이었다** — 2026-08-01 ⑩과 같은 유형.

| 안 | 건수 | 업종 정확 매칭 | 판정 |
|---|---|---|---|
| A 라벨 없음(5-2까지의 상태) | 6 | **0** | 전부 범용(수출 바우처·SMTECH R&D) |
| B `'서비스업'` 재사용 | 6 | 4 | ⚠ **3위가 「프랜차이즈 가맹본부 경쟁력 강화」** · local_service와 결과 100% 동일 |
| C `'시설관리·경비'` 태그 미추가 | 6 | **0** | A와 실질 동일 |
| **D `'시설관리·경비'` + 태그 5건** | 6 | **5** | 채택 |

#### D안 — 회귀 위험이 0인 것이 근거다
`p.industry` 배열에 문자열을 **추가**해도 다른 업종의 `includes(다른라벨)`·`includes('all')` 결과는
바뀌지 않는다. → **`'시설관리·경비'` 라벨을 가진 사용자에게만 영향이 간다.**
⚠ 라벨을 `'시설관리·경비'`(가운뎃점)로 둔 것은 **`INDUSTRY_LABEL`은 `wizard.js INDUSTRY_MAP`을
그대로 반전한 값**이라는 기존 원칙 때문이다([wizard.js:22](js/wizard.js#L22)). 형식 통일보다 원칙이 우선한다.

**태그를 붙인 5건과 근거**
| 사업 | 근거 |
|---|---|
| 고용촉진장려금 · 청년일자리도약장려금 | **인건비가 원가의 대부분인 업종에 가장 적합한데 `'all'`이라 +1만 받고 있었다** |
| 소상공인 경영컨설팅 지원 | 원가 구조 진단(`1_3`·`1_4`)이 이 업종의 핵심 과제 |
| 소상공인 스마트화 (디지털 전환) | 근태·점검 기록 자동화(`4_2`·`4_5`)와 직결 |
| 소상공인 정책자금 | 소상공인 규모 업체의 실제 대상 |

⚠ **붙이지 않은 것**: 프랜차이즈(가맹본부가 아니다) / 스마트공장·HACCP·브랜드K(제조·식품 전용) /
**클라우드·비대면 바우처 — 근태 앱이 대상 품목인지 확인되지 않아 넣지 않았다**
(정책자금에서 세운 *"확인되지 않은 것은 넣지 않는다"* 원칙).

#### ⚠ 조사 중 발견한 실제 오진 — `'facility_service'`가 IT 인과사슬을 받고 있었다
[ai-engine.js `buildCausalChain()`](js/ai-engine.js)의
```js
if (/it|소프트웨어|테크|개발|knowledge_it/.test(ind))
```
에서 **`facil-it-y_service`의 `it`이 매칭된다.** 시설관리·경비 업체가
`ARR 누적 실패`·`MRR 불규칙` 같은 **SaaS 인과사슬 진단을 받아 AI 프롬프트로 들어가고 있었다.**

→ **`it` 단독 대안을 제거**했다. `'IT/소프트웨어'` 라벨은 `소프트웨어`가 이미 잡으므로 손실이 없다.
`\bit\b`로 막는 방법도 있으나 **패치 과정에서 백슬래시가 소실돼 실제 백스페이스 문자(0x08)가
파일에 기록되는 사고가 있었다** — 되돌리고 백슬래시를 쓰지 않는 형태로 확정했다.
⚠ **셸 경유 패치에서 `\b`·`\n` 같은 이스케이프는 층을 하나 잃을 수 있다.**
커밋 전 제어문자 검사(0x00~0x1F, 개행·탭 제외)를 반드시 돌릴 것.

**전용 인과사슬 분기도 신설**했다(3갈래): 실질 인건비 미파악 × 도급 단가 고정 /
현장 인력 이직 × 결원의 계약 위반 연쇄 / 계약처 편중 × 가격 외 차별화 부재.

#### 시스템별 조치
| 시스템 | 조치 |
|---|---|
| `gov-support.js` | `INDUSTRY_LABEL` + PROGRAMS 태그 5건 |
| `api/kosis-survival.js` | `facility_service: 'S'` — **실제 KSIC는 N(사업시설관리·사업지원)이나 `FALLBACK`에 N이 없어 근사.** 주석 명시 |
| `js/diagnosis/funding-rules.js` | `INDUSTRY_WATCH: ['68', '75330중', '75993']` + `KOSMES_NON_MFG_INDUSTRIES` 라벨 |
| `js/diagnosis/cross-context.js` | `INDUSTRY_ID_MAP`에 `'시설관리·경비'` 역매핑 |
| `js/ai-engine.js` | `INDUSTRY_CLOSURE_CAUSES` + 정규식 오매칭 수정 + 전용 인과사슬 |
| `js/dashboard.js` | `_INDUSTRY_KR` + **죽어 있던 조회 복구**(아래) |
| `js/reference-db.js` | **항목을 만들지 않았다** — 확인된 벤치마크 수치가 없다 |
| `js/pattern-db.js` | **건드리지 않았다** (아래) |

⚠ **`funding-rules`의 `68`(부동산업) 주의가 이 업종에 실질적으로 중요하다.**
빌딩관리(FM)가 부동산업으로 분류될까 우려할 수 있는데 **예외 조항에 `부동산관리업(6821)`이
명시돼 있어 신청 가능**하다. 경비업은 흥신소(`75330중`)·추심대행(`75993`)을 겸하면 제외 대상이다.

#### ⚠ `dashboard.js` — 업종 라벨 조회가 죽어 있었다
```js
const industryKr = _INDUSTRY_KR[fd.industry] || fd.industry || '—';
```
`fd.industry`는 `#industry` select 제거(2026-04-17) 이후 **항상 `''`**이라
**PDF 표지의 업종이 17개 업종 전부 `'—'`로 나오고 있었다.**
`_INDUSTRY_KR[fd.industryKey]`를 먼저 보도록 고쳤다.
⚠ **이건 facility_service만의 변화가 아니다** — 전 업종의 PDF 표지 업종명이
`'—'`에서 실제 라벨로 바뀐다. 비어 있던 것이 채워지는 방향이라 회귀가 아니라고 판단했다.

#### ⚠ `pattern-db.js`는 손대지 않았다 — 근거
`match()`의 폴백이 `INDUSTRY_CONTEXT[industryKey] || INDUSTRY_CONTEXT.local_service`인데,
호출부가 `diagData.industry`(**항상 `''`**)를 넘기므로 **이미 전 사용자가 local_service 통계**
(`월평균 1,580만원`·`노쇼·예약 취소`)를 받고 있다. facility_service 고유 문제가 아니다.
- 항목을 신설하려면 `sampleSize`·`avgMonthlyRev`·`bepRange` 같은 **수치를 지어내야 한다 — 금지**
- 폴백을 없애면 **전 업종에서 블록이 사라진다** — 큰 회귀
- 호출부를 `industryKey`로 고치면 **16개 업종의 PatternDB 표시가 한꺼번에 바뀐다**
→ CLAUDE.md에 이미 *"PatternDB 4축과 D1~D7 매핑 설계가 선행되어야 한다"*고 기록된 사안이다.

#### 검증 (전 항목 통과)
| 항목 | 결과 |
|---|---|
| **[1] 매칭 상위 6개** | **6건** · 업종 정확 매칭 **5건** — D안 사전 시뮬레이션과 **완전 일치** ✓ |
| **[2] 라벨 ↔ 태그 문자열** | `INDUSTRY_LABEL = "시설관리·경비"` · 태그 보유 5건 · **코드포인트 일치** ✓ |
| **[3] 기존 17업종 회귀** | 17업종 × 4조직형태 × 7관심분야 × 4규모 = **1,904케이스 · 불일치 0건** ✓ |
| **[4] 8개 시스템 undefined** | gov `'시설관리·경비'` / kosis `'S'`+`FALLBACK.S` / reference **블록 0자 생략** / cross-context 예외 없음 / funding-rules 2기관·**중진공 재확인 경고 정상 발동** / ai-engine 폐업원인 3축·`buildInsightsSummary` 빈 문자열 / dashboard 라벨·조회 복구 ✓ |
| **[5] 생존율** | `S` → `기타개인서비스` **y1 49.8 / y3 29.1 / y5 18.6 · 폐업 고위험**, 긴급 생존 모드 발동 ✓ |
| **[5] 폐업 원인 연결** | 재무 2.0 → `최저임금 인상에 따른 인건비 압박·계약 단가 동결` 등 3축 정상 출력 ✓ |
| **[5] 벤치마크** | `getBench` `null` · **블록 깨끗이 생략** ✓ |
| **정규식** | `facility_service` **매칭 없음** · knowledge_it·local_service·construction·logistics·mfg_parts·restaurant·fashion·medical·education **전부 기존 분기 유지** ✓ |
| 제어문자 | 패치 7개 파일 **0x00~0x1F 0개** · `node -c` 문법 전부 통과 ✓ |

#### 캐시버스팅
`?v=` 52곳 `20260910e`

### ⑫ 5-4 — facility_service ACTION_PLAN 35항목 (2026-09-10) · **5단계 완료**

**5-3까지가 가장 나쁜 상태였다.** 문항은 인건비·계약 관리인데
`ACTION_PLAN_7DAY_BY_GROUP`에 항목이 없어 **base(외식)로 폴백**해
시설관리 사장님이 `Dog 메뉴 제거`·`테이블에 QR 스탠드 부착`을 받았다.
4단계 때 확인했듯 **7일 계획은 사용자가 실제로 읽고 행동하는 부분이라 문항보다 체감 영향이 크다.**

#### profit_ops — 7일 안에 실질 인건비 계산을 끝내게 만든다
이 업종의 핵심이 인건비 원가 파악이므로 7일 계획이 그 계산을 **실제로 수행**하도록 설계했다.
```
Day1 계약 3곳 계약서에서 월 도급 단가 + 계약 기간·단가 조정 조항 확인
Day2 현장별 배치 인원 명단 — 이름·근무 형태(주간/교대)·주당 근무시간
Day3 인원별 기본급 집계 — 시급 × 월 근무시간, 야간·휴일 근무분 별도 표시
Day4 ⚠ 4대보험 사업주 부담분·퇴직충당금·연차수당을 더해 실질 인건비 산출
Day5 현장별 도급 단가 − 실질 인건비 − 소모품·장비·이동비 = 남는 금액
Day6 적자·저마진 현장 식별 — 배치 인원이 많은 현장이 실제로도 남는지 대조
Day7 실질 인건비 단가표 확정 — 다음 견적부터 적용 + 월 집계 양식
```
⚠ **Day1에 단가 조정 조항 확인을 넣었다.** 최저임금이 오르면 곧바로 적자로 돌아서는 구조라
계산보다 먼저 확인해야 할 계약 조건이다(`3_3`과 이어진다).
⚠ **Day6이 `1_4`의 축을 그대로 잇는다** — 배치 인원이 많은 계약이 반드시 남는 계약은 아니다.

#### 나머지 4키
| 키 | Day1 → Day7 요지 |
|---|---|
| `place_seo` | 관리 실적 목록 · 인력·자격 명부 · **허가 유효기간 점검** · 보험 보장 범위 · 소개 자료 1장 · 채널 정보 일치 확인 · 분기 점검 일정 |
| `multichannel` | 1위 발주처 비중 · 만료일 달력 표시 · **단가 조정 조항 없는 계약 목록** · 처리 실적 정리 · 견적 성사율·탈락 사유 · 인상 필요 계약 선별 · 갱신 접촉 + 후보 10곳 |
| `smart_dx` | 근무일지 양식 통일 · 결원 건수·대체 소요시간 · 대기 인력 명단 · **결원 연락 순서 1장 매뉴얼** · 점검 체크리스트 · 민원 처리 시간 · 사진 전송 당일 취합 |
| `sns_ai` | 소개 자료 수치 갱신 · 지역명+업무명 노출 문구 · 견적 문의 양식 4항목 · **24시간 회신 규칙** · 실적 확인서 3곳 요청 · 제안서 표준 문구 · 유입 경로 집계 |

⚠ **"근태 시스템 도입"은 7일에 못 한다.** `현장 3곳 근무일지 양식 통일`·`휴대폰 사진 전송으로
당일 취합` 수준으로 맞췄다. `{ day, title }`만 쓰고 `desc`는 만들지 않았다(그룹 계획에 없는 필드).
⚠ 한쪽 전용어(`순찰`·`약품`·`구역별`·`초소`·`승강기`)를 title에 쓰지 않고
`점검 항목`·`소모품`·`수행 주기` 같은 상위 개념으로 썼다 — 검사 0건.

#### 검증 (전 항목 통과)
| 항목 | 결과 |
|---|---|
| **[1] 항목 수** | 5키 × 7일 = **35항목** · `day` 1~7 순차 · `desc` **0건** ✓ |
| **[2] 외식·소매 용어** | 메뉴·매장·플레이스·테이블·시술·POS 등 **19종 0건** · 한쪽 전용어 **0건** ✓ |
| **[3] 기존 그룹 불변** | 5키 × 9그룹 = **45조합 JSON 동일** · base `ACTION_PLAN_7DAY` 불변 · **food 폴백(null) 유지** ✓ |
| **[4] profit_ops 인건비 관점** | 4대보험·퇴직충당금·연차수당·배치 인원·시급·도급 단가·실질 인건비 **7개 전부 포함** ✓ |
| **⚠ 10그룹 키 개수** | 전 그룹 35/35 · 문항 JSON 동일 · DOMAINS 불변 · calcScores 30조합 불일치 0 ✓ |
| **end-to-end** | `buildPromptSummary`가 D1 취약 시 **7일 계획을 프롬프트에 실제로 주입** · 외식 폴백 오염 0 ✓ |

⚠ **프롬프트에 `점포환경·네이버 플레이스` 1건이 남는다** — `DOMAINS.label`이며
construction·trade_logistics도 동일하다(설계상 label을 분기하지 않는다). 아래 남은 이슈 참조.

#### 캐시버스팅
`?v=` 52곳 `20260910f`

---

## 최근 수정 이력 (2026-09-11) — 기존 4그룹 전용어 정리 (6단계) · 1단계 etc

**전수 점검에서 5건이 나왔다. 전부 같은 패턴이다** —
그룹 안에 이질적 업종이 섞여 있는데 문구가 그중 **한 업종 전용**이다.

| 그룹 | 잔존 전용어 | 부적합 대상 |
|---|---|---|
| **etc** | 매장 전면·파사드 / 플레이스 | **업종 미판별 전원** |
| beauty | 시술19 샴푸1 홈케어2 매장10 | 세탁소·수선집·펫샵·필라테스 |
| retail | 매장11 진열4 계산대6 | 무역상사·패션 제조 |
| pro_service | 수임8 상담실3 파일실1 | IT·SaaS·미디어 |
| edu_service | 교실3 원장2 | 온라인강의 |

⚠ `manufacturing`·`construction`·`trade_logistics`·`facility_service`는 **0건**이다.
신설할 때 *"본문은 중립, 전용어는 guide"* 원칙을 지켰기 때문이다.
기존 4그룹은 **그 원칙이 서기 전에 만들어져** 전용어가 본문에 박혀 있다.

### ① 1단계 — etc 5문항 (2026-09-11)

⚠ **지시는 3문항(`2_1`·`2_2`·`7_5`)이었으나 실측 결과 5문항이었다.**
`7_1`(`네이버 스마트플레이스`·`인스타그램`)·`7_4`(`자사 플레이스 채널`)도 해당한다.
3개만 고치면 **검증 기준 "전용어 0건"을 만족할 수 없어** 5개를 함께 처리했다.

| 문항 | before | after |
|---|---|---|
| `2_1` | **109초 시선 주목도(매장 전면·파사드)** — 보행자 시선 | **첫 접점에서의 인지성** — 처음 접하는 지점에서 무엇을 하는 곳인지 |
| `2_2` | **네이버 플레이스 정보 일치도** — 오프라인 간판 | **온라인 사업장 정보 일치도** — 채널 간 상호·주소·연락처 일치 |
| `7_1` | 인스타그램 피드·네이버 스마트플레이스 소식 | 홍보 문구·소식 글 |
| `7_4` | **플레이스 저장하기(하트) 유도** | **관심 고객 재접촉 수단 확보** — 즐겨찾기·알림·연락처 등록 |
| `7_5` | **플레이스 CTR 최적화** | **노출 대비 반응률 관리** |

⚠ **`etc`의 guide에는 특정 업종 예시를 넣지 않았다.** 다른 그룹은 guide에 업종별 예시를
병기하지만 `etc`는 **업종을 특정할 수 없는 자리**다. 대신 사업 형태로 갈라 서술했다:
> `2_1` — "첫 접점이 어디인지는 사업 형태에 따라 다르다. **오프라인 접점이 있으면** 외부 표시물과
> 안내 문구, **없으면** 검색 결과와 소개 자료의 첫 화면이 그 자리다"

⚠ **나머지 30문항은 덮지 않았다.** 기본 ITEMS가 이미 중립이며,
`etc` 오버라이드의 원칙은 *"기본보다 더 중립적인 것만 덮는다"*이다.

#### 부수 효과 — 영문 약자가 오히려 줄었다
전문용어 정리(2026-09-07) 이후에도 이 5문항에 남아 있던 것이 함께 정리됐다.
| 문항 | before | after |
|---|---|---|
| `2_1` | `USP` | 없음 |
| `7_1` | `AI` `ChatGPT`×3 `SNS` `USP`×2 | `AI`만(label) |
| `7_4` | `SEO` `QR` | `QR`만(승인 목록) |
| `7_5` | `CTR`×3 | 없음 |

**신규 도입 영문 0건.** `ChatGPT`는 guide에 풀네임 설명과 함께 남겼다.

#### 검증 (전 항목 통과)
| 항목 | 결과 |
|---|---|
| **[1] etc 본문 전용어** | 매장·플레이스·파사드·보행자·간판·계산대·진열·POS·메뉴·시술·테이블·인스타그램 등 **20종 0건** ✓ |
| **[2] guide** | 35/35 존재 · etc는 업종 예시 대신 **사업 형태별 서술**(설계대로) ✓ |
| **[3] 다른 9그룹** | `items`·`domains` **JSON 동일** · 기본 `ITEMS` 불변 · `ACTION_PLAN` 불변 ✓ |
| **[4] calcScores** | 10그룹 × 1·3·5점 **30조합 불일치 0** ✓ |
| **⚠ 10그룹 키 개수** | 전 그룹 **35/35** · etc **실효 5**(0 → 5) ✓ |
| ai_trigger·scale | etc **35/35 기본과 동일** · scale 5단계 전수 ✓ |
| 신규 영문 | **0건** ✓ |

#### 캐시버스팅
`?v=` 52곳 `20260911a`

### ② 2단계 — beauty 그룹 (2026-09-11)

**사용자가 실사용에서 확인한 문제다** — *"일반 세탁소인데 시술, 홈케어 샴푸 예시가 나온다"*.
`beauty`는 **헤어·네일·피부샵 / 세탁소 / 수선집 / 반려동물샵 / 필라테스·요가** 5개 업종을 담는데
문구가 **미용실 전용**이었다(시술19 · 샴푸1 · 홈케어2).

#### manufacturing 방식 — 본문 중립화 + guide에 미용 예시 첫 번째
5개 업종의 공통 구조는 **접수 → 작업 → 인도**다. 본문을 그 층위로 올렸다.
| before | after |
|---|---|
| `시술별로 손이 가는 만큼 남는지 계산` | `작업별로 손이 가는 만큼 남는지 계산` |
| `시술재료비` | `작업 재료비` |
| `시술실` / `시술석` | `작업 공간` / `작업 자리` |
| `홈케어 제품 패키지 판매` | `부가 상품 묶음 판매` |
| `홈케어 제품(샴푸·앰플·크림)` | `부가 상품` |
| `비포애프터` | `작업 전·후` |

⚠ **`매장`은 유지했다.** 5개 업종 모두 점포가 있고,
2026-09-04에 *"beauty·retail·edu_service에는 자연스러운 표현"*으로 이미 승인된 결정이다.
⚠ **guide는 미용 예시를 반드시 첫 번째에 둔다** — 15문항 전부 확인했다.
미용실 사장님이 받는 문항 품질이 떨어지면 안 되기 때문이다.
> `1_4` — `예: 미용은 시술별 재료·소요 시간, 세탁은 품목별 세제·건조 시간,
> 수선은 작업 난이도별 시간, 펫샵은 미용 건별 재료·소요 시간, 필라테스는 수업 회차별 강사 시간`

#### ⚠ 필라테스·요가 — `1_3`이 성립하지 않아 guide에 명시했다
나머지 4개는 재료비가 실재하지만 **필라테스는 재료비가 거의 없고 강사 인건비가 원가의 대부분**이다.
문항을 억지로 맞추지 않고 `1_3` guide에 그대로 적었다.
> `⚠ 필라테스는 재료비가 거의 없고 강사 인건비가 원가의 대부분이라 인건비 비율만 보면 된다`

`2_5`(보관→준비→작업 3각 동선)도 필라테스에는 약하나 `소도구 보관→기구 배치`로 성립한다.

#### ⚠ 작업 중 사고 2건 — 기계적 치환의 위험
**① 한국어 조사가 깨졌다(4건).** `시술`(받침 ㄹ) → `서비스`(받침 없음) 치환이
앞뒤 조사를 그대로 두었다.
```
저수익 시술을  →  저수익 서비스을   ✗ (서비스를)
주요 시술은    →  주요 서비스은     ✗ (서비스는)
대표 시술과    →  대표 서비스와 ✗ → 2곳
```
→ **조사 파손 전용 검사기**(`서비스을`·`상품를`·`작업를` 등 14패턴)를 만들어 잡았다.
⚠ **받침 유무가 바뀌는 치환은 반드시 조사까지 함께 본다.**

**② 검사기 오탐 1건.** `썸네일`의 `네일`이 미용 전용어로 잡혔다.
2026-09-10 `전환율`→`환율`과 같은 유형이다. 앞 글자가 `썸`인 경우를 제외해 재검사했다.
**용어 검사기는 단어 경계를 고려하지 않으면 반드시 오탐이 난다.**

#### ACTION_PLAN도 함께 처리했다
`Dog 시술 예약 목록` · `시술실·샴푸대·접수대 동선 실측` · `홈케어 제품(샴푸·앰플) 패키지` 등
**19건**을 중립화했다. ⚠ `Dog`은 4분면 분류 용어라 전 그룹 공통이므로 유지했다.

#### 검증 (전 항목 통과)
| 항목 | 결과 |
|---|---|
| **[1] 본문 전용어** | 시술·샴푸·홈케어·에스테틱·헤어·모발·펌제·염모제·네일 **0건**(label·question·scale·warning_msg) ✓ |
| **[2] guide 5업종 병기** | **15문항** 전부 미용·세탁·수선·펫샵·필라테스 포함 ✓ |
| **[3] 미용 첫 번째** | **15/15** — 미용실 관점 품질 유지 ✓ |
| **[4] 다른 9그룹** | `items`·`domains`·`ACTION_PLAN` **전부 JSON 동일** · 기본 `ITEMS` 불변 ✓ |
| **[5] calcScores** | 10그룹 × 1·3·5점 **30조합 불일치 0** ✓ |
| **⚠ 10그룹 키 개수** | 전 그룹 **35/35** · beauty 실효 26 유지 ✓ |
| 조사 파손 | 14패턴 검사 **0건** ✓ |
| ai_trigger·scale | `warning` 코드 전수 보존 · scale 5단계 전수 ✓ |
| ACTION_PLAN | beauty 전용어 **0건** ✓ |

#### 캐시버스팅
`?v=` 52곳 `20260911b` · 잔여 조사 수정 `20260911c`

⚠ **push 직후 조사 파손 1건이 더 나왔다** — `작업 자리이`(시술석 → 작업 자리).
앞서 4건을 잡은 검사 패턴에 `자리이`가 빠져 있었다. 별도 커밋으로 수정했다.

### ③ 3단계 — retail 그룹 (2026-09-15)

`retail`은 **도소매·유통·무역상사·대리점·총판(`wholesale`)** 과
**의류·패션잡화 디자인·제조·유통 브랜드(`fashion`)** 를 담는데
문구가 **편의점·의류매장 같은 소매점포 전제**였다(매장11 · 진열4 · 계산대6 · 바코드1).

#### ⚠ beauty와 달리 `매장`을 중립화했다 — 판단 근거
beauty는 5개 업종이 **모두 점포를 가지므로** `매장`을 유지했다.
retail은 **무역상사·총판이 B2B 유통이고 패션 브랜드는 디자인·제조가 본업**이라
점포가 없을 수 있다. → `매장` → **`사업장`**, `점주` → **`대표`**(기본 ITEMS가 쓰는 말).

| before | after |
|---|---|
| `109초 시선 주목도(매장 전면·파사드)` | **`첫 접점 상품 인지성`** |
| `POS 판매 데이터 분석` | `판매·주문 데이터 분석` |
| `매장 쇼핑 편의 동선` | `고객·상품 이동 동선` |
| `창고·진열·계산 동선` | `창고·출고·정산 동선` |
| `계산 처리 편차 지수` / `계산 대기 지연율` | `주문·결제 처리 편차` / `주문·결제 처리 지연율` |
| `바코드·자동 집계 장비` | `상품 코드·자동 집계 장비` |
| `피크타임` | `주문이 몰리는 시점` |

#### fashion 제조 관점 — 필요하다. guide에 넣고 본문은 바꾸지 않았다
검토 결과 **`1_3`·`1_4`·`3_2`·`5_2` 네 곳에서 제조 관점이 실제로 필요**하다.
`패션 브랜드는 원단·부자재비와 생산 발주 단가` / `품번별 원단·부자재비와 샘플·생산 관리 시간` /
`시즌 코디 세트` / **`생산 발주 선지출과 판매 대금 회수 시점의 간격`**.
⚠ 본문을 제조업으로 바꾸지 않았다 — `fashion`은 유통 브랜드이기도 하며
`wholesale`과 한 그룹이므로 본문이 제조 전제가 되면 도매·총판이 성립하지 않는다.

#### guide 3축 — 소매점을 첫 번째로
`소매점 → 도매·총판 → 패션 브랜드` 순서로 **14문항**에 병기했다.
소매점 예시를 빼지 않았으므로 **소매점 관점 품질은 유지된다**
(guide에 `매장`·`진열`·`POS`·`바코드`가 남아 있는 것은 **의도된 소매점 예시**다).

#### ⚠ 이번에 드러난 결함 3가지
**① label 상속을 놓쳤다.** `2_1`은 question·scale을 전부 중립화했는데
**label만 기본 ITEMS에서 상속**되어 `109초 시선 주목도(매장 전면·파사드)`가 그대로 나갔다.
→ **오버라이드 검사는 `label`·`question`·`scale`·`guide` 네 필드를 모두 봐야 한다.**
전수 점검 결과 retail에서 label을 상속하는 문항이 **13개**이며,
그중 `2_2`·`7_4`·`7_5`의 `네이버 플레이스`·`플레이스 CTR`은 **전 그룹 공통 미해결 이슈**라
retail 단독으로 바꾸지 않았다(아래 남은 이슈).

**② 치환 순서 버그.** `계산 대기 지연율`(짧은 것)을 먼저 적용해
`피크타임 밀집 상황 시, 계산대 병목이나…`(긴 것)의 패턴이 어긋나 미적용됐다.
→ **긴 패턴 우선 원칙을 어기면 조용히 누락된다.**

**③ 죽은 오버라이드가 생겼다.** `5_1`의 `점주 가지급금` → `대표 가지급금` 치환으로
**기본 ITEMS와 완전히 동일**해졌다(기본이 이미 `대표 가지급금`). 오버라이드를 **제거**했다.
→ retail 실효 **26 → 25**. 2026-09-04 manufacturing에서 겪은 것과 같은 유형이다.
⚠ 같은 형태가 `construction`·`trade_logistics`의 `5_1`에도 있어 보인다 — 별도 확인 필요.

#### ⚠ 검사기 오탐 2건 — 조사 판정을 수동 분류하면 안 된다
`채널을`·`채널이`·`채널로`가 파손으로 잡혔으나 **전부 정확한 한국어**다.
- `채널`은 받침 ㄹ이 있어 `을/이`가 맞다 — 수동으로 "받침 없음"에 넣은 것이 원인
- **ㄹ 받침은 `으로`가 아니라 `로`를 쓴다**(서울로, 채널로)

→ **받침을 코드포인트로 판정**하도록 검사기를 고쳤다(`(charCode - 0xAC00) % 28`,
ㄹ은 `% 28 === 8`). 재검사 결과 **현재 0건 / 패치 전 0건**.

#### `스마트스토어`는 `3_1`에만 남겼다 — 의도
판로 나열이고 `등`이 붙어 있으며 **`B2B 도매 납품`이 함께 나열**되어 3축이 모두 커버된다.
반면 `3_5` scale3의 `스마트스토어 알림받기`는 도매·총판이 쓰지 않는 전제라
`온라인 채널 알림받기`로 중립화했다.

#### 검증 (전 항목 통과)
| 항목 | 결과 |
|---|---|
| **[1] 본문 전용어** | 진열·계산대·바코드·매장·점주·파사드·보행자·피크타임·POS·쇼핑몰 **0건**(label·question·scale·warning_msg) ✓ |
| **[2] guide 3축** | **14문항** 소매점·도매/총판·패션 브랜드 병기 ✓ |
| **[3] 소매점 첫 번째** | **14/14** — 소매점 관점 품질 유지 ✓ |
| **[4] 다른 9그룹** | `items`·`domains`·`ACTION_PLAN` **전부 JSON 동일** · 기본 `ITEMS`·base `ACTION_PLAN` 불변 ✓ |
| **[5] calcScores** | 10그룹 × 1·3·5점 **30조합 불일치 0** ✓ |
| **⚠ 10그룹 키 개수** | 전 그룹 **35/35** · retail 실효 **25**(죽은 오버라이드 1건 제거) ✓ |
| 조사 파손 | 코드포인트 판정 **0건** ✓ |
| ai_trigger·scale·guide | `warning` 코드 전수 보존 · scale 5단계 전수 · guide 35/35 ✓ |
| ACTION_PLAN | retail 전용어 **0건**(11건 치환) ✓ |

#### 캐시버스팅
`?v=` 52곳 `20260915a`

### ④ 4단계 — pro_service 그룹 (2026-09-15)

`pro_service`는 **의원·약국·헬스장(`medical`) / IT·SaaS·컨설팅·회계·법무·HR(`knowledge_it`) /
금융·핀테크·보험(`finance`) / 미디어·영상제작·광고(`media`)** 4개 업종을 담는데
문구가 **세무·법무 사무소 전용**이었다(수임8 · 상담실3 · 파일실1 · 크몽1 · 숨고1).
전수 검사에서는 `사무소`·`의뢰인`·`접수대`·`대기실`·`자문`까지 더해 **51건**이 나왔다.

| before | after |
|---|---|
| `수임 건수` / `수임료` | `계약 건수` / `계약 단가` |
| `의뢰인` | `고객` |
| `사무소` | `사업장` |
| `상담실` / `파일실` / `접수대` / `대기실` | `업무 공간` / `자료 보관 공간` / `접수 창구` / `대기 공간` |
| `크몽·숨고 등 온라인 플랫폼` | `온라인 수주 채널` |
| `B2B 자문 계약` | `기업 대상 계약` |
| `대기실·접수 편의 동선` | `대기·접수 동선` |
| `상담 처리 편차 지수` | `처리 시간 편차` |
| `D2C 재의뢰 직계약 인프라` | `D2C 재거래 직접 계약 인프라` |

#### ⚠ 세무·법무 품질 유지 — guide에 `수임 건수·수임료`를 명시했다
본문에서 `수임`을 걷어냈으므로 **guide 21문항에 `세무·법무`를 명시**했고,
특히 `1_2`에 **`세무·법무는 수임 건수·건당 수임료`**를 그대로 남겼다.
`3_3`에도 `세무·법무는 플랫폼 전용 수임료를 직접 계약 단가와 구분해 산정했는지`를 두었다.

#### ⚠ medical은 나머지 3개와 성격이 달라 `2_3`에 단서를 달았다
의원·헬스장은 **개인 고객 대면·재방문 구조**라 `2_3`(대기·접수 동선)이 잘 맞지만
**IT·SaaS·미디어는 물리적 동선 개념이 약하다.** 문항을 억지로 맞추지 않고 guide에 적었다.
> `⚠ 온라인 중심인 IT·SaaS·미디어는 물리적 동선 대신 상담·미팅 공간과 작업 공간의 분리로 본다`

#### ⚠ 1차 패치 후 금융(finance) 축이 광범위하게 빠져 있었다
guide 축 커버리지를 문항별로 실측한 결과 **오버라이드 27문항 중 금융 누락이 15건**이었다.
`medical`·`knowledge_it`·`media`는 원문에 흔적이 있었으나 금융은 거의 없었다.
→ 핵심 15문항에 **2차 패치로 보강**했다(`금융은 상품 판매 건수·상담 전환`,
`모집 수수료와 시스템 이용료`, `전자청약과 계약 관리 자동화` 등).
**오버라이드 27문항 중 4축 완비 21문항.**
⚠ 나머지 6문항(`4_3`·`4_4`·`4_5`·`6_4`·`7_2`·`7_4`)은 **업종 무관 문항**이라 4축 병기가 불필요하다.

#### ⚠ 또 부분 문자열 오탐 — `전자문서`의 `자문`
`자문`(B2B 자문 계약) 검사에서 **`전-자-문-서`의 `자문`이 5건** 잡혔다.
`썸네일`→`네일`(6-2), `전환율`→`환율`(3단계)에 이어 **세 번째**다.
실제 `자문`은 `3_1`에만 있었고 전부 `기업 대상 계약`으로 교체해 **실제 0건 / 오탐 5건**으로 분리 확인했다.

#### 검증 (전 항목 통과)
| 항목 | 결과 |
|---|---|
| **[1] 본문 전용어** | 수임·상담실·파일실·크몽·숨고·사무소·의뢰인·접수대·대기실 등 16종 **0건** ✓ |
| **[2] guide 4축** | 오버라이드 27문항 중 **4축 완비 21문항**(나머지 6은 업종 무관) ✓ |
| **[3] 세무·법무 품질** | guide **21문항**에 명시 · `1_2`의 `수임 건수·건당 수임료` 보존 ✓ |
| **[4] 다른 9그룹** | `items`·`domains`·`ACTION_PLAN` **전부 JSON 동일** · 기본 `ITEMS`·base `ACTION_PLAN` 불변 ✓ |
| **[5] calcScores** | 10그룹 × 1·3·5점 **30조합 불일치 0** ✓ |
| **[6] 조사 파손** | 코드포인트 판정 + ㄹ 예외 **0건** ✓ |
| **⚠ 10그룹 키 개수** | 전 그룹 **35/35** · pro_service 실효 **27** 유지 ✓ |
| label 상속 | 12문항이 label을 상속하나 **전용어 포함 0건** ✓ |
| ai_trigger·scale·guide | `warning` 코드 전수 보존 · scale 5단계 전수 · guide 35/35 ✓ |
| ACTION_PLAN | pro_service 전용어 **0건**(6건 치환) ✓ |

#### 캐시버스팅
`?v=` 52곳 `20260915b`

### ⑤ 5단계 — edu_service 그룹 (2026-09-16) · **6단계 완료**

`edu_service`는 **학원·교습소·온라인강의·직업훈련기관**을 담는데
문구가 **오프라인 학원 전용**이었다. 전수 검사 **125건**.

#### ⚠ `수강`·`강좌`·`강사`는 유지했다 — 판단 근거
125건 중 다수가 이 세 단어였으나 **4개 형태에 모두 통용된다.**
온라인강의도 `수강 완주율`, 직업훈련기관도 `훈련과정 수강`이라 쓴다.
중립화하면 오히려 어색해지므로 그대로 두었다.

**중립화한 것**: `교실`→`강의 공간` / `원장`·`부원장`→`운영 책임자` /
`점주`→`대표` / `매장`→`사업장` / `상담실`→`상담 공간` /
본문의 `학원`→`사업장`·`수강 관리`(guide에는 학원 예시로 남김) /
`매장 전면(파사드)·보행자`→`첫 접점`.

| before | after |
|---|---|
| `109초 시선 주목도(매장 전면·파사드)` | **`첫 접점 인지성`**(label 오버라이드 신설) |
| `교실·대기실 편의 동선` | `강의 공간·대기 동선` |
| `교구·교실·출력 동선` | `교구·강의 공간·출력 동선` |
| `학원 관리 시스템 … 점주가 학원에 부재하더라도` | `수강 관리 시스템 … 대표가 사업장에 부재하더라도` |
| `클래스101·탈잉 등 온라인 플랫폼` | `온라인 강의 플랫폼` |

#### ⚠ 온라인강의가 성립하지 않는 문항 5건 — 억지로 맞추지 않고 guide에 단서를 달았다
| 문항 | 단서 |
|---|---|
| `1_1` 상권 분석 | `⚠ 온라인강의는 지역 상권 대신 검색 수요와 경쟁 강좌 수로 바꿔 본다` |
| `2_1` 첫 접점 | `⚠ 온라인강의는 물리적 간판이 없으므로 강좌 소개 페이지 첫 화면과 대표 이미지가 그 자리다` |
| `2_3` 대기·동선 | `⚠ 온라인강의는 물리적 동선이 없으므로 수강 신청부터 학습 시작까지의 화면 흐름으로 바꿔 본다` |
| `2_5` 3각 동선 | `⚠ 온라인강의는 물리 동선 대신 촬영 장비·소재 보관과 편집 작업 공간의 배치로 본다` |
| `3_2` 콘텐츠 패키지화 | `⚠ 온라인강의는 이미 이 항목이 본업이므로 강좌 규격화와 재판매 구조 수준으로 본다` |

⚠ `3_2`는 반대 방향이다 — 다른 형태에는 "전환 과제"인데 **온라인강의에는 이미 본업**이라
같은 문항이 난이도가 전혀 다르다. 문항을 나누지 않고 해석 기준만 달리 제시했다.

#### 검증 (전 항목 통과)
| 항목 | 결과 |
|---|---|
| **[1] 본문 전용어** | 교실·원장·매장·상담실·파사드·보행자·점주·학원 **0건** ✓ |
| **[2][3] guide** | 오버라이드 27문항 중 **3형태 완비 19문항** · **학원 첫 번째 19/19** ✓ |
| **[4] 다른 9그룹** | `items`·`domains`·`ACTION_PLAN` **전부 JSON 동일** ✓ |
| **[5] calcScores** | 30조합 불일치 **0** ✓ |
| **[6] 조사 파손** | 코드포인트 판정 + ㄹ 예외 **0건** ✓ |
| 10그룹 키 개수 | 전 그룹 **35/35** · edu_service 실효 **27** 유지 ✓ |
| ACTION_PLAN | 전용어 **0건**(2건 치환) ✓ |

#### 캐시버스팅
`?v=` 52곳 `20260916a`

### ⑥ 6단계 완료 — 10그룹 전수 재검사 결과

본문(`label`·`question`·`scale`·`warning_msg`) 기준. **자기 업종어와 타 업종어를 구분**했다.

| 그룹 | 자기 업종어 (정상) | 타 업종어 |
|---|---|---|
| **food** | 메뉴29 조리17 주방6 테이블7 | 매장26 파사드4 플레이스11 |
| **beauty** | — | 매장17 파사드4 플레이스11 |
| **retail** | — | 플레이스10 |
| **edu_service** | — | 플레이스10 |
| **pro_service** | — | 플레이스10 |
| **manufacturing** | — | **0건** |
| **construction** | — | **0건** |
| **trade_logistics** | — | **0건** |
| **facility_service** | — | **0건** |
| **etc** | — | **0건** |

#### 판정 — 지시한 17개 검사어 중 **13개가 전 그룹 0건**
`메뉴·조리·주방·테이블`은 **food(restaurant 단독)의 자기 업종어**이며 지시대로 손대지 않았다.
`시술·샴푸·홈케어·진열·계산대·수임·상담실·파일실·교실·원장`은 **전 그룹에서 완전히 사라졌다.**

#### ⚠ 남은 것은 `매장`·`파사드`·`플레이스` 3종이며 전부 **의도적 유지 또는 기존 미해결 이슈**다
**① `매장`(food26 · beauty17)** — 의도적 유지.
food는 restaurant 단독이라 정상이고, beauty는 5개 업종이 **모두 점포를 가지므로**
6-2에서 유지 판단했다(2026-09-04 승인 결정). retail은 무역상사·패션 브랜드 때문에 `사업장`으로 바꿨다.

**② `파사드`(food4 · beauty4)** — `2_1`·`2_2`의 **기본 ITEMS 상속**이다.
두 그룹 다 점포 업종이라 성립하지만, **etc·retail·edu_service·pro_service는 `첫 접점`으로
중립화했으므로 그룹 간 표현이 갈린다.** 점포 업종에는 `파사드`가 더 구체적이라 그대로 두었다.

**③ `플레이스`(5개 그룹 각 10~11건)** — **전 그룹 공통 미해결 이슈**다.
`2_2`·`7_4`·`7_5`의 **label이 기본 ITEMS에서 상속**되고(`네이버 플레이스 정보 일치도`·
`플레이스 저장하기(하트) 유도`·`플레이스 CTR 최적화`), `DOMAINS.label`도
`점포환경·네이버 플레이스`다. **`etc`에서만 중립화했는데 그쪽은 "업종 미판별" 특수 케이스**였다.
⚠ `label`은 `calcScores`가 레이더차트·대시보드·AI 프롬프트로 그대로 흘려보내므로
**한 그룹만 바꾸면 표시가 갈린다.** 전 그룹을 함께 바꾸려면 표시 계층까지 설계해야 한다.

### ⑦ 6단계 마무리 — `플레이스`·`파사드` 정리 (2026-09-16)

⑥의 전수 재검사에서 남은 2건을 처리했다.

#### ⚠ 원인 — 기본 ITEMS 상속 label·question이었다
`2_2`·`7_4`·`7_5`의 **`label`과 `question`이 그룹 오버라이드에 없어 기본 ITEMS에서 상속**되고 있었다.
1차 치환이 오버라이드 블록만 훑어 **닿지 않았고**, 재검사에서 21건이 그대로 남았다.
→ **4개 그룹에 `label`·`question` 필드를 신설**(21줄)해 해결했다.
**6-3의 `2_1.label` 상속 누락과 같은 유형이 세 번째다.**

| 문항 | before (기본 상속) | after (4그룹 공통) |
|---|---|---|
| `2_2.label` | `네이버 플레이스 정보 일치도` | **`온라인 등록 정보 일치도`** |
| `7_4.label` | `플레이스 저장하기(하트) 유도` | **`관심 고객 재접촉 수단 확보`** |
| `7_5.label` | `플레이스 CTR 최적화` | **`노출 대비 반응률 관리`** |
| `2_2.question` | `네이버·카카오 플레이스 등 온라인 지도 플랫폼에` | `온라인에 등록된 상호·주소·연락처·영업시간·취급 내용이 채널마다` |
| `7_4.question` | `자사 플레이스 채널을 향한 "저장하기" 트래픽 지수` | `즐겨찾기·알림 설정·채널 추가 같은 재접촉 수단` |

`etc`에서 쓴 라벨과 **같은 표현으로 통일**했다.

#### 판단 — food는 유지, beauty는 중립화
**① `food`의 `플레이스`·`파사드`는 유지했다.**
`restaurant` **단독 그룹**이고 네이버 플레이스는 외식업의 **실제 핵심 유입 채널**이다.
매장 전면(파사드)도 외식업 진입 동선의 실제 축이라 중립화하면 구체성만 잃는다.
→ 전수 재검사 표에서 food의 `파사드4 플레이스11`은 **외식 전용어로 분류**된다.

**② `beauty`의 `파사드`는 제거했다.** 미용실·세탁소·수선집·펫샵은 점포 전면이 실제로 중요하지만
**필라테스는 건물 안 층에 있는 경우가 많아 파사드가 성립하지 않는다.**
⚠ 다만 `매장`은 그대로 두었다 — 5개 업종이 모두 점포를 가지며 2026-09-04 승인 결정이다.
`매장 전면(파사드)` → **`매장 전면`**으로 전문용어만 덜어냈다.

**③ `beauty`의 `플레이스`도 중립화했다.** 미용·세탁·수선·펫샵은 실제로 쓰지만
필라테스는 예약 앱·홈페이지가 주 채널이다. **guide 첫 번째에 `네이버 플레이스·카카오맵`을
남겨 손실이 없다.**

#### guide — 업종별 채널을 병기했다
| 그룹 | `2_2` guide |
|---|---|
| beauty | `미용·세탁·수선·펫샵은 네이버 플레이스·카카오맵이 주 채널이고, 필라테스는 예약 앱과 홈페이지도 함께 본다` |
| retail | `소매점은 네이버 플레이스·카카오맵, 도매·총판은 기업정보 디렉터리, 패션 브랜드는 온라인몰·입점 채널` |
| edu_service | `학원·교습소는 네이버 플레이스…. ⚠ 온라인강의는 지도 등록 대신 강좌 목록과 강사 소개 페이지가 그 자리다` |
| pro_service | `의원·헬스장은 네이버 플레이스…. ⚠ IT·SaaS·미디어는 지도 등록 대신 홈페이지·포트폴리오 사이트가 그 자리다` |

#### 검증
| 항목 | 결과 |
|---|---|
| **[1] 10그룹 전수** | **food 외 9그룹 전부 0건** · food는 외식 전용어(정상) ✓ |
| **[2] guide 병기** | 4그룹 × `2_2`·`7_4`·`7_5` = **12건 전부 업종 예시 포함** ✓ |
| **[3] calcScores** | 30조합 불일치 **0** ✓ |
| **[4] 조사 파손** | 코드포인트 판정 **0건** ✓ |
| 불변 | 기본 `ITEMS`·`ACTION_PLAN` 전체·`DOMAINS` 전 그룹 · **food·신설4·etc 6그룹 JSON 동일** ✓ |
| 키 개수 | 전 그룹 **35/35** · 실효 개수 전부 유지 ✓ |

#### ⚠ `DOMAINS.label`은 여전히 `점포환경·네이버 플레이스`다
`ITEMS`의 label은 그룹 오버라이드가 정식 메커니즘이라 바꿨지만,
`DOMAINS`는 **`label`·`key`·`weight`·`id`를 절대 분기하지 않는다**는 이 모듈의 원칙이 있다
(`calcScores`가 `domain.label`을 레이더차트·대시보드·AI 프롬프트로 그대로 흘려보낸다).
**전 그룹 공통으로 바꾸려면 표시 계층까지 함께 설계해야 한다.**

#### 캐시버스팅
`?v=` 52곳 `20260916b`

### ⑧ ⚠ 누락 보완 — `5c88e1d`에 CLAUDE.md 기록이 없었다
작업 규칙에 *"모든 커밋에 CLAUDE.md 이력 기록을 포함한다"*가 있으나
`5c88e1d`(beauty `2_5` 조사 파손 수정)는 코드와 `?v=`만 바꾸고 기록을 남기지 않았다.
내용은 ②의 캐시버스팅 항목에 이미 서술돼 있었으나 **커밋 단위로는 누락**이었다.
→ 이 커밋에서 보완한다. **나머지 14개 커밋은 전부 기록되어 있음을 확인했다.**

### ⑨ facility_service 업종 특화 진단 모듈 신설 (2026-09-16)

**5단계에서 그룹만 신설하고 `js/diagnosis/industry/facility_service.js`를 만들지 않아
업종 특화 탭이 비어 있었다.** 사용자가 브라우저에서 확인했다 —
*"경비·보안업 맞춤진단 설문지는 오케이. 업종별 특화진단은 없음"*.
다른 17개 업종은 전부 모듈이 있었다.

#### ⚠ 맞춤진단과 축을 나눴다 — 중복 회피가 설계의 출발점
| | 무엇을 보는가 |
|---|---|
| **맞춤진단**(DiagMicro facility_service 35문항) | **경영 관리 수준** — 실질 인건비·계약별 수익성·관리 실적·갱신·근태 기록 |
| **업종 특화**(신설 16문항) | **현장 실행 수준** — 배치 설계·작업 표준·안전 이행·발주처 운영 |

⚠ **지시 초안을 그대로 쓰지 않았다.** 초안의 `결원 대체 투입`·`민원 처리 기록`·
`근무일지·점검 기록`·`4대보험 가입 판단`·`단가 인상 조항`·`배상책임보험`·`허가 유효`는
**전부 맞춤진단이 이미 다루는 축**이다(`4_1`·`4_5`·`5_3`·`3_3`·`2_3`·`2_2`).
그대로 넣으면 같은 질문을 두 번 묻게 되므로 **한 단계 더 현장으로 내렸다.**

| 영역 | 문항 | 맞춤진단과의 차이 |
|---|---|---|
| **현장 배치·교대 운영** | 배치 인원 산정 근거 · 교대 인수인계 · 현장 책임자 권한 · 다현장 관리 부담 | 맞춤은 "결원 대응 속도", 여기는 **배치 설계 자체** |
| **작업 표준·품질 증빙** | 업무 범위 표준서 · 작업 주기표 · 발주처 확인 요청 대응 · 소모품·장비 사용 기준 | 맞춤은 "편차 통제", 여기는 **표준의 존재와 증빙** |
| **안전·법정 의무 이행** | 작업 전 위험 확인 · 사고 보고 절차 · 보호구 관리 · 법정 교육·신고 기한 | 맞춤은 "위험 지점 점검·보험 가입", 여기는 **사고 대응과 기한 관리** |
| **발주처 운영·현장 이관** | 정기 보고 · 요청사항 반영 이력 · 신규 현장 인수 점검 · 계약 종료 정리 | 맞춤은 "계약 조건·갱신", 여기는 **운영 중 관계와 인수·인계** |

⚠ **`신규 현장 인수 점검`과 `계약 종료 정리 절차`는 어느 쪽에도 없던 축이다.**
정기 도급은 계약 시작과 끝에서 손실이 반복되는데(인수 전 하자를 떠안거나
종료 시 숙련 인력이 갈 곳 없이 이탈) 진단 어디에서도 묻지 않고 있었다.

#### 한쪽 전용어는 scale 예시로만 처리했다
`순찰`·`약품`·`초소`·`승강기` 등을 **question에 0건**으로 두고,
필요한 곳은 scale 서술에 예시로 넣었다(`예: 세제·소모 자재·장비 부품`,
`예: 높은 곳 작업·전기 설비·밀폐 공간`). 영문 약자도 본문 0건이다.

#### ⚠ 등록 지점은 3곳이며 전수 확인했다
| 지점 | 역할 |
|---|---|
| `index.html` 스크립트 태그 | 로드 — `agri_food.js` 다음, `app.js`보다 앞 |
| `wizard.js` `industryVarMap` | **업종 탭 렌더링** — 이것이 없어 탭이 비었다 |
| `ai-engine.js` `industryVarMap` | AI 프롬프트 `insights` 주입 |

⚠ **`ai-engine`의 등록은 현재 전 업종에서 무효다.** `buildInsightsSummary()`가
`industryData.insights`를 읽는데 **18개 모듈 중 `insights` 필드를 가진 것이 하나도 없다**
(v2.0 재작성 때 `ai_analysis`로 바뀌면서 사라진 것으로 보인다).
일관성을 위해 등록은 했으나 **동작에 변화가 없으며, 별도 확인이 필요한 기존 결함이다.**

#### 검증 (전 항목 통과)
| 항목 | 결과 |
|---|---|
| **[1] 구조** | 4영역 × 4문항 = **16문항** · `type:'bars'` 전수 · **scale 5단계 전수** · id 중복 0 · `ai_trigger` 16건 ✓ |
| **[2] 렌더링** | 18개 모듈 동시 로드 시뮬레이션 → `industryVarMap['facility_service']`가 **모듈 반환(16문항)** → `renderDiagModule` 실행 ✓ |
| **[3] 진행률** | DiagMicro **35** + 업종 특화 **16** = **51** ✓ |
| **[4] 중복** | 라벨 완전 일치 **0건** · 맞춤진단 9개 축 **재등장 0건** ✓ |
| **[5] 다른 17개 모듈** | **변경 없음**(신규 파일 1개만) ✓ |
| **[6] 전용어** | 외식·소매·타 업종 18종 **0건** · 영문 약자 **0건** · question에 한쪽 전용어 **0건** ✓ |
| `ai_analysis` | 6건 · trigger 참조 **전부 유효** ✓ |

#### 캐시버스팅
`?v=` **53곳**(신규 스크립트 1개 추가로 52 → 53) `20260916c`

### ⑬ 5단계 완료 상태 — 10그룹
| 그룹 | 문항 오버라이드 | ACTION_PLAN |
|---|---|---|
| food | 29 | base 폴백(설계대로) |
| beauty / retail / edu_service | 26 / 26 / 27 | 각 35 |
| pro_service / manufacturing | 27 / 31 | 각 35 |
| construction / trade_logistics | 26 / 26 | 각 35 |
| **facility_service** | **26** | **35** |
| etc | 0(중립 기본, 의도) | 35 |

### ⑨ 남은 단계와 이슈
- ✅ **6단계 완료 — 기존 4그룹 + etc 전용어 정리.**
  `etc`(5문항) · `beauty`(시술·샴푸·홈케어) · `retail`(매장·진열·계산대) ·
  `pro_service`(수임·상담실·파일실) · `edu_service`(교실·원장) 전부 처리했다.
  **17개 검사어 중 13개가 전 그룹 0건**이며, 남은 `매장`·`파사드`·`플레이스`는
  의도적 유지이거나 기존 미해결 이슈다(⑥ 참조).
- ✅ **`플레이스`·`파사드` 정리 완료(⑦).** `2_2`·`7_4`·`7_5`의 label·question을
  4그룹에 신설해 **food 외 9그룹 전부 0건**이 됐다. food는 restaurant 단독이고
  네이버 플레이스가 실제 핵심 채널이라 의도적으로 유지한다.
- ⚠ **`DOMAINS.label`은 여전히 `점포환경·네이버 플레이스`·`SNS·생성형AI`다.**
  `DOMAINS`는 `label`·`key`·`weight`·`id`를 절대 분기하지 않는다는 원칙이 있다
  (`calcScores`가 `domain.label`을 레이더차트·대시보드·AI 프롬프트로 그대로 흘려보낸다).
  **전 그룹 공통으로 바꾸려면 표시 계층까지 함께 설계해야 한다**
- ✅ **별건 ① 해결 — `facility_service` 업종 특화 모듈 신설(⑨).**
  4영역 × 4문항 = 16문항, 등록 3곳 완료. **진행률 35 → 51**.
- ⚠ **`ai-engine`의 `industryVarMap`은 전 업종에서 무효다.**
  `buildInsightsSummary()`가 `industryData.insights`를 읽는데
  **18개 업종 모듈 중 `insights` 필드를 가진 것이 하나도 없다**
  (v2.0 재작성 때 `ai_analysis`로 바뀌면서 사라진 것으로 보인다).
  업종 인사이트가 AI 프롬프트에 주입되지 않고 있다 — **별도 확인이 필요하다**
- ⚠ **별건 ②: 주유소가 `wholesale`로 분류되는데도 맞춤진단이 헤어숍과 동일하다는 보고.**
  5-1에서 주유소를 `local_service` → `wholesale`(retail 그룹)로 옮겼으나,
  **AI(`api/analyze-biz.js`)가 실제로 어떤 키를 반환하는지 확인되지 않았다.**
  `local_service`를 반환하면 `beauty` 그룹(시술·예약) 문구를 그대로 받는다.
  **실측이 선행되어야 한다 — 분류 기준 문구만 바꾸고 검증하지 않은 것이 원인일 수 있다**
- ✅ **5-4 완료 — 5단계 전체 완료.** `local_service` 분리 → `facility_service` 신설 →
  문항 26 → 부속 시스템 → ACTION_PLAN 35까지 갖춰졌다
- ⚠ **`DOMAINS.label`이 `점포환경·네이버 플레이스`·`SNS·생성형AI`인 채로 남아 있다.**
  `desc`만 분기하는 것이 이 모듈의 원칙이라(`calcScores`가 `domain.label`을 레이더차트·
  대시보드·AI 프롬프트로 그대로 흘려보낸다) 손대지 않았다. construction·trade_logistics·
  manufacturing도 같은 상태다. **바꾸려면 레이더차트·대시보드 표시까지 함께 설계해야 한다**
- ⚠ **`DOMAIN_TO_ACTION_KEY`에 D5·D6이 없어 모든 그룹이 그 두 도메인은 base를 쓴다.**
  두 title(`사업 전용 계좌 분리…`·`월간 BEP 돌파율…`)이 중립이라 지금 문제는 없다
- ✅ **5-3 완료** — 부속 시스템 6곳 연결 (reference-db·pattern-db는 의도적 미처리, 근거는 ⑪)
- ⚠ **KOSIS `facility_service: 'S'`는 근사값이다.** 실제 KSIC는 **N(사업시설관리·사업지원 서비스업)**
  이나 `FALLBACK` 테이블에 N 항목이 없다. `energy: 'C' // 제조업 근사`와 같은 방식이며
  **N 통계를 확보하면 교체할 것.** 현재는 기타개인서비스(y3 29.1%) 기준으로 표시된다
- ⚠ **`pattern-db`는 전 업종이 local_service 통계로 폴백 중이다**(기존 결함, ⑪ 참조).
  `facility_service`·`etc` 항목이 없는 것은 그 결함의 일부이며,
  **PatternDB 4축 ↔ D1~D7 매핑 설계가 선행되어야 한다**
- ⚠ **`reference-db`에 `facility_service`·`etc` 벤치마크가 없다.** 블록이 생략되므로
  깨지지는 않으나 AI 프롬프트에서 업종 준거 수치가 빠진다.
  **확인된 통계를 확보한 뒤 추가할 것 — 지어내지 말 것**
- ⚠ **`industryVarMap`에 `facility_service`·`etc`가 없다**(전용 업종 진단 모듈 부재).
  `if (industryData && industryData.insights)` 가드가 있어 안전하며, 진행률이 35로 나오는 것과
  같은 원인이다(5-1 ⑥)
- ✅ **5-2 완료** — facility_service 문항 26개 오버라이드 + `DOMAIN_DESC` 6개 도메인
- ⚠ **5-2 시점의 미해결**: `ACTION_PLAN_7DAY_BY_GROUP`에 `facility_service`가 없어
  **7일 실행계획이 base(외식)로 폴백한다.** 시설관리 사장님이 `Dog 메뉴 제거`·
  `테이블에 QR 스탠드 부착`을 받는다. **5-4에서 반드시 처리할 것** — 4단계 때 확인했듯
  7일 계획은 사용자가 실제로 읽고 행동하는 부분이라 문항보다 체감 영향이 크다
- ⚠ **`DOMAINS.label`은 여전히 `점포환경·네이버 플레이스`·`SNS·생성형AI`다.**
  `desc`만 분기하는 것이 이 모듈의 원칙이라(레이더차트·대시보드가 `label`을 그대로 받는다)
  손대지 않았다. construction·trade_logistics·manufacturing도 같은 상태다 —
  **라벨을 업종 중립으로 바꾸려면 레이더차트·대시보드 표시까지 함께 설계해야 한다**
- **5-3** 부속 시스템 9개 파일. ⚠ **`gov-support`의 `PROGRAMS.industry` 태그가 한국어 `'서비스업'`이라
  새 라벨을 쓰면 매칭이 0이 된다** — 라벨 처리 방침을 5-3에서 판단한다
- **5-4** ACTION_PLAN 5키 × 7일 = 35항목
- ⚠ **KOSIS**: `facility_service`의 실제 KSIC는 **N(사업시설관리·사업지원)**이나
  `FALLBACK` 테이블에 N이 없어 **`'S'`(기타개인서비스)로 근사**할 예정이다(5-3).
  `energy: 'C' // 제조업 근사`와 같은 방식. **통계 확보 시 N으로 교체할 것**
- ⚠ **업종 수동 정정 UI가 경영진단 경로에 없다.** `loadDiagnosisUI(forceIndustryKey)` 인자는 있으나
  사용자가 업종을 고를 UI는 정책자금 경로(`fundIndustryOverride`)에만 있다.
  AI가 실패하면 사용자가 바로잡을 방법이 없다 — **별도 작업으로 검토 필요**
- ⚠ **etc 문항의 잔여 6건**(2_1 파사드 · 2_2·7_1·7_4·7_5 플레이스)은 그대로다.
  폴백이 etc로 바뀌면서 **영향 범위가 넓어졌다** — 우선순위를 올려 검토할 것

---

## 최근 수정 이력 (2026-09-09) — DiagMicro 업종→그룹 전면 재편 (6그룹 → 9그룹) · 1단계 골격

**"인테리어 공사·가구 제조 복합업"으로 진단했더니 매장 문항이 나왔다는 실사용 보고에서 출발했다.**
> "매장 반경 500m 이내 유동인구 증감 추이" · "일별·시간대별 POS 결제 내역" ·
> "매장 내 재고 관리, 고객 응대, 상품 발주 업무 중 점주가 직접…" · "네이버 플레이스"

**현장 시공업에 소매점 문항이 나가고 있었다.**

### ① 원인 — retail이 "기타" 통이 됐다
16개 업종이 6그룹에 배정돼 있었는데 **retail 하나에 5개 업종**(wholesale·fashion·
logistics·construction·export_sme)이 몰려 사실상 분류 기능을 잃었다.

| 업종 | 받고 있던 문구 | 실제 필요한 축 |
|---|---|---|
| construction | 매장·POS·계산대 | 현장 시공·기성 수금·공정률 |
| logistics | 매장 | 차량·배송 거점·배차 |
| export_sme | 국내 상권 | 해외 바이어·통관·환율 |
| agri_food | 외식 홀 | 1차 생산·원물 |
| energy | 상담실(pro_service) | 설비 시공·인증 |

### ② ⚠ 사전 확인에서 더 큰 것이 나왔다 — `etc`가 외식을 받고 있었다
`wizard.js`는 매핑되지 않은 업종을 **`'etc'`로 떨어뜨린다**(`INDUSTRY_MAP[industry] || 'etc'`, 2곳).
`INDUSTRY_MAP`에 `'기타': 'etc'`도 실재한다. 그런데 `INDUSTRY_GROUP_MAP`에 `etc`가 없어
`getGroup`의 `|| 'food'` 폴백이 걸렸다 →
**기타 업종·업종 미판별 사용자가 외식 35문항을 100% 받고 있었다.**
2026-09-03에 고친 `mfg_parts` 오매핑과 같은 구조다.

⚠ **`getGroup` 기본값만 중립으로 바꾸는 것으로는 절반만 해결된다.** 실측 결과
미등록 그룹은 **부속 테이블 3개가 여전히 food로 폴백**했다:
| 테이블 | 미등록 그룹의 실제 출력 |
|---|---|
| `WARN_WORDING` | `"…실제로 남는 돈이 적은 **메뉴** 정리가 즉각 필요합니다"` |
| `DOMAIN_DESC_BY_GROUP` | D2 `"**매장 전면(파사드)**…**네이버 플레이스** 연동"` |
| `ACTION_PLAN_7DAY_BY_GROUP` | `null` → base(외식) 폴백 |

→ **`etc`를 정식 그룹으로 등록**하고 네 테이블을 모두 채우는 방식(A안)을 채택했다.
`INDUSTRY_WORDING.etc`만 의도적으로 비워 둔다 — 업종을 특정할 수 없으므로 중립 기본이 맞다.

### ③ 확정 그룹 9개
| 그룹 | 업종 |
|---|---|
| food | restaurant |
| beauty | local_service |
| retail | fashion, wholesale |
| edu_service | education |
| pro_service | medical, knowledge_it, finance, media |
| manufacturing | mfg_parts, food_mfg, **agri_food**(이동) |
| **construction**(신설) | construction, **energy**(이동) |
| **trade_logistics**(신설) | logistics, export_sme |
| **etc**(신설) | etc + **모든 미판별 폴백** |

### ④ 1단계에서 한 것 (골격만)
`INDUSTRY_GROUP_MAP` 재배정 · `GROUP_LABELS` 3개 추가 ·
`INDUSTRY_WORDING`에 3그룹 **빈 객체**(2·3단계 앵커) ·
`WARN_WORDING` 3그룹(8필드) · `DOMAIN_DESC_BY_GROUP` 3그룹(도메인 1·2·3·4) ·
`getGroup` 폴백 `'food'` → **`'etc'`**.

⚠ `WARN_WORDING`은 **완성된 구(句)로 저장**한다 — 어절만 바꾸면 한국어 조사가 어긋난다.
2026-09-04에 어절 단위로 바꿨다가 food 문구까지 훼손한 전례가 있다.
⚠ `DOMAIN_DESC_BY_GROUP`은 **`desc`만** 덮는다. `label`·`key`·`weight`·`id`는 절대 분기하지 않는다 —
`calcScores`가 `domain.label`을 반환값에 실어 레이더차트·대시보드로 흘려보낸다.
⚠ `construction`은 인테리어 시공과 태양광 설비를 모두 포괄해야 하므로
한쪽 전용어(도배·타일 / 모듈·인버터)를 쓰지 않았다. `subj`는 `'이 사업장은'`.

### ⑤ 1단계 시점 상태 — 중립이 retail보다 명확히 낫다 (실측)
본문(`label`·`question`·`scale`)의 소매/외식 용어 건수:
| 상태 | 건수 | 내역 |
|---|---|---|
| **이전(retail)** | **26건** | 매장 11 · 계산대 6 · 진열 4 · 플레이스 3 · POS 2 |
| **1단계 후(중립 기본)** | **6건** | 매장 2 · 플레이스 4 |

⚠ **0건은 아니다.** 남은 6건의 정확한 위치이며 **2·3단계에서 반드시 덮어야 한다**:
`2_1`(label·scale4 — 매장 전면·파사드) · `2_2`(label·question·scale3~5 — 네이버 플레이스) ·
`7_1`(question) · `7_4`(label·question — 플레이스 저장하기) · `7_5`(label·question·scale1 — 플레이스 CTR)

### ⑥ 검증 (57/57 + 렌더 12/12)
| 항목 | 결과 |
|---|---|
| **[1] 9그룹 매핑** | 17개 업종 키 전부 확정안대로 ✓ |
| **[7] 폴백** | `''`·`'unknown'`·`undefined`·`null`·미등록 전부 **`etc`** ✓ |
| **⚠ 9그룹 키 개수(경계 침범 검사)** | 전 그룹 **35/35문항**. 실효 오버라이드 food 29 / beauty 26 / retail 26 / edu 27 / pro 27 / manufacturing 31 / **신설 3그룹 0**(의도) ✓ |
| **[4] 기존 6그룹 불변** | `items`·`domains`·`ACTION_PLAN` 5키 **전부 JSON 동일** ✓ |
| **[5] calcScores** | 1~5점 전부 20/40/60/80/100 동일 ✓ |
| **[6] guide** | **9그룹 × 35 = 315건** ✓ |
| 부속 테이블 폴백 | 신설 3그룹의 경고·D2 desc에 외식/소매 용어 **0건**(food 폴백 안 함) ✓ |
| **[8] 진행률** | 신설 3그룹 전부 **35 + 16 = 51문항** ✓ |
| **[9] 레이더** | 신설 3그룹 전부 **d1~d7 산출, 0점 영역 없음** ✓ |
| scale·ai_trigger | 5단계 전수 · 35건 보존 ✓ |

`DIAG_CONTAINERS`·`_countDiagItems`·`_calcMicroDomainScores`는 **컨테이너 id 고정**이고
group 인자를 받지 않으므로 그룹 추가와 무관하다(코드 확인 + 렌더 실측).

### ⑦ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260909a`

### ⑩ 2단계 — construction 문항 오버라이드 26문항 (2026-09-09)

**사용자가 실제로 보고 문제 제기한 문구가 전부 이 단계에서 사라진다.**

| 보고된 증상 | 출처 | 조치 후 |
|---|---|---|
| "매장 반경 500m 이내 유동인구 증감 추이" | 기본 `1_1` 상권 빅데이터 분석 | **발주 정보 분석** — 발주 물량 추이·경쟁 입찰 참여 |
| "일별·시간대별 POS 결제 내역" | 기본 `1_2` 판매 데이터 분석 | **현장별 수익 데이터 분석** — 실행예산 대비 실투입 |
| "매장 내 재고 관리, 고객 응대, 상품 발주 … 점주가 직접" | 기본 `1_5` | **현장 관리·발주처 응대·자재 발주**의 현장소장 위임 |
| "네이버 플레이스" | 기본 `2_2`·`7_4`·`7_5` | **온라인 사업자 정보 일치도** / **견적 문의 전환** / **노출 대비 문의 전환** |

#### D2를 "발주처가 우리를 어떻게 찾고 신뢰하는가"로 재해석했다
건설·인테리어에는 매장이 없으므로 파사드·보행자 축이 성립하지 않는다.
manufacturing이 `2_1`을 "기업 신뢰도 노출"로 바꾼 것과 같은 처리다.
- `2_1` → **시공 사례·포트폴리오 노출**(완공 사진·시공 전후 비교)
- `2_2` → **온라인 사업자 정보 일치**(상호·사업자 정보·**보유 면허와 인증**)
- `7_4`·`7_5` → **견적 문의 전환**(문의 양식·회신 시간 → 현장 실측 방문)

⚠ **완전히 B2B로 쓰지 않았다.** 인테리어는 개인 발주 비중이 크므로
`3_1`(발주처 다변화)에 `원청·시행사·개인 발주`를 함께 두고,
`2_1` guide에 "개인 발주는 완성도, 원청·시행사는 공기 준수 실적을 먼저 본다"를 명시했다.

#### 축별 반영
| 축 | 문항 |
|---|---|
| 원가 | `1_3` 공사 원가(자재비+노무비+외주비) · `1_4` 공종별 실제 남는 금액(투입 공수 반영) |
| 수주 | `3_4` **견적 대비 수주율**(탈락 사유 유형화) · `3_2` 표준 견적 패키지 · `3_1` 발주처 다변화 |
| 현장 | `4_1` 공정률·공기 준수 · `2_3` 출입·자재 반입 동선 · `2_5` 자재 적치 · `2_4` 마감 품질 편차 |
| 대금 | `3_3` 기성 청구·수금 지연·**하자보수 예치금·어음 만기** · `5_2` 기성 수금과 지출 시점의 균형 |
| 인력 | `5_3` 일용직·기능공 근로계약 + 안전 교육 · `1_5`·`4_5` 현장 위임 |
| 인증 | `2_2`(면허·인증 일치) · `5_4`(폐기물 적법 처리) |
| 판로 | `3_5` 소개·재의뢰 · `7_3` 발주처 후기·추천 |

#### ⚠ 오버라이드하지 않은 9문항 — 억지로 채우지 않았다
`5_1`(가계·사업 계좌) · `5_5`(정책자금·보증) · `6_1`·`6_2`·`6_3`·`6_5`(폐업 법령·제도) ·
`4_3`·`4_4`(도입 기기 효과) · `7_2`(무료 디자인 도구) — **업종과 무관하게 기본 문항이 정확하다.**

#### ⚠ 법령 수치는 확신한 것만 썼다
`5_3` guide에 **"일용직도 근로계약서 작성·교부 의무가 있고, 산재보험은 근로자를 쓰면 원칙적으로 가입 대상"**까지만 적고,
**적용 범위·절차는 "관할 기관에 확인할 것"으로 남겼다.** 건설업 산재 적용은 공사 금액·기간에 따라 갈려
확정 수치를 쓰면 오정보가 된다. 2026-09-07 1인 사업장 작업의 법령 기준을 그대로 적용했다.

#### 검증 (구조 57/57 · 렌더 12/12 · 용어 0건)
| 항목 | 결과 |
|---|---|
| **[1] 부적합 용어** | 본문(label·question·scale)에 매장·POS·계산대·진열·플레이스·파사드·유동인구·상권·보행자·점주 **0건** ✓ |
| 보고 증상 4건 | `1_1`·`1_2`·`1_5`·`2_2` 전부 대체 확인 ✓ |
| **[2] 기존 6그룹** | `items`·`domains`·`ACTION_PLAN` **JSON 동일** ✓ |
| **[3] calcScores** | 1~5점 20/40/60/80/100 동일 ✓ |
| **[4] 진행률·레이더** | 51문항 · d1~d7 산출, 0점 영역 없음 ✓ |
| **⚠ 9그룹 키 개수** | 전 그룹 35/35. construction **실효 26문항**(food 29 / manufacturing 31과 같은 부분 커버리지) ✓ |
| scale·guide·ai_trigger | 5단계 35/35 · guide 35/35 · ai_trigger 35/35 보존 ✓ |
| guide 양쪽 예시 | 인테리어·태양광 병기 **20문항** ✓ |

⚠ 패치 시 `INDUSTRY_WORDING` 앵커 이후에서만 탐색하고 그룹 경계 assert를 걸었다 —
`construction`은 `DOMAIN_DESC_BY_GROUP`에도 같은 키가 있어 D4 때와 동일한 조건이다.

#### 캐시버스팅
`?v=` 52곳 `20260909b`

### ⑪ 3단계 — trade_logistics 문항 오버라이드 26문항 (2026-09-10)

**이번 단계의 난점은 한 그룹이 축이 다른 두 업종을 담는다는 것이었다.**
`logistics`(물류운송)와 `export_sme`(수출중소기업)는 원가 구성부터 다르다 —
물류는 `유류비+인건비+차량 유지비`, 수출은 `매입가+물류비+관세`.

#### 해결 — 본문은 상위 개념, guide에서 갈라 준다
`배차`는 물류만, `통관`·`선적`·`환율`은 수출만 쓰는 말이라 **본문에 쓰지 않았다.**
| 본문(공통 상위 개념) | guide (양쪽 병기) |
|---|---|
| `직접 원가 비율 통제` | 물류 = 유류비+기사 인건비+차량 유지비 / 수출 = 매입가+물류비+관세 |
| `운행·출고 계획 수립` | 물류 = 운행 배차 / 수출 = 선적 일정 조율 |
| `납기 준수·지연율` | 물류 = 인도 지연(차량 고장·상하차 대기) / 수출 = 출고 지연(자재 수급·선적 일정) |
| `수금 주기와 지출 시점의 균형` | 물류 = 경유 시세 / 수출 = 환율 변동 |
| `제안 대비 성사율` | 물류 = 운임 견적 대비 배정률 / 수출 = 해외 문의 대비 주문 전환율 |

**guide에 물류·수출을 함께 명시한 문항이 24개**다.

#### D2를 "거래처가 우리를 평가하는 첫 접점"으로 상위화했다
⚠ **두 업종이 가장 크게 갈리는 지점이었다.** 물류는 차고지·차량·안전 실적이,
수출은 회사 신뢰도·거래 실적이 그 자리에 온다. 둘을 한 문장에 담을 수 없어
**`2_1`을 "거래처가 처음 검토할 때 보게 되는 자료(보유 자원·처리 실적·회사 소개)"**로 올리고
구체는 guide로 내렸다(물류 = 차량 대수·거점·안전 운행 실적 / 수출 = 수출 실적국·공급 능력·품질 인증).

#### 무역 약자는 없애지 않고 guide에 풀네임 + 정의를 달았다
업계에서 실제로 쓰이는 말이라 없애면 오히려 소통이 안 된다.
전문용어 정리 4단계(2026-09-07)에서 세운 **"본문은 쉬운 말, guide에 풀네임+정의"** 원칙을 그대로 적용했다.
- `3_3` guide — **신용장(L/C, Letter of Credit — 은행이 대금 지급을 보증하는 방식)**은 안전하나 수수료·서류 부담이 있고,
  **전신송금(T/T, Telegraphic Transfer — 은행 계좌 송금)**은 간편하나 선적 후 송금이면 미수 위험이 있다
- `1_3`·`2_2` guide — **자유무역협정(FTA)** 원산지 증명·원산지 인증수출자 지정
본문은 `결제 조건`, `보유 면허와 인증`처럼 쉬운 말로만 썼다.

#### ⚠ 함께 넣은 실무 주의 2건
- `3_5`(직접 거래 전환) — **"중개 계약에 직접 거래를 제한하는 조항이 있는지 먼저 확인해야 한다."**
  수수료 절감만 권하면 계약 위반을 유도하게 된다
- `5_3` — **위·수탁(지입) 계약은 근로자성 판단이 갈릴 수 있어** 계약 형태 확인이 필요하다고만 적고
  단정하지 않았다. 건설 `5_3`과 같은 기준이다

#### ⚠ 오버라이드하지 않은 9문항
`5_1`·`5_5`·`6_1`·`6_2`·`6_3`·`6_5`·`4_3`·`4_4`·`7_2` — construction과 동일하게
**업종과 무관한 법령·제도·일반 도구 문항**이라 기본을 그대로 쓴다.

#### 검증 (구조 · 용어 · 렌더 전부 통과)
| 항목 | 결과 |
|---|---|
| **[1] 매장·소매 용어** | 본문 **0건**(매장·POS·계산대·진열·플레이스·유동인구·메뉴 등 17종) ✓ |
| **[2] 한쪽 전용어 혼입** | 물류 전용어 **0건** / 수출 전용어 **0건** ✓ |
| **[3] guide 양쪽 병기** | 물류·수출 함께 명시 **24문항** · L/C·T/T·FTA 풀네임 병기 ✓ |
| **[4] 기존 7그룹 + etc** | `items`·`domains` **JSON 동일** ✓ |
| **[5] calcScores·진행률·레이더** | 1·3·5점 동일 · 51문항 · d1~d7 산출 ✓ |
| **⚠ 9그룹 키 개수** | 전 그룹 35/35. trade_logistics **실효 26문항** ✓ |
| scale·guide·ai_trigger | 5단계 35/35 · guide 35/35 · ai_trigger 35/35 ✓ |

⚠ **검사기 오탐 1건을 확인했다.** `7_4`의 `"거래 전환율"`이 `환율`로 잡혔다.
부분 문자열 매칭이라 앞 글자가 `전`인 경우를 제외해 재검사했고 실제 위반은 0건이다.
**용어 검사기는 단어 경계를 고려하지 않으면 오탐이 난다** — `포스터`가 `포스`로 잡혔던 것과 같은 유형이다.

#### 캐시버스팅
`?v=` 52곳 `20260910a`

### ⑫ 4단계 — ACTION_PLAN 신설 3그룹 105항목 (2026-09-10) · 재편 완료

**⚠ 3단계까지가 가장 나쁜 상태였다.** 문항은 건설·물류로 바뀌었는데
`ACTION_PLAN_7DAY_BY_GROUP`에 항목이 없어 **base(외식)로 폴백**했다.
건설업 사장님이 `"Dog 메뉴 제거"`·`"플레이스 대표 사진 교체"`·`"테이블에 QR 스탠드 부착"`을 받았다.
**7일 계획은 사용자가 실제로 읽고 행동하는 부분이라 문항보다 체감 영향이 크다.**

`construction` · `trade_logistics` · `etc` × 5키 × 7일 = **105항목** 신설.

#### 작성 축 (3단계까지 확정한 것을 그대로 이었다)
| 키 | construction | trade_logistics | etc |
|---|---|---|---|
| `profit_ops` | 실행예산 대비 실투입 → 적자 공종 식별 → 견적 단가 갱신 | 건별 직접 원가+인건비 → 적자 거래 → 단가 재협상 | 품목별 원가 → 4분면 분류 → 취급 판단 |
| `place_seo` | 시공 사례 정리 · **면허 유효기간 점검** · 채널 정보 일치 | 보유 자원·처리 실적 1장 · 면허·보험 점검 | 온라인 정보 전수 확인 · 불일치 수정 |
| `multichannel` | **견적 대비 수주율** · 탈락 사유 분류 · 소개 요청 | 1위 의존도 · 성사율 · **중개 계약 제한 조항 확인** | 채널별 수수료 차감 후 수익 비교 |
| `smart_dx` | 공정표·진행률 · 기성 입금과 지출 대조 | 지연율 집계 · 입금·지출 대조 · 서류 양식화 | 주간 매출·입금 양식 · 중복 입력 제거 |
| `sns_ai` | 완공 사진 3장 · 견적 문의 양식 · 24시간 회신 | 회사 소개 · 검색어 기준 품목명 · 문의 양식 | 대표 이미지 · 소개 글 · 문의 양식 |

#### ⚠ 7일 안에 실제로 할 수 있는 일로만 썼다
`ERP 도입` 같은 것은 넣지 않고 **`견적서 양식에 납기 확약란과 기성 지급 조건란 추가`**,
**`반복 서류 중 1건을 양식화`**, **`중개 계약서에서 직접 거래 제한 조항 유무 확인`** 수준으로 맞췄다.
`{ day, title }` 형식만 쓰고 `desc`는 만들지 않았다(그룹 계획에는 원래 없는 필드다).

⚠ 한쪽 전용어를 `title`에 쓰지 않았다. `trade_logistics`는
`"물류는 유류비·통행료, 수출은 매입가·물류비"`처럼 **한 title 안에 양쪽을 병기**했다.
⚠ `etc`는 업종을 특정할 수 없으므로 **공사·배차·메뉴·시술 같은 전용 행동을 일절 쓰지 않고**
매출·원가 파악, 고객 관리, 온라인 정보 정비, 자금 계획 같은 일반 항목으로만 구성했다.

#### 검증 (전 항목 통과)
| 항목 | 결과 |
|---|---|
| **[1] 항목 수** | 3그룹 각 35 = **105항목** · `day` 1~7 순차 · `title` 전수 존재 · `desc` **0건** ✓ |
| **[2] 외식·소매 용어** | construction **0건** / trade_logistics **0건**(15종 검사) ✓ |
| **[3] etc 전용어** | 특정 업종 전용어 **0건**(외식·소매 15종 + 건설·물류·수출 15종 = 30종 검사) ✓ |
| **[4] 기존 6그룹** | `ACTION_PLAN` 5키 전부 **JSON 동일** ✓ |
| **[5] food 폴백** | 5키 전부 `null` 유지 — **base로 폴백하는 기존 동작 불변** ✓ |
| 문항·guide | 9그룹 `items` 전부 **JSON 동일**(3단계 결과 보존) ✓ |
| **⚠ 9그룹 키 개수** | 전 그룹 35/35 — 경계 침범 없음 ✓ |

#### 캐시버스팅
`?v=` 52곳 `20260910b`

### ⑬ 재편 완료 상태와 남은 것

**6그룹 → 9그룹 재편이 1~4단계로 완료됐다.**
| 그룹 | 문항 오버라이드 | ACTION_PLAN |
|---|---|---|
| food | 29 | base 폴백(설계대로) |
| beauty / retail / edu_service | 26 / 26 / 27 | 각 35 |
| pro_service | 27 | 35 |
| manufacturing | 31 | 35 |
| **construction** | **26** | **35** |
| **trade_logistics** | **26** | **35** |
| **etc** | **0**(중립 기본, 의도) | **35** |

#### ⚠ 남은 것 — etc 문항의 잔여 6건
**`etc`는 ACTION_PLAN은 갖췄으나 문항에 업종 전제가 남아 있다.**
기본 ITEMS의 `2_1`(매장 전면·파사드) · `2_2`·`7_1`·`7_4`·`7_5`(네이버 플레이스) **6건**이며,
`etc` 사용자는 여전히 `"매장 전면"`·`"네이버 플레이스"` 문항을 받는다.

4단계 지시가 **"문항·guide는 건드리지 마라. 3단계에서 끝났다"**였으므로 손대지 않았다.
처리하려면 `INDUSTRY_WORDING.etc`에 5문항 오버라이드를 넣으면 된다
(예: `2_2` → `"온라인에 등록된 사업장 정보가 서로 일치하는가"`).
⚠ **`etc`는 업종 미판별 사용자 전원이 오는 자리라 영향 범위가 좁지 않다.**

#### 5단계 — local_service 분리 (⑧ 참조, 변동 없음)

### ⑧ ⚠ 5단계로 분리한 것 — `local_service` 분리 (반드시 처리할 것)
**`local_service` 하나에 미용실·세탁소·펫샵·주유소·FM·경비·청소대행이 섞여 있다.**
`beauty` 그룹의 **시술·예약 밀도** 문구가 FM·경비·주유소에 부적합하다.

⚠ **그룹만 옮기면 미용실이 그룹을 잃는다.** `api/analyze-biz.js`의 분류 기준상
`local_service`에 **헤어·네일·피부샵이 첫 번째로 명시**돼 있고, `fashion`은
`"(미용 서비스 제외)"`로 명시적으로 배제하므로 미용실이 갈 곳이 없다.
`beauty` 그룹에 배정된 업종도 `local_service` **단 하나**다.

**해결하려면 `api/analyze-biz.js`의 16개 분류에 `facility_service`를 신설해야 하며,
이는 AI 업종 판별 결과가 전 사용자에게 바뀌는 작업이다.**
그룹 재편(1~4단계)과 위험 성격이 달라 **5단계로 분리했다.**
**반드시 처리할 것 — 미루면 세탁소·경비업 사장님이 '시술 재료비'를 계속 받는다.**

5단계 착수 시 확인할 것(1~4단계 중에는 하지 않는다):
- 분류 기준 변경의 파급 범위 — `INDUSTRY_MAP` · `INDUSTRY_GROUP_MAP` ·
  `js/diagnosis/industry/` 18개 모듈 · `gov-support` 태그 · `CrossContext` · `funding-rules`
- `facility_service` 신설 시 **AI 재분류 품질을 어떻게 검증할지**(실측 수단 확보)
- 기존 사용자의 **저장된 진단 이력**(HistoryTracker 스냅샷)에 미치는 영향

### ⑨ 남은 단계
2단계 `construction` D1~D7 · 3단계 `trade_logistics` D1~D7 ·
4단계 `ACTION_PLAN_7DAY_BY_GROUP` **3그룹 × 5키 × 7일 = 105항목**(`etc` 포함).
⚠ `etc`의 ACTION_PLAN을 빠뜨리면 기타 업종 사장님이 `"Dog 메뉴 제거"`·`"플레이스 사진 교체"`를 받는다.

---

## 최근 수정 이력 (2026-09-08) — micro 2차 낭비 필드 제거 (60초 초과 해결 2순위)

**1순위(`_SYSTEM_MICRO_1`) 배포 후 Vercel 로그 실측으로 병목이 2차임이 확정됐다.**

| 호출 | 소요 | output_tokens |
|---|---|---|
| 1차-micro | **47초** | — (49808a9로 92초 → 47초) |
| **2차-micro** | **141초** | **8,349** ← 60초 상한 초과 |
| 3차-micro | 141초 구간 이후 | 4,578 |

경로는 `[1차-micro]` 로그로 micro 정상 확인됐다. **2차 생성량만 줄이면 된다.**

### ① 제거 대상 4개 — micro에서 소비처가 0인 것을 전수 재확인
| 소비처 | keyStrategies · fourP · specializedAnalysis · kpi |
|---|---|
| `dashboard.js` | `sec-strategy`·`sec-4p`·`sec-consulting`·`sec-kpi`가 전부 **`smeOnly` → micro에서 `display:none`**. `renderSpecializedSection`은 `if (!isMicro)`라 **호출조차 되지 않는다** |
| `ppt-export.js` | 네 필드 참조가 전부 **`_buildSme`(331~470) 안**. micro 8장(`_buildMicro` 248~330)은 `executiveSummary`·`lifecycleStage`·`stp`·`tam`/`sam`/`som`·`sixSystems`·`plan90days`만 쓴다 |
| `history-tracker.js` | **참조 0건** (`executiveSummary` 200자만 저장) |
| 3차 프롬프트 | 네 필드를 입력으로 쓰지 않는다 |

→ **141초를 들여 만들고 버리고 있었다.**

⚠ **`roadmap`은 제거하지 않았다.** `_buildPrompt3Micro`가 `r2.roadmap[0].tasks[0]` **한 줄**을
3차 프롬프트에 넣는다(`1단계: …`). 제거하면 3차가 `'즉시 실행 액션'` 폴백으로 떨어진다.
→ **1단계 × task 1개**의 최소 형태로 줄였다. 화면에는 나오지 않으므로(`sec-roadmap` = smeOnly) 손실이 없다.

⚠ **`sixSystems` D1~D4는 건드리지 않았다.** `sec-six-systems`와 PPT '영역별 처방'에 실제로 나온다.

### ② 예상 효과 (실측 8,349토큰 / 141초 = 59 tok/s 기준)
스펙 분량을 출력량의 대리지표로 안분했다.

| 필드 | 스펙 | 비중 | 추정 output | 조치 |
|---|---|---|---|---|
| specializedAnalysis | 341자 | 13.1% | ~1,090 | 제거 |
| keyStrategies | 275자 | 10.5% | ~879 | 제거 |
| kpi | 267자 | 10.2% | ~853 | 제거 |
| fourP | 248자 | 9.5% | ~793 | 제거 |
| roadmap | 561자 | 21.5% | ~1,793 | **최소화(~40)** |
| **sixSystems** | 920자 | 35.2% | ~2,941 | **유지** |

**예상 output ~2,981토큰(36%) → ~51초.**
보수 시나리오(sixSystems가 실제로 40% 차지)에서도 **3,380토큰 → 57초**로 상한 안이다.

입력도 함께 줄었다 — `_SYSTEM_EXEC_MICRO_2` **2,722자 → 1,269자**(~1,701 → ~793 토큰).

### ③ 더 줄여야 할 경우의 후보 (임의로 적용하지 않았다)
57초는 여유가 크지 않다. 배포 후 실측이 60초를 넘으면 아래 순으로 검토한다.
1. `sixSystems.issue` **"2~3문장" → "2문장"** — 화면에 나오지만 길이만 줄이므로 손실이 가장 작다
2. `sixSystems.actions` **3개 → 2개** — 처방 개수가 줄어 체감이 있다
3. `roadmap` 완전 제거 + `_buildPrompt3Micro`가 폴백 문자열을 쓰게 함 — 3차 입력 품질이 미세하게 떨어진다
4. 2·3차의 `microPrompt.substring(0, 500)` 재검토 — 입력 축소라 소요 단축 효과는 작다

⚠ **`sixSystems` 항목 수(D1~D4)는 줄이면 안 된다.** 화면의 7대 영역 처방이 비게 된다.

### ④ 검증 (16/16 통과)
| 항목 | 결과 |
|---|---|
| **[1] 제거** | micro 2차 요청에서 `keyStrategies`·`fourP`·`specializedAnalysis`·`kpi`·`leanCanvas`·`plan90days` **JSON 명세 0건** ✓ |
| 유지 | `sixSystems` D1~D4 **4개** · `roadmap` **1단계만** ✓ |
| **[2] 대시보드 7섹션** | `sec-summary`·`sec-lifecycle`·`sec-market-micro`(1차) · `sec-diag`(진단) · `sec-six-systems`(2·3차) · `sec-plan90`(3차) · `sec-gov`(로컬) — **4개 필드에 의존하는 섹션 없음**. `if (data.kpi)`·`if (data.fourP)`·`if (data.keyStrategies)`·`if (data.roadmap)` 전부 undefined 가드 있음 ✓ |
| **[3] sme 완전 불변** | `SYSTEM`·`_SYSTEM_EXEC`·`_SYSTEM_MICRO_1`·`_SYSTEM_EXEC_MICRO_3` **바이트 동일** · sme 1차·2차 유저 프롬프트 **바이트 동일** ✓ |
| 무변경 | `wizard.js`·`dashboard.js`·`app.js`·`api/claude-analyze-1/2/3`·`vercel.json`·`ppt-export.js`·`history-tracker.js` **전부 바이트 동일** ✓ |

**변경 파일은 `ai-engine.js` 하나뿐이다.**

### ⑤ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260908c`

### ⑥ 남은 이슈
1. **3차(4,578토큰)는 손대지 않았다.** 141초 구간이 2차였으므로 3차 단독 소요를 아직 모른다.
   배포 후 `[3차]` Duration을 확인해 60초를 넘으면 `plan90days`·D5~D7 분량을 검토한다
2. (기존) `dashboard.js`·`ppt-export.js`가 `_diagPathOf`를 쓰지 않는다
3. (기존) 2·3차의 `substring(0, 500)` 컷 근거 불명

---

## 최근 수정 이력 (2026-09-08) — ⚠ 창업 초기 소상공인 진단 점수 전부 0 (경로 판정 비대칭) · _diagPathOf 신설

**"진단을 다 입력했는데 7대 영역이 전부 비고 'D1 미입력'으로 최종 보고서 진입이 막힌다."**
개업 1년 미만 외식업, 종업원 `사장+알바`(solo_pt) 조건에서 보고됐다.

### ① 증상 1·2·3은 하나의 원인이다 — AI 결과 문제가 아니었다
셋 다 `domainScores`가 전부 0인 결과이며, 전부 **AI 호출 전** 진단 점수만으로 그리는 부분이다.
| 증상 | 코드 |
|---|---|
| 7대 영역 프로파일 빔 | `drScoreList` — `avg>0 ? avg.toFixed(1) : '—'` + `'미입력'` |
| 미리보기 내용 빈칸 | `drDomainGuide` — `if (d.avg === 0) return '';` → 카드 전부 소멸 |
| D1 미입력 잠금 | `d1Avg === 0` → `disabled` |

⚠ **`49808a9`(micro 1차 프롬프트)와 무관하다.** 그 커밋에서 `wizard.js`·`app.js`·`dashboard.js`는 바이트 동일이었다.

### ② ⚠ 1인 사업장은 원인이 아니었다 — 지목된 가설을 실측으로 기각
`getSchema('food', 'team'/'solo'/'solo_pt')` 전부 **35문항 · 키 `1_1`~`7_5` 동일**이고,
컨테이너 id도 scale과 무관하게 `'diag-micro-container'` 고정이다. 세 구간 모두 정규식 매칭 정상.
**`SCALE_WORDING`은 문구만 덮고 키를 건드리지 않는다** — 설계대로였다.

### ③ 진짜 원인 — 두 함수가 같은 판정을 각자 하고 있었다
```
loadDiagnosisUI : isMicro = bizScale==='micro' && !isStartupMode && DiagMicro존재  ← 어디에 그릴지
showDiagReveal  : isMicro = !isSocial && bizScale==='micro'                        ← 어느 키로 읽을지
```
`bizScale` 산출식은 양쪽이 글자까지 같아 어긋날 수 없다. **차이는 `isStartupMode` 하나뿐이었다.**

→ 개업 1년 미만 micro는 STARTUP 8문항이 `diag-common-container`에 렌더되어
점수가 **`diag-common-container_s1_1`**로 저장되는데,
`showDiagReveal`은 `isMicro=true`로 판정해 **`/^diag-micro-container_(\d)_/`**로 읽는다.
하나도 매칭되지 않아 전 영역 0 → 세 증상이 동시에 나온다.

**출처를 특정했다 — `e7c4056`(2026-09-03).** 그 커밋이 `loadDiagnosisUI`의 `isMicro`에
`!isStartupMode`를 넣으면서 **`showDiagReveal`을 손대지 않았다.**
검증 항목도 문항 수·진행률·탭 라벨·활성 컨테이너뿐이고 **결과 화면이 빠져 있었다** —
CLAUDE.md의 *"진단 모듈을 추가할 때는 결과 화면까지 함께 확인할 것"* 규칙을 그 커밋이 위반했다.

⚠ 재현으로 확정했다. 사용자 콘솔 실측 키가 `diag-common-container_s1_1`이었다.

### ④ ⚠ 조사 중 발견한 세 번째 결함 — 허위 진단이 AI로 나가고 있었다
`collect()`의 micro 분기가 `bizScale === 'micro' && window.DiagMicro`뿐이라
창업 초기도 여기 걸렸다. `DiagMicro.calcScores(STARTUP 키)` → **전 항목 0점**,
`buildPromptSummary`가 그 0점으로 **CRITICAL 경고 6건을 지어내 AI에 보냈다**:
> `종합 0점 / 100점` · `재료비+인건비 60% 초과` · `BEP 미관리` · `계좌 혼용 + 현금흐름 미관리`

**사용자가 답한 적 없는 내용이다.** 창업자에게 "재료비 60% 초과"를 진단하는 보고서가 나갔다.
`DiagSme`가 이미 같은 이유로 접두어 가드를 쓰고 있었는데 micro만 빠져 있었다.
→ `_diagPathOf(data).isMicro`로 교체해 차단했다. 창업 초기는 `sme+창업초기`와 동일하게
`microPrompt`를 만들지 않는다(허위 요약보다 없는 편이 낫다).

### ⑤ 근본 대책 — `_diagPathOf()` 단일 판정 함수 신설
**이 사고는 세 번째다.** 전부 "그리는 쪽"과 "읽는 쪽"이 각자 판정해서 났다.
| 회차 | 시점 | 내용 |
|---|---|---|
| 1 | 2026-08-16 | 사회적기업 — `bizScale`만 보고 `orgType`을 몰라 `s1_1`을 못 읽음 |
| 2 | 2026-09-02 | 소셜벤처 — 접두어 정규식 하드코딩 |
| 3 | 2026-09-03 | `e7c4056` — `isStartupMode` 누락 (이번 건) |

```js
_diagPathOf({ bizScale, isStartup, orgType })
  → { isSocial, isMicro, isStartup, orgMod, containerId, keyPrefix }
```
판정 축은 셋이고 **우선순위가 있다: 조직형태 → 창업초기 → 규모.**
⚠ `isSocial`이 맨 앞이어야 한다 — 사회적경제도 `bizScale`은 micro라 순서가 바뀌면 영원히 micro로 빠진다.
⚠ `isStartup`이 `isMicro`보다 앞이다 — 개업 1년 미만은 실적 기반 35문항을 답할 수 없다.

**이번엔 `loadDiagnosisUI`·`showDiagReveal`·`collect()` 세 곳만 교체했다.**
한 커밋에서 전부 갈아엎으면 회귀 원인 추적이 어려워진다. `dashboard.js`는 남은 이슈로 둔다.

### ⑥ 진행 버튼 잠금 — 원상복구 분기를 명시했다
기존에는 잠금 처리가 `if (isMicro && drProceedBtn)` **안에만** 있어,
경로가 바뀌면 이전 렌더의 `disabled`와 `⚠ D1 미입력` 문구가 **그대로 남았다.**
→ `if (drProceedBtn)`으로 열고 `else`에서 HTML 기본값(`솔루션 전체 보고서 보기 →` · `disabled` 없음)으로 되돌린다.
⚠ **STARTUP·사회적경제·sme에는 `d1`이 존재하지 않으므로 `d1Avg`를 보면 안 된다.** 잠금은 micro(비창업) 전용이다.

### ⑦ 대시보드 7섹션 전수 확인 — `sec-diag` 하나만 비어 있었다
| 섹션 | 데이터 출처 | 창업 초기 |
|---|---|---|
| `sec-summary` · `sec-lifecycle` · `sec-market-micro` | AI 1차 | 채워짐 (진단 점수 비의존) |
| `sec-six-systems` · `sec-plan90` | AI 2·3차 | 채워짐 |
| `sec-gov` | `GovSupport.match` | 채워짐 |
| **`sec-diag`** | `fd.diagScores` → `calcDiagScores` | **비어 있었다** |

**원인**: STARTUP 키는 `calcDiagScores`에서 `area_s1`~`area_s4`가 되는데
`renderRadar`·`renderWeakAreas`가 **`['area_1'..'area_4']`를 하드코딩**해 하나도 잡지 못했다.
남는 축이 업종특화 1개뿐이라 `labels.length < 3`에서 `return` → 레이더가 통째로 비었다.

⚠ **억지로 7개(D1~D7)에 맞추지 않았다.** 창업 초기는 실제로 **4영역 + 업종특화 = 5축**이며,
`diag-reveal`의 창업 5축(`calcDomainScores(scores, true)`)과 축 수가 정확히 일치한다.
`STARTUP_AREA_LABELS`(사업 검증·현금 생존력·고객 확보·운영 준비도)를 신설하고,
`_commonAreaList(scores)`가 **점수 키에서 어느 표를 쓸지 파생**하게 했다 — 배열 하드코딩을 없앴다.

⚠ **이 수정은 `sme+창업초기`의 `sec-diag`도 함께 살린다.** 그쪽도 같은 이유로 비어 있었다
(`e7c4056` 이전부터의 문제이며 이번 원인과는 별개다). **"기존과 동일"에서 벗어나지만
비어 있던 것이 채워지는 방향이므로 회귀가 아니라고 판단했다.** 비창업 경로의 축 라벨·개수는 불변이다.

### ⑧ 검증 (23/23 통과)
| 항목 | 결과 |
|---|---|
| **[1] micro+창업초기** | `isMicro=false` → D1~D7 정규식 미사용 · `isStartup=true` → 창업 5축 · **읽기 접두어 = 저장 접두어** · D1 잠금 미적용 ✓ |
| **[2] 대시보드 7섹션** | `sec-diag` 축 **4 + 업종특화 = 5** (`사업 검증·현금 생존력·고객 확보·운영 준비도`) · 나머지 6섹션 진단 점수 비의존 ✓ |
| **[3] micro 일반 회귀** | `diag-micro-container_` 저장==읽기 · 공통 축 라벨 `재무건전성…경영역량` 불변 ✓ |
| **[4] sme+창업초기 회귀** | 경로 판정 불변(`diag-common-container_`) ✓ (`sec-diag`는 ⑦대로 개선) |
| **[5] 사회적경제 3유형** | 사회적기업·협동조합·소셜벤처 저장==읽기 전부 불변 ✓ |
| 우선순위 | **사회적기업+창업초기 → 사회적기업 경로 유지**(isSocial 우선) ✓ |
| 무변경 | `ai-engine.js`·`app.js`·`diagnosis-micro.js`·`common.js`·`startup.js`·사회적경제 3모듈·`gov-support.js`·`ppt-export.js`·`funding-rules.js`·`api/`·`vercel.json` **전부 바이트 동일** ✓ |

**변경 파일은 `wizard.js`·`dashboard.js` 둘뿐이다.** 진단 문항·용어 정리·1인 사업장 작업은 손대지 않았다.

### ⑨ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260908b`

### ⑩ 남은 이슈
1. **`dashboard.js`가 아직 `_diagPathOf`를 쓰지 않는다.** `isMicro = fd.bizScale === 'micro'`로
   자체 판정하므로 **micro+창업초기 대시보드가 micro 7섹션 구성으로 뜬다.**
   AI도 `bizScale` 기준이라 D1~D7 처방을 생성하므로 지금은 앞뒤가 맞지만,
   **진단은 4영역인데 처방은 7영역**이라는 축 불일치가 남는다.
   창업 초기 전용 리포트 구성이 필요한지는 별도 판단 사안이다
2. **`_diagPathOf`를 쓰지 않는 나머지 지점** — `dashboard.js`·`ppt-export.js`.
   네 번째 사고를 막으려면 결국 전부 이 함수를 거쳐야 한다
3. (기존) 2·3차의 `substring(0, 500)` 컷 근거 불명 / micro 2차 낭비 필드 제거(2순위 보류)

---

## 최근 수정 이력 (2026-09-08) — micro 1차 전용 시스템 프롬프트 신설 (Vercel Hobby 60초 초과 해결 1순위)

**"소상공인이 쓰지 않는 것을 AI가 만들어서 버린다"는 지적에서 출발했으나,
조사 결과 진짜 원인은 버려지는 필드가 아니라 프롬프트 자기모순이었다.**

### ① 조사 — micro 경로 생성 필드 ↔ 렌더링 전수 대조
`dashboard.js`·`ppt-export.js`·`history-tracker.js`·`ai-engine.js`(2·3차 입력)를 전부 grep으로 확인했다.

| 필드 | 생성 | 대시보드 | PPT(micro 8장) | History | 2·3차 입력 | 판정 |
|---|---|---|---|---|---|---|
| `executiveSummary` | 1차 | `sec-summary` ✔ | 사용 ✔ | 200자 ✔ | 2차·3차 | **필요** |
| `lifecycleStage` | 1차 | `sec-lifecycle` ✔ | 사용 ✔ | — | 2차·3차 | **필요** |
| `stp` | 1차 | `sec-market-micro` ✔ | 사용 ✔ | — | — | **필요** |
| `tam`/`sam`/`som` | 1차 | `sec-market-micro` ✔ | 사용 ✔ | — | — | **필요** |
| `swot` | 1차 | `sec-swot` = smeOnly **hidden** | 미사용 | — | `strengths[0]`·`weaknesses[0]` | **간접 필요(2개)** |
| `sixSystems` D1~D4 / D5~D7 | 2·3차 | `sec-six-systems` ✔ | 사용 ✔ | — | 3차 | **필요** |
| `plan90days` | 3차 | `sec-plan90` ✔ | 사용 ✔ | — | — | **필요** |
| `roadmap` | 2차 | `sec-roadmap` hidden | 미사용 | — | `[0].tasks[0]` 1줄 | 거의 버림 |
| `kpi`·`keyStrategies`·`fourP`·`specializedAnalysis` | 2차 | **전부 hidden / 미호출** | 미사용 | — | — | **버림** |

⚠ **`sec-stp` 섹션이 없다고 STP를 "버려지는 것"으로 판단하면 틀린다.**
`renderMarketMicro(data)`가 `data.stp`(segmentation/targeting/target/positioning)와
`data.tam`/`sam`/`som`을 실제로 읽는다. **STP·TAM/SAM/SOM은 필요한 것이다.**
⚠ `swot`도 제거 대상이 아니다 — `_buildPrompt2Micro`가 `r1.swot.strengths[0]`·`weaknesses[0]`을 읽는다.
⚠ `specializedAnalysis`·`leanCanvas`는 `if (!isMicro)` 가드로 **애초에 렌더 함수가 호출되지 않는다.**

### ② ⚠ 진짜 원인 — 한 요청 안에 상반된 두 명세가 들어가 있었다
`callClaude()`가 micro 1차에도 **sme용 `SYSTEM`을 그대로** 보내고 있었다.

| | 분량 |
|---|---|
| `SYSTEM` 전체 | 11,942자 ≈ **7,464 입력토큰** |
| └ 그중 sme JSON 템플릿 | 7,124자 ≈ **4,453토큰** |

그 템플릿이 요구하는 것 — `swot` 4분면 × **6개** × `{item,evidence}` = **24객체**(880tok) ·
`kpi` 10개(875) · `roadmap` 3단계×task6+budget(673) · `keyStrategies` 6개(518) ·
`leanCanvas` 9블록(398) · `specializedAnalysis`(368) · `fourP`(318) ·
`executiveSummary` "5~7문장, 수치 포함 필수"(165).

반면 `buildPrompt1` micro 분기는 **"7개 필드만, 각 1개, [운영현황]/[핵심위험]/[즉시과제]"**라고 말한다.
**모델은 최대 명세 쪽으로 쏠린다** → 실측 output **8,964토큰 / 92초** → Vercel Hobby 60초 초과.

**버려지는 필드 이론보다 이쪽이 훨씬 잘 설명한다.** micro 1차가 실제로 필요한 7개 필드를
명세대로만 쓰면 1,000토큰 안팎이면 끝난다.

⚠ **2026-05-25 `22ff99e`가 절반만 정리한 결과였다.** 그때 `SYSTEM`에서 `sixSystems`·
`plan90days` 템플릿과 인과사슬 블록은 걷어냈지만 **SWOT·STP·4P·keyStrategies·kpi·
roadmap·leanCanvas 템플릿은 남겨 뒀다.**

### ③ 조치 — `_SYSTEM_MICRO_1` 신설 (1순위만. 2순위는 보류)
```js
const text1 = await apiCall(_isMicro ? _SYSTEM_MICRO_1 : SYSTEM, buildPrompt1(formData), '1차', _opts1);
```
⚠ **`SYSTEM` 상수는 한 글자도 건드리지 않았다.** sme 1차가 공유하므로 손대면 그쪽 품질이 깎인다.
⚠ **명세는 `_SYSTEM_MICRO_1` 한 곳에만 둔다.** `buildPrompt1` micro 분기의 JSON 템플릿을
   제거하고 "시스템 프롬프트의 구조를 따르라"는 3줄로 대체했다. **두 곳에 두는 것 자체가
   이번 문제의 원인이었으므로**, 코드에 "여기 다시 적지 마라"는 주석을 남겼다.

**남긴 블록과 근거** — "절대 규칙·페르소나·언어 원칙만"이 지시였으나 두 블록을 더 남겼다:
- **필수 반영 원칙** — 1차가 실제로 만드는 SWOT·STP·executiveSummary를 직접 지배한다.
  특히 4번(진단 점수 등급별 반영)이 빠지면 **진단 결과가 보고서에 반영되지 않는다.**
  2차·3차 대상인 6번(정부지원→로드맵)만 덜어내고, 프레임워크 10권 중 SWOT·
  executiveSummary 대상인 ①블루오션·⑥제로투원만 6번 한 줄로 압축했다
- **응답자 메모 활용 원칙** — 2026-09-07에 넣은 것으로 `executiveSummary`·SWOT에 메모를
  반영시키는 **유일한 지시**다. 빼면 그 작업이 micro에서 조용히 무효가 된다

**제외한 블록** (전부 2차·3차 필드 전용이라 1차에 도달할 대상이 없다):
현금 런웨이 4축(plan90days·sixSystems) / 카드사 위험 신호 3개(sixSystems) /
정책금융 우선순위 4단계 / 정부지원 평가기준(keyStrategies·roadmap) /
프레임워크 ②③④⑤⑦⑨⑩(keyStrategies·KPI·roadmap·fourP) / consultingType specializedAnalysis(2차)

### ④ 검증 (지시된 4항목 전부 통과)
| 항목 | 결과 |
|---|---|
| **[1] 제거 확인** | micro 1차 요청 전체에서 `kpi`·`roadmap`·`keyStrategies`·`leanCanvas`·`specializedAnalysis`·`fourP`의 **JSON 명세 0건** (금지 지시로만 언급) ✓ |
| **[2] 7필드 잔존** | `executiveSummary`·`lifecycleStage`·`swot`·`stp`·`tam`·`sam`·`som` 명세 **전부 유지** ✓ |
| **[3] sme 무영향** | sme 1차 `SYSTEM` **바이트 동일**(11,942자) · sme 1차 유저 프롬프트 **바이트 동일**(2,994자) ✓ |
| 보존 확인 | 메모 원칙 · 진단 점수 등급 · 언어 원칙 · 페르소나 · 절대 규칙 전부 유지 ✓ |
| 경로 무변경 | `api/claude-analyze-1/2/3` · `-social` · `-funding` · `lib/claude-stream.js` · `vercel.json` · `dashboard.js` · `wizard.js` · `app.js` · `diagnosis-micro.js` **전부 바이트 동일** ✓ |

**[4] 입력 토큰 실측 (micro 1차 요청 전체)**
| | 시스템 | 유저 | 합계 |
|---|---|---|---|
| before | 11,942자 | 3,540자 | 15,482자 ≈ **9,676 토큰** |
| after | **2,064자** | 3,174자 | 5,238자 ≈ **3,274 토큰** |

**예상 output**: 8,964 → **1,000~1,500토큰**. 실측 97.4 tok/s 기준 **92초 → 10~16초**.

### ⑤ 2순위는 보류 — 배포 후 실측으로 결정
`kpi`·`keyStrategies`·`fourP`·`specializedAnalysis`는 micro 대시보드·PPT·History 어디에도
나오지 않지만 **전부 2차 생성**이다. 1차가 15초로 줄고 2·3차가 60초 안에 들어오면 불필요하다.
**두 순위를 한 번에 하면 문제 발생 시 원인 구분이 안 되므로 나눴다.**

### ⑥ 사용자가 Vercel Logs에서 확인할 것
Vercel → 프로젝트 → **Logs** → Function 필터. 함수별로 다음 줄을 본다.

| 로그 | 의미 |
|---|---|
| `[1차-micro] 완료: turns=N, 누적 output_tokens=…` | 1차 생성량. **1,500 이하면 성공** |
| `[2차-micro]` / `[3차]` 같은 줄 | 2·3차 생성량 — **60초 초과 여부의 판단 근거** |
| 각 요청 행의 **Duration** | 함수별 실제 소요. 60초 상한은 **호출당** 적용된다 |
| `504` / `FUNCTION_INVOCATION_TIMEOUT` | 어느 함수에서 나는지가 2순위 우선도를 정한다 |

⚠ **Duration은 함수마다 따로 찍힌다.** `claude-analyze-1`만 60초를 넘고 있었다면 이번 수정으로
끝나고, `-2`·`-3`도 넘고 있다면 2순위(2차 낭비 필드 제거)를 바로 진행해야 한다.

### ⑦ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260908a`

### ⑧ 남은 이슈
1. **2순위 보류 중** — 2차의 `kpi`·`keyStrategies`·`fourP`·`specializedAnalysis`는 micro에서
   완전히 버려진다. `roadmap`도 3차 프롬프트가 `[0].tasks[0]` 1줄만 쓴다. 배포 후 결정
2. (기존) 2·3차의 `substring(0, 500)` 컷 근거 불명 — 변동 없음
3. (기존) `_renderSolution()`을 `try` 안에 다시 넣지 마라 — 렌더 오류가 AI 실패로 분류된다

---

## 최근 수정 이력 (2026-09-08) — ⚠ 백그라운드 AI 되돌림 (보고서가 나오지 않는 증상)

**속도 개선(`397fac6`) 배포 후 최종 보고서가 나오지 않는다는 보고를 받고 `js/app.js`를 되돌렸다.**
> "진단 대시보드만 먼저 보이고, 최종 보고서 도출은 '다시 시도' 메뉴가 나오는 등 화면이 넘어가지 않는다"

### ① ⚠ 요구를 오해했던 것도 함께 정정한다
사용자가 원한 것은 **"모든 보고서를 빨리 도출"**이었다.
`diag-reveal`을 먼저 띄우는 것은 **체감** 개선일 뿐 보고서 자체를 빠르게 하지 않는다.
방향이 어긋난 데다 보고서를 아예 못 보게 만들었다.

### ② 조사 — 추측하지 않고 대조·실행으로 확인했다
| 확인 | 결과 |
|---|---|
| `git diff 397fac6^ 397fac6 -- js/app.js` | `callClaude(data)` **인자 동일** · 호출 횟수·프롬프트 불변 |
| `showDiagReveal`·`HistoryTracker.save`가 `data`를 훼손하는가 | **대입 0건** — 둘 다 읽기 전용 |
| `callClaude`를 fetch 스텁으로 **실제 실행** | **정상** — 3회 호출(`/api/claude-analyze-1/2/3`) · 14필드 반환 |
| `Dashboard.render`를 DOM 스텁으로 실행 | 완전한 결과·**부분 결과 모두 던지지 않음** |

→ **app.js 변경분에서 AI 호출을 실패시키는 원인을 찾지 못했다.**

### ③ ⚠ 다만 진짜 결함을 하나 찾았다 — 재도입 금지
```js
try {
  const result = await AIEngine.callClaude(data);
  ...
  if (_awaitingSolution) { _renderSolution(); }   // ← try 안에 있었다
} catch (e) {
  _pendingResult = null;                          // ← 성공한 결과를 지운다
  _aiState = 'error';
}
```
**`Dashboard.render()`가 던지면 AI 성공을 "AI 분석 실패"로 뒤집고 결과까지 지운다.**
그 뒤 [다시 시도] → AI 재성공 → 렌더 재실패 → **무한 "다시 시도" 루프**가 된다.
`proceedToSolution`의 `_renderSolution()`도 감싸지 않아 던지면 **화면이 그대로 멈춘다**.
보고된 두 증상이 모두 이 형태와 일치한다.

⚠ **렌더링 오류를 AI 실패로 분류하면 안 된다.** 다시 시도할 때는
`_renderSolution()`을 `try` 밖으로 빼고 자체 오류 처리를 붙여야 한다.

### ④ 판단 — 고치지 않고 되돌렸다
결함(③)은 실재하지만 **그것이 원인이라는 증거가 없다.** 로컬에서 실제 API 실패를 재현할 수 없어
"고쳐 놓고 지켜보는" 방식은 원인을 더 모호하게 만든다.

→ **`js/app.js`만 `397fac6^` 상태로 되돌렸다.** 변수가 하나로 줄어 다음 진단이 명확해진다.
- 되돌린 뒤에도 보고서가 안 나오면 → 원인은 **AI/API 상층**이지 화면 순서가 아니다
- 되돌린 뒤 정상이면 → 변경분이 원인이며 ③을 고쳐 다시 접근한다

⚠ `git revert`를 쓰지 않았다. `397fac6`은 `index.html`·`CLAUDE.md`도 건드렸는데
그 뒤 `?v=`가 여러 번 바뀌어 충돌이 난다. **`git checkout 397fac6^ -- js/app.js`로
동작만 정확히 되돌리고** 문서·캐시버스팅은 앞으로 두었다.

### ⑤ 사용자가 확인할 것 (브라우저 콘솔)
`F12` → **Console** 탭에서:
| 로그 | 의미 |
|---|---|
| `[AI 분석 실패]` **뒤의 원본 메시지** | 가장 중요하다. `max_tokens`/JSON 조각/타임아웃 중 무엇인지 드러난다 |
| `1차 응답 (처음 400자):` 가 찍히는가 | 안 찍히면 1차부터 실패 |
| `[BizNavi] 2차 분석 시작` / `3차 분석 시작` | 어느 단계까지 갔는지 |

**Network** 탭에서 `claude-analyze-1/2/3`의 **상태 코드**(200 / 500 / 504)를 함께 확인.

### ⑥ 불변 확인
`js/app.js`·`index.html`(`?v=`)만 변경했다. **진단 문항·용어 정리·1인 사업장 작업은
전부 바이트 동일**이며 사회적경제·정책자금·재무분석 경로도 손대지 않았다.

### ⑦ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818z`

### ⑧ 다음 방향 — 체감이 아니라 생성량
"보고서 자체가 빨리 나오게" 하려면 화면 순서가 아니라 **생성량**을 줄여야 한다.
AI 3회 호출 축소 · sme 13섹션 정리 · `max_tokens` 조정 중 무엇을 택할지 결정이 필요하다.
⚠ **결과 품질을 낮추는 방식은 안 된다**는 것이 확정된 제약이다. **별도 작업으로 진행한다.**

---

## 최근 수정 이력 (2026-09-07) — 1인 사업장 전용 진단 신설 (SCALE_WORDING 2차 오버라이드)

**"외식업·미용업 소상공인은 사장과 종업원 1명이 거의 전부인데 진단이 너무 무겁다."**
직원이 없는데 위임·교육·근로계약을 묻고 있었다.

### ① ⚠ 지목된 D3가 DiagMicro에 없었다 — 착수 전 정정
지시에 예시로 나온 `3_1 전 직원과 목표 공유`·`3_2 역할·책임 체크리스트`·
`3_3 대표 부재 시 원격 통제`·`3_4 신입 교육 SOP`는 **`DiagCommon`의 도메인 3
(시스템화·위임 지수)**이고, **`DiagMicro`의 D3는 `다채널 판로`**다.
"35문항·7대 영역"은 DiagMicro인데 문항 예시는 DiagCommon이었다.
**착수 전 보고해 정정받았다.** (2026-09-07 "플랫폼 리뷰"와 같은 어긋남)

→ 전수 조사로 **DiagMicro에서 실제로 고용을 전제하는 8문항**을 찾았다.
조직 관련 축이 D3 한 곳이 아니라 여러 영역에 흩어져 있다.

| 문항 | 고용어 | 문제 |
|---|---|---|
| **`5_3` 근로계약** | **14회** | `알바`·`주휴수당`·`4대 보험` — 고용이 없으면 통째로 해당 없음 |
| `1_5` 위임 | 6회 | 위임할 사람이 없다 |
| **`4_5` 자리 비우기** | 5회 | ⚠ **1인 사업장 최대 위험**(아프면 매출 0)인데 직원 전제로 물어 답할 수 없다 |
| `4_3` 시간 단축 | 5회 | `파트타임 1명 축소` → 혼자면 본인 시간 |
| `2_4` 작업 편차 | 5회 | `숙련 직원과 신규 직원 간 편차` |
| `6_3` 직원 정리 | 4회 | 해고 예고·퇴직금 |
| `4_2` 노동 단축 | 3회 | `홀 직원의 보조 노동` |
| `2_3` 고객 동선 | 2회 | `직원과 고객이 부딪혀` |

⚠ `1_3`·`1_4`·`4_4`의 **`인건비`는 1인도 본인 인건비로 성립**하므로 대상이 아니다.

### ② ⚠ 2구간이 아니라 3구간이다 — 실무 지적으로 설계 변경
> "1인 사업장도 실제로 **알바 등을 고용**하여 운영할 수 있으니 1인 사업장과 2~5인 등으로 구분해야"

| 구간 | 정의 | 오버라이드 |
|---|---|---|
| `solo` | 대표만. 고용 없음 | 8문항 |
| **`solo_pt`** | 대표 + 파트타임(알바) | 4문항 재작성 + solo 상속 |
| `team` | 정규 직원 (기본) | **없음 — 기존 동작 100% 유지** |

⚠ **`solo_pt`가 실무에서 가장 흔하고 가장 위험하다.** 주휴수당·4대보험 미가입 문제가
여기서 터진다. `solo`로 묶으면 `5_3`을 통째로 건너뛰어 **그 위험을 못 잡는다.**

### ③ 어느 문항이 갈리고 어느 것이 공통인가 (판단 근거)
**갈린다 (4문항)** — `solo`와 `solo_pt`의 답이 실제로 달라진다
`1_5`(기록·자동화 ↔ 알바 위임) · `4_3`(본인 시간 ↔ 본인+알바 시간) ·
`4_5`(휴업 대비 ↔ 알바가 버티기) · `5_3`(향후 고용 대비 ↔ 단시간 근로자 의무)

**공통 (3문항)** — `solo` 것을 `solo_pt`가 그대로 상속
`2_4`(알바가 있어도 "본인과 알바 간 편차"로 같은 문장) ·
`4_2`(알바 유무로 설비 도입 판단이 달라지지 않는다) ·
`2_3`(고용어가 scale2 한 줄뿐)

**⚠ `6_3`은 특수 — `solo_pt`가 기본(team)으로 되돌아간다**
`해고 예고 30일`·`퇴직금 14일`은 **단시간 근로자에게도 그대로 적용된다.**
`solo`의 "고용 없음" 문구를 물려받으면 **알바 퇴직금 의무를 아예 묻지 않게 된다.**
실무에서 "알바는 퇴직금이 없다"는 오해가 흔해 **기본 문항을 그대로 두는 것이 안전**하다.

### ④ 구조 — 3층 상속 (`_extends` + `null` 취소)
```
기본 ITEMS → INDUSTRY_WORDING[group] → SCALE_WORDING[scale]
```
⚠ **이 순서를 지켜야 업종 문구 위에 규모 문구가 얹힌다.** 뒤바뀌면 업종이 규모를 지운다.
⚠ 6업종 × 3규모 = 18조합을 각각 쓰면 630문항이라 불가능하다. **규모 층은 업종과
무관한 부분(고용·위임·본인 시간)만 덮는다.**

**중복 작성 대신 상속을 택했다.** `solo_pt`가 `solo`와 공통인 것이 3문항인데
중복 작성하면 두 벌 관리가 되고 한쪽만 고치면 조용히 어긋난다.
```js
solo_pt: { _extends: 'solo', '1_5':…, '4_3':…, '4_5':…, '6_3': null }
```
**`null`은 상속 취소**다. 이 장치가 없으면 `6_3`을 기본으로 되돌릴 방법이 없다.

`getSchema(industryGroup, scaleMode)` — **`scaleMode`를 안 주면 기존과 완전히 동일**하다.
기존 호출부는 수정하지 않았다.

### ⑤ 판별 UI — 라디오 3지선다, 기본 `team`
step1 직원수가 **`1~5명`일 때만** 노출한다(6명 이상인데 1인 사업장일 수 없다).
⚠ **숫자 입력을 요구하지 않는다.** 사장님이 부담스러워한다.
⚠ 기본값이 `team`이라 **라디오를 건드리지 않으면 기존과 100% 동일**하다.
⚠ 직원수를 6명 이상으로 바꾸면 **`team`으로 되돌린다** — `solo` 선택이 남으면 안 된다.
⚠ 이벤트는 HTML 속성이 아니라 `DOMContentLoaded` 리스너로 등록했다(2026-06-05 `c317c03` 참조).

### ⑥ 단계 분할
| 단계 | 범위 | push |
|---|---|---|
| 1 | 라디오 UI · `SCALE_WORDING` 골격 · 병합 로직 · 배선 | 보류 |
| 2 | 핵심 3문항 `1_5`·`4_3`·`4_5` | 1단계와 함께 |
| 3 | 나머지 5문항 `5_3`·`6_3`·`2_3`·`2_4`·`4_2` | 완료 |

⚠ **1단계만 배포하면 라디오는 있는데 아무 변화가 없다.** 2단계까지 끝내고 함께 push했다.

### ⑦ 검증 (23/23)
| 항목 | 결과 |
|---|---|
| **미선택(team·null·미등록 scale)** | 6그룹 문항 **기존과 완전 동일** ✓ |
| `solo` | **3문항만** 변경(6그룹 전부) ✓ |
| `solo_pt` | 3문항 변경 · **solo와 서로 다름**(중복 아님) · **`6_3` 상속 취소 확인** ✓ |
| 업종 문구 유지 | 미용실 solo → `시술` 문구 + 1인 맥락 **공존** ✓ |
| 라디오 노출 | `1~5명`만 노출 · 6명 이상 숨김 · **숨길 때 team 복귀** ✓ |
| 렌더링 | 세 구간 전부 **35문항 · guide 35건** ✓ |
| `calcScores` | 1~5점 전부 동일 ✓ |
| 기본 ITEMS | **35문항 완전 불변** ✓ |

### ⑦-2 3단계 — 나머지 5문항 (`5_3`이 핵심이었다)
**갈림 3 / 공통 2**로 갈렸다.

| 문항 | solo | solo_pt |
|---|---|---|
| **`5_3`** | **대표 본인 보험·향후 채용 대비** — 고용이 없으니 본인 국민연금·건강보험 상태와 "사람을 쓰게 될 때 필요한 절차"로 축을 옮겼다 | ⚠ **강화.** 근로계약서 **서면 작성·교부** / **1주 15시간 이상 주휴수당** / **4대 보험 대상 확인** 3쟁점을 명시 |
| **`6_3`** | **임대차·설비 정리** — 해고 예고·퇴직금을 덜어내고 원상복구 범위·설비 처분에 집중 | **알바 정리·임대차 정리** — 근무 기간에 따라 해고 예고·퇴직금 대상이 달라진다는 것을 아는가 |
| **`2_4`** | 컨디션·시간대별 편차 | 본인과 알바 간 편차 |
| `2_3`·`4_2` | **공통 — solo만 쓰고 solo_pt가 상속** | (상속) |

⚠ **`5_3`이 `solo_pt`에게 가장 값진 문항이다.** 주휴수당 미지급·4대 보험 미가입·
근로계약서 미교부는 **실무에서 실제로 사고가 나는 지점**이고, `solo`로 묶었으면
이 문항을 통째로 건너뛰어 위험을 못 잡았을 것이다.

### ⑦-3 ⚠ 1단계 설계를 한 곳 바꿨다 — `6_3`의 `null` 제거
1단계에서 `solo_pt`의 `'6_3': null`(기본 유지)로 뒀다. 근거는 "해고 예고·퇴직금은
단시간 근로자에게도 적용되므로 기본 문항이 그대로 맞다"였다.

**그러나 기본 문항은 정규직 전제라 "알바는 언제부터 대상인가"라는 실제 쟁점을 묻지 못한다.**
→ `null`을 제거하고 **`solo_pt` 전용 문항**을 썼다.
⚠ `null` 장치 자체는 코드에 남겨 뒀다(주석으로 미사용임을 명시). 앞으로 같은 상황이 생길 수 있다.

### ⑦-4 ⚠ 법령 수치는 확신한 것만 썼다
잘못된 법령 정보는 진단 신뢰를 크게 깎는다. 기준을 나눠 적용했다.

| 쓴 것 (확실) | 쓰지 않은 것 |
|---|---|
| **1주 15시간 이상 → 주휴수당 발생** (근로기준법 §18③·§55) | 과태료·벌금 **금액** |
| **1년 이상 계속 근로 + 4주 평균 주 15시간 → 퇴직금** (퇴직급여보장법 §4①) | **최저임금액** (연도별 변동) |
| **계속 근로 3개월 미만 → 해고 예고 의무 없음** (근로기준법 §26 단서) | 4대 보험 세부 가입 요건 — 보험별로 달라 **"근로시간·기간에 따라 대상이 달라진다"는 개념만** |
| 근로계약서 **서면 작성·교부 의무** (§17) | 수습 감액률 — 조건이 많아 개념만 |

### ⑦-5 3단계 검증 (17/17 + 렌더 11/11)
| 항목 | 결과 |
|---|---|
| team·null·미등록 scale | 6그룹 **기존과 완전 동일** ✓ |
| `solo` | **정확히 8문항** 변경(6그룹 전부) ✓ |
| `solo_pt` | 8문항 변경 · **전용 6문항이 solo와 다름** · **`2_3`·`4_2`는 상속**(중복 없음) ✓ |
| `5_3` | solo_pt에 3쟁점 전부 포함 · solo는 본인 보험 축 ✓ |
| **법령 수치** | 과태료·최저임금액 **0건** · 해고 예고·퇴직금 요건 정확히 명시 ✓ |
| 업종 문구 | 미용실 solo_pt → `시술` + 알바 근로조건 **공존** ✓ |
| 렌더링 | 세 구간 전부 **35문항 · guide 35건** ✓ |
| `calcScores`·기본 ITEMS | 1~5점 동일 · **35문항 완전 불변** ✓ |
| scale·ai_trigger·guide | **전 18조합**(6그룹 × 3구간) 유지 ✓ |

### ⑧ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** — 1·2단계 `20260818x` · 3단계 `20260818y`

### ⑨ 남은 이슈
1. ✅ **3단계 완료.** 8문항 전부 1인 사업장 맥락으로 갖춰졌다
   (solo 8문항 · solo_pt 전용 6 + 상속 2)
2. ⚠ **AI 업종분석이 `biz_scale`을 `sme`로 반환하면 `DiagCommon`이 뜨고
   `SCALE_WORDING`이 적용되지 않는다.** 1~5명인데 sme로 분류되는 드문 경우다.
   `DiagCommon`은 업종 그룹 구조가 없어 규모 층을 붙이려면 별도 설계가 필요하다

---

## 최근 수정 이력 (2026-09-07) — 경영진단(micro·sme) 체감 속도 개선 (백그라운드 AI)

**진단 완료 후 AI 3회 호출이 전부 끝날 때까지 로딩 화면만 보고 있었다.**
그런데 `diag-reveal`의 내용(레이더차트·영역 점수·해설 카드·KOSIS 생존율·진단유형)은
**전부 AI 없이 즉시 계산된다.** 사회적경제가 이미 쓰던 방식을 일반 경로에 적용했다.

### ① ⚠ 결과 품질은 한 글자도 건드리지 않았다
이번 작업은 **화면 전환 순서만** 바꾼 것이다. 해시로 불변을 확인했다.

| 파일 | 결과 |
|---|---|
| `js/ai-engine.js` (프롬프트·호출 로직) | **바이트 동일** ✓ |
| `api/claude-analyze-1/2/3.js` · `-social` · `-funding` | **바이트 동일** ✓ |
| `lib/claude-stream.js` · `vercel.json`(maxDuration) | **바이트 동일** ✓ |
| `js/dashboard.js` · `js/wizard.js` | **바이트 동일** ✓ |

**변경 파일은 `js/app.js` 하나뿐이다.** 호출 횟수 3회·`max_tokens` 16000·타임아웃·재시도 정책
모두 그대로이며, **부분 렌더링도 하지 않는다** — 보고서는 AI가 완전히 끝난 뒤 전체를 렌더링한다.

### ② 사전 확인에서 나온 사실 4가지
1. **`showDiagReveal(data, currentSnap)`의 2번째 인자는 AI 결과가 아니라 이력 스냅샷이다.**
   본문 220줄에 `result`·`executiveSummary`·`swot` 참조가 **0건** — 기다릴 이유가 구조적으로 없었다
2. `proceedToSolution()`이 `if (!_pendingResult) return;`으로 **조용히 반환**한다.
   백그라운드로 바꾸면 AI 진행 중 클릭 시 **버튼이 먹통**이 되므로 반드시 고쳐야 했다
3. `_socialReqId` 경쟁조건 방어 구조가 이미 있다
4. `HistoryTracker.save(data, result)`의 result 의존은 `execSummary` **1줄뿐**이고 `?.`로 접근한다

### ③ 변경 내용
```
진단 완료 → save(data, null) → showDiagReveal → show('diag-reveal')   ← 즉시
          ↘ 동시에 백그라운드로 callClaude()
[사용자가 레이더차트를 읽는 시간 = AI 생성 시간]
[솔루션 보기] → done  : 전체 렌더링
              → loading: 로딩 화면 대기 → 완료 시 자동으로 열림
              → error : 에러 화면 + 재시도
```

⚠ **카운터를 `_aiReqId` 하나로 통일했다**(복제하지 않았다). 두 경로는 동시에 진행될 수 없고,
카운터가 하나면 `restart()`가 `_aiReqId++` 한 번으로 **양쪽을 동시에 무효화**한다.
둘로 나누면 경로를 바꿔 재진단할 때 한쪽이 살아남는다.

⚠ **`HistoryTracker.save`를 두 번 호출한다.** 즉시 `save(data, null)`로 저장해야
`showDiagReveal`이 "지난 분기 대비"를 그릴 수 있고, AI 완료 후 `save(data, result)`로
`execSummary`를 채운다. 같은 분기+회사명이면 덮어쓰므로 중복 스냅샷이 생기지 않는다.
**재저장에도 `reqId` 검사를 적용**했다 — 없으면 새 진단의 스냅샷을 낡은 응답이 덮어쓴다.

⚠ **백그라운드 실패를 `diag-reveal`에 띄우지 않는다.** 그 시점의 사용자는 레이더차트를
보고 있다. 에러는 보고서를 요청한 순간에만 표시한다.

### ④ ⚠ `animateLoading()`은 두 번 호출하지 않는다
첫 호출의 `setTimeout` 체인이 `diag-reveal`을 보는 동안에도 계속 진행되므로,
보고서 대기 화면에서 다시 부르면 애니메이션이 겹친다. **문구만 교체**한다.
| 시점 | 문구 |
|---|---|
| 첫 로딩 | `진단 결과 정리 중…` / `통계·정부지원사업 데이터를 불러오고 있습니다` |
| 보고서 대기 | `AI 보고서 생성 중…` / `Claude AI가 보고서를 작성하고 있습니다. 완료되면 자동으로 열립니다` |

⚠ KOSIS·기업마당 병렬 조회는 AI 앞에 있어 그 구간은 여전히 기다린다(짧다).

### ⑤ 검증 (19/19 · DOM 스텁)
| 항목 | 결과 |
|---|---|
| **AI 품질 파일 8종 + dashboard·wizard** | **바이트 동일** ✓ (최우선 항목) |
| micro·sme | AI 응답 전 `diag-reveal` **즉시 표시** · 로딩 화면 벗어남 ✓ |
| 즉시 클릭 | 부분 렌더링 없음 · 로딩 전환 · 완료 시 **자동으로 열림** ✓ |
| 오래 머문 뒤 클릭 | 즉시 전체 렌더링 ✓ |
| AI 실패 | `diag-reveal` 유지·에러 없음 → **클릭 시점에만** 에러 화면 ✓ |
| 새 진단(경쟁조건) | 낡은 응답이 화면에 안 꽂힘 · **스냅샷도 덮어쓰지 않음** ✓ |
| 회귀 | 사회적경제 경로 동작 불변 ✓ |

### ⑥ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818w`

### ⑦ 남은 이슈
1. **sme 리포트 생성량 자체를 줄이는 것은 별도 작업이다.** 이번엔 체감 속도만 개선했고
   호출 횟수·`max_tokens`는 그대로다
2. (기존) 2·3차의 `substring(0, 500)` 컷 근거 불명 — 변동 없음

---

## 최근 수정 이력 (2026-09-07) — 진단 문항 전문용어 정리 (3단계, 커밋 4건)

**"진단이 무겁다. 전문용어가 너무 많이 나온다"는 지적에서 출발했다.**
`Cpk`·`ACM`·`Prime Cost`·`CCC`·`ROAS`·`Work Triangle` 같은 용어를 소상공인 사장님이
읽고 답할 수 없다. **규모와 무관한 문제**였다.

### ① ⚠ 판단 기준은 "컨설턴트가 아는 말"이 아니라 "응답자가 아는 말"이다
사용자(경영지도사, 소상공인 컨설팅 실무)의 확인 결과가 기준이 됐다.

> "프라임 코스트·ACM·CCC 등 **영어 약자는 사장님들이 모른다.**
>  꼭 쓸 거면 **풀네임을 내가(컨설턴트가) 설명할 수 있게 진단지 옆에 별도 주석**을 달아라."

→ **처리 원칙: 본문은 쉬운 말로만 쓰고, `guide`에 풀네임 + 한 줄 정의를 넣는다.**
`guide`는 **컨설턴트가 사장님께 "이게 업계에서 말하는 ○○입니다"라고 짚어줄 때 쓰는 자료**다.
⚠ **약자만 쓰지 않는다.** 반드시 풀네임과 정의를 함께 적는다.

### ② 실무 판단에 따라 용어별로 처리가 갈렸다
| 용어 | 처리 | 근거 |
|---|---|---|
| **OHI** | **완전 제거. guide에도 넣지 않았다** | 맥킨지 컨설팅 용어다. **소상공인이 평생 접할 일이 없다.** `대표 없이도 돌아가는 정도`가 같은 뜻이고 더 정확하다 |
| **BEP(손익분기점)** | **그대로 유지** | **사장님들이 아는 말이다.** 이미 설명도 있어 손대지 않았다 |
| **공헌이익** | **다른 용어보다 설명을 길게 썼다** | "손익분기점은 알지만 **공헌이익 개념을 아는 사업자는 거의 없다. 중요한 개념이니 반드시 쉽게 설명하라**"는 판단 |
| ACM·프라임 코스트·CCC·TCO | 본문 제거 + guide 정의 | 업계에서 실제로 통용되므로 없애지 않는다. 사장님이 나중에 그 말을 들었을 때 알아야 한다 |
| SVI·IMP·SROI | 유지 + guide 정의 | 사회적경제 **공식 지표**다. 바꾸면 제도와 어긋난다 |
| 이용고 배당·법정적립금 | 유지 + 정의 병기 | **협동조합기본법 용어**다 |
| POS·ERP·CRM·QR·ISO·HACCP·MOU·TIPS | 그대로 | 이미 익숙하다 |

**공헌이익 설명문**(social `s4_3`·`s4_5` / coop `c4_3`):
> `※ 공헌이익 = 판매가 − 변동비(재료비 등). 고정비(임대료·인건비)를 갚는 데 쓰이는 돈이다.`
> `매출이 아니라 이 금액이 클수록 남는 장사다`

### ③ 단계 분할과 커밋
| 단계 | 범위 | 커밋 |
|---|---|---|
| 1 | DiagMicro 6그룹 (a)류 15종 일괄 제거 | `3fc5799` |
| 2 | DiagMicro (b)류 설명 병기 + manufacturing 전용어 | `94aa49e` |
| 2 보완 | **`DOMAINS.desc`·`output`·경고 msg 누락분** | `e82f95e` |
| 3 | `common.js` + 사회적경제 3유형 + `startup.js` (+ 파사드 병기) | `36f5d70` |

### ④ ⚠ 2단계에서 검사 범위를 좁게 잡아 놓쳤다 — 재발 방지 기록
`label`·`question`·`scale`만 훑어 **`DOMAINS.desc`와 `ACTION_PLAN.output`을 놓쳤다.**
보완 커밋(`e82f95e`)이 따로 필요했다.

⚠ **`DOMAINS.desc`에는 약자를 절대 넣으면 안 된다.**
`guide`가 없는 자리이고, **영역 제목 아래 첫 화면에 바로 보인다.**
사장님이 진단을 시작하자마자 읽는 문구라 본문보다 오히려 중요하다.
> `POS·ACM·프라임코스트 기반 정량 수익 구조` → `판매 데이터와 원가를 바탕으로 실제로 남는 구조인지`

→ **이후 모든 텍스트 필드를 대상으로 재귀 검사기를 만들어 썼다:**
`label`·`question`·`guide`·`desc`·`scale[].desc`·`tool`·`output`·`DOMAINS.desc`·
`DOMAIN_DESC_BY_GROUP`·`ACTION_PLAN title`·경고 `msg`

⚠ **전용어 검사는 `label`·`question`·`scale`만 대상으로 해야 한다.**
`guide`에는 `POS`·`식재료`·`플레이스` 같은 업종어가 **의도적으로** 들어간다(업종별 예시 나열).
guide까지 훑으면 정상 상태가 실패로 잡힌다.

### ⑤ 주요 before / after
| 문항 | before | after |
|---|---|---|
| micro `1_5` | **OHI** 조직 위임도 | 대표 없이도 돌아가는 정도 |
| micro `2_4` | 조리 편차 지수**(σ_prep)** | 조리 시간 들쑥날쑥 정도 |
| micro `1_4` | 조정 기여 마진**(ACM)** 산출 | 손이 가는 만큼 남는지 계산 |
| micro `1_3` | **프라임 코스트** 통제 | 재료비+인건비 비율 통제 |
| micro `2_2` | 로컬 SEO**(PLACE)** 일치도 | 네이버 플레이스 정보 일치도 |
| common `1_2` | 현금 전환 사이클**(CCC)** 관리 — `원재료 매입 대금 지급일부터 매출 채권 회수일까지` | 돈이 묶여 있는 기간 관리 — `물건·재료 값을 치른 날부터 판 돈이 들어오는 날까지 며칠이 걸리는지` |
| common `3_4` | **SOP**·교육 매뉴얼 체계 **(Capability)** | 업무 매뉴얼·교육 체계 |
| common `5_1`~`5_4` | 탄소 배출 인벤토리 **(E)** | 탄소 배출 관리 **(환경)** |

⚠ **`PLACE SEO`는 3개 파일에 하드코딩돼 있었다** — `DOMAINS[1].label`,
`wizard.js`(레이더차트 라벨), `ai-engine.js`(AI 프롬프트 sixSystems 이름).
**한 곳만 고치면 화면마다 다른 이름이 뜬다.** 셋 다 바꿨다.

⚠ **파사드는 용어 병기만 했다.** 문항 재설계는 하지 않았다(25곳).
조사 결과 scale이 고비용 행동(리뉴얼·간판 교체)을 요구하지 않고
7일 실행계획도 `낡은 홍보 스티커 제거` 수준이라 **재설계할 근거가 없었다.**

### ⑥ `startup.js` — `guide` 필드를 신설했다
`guide`가 없어 인라인 병기만 가능한 줄 알았으나,
`renderDiagModule` → `_renderItemHtml`이 `item.guide`를 출력하므로
**필드를 추가하면 그대로 렌더링된다.** 8문항 전부 신설했다.

### ⑦ 검증
| 단계 | 결과 |
|---|---|
| 1 | 대상 용어 6그룹 전수 **0건**(본문+guide) · 7일 실행계획 0건 · `OHI` 완전 제거 |
| 2 | (b)류 본문 0건 · ACM·`1_3`·TCO guide 정의 6그룹 전수 |
| 2 보완 | `DOMAINS` label·desc 약자 0건 · base `ACTION_PLAN`(desc·tool·output) 0건 |
| 3 | **설명 없는 전문어 0건**(승인된 (c) 목록 제외) |
| 공통 | `calcScores` 점수 동일 · 문항 수 불변 · `ai_trigger` 불변 · scale 5단계 전수 |

⚠ DOMAINS 라벨 변경으로 `calcScores` 반환값의 `label` **문자열**이 바뀐다.
점수(`avg`·`pct`·`weight`·`total`)는 불변이며 외부 하드코딩 참조가 없음을 확인했다.

### ⑧ 캐시버스팅
`20260818s`(1단계) · `t`(2단계) · `u`(2단계 보완) · `v`(3단계)

### ⑨ 남은 이슈
1. **범위 밖에서 같은 문제 2건을 찾았으나 손대지 않았다.**
   - `ai-engine.js` 583·590·610 — AI 프롬프트 JSON 템플릿의 `Prime Cost 현황`·`ACM 비율 진단`.
     **AI가 이 문구를 그대로 최종 보고서에 쓴다.** 문항을 다 쉽게 해도 보고서에 약자가 나오면 효과가 반감된다
   - `cross-context.js:495` — 교차 경고 msg의 `Prime Cost까지 높아`·`Work Triangle 재배치와 Portion Control`
2. `DiagCommon`의 "업종 무관" 분류를 **B2B 관점에서 재점검**할 필요가 있다.
   `2_3`·`2_4`가 이미 뒤집혔고, `4_1`(인스타그램)·`5_4`(플랫폼)에 B2C 흔적이 남아 있다

---

## 최근 수정 이력 (2026-09-07) — 진단 결과의 AI 프롬프트 반영 강화 (응답자 메모 + 점수 분포)

**"꼼꼼하게 진단했으면 최종 보고서에 그 내용이 언급되어야 신뢰할 수 있다"는 지적.**
확인 결과 `buildPromptSummary`가 AI에 넘기던 것은 **영역 점수 + 교차 경고 + 2점 이하 label**뿐이었다.

### ① ⚠ 메모는 모듈에 도달할 수 없는 구조였다
사장님이 쓴 메모는 `diagScores[key].memo`·`diagMemos[key]`에 **정상 저장**되고
`data.diagScores`로 `collect()`에도 넘어간다. 그러나
```js
function collectAllScores() {           // wizard.js
  const s = Number(diagScores[k]?.score || 0);
  if (s > 0) all[k] = s;                // ← 숫자만. memo 탈락
}
```
`buildPromptSummary(allScores)`가 받는 것이 **평면 숫자 맵**이라 모듈 안에서 메모를 볼 방법이 없었다.
**전달 0건.**

⚠ 이 평면 숫자 계약은 `DiagMicro.calcScores`·`CrossContext.buildScoreMap`이 함께 의존한다.
→ **`buildPromptSummary(scores, group, memos)` 별도 인자**로 넘긴다. 계약은 손대지 않았다.
호출부가 인자를 안 주면 블록 자체가 생략되므로 기존 경로는 무해하다.

### ② 블록 순서를 바꿨다 — 2·3차가 앞 500자만 보기 때문
| 순서 | 블록 |
|---|---|
| 0 | 헤더(업종 그룹·종합 점수) — 2줄로 압축 |
| **1** | **응답자 메모** ← 가장 값진 정보를 500자 컷 안에 넣는다 |
| 2 | 복합 경고 신호 |
| 3 | 영역별 점수 |
| 4 | 취약 항목 |
| 5 | **우수 항목(신설)** |
| 6 | 7일 실행계획 |

⚠ 메모 상한 — 문항당 **200자**, 블록 총량 **400자**. 초과 시 `…`·`※ 외 N건 생략`.
**낮은 점수 문항의 메모를 우선 수록**한다(취약할수록 현장 맥락이 중요하다).
상한이 없으면 메모만으로 500자를 채워 경고·점수가 밀려난다.

### ③ 점수 분포 — (b)안 채택, 상한 15개
실측 후 3안을 비교했다(혼합 분포 기준):
| 방식 | 길이 |
|---|---|
| (a) 전 35문항 나열 | 488토큰 |
| **(b) 취약 전체 + 우수(5점) 전체 + 보통 개수** | **243토큰** |
| (c) 취약 + 영역별 최고/최저 | 426토큰 |

⚠ **(b)는 전항목 1점일 때 (a)보다 길어진다**(518 vs 488) — 취약 35개가 전부 나열되기 때문.
→ **취약 항목 상한 15개(낮은 점수 우선)** + `※ 외 N개 항목이 2점 이하 — 취약 항목은 총 N개`.
숫자를 남기지 않으면 AI가 전체 심각도를 오판한다.

**"이미 잘하고 있는 항목(5점)" 블록을 신설**했다 — 보고서 신뢰는 못하는 것만 짚어서는 올라가지 않는다.

### ④ AI 시스템 프롬프트 지시 (경영진단 `SYSTEM` + `_SYSTEM_SOCIAL` 2곳)
```
[응답자 메모 활용 원칙 — 반드시 지킬 것]
- 진단 결과에 [응답자 메모] 블록이 있으면 그것은 점수만으로 알 수 없는 현장 맥락이다.
- 보고서 작성 시 메모 내용을 반드시 반영하라.
- 메모와 점수가 어긋나면 메모를 우선 고려하라.
- 메모를 인용할 때는 응답자가 쓴 표현을 그대로 살려라. 자기 말로 바꾸지 마라.
```
⚠ **마지막 줄이 핵심이다.** AI가 메모를 자기 말로 바꾸면 응답자가 "내 이야기가 반영됐다"고
느끼지 못한다.

### ⑤ 사회적경제 3유형 — 코드 복제를 감수했다
`DiagSocial`·`DiagVenture`·`DiagCoop`에 동일 헬퍼(`_memoBlock`·`_scoreDistBlock`)를 넣었다.
시그니처는 `buildPromptSummary(scores, memos)`(group 인자 없음).

### ⑥ 검증 (DiagMicro 13/13 · 3유형 21/21 · end-to-end)
| 항목 | 결과 |
|---|---|
| 메모 입력 시 포함 · 원문 그대로 | ✓ |
| 메모 없음 / 빈 객체 | 블록 **생략** ✓ |
| **`substring(0,500)` 안에 메모 본문** | ✓ (4개 모듈 전부) |
| 취약 15개 상한 + `외 20개` 표시 | ✓ |
| 영역 점수·교차 경고·7일 액션 블록 내용 | **불변** ✓ |
| `calcScores`·문항 수 | 불변 ✓ |
| **end-to-end** | `W.setMemo()` → `collect()` → `microPrompt` 반영 확인 ✓ |

**길이 (DiagMicro)**
| 케이스 | 토큰 |
|---|---|
| 수정 전(혼합) | 813 |
| 혼합·메모 없음 | 896 |
| **혼합·메모 3건** | **1,052** (+29%) |
| 최악(전항목 1점)·메모 3건 | 1,601 |

1차 `max_tokens` 16,000 대비 입력 증가분은 부담이 크지 않다.

### ⑦ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818r`

### ⑧ 남은 이슈
1. **⚠ 2·3차 호출이 `microPrompt`를 `substring(0, 500)`으로 자른다.**
   ([ai-engine.js:1306·1331](js/ai-engine.js#L1306)) 도입은 `ab0dbb3`(2026-04-29 2회 호출 분할)이며
   **왜 500인지 근거 기록이 없다.** 현재 1,300~1,600자 중 대부분이 버려진다.
   메모를 맨 앞에 둔 것은 **임시방편**이다. 2·3차가 실제로 필요로 하는 정보량을 재검토해
   적정 값을 정해야 한다
2. **`DiagMicro`/`Social`/`Venture`/`Coop`의 `buildPromptSummary`가 거의 동일한 코드다.**
   이번에 `_memoBlock`·`_scoreDistBlock`까지 4벌로 복제됐다. 시그니처가 다르고
   (micro만 group 인자) 서로 독립 IIFE라 **공용 모듈 신설 + 로드 순서 조정**이 필요하다.
   별도 작업으로 분리
3. **문항의 `question` 본문은 여전히 전달되지 않는다.** label만 간다.
   "무엇을 물었는지"까지 넣으면 프롬프트가 크게 늘어 이번엔 넣지 않았다

---

## 최근 수정 이력 (2026-09-07) — DiagCommon 2_3·2_4 B2C 전제 수정 · pro_service 7_1

**"제조업 진단에 플랫폼 리뷰·48시간 답글·채널별 광고가 나온다"는 보고를 받았으나
`manufacturing` 그룹에는 그 문구가 0건이었다.**

### ① ⚠ 증상의 출처가 지목된 곳이 아니었다 — 렌더링으로 확정
| 경로 | DiagCommon | DiagMicro(manufacturing) |
|---|---|---|
| `mfg_parts` / **micro** | 렌더 안 됨 | 렌더 O · **증상 0건** |
| `mfg_parts` / **sme** | 렌더 O · **증상 3건** | 렌더 안 됨 |

- `플랫폼 리뷰`·`48시간 내 맞춤형 답글` → **`DiagCommon 2_3`**
- `채널별 광고` → **`DiagCommon 2_4`**

즉 **직원 6명 이상(sme)으로 진단**해 DiagCommon이 나온 것이다.
`manufacturing`의 `2_3`은 `자재·완제품 이동 동선`, `3_4`는 `수주 영업 효율`로 이미 정상이었다.
**증상을 그룹에서 찾기 전에 어느 모듈이 렌더링되는지부터 확인해야 한다.**

### ② ⚠ 2026-09-04 DiagCommon 작업의 분류 오류가 드러났다
`2_3`·`2_4`는 그때 **"업종 무관 12문항"으로 분류돼 손대지 않기로 한 것**이다.
B2C 기준으로는 맞았지만 **B2B에서는 성립하지 않는다** — 거래처는 플랫폼에 리뷰를 남기지 않고
채널별 광고를 집행하지도 않는다. **"업종 무관"은 B2C 안에서만 무관이었다.**

### ③ 조치 — 그룹 구조를 만들지 않고 중립화 + guide 다중 예시
⚠ `DiagCommon`에는 오버라이드 구조가 없어 **ITEMS를 직접 수정**해야 하고,
그 결과 **외식업 sme도 함께 바뀐다.** 그룹 구조 이식(근본 해결)은 2문항 때문에 하기엔 과해
지난 8문항과 같은 방식(중립화 + guide 병기)을 택했다.

| 문항 | before | after |
|---|---|---|
| `2_3` | **고객 리뷰·평판 관리** — `플랫폼 리뷰를 실시간 모니터링하며 48시간 내 맞춤형 답글` | **고객 만족도·평판 관리** — `고객 평가와 불만을 실시간으로 파악하며 정해진 시간(48시간) 안에 대응` |
| `2_4` | **다채널 광고·마케팅 ROAS** — `채널별 광고비 투입 대비 실질 매출(ROAS)` | **판로별 마케팅 효율** — `판로별로 투입한 마케팅·영업 비용 대비 실질 매출` |

**scale 5단계도 함께 중립화했다** — `리뷰 모니터링 없음` → `고객 평가를 확인하지 않음`,
`영수증·예약 연동 자동 리뷰 수집` → `거래·이용 완료 시점과 연동한 자동 평가 수집` 등 10줄.
**본문만 고치고 scale을 두면 반쪽이 된다.**

guide에 3개 축을 모두 담아 외식업 손실을 최소화했다:
> `2_3` — `예: 소매·외식은 플랫폼 리뷰·답글·평점 4.0 관리, B2B 제조는 거래처 만족도·품질 클레임 대응 리드타임, 전문서비스는 상담 후기·재의뢰율`
> `2_4` — `예: 소매·외식은 광고 ROAS, B2B 제조는 전시회·조달 등록·중개상사별 수주 성과, 전문서비스는 채널별 상담 유입 대비 수임`

### ④ `pro_service` — 플랫폼 문구를 제거하지 않았다
7곳에 있으나 **유지가 맞다**: `pro_service`는 `medical`(의원)·`finance`·`media`·`energy`·
`knowledge_it`로 구성되고, **의원은 네이버 플레이스가 핵심 채널이며 리뷰가 매출을 좌우한다.**
`3_1`의 `크몽·숨고`도 실제 판로다.

**`7_1`의 `인스타그램 피드` 한 곳만** `온라인 콘텐츠 채널`로 상위화했다.
⚠ 제거하지 않고 **guide에 병기**했다 — `media`(미디어·콘텐츠)는 인스타그램이 본업일 수 있다.
> `예: 의원·미용은 인스타그램·블로그, 세무·법무는 블로그·뉴스레터, 미디어는 인스타그램·유튜브`

### ⑤ 검증 (12/12)
| 항목 | 결과 |
|---|---|
| `2_3`·`2_4` 본문·scale | B2C 전용어 **0건** ✓ |
| guide | 소매·외식 / B2B 제조 / 전문서비스 **3축 모두 포함** ✓ |
| DiagCommon 나머지 18문항 | **완전 불변** ✓ |
| DiagMicro food·beauty·retail·edu·manufacturing | **완전 불변** ✓ |
| `pro_service` | 변경 **`7_1` 1건뿐** · 플랫폼·플레이스·리뷰 문구 **7곳 유지** ✓ |
| `calcScores` | DiagCommon·DiagMicro 1~5점 전부 동일 ✓ |
| 문항 수 | 20·35 불변 · guide 20건 유지 ✓ |
| **렌더링 재현 검사** | sme 4개 업종 본문 증상 **0건** ✓ |

⚠ 렌더 HTML 전체를 훑으면 `플랫폼 리뷰`가 2건 잡히는데 **`guide`의 의도된 업종 예시**다.
**전용어 검사는 반드시 `label`·`question`·`scale`만 대상으로 해야 한다.**

### ⑥ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818q`

### ⑦ 남은 이슈
1. **`DiagCommon`의 "업종 무관" 분류를 B2B 관점에서 재점검할 필요가 있다.**
   이번에 `2_3`·`2_4`가 뒤집혔다. 남은 18문항 중 `4_1`(인스타그램)·`5_4`(플랫폼)에도
   B2C 흔적이 있으나 **이번 지시 범위 밖이라 건드리지 않았다**
2. (기존) `DiagCommon`에 업종 그룹 구조가 없다. sme 경로 분화가 계속 필요해지면
   `DiagMicro`의 `INDUSTRY_WORDING` 방식 이식을 검토해야 한다

---

## 최근 수정 이력 (2026-09-04) — 제조업 전용 그룹(manufacturing) 신설 · DiagMicro 6번째 그룹

**retail에 7개 업종이 몰려 사실상 "기타"가 되어 분류 기능을 잃었다.**
특히 `1_3`·`1_4`의 **`매입 원가`는 제조업에 성립하지 않는 개념**이다(제조 원가는 재료비+노무비).

### ① 이동 업종 — 명확한 제조업 2개만
`mfg_parts`(뿌리 제조·부품가공) · `food_mfg`(식품 제조·가공) → `manufacturing`
⚠ `wholesale`·`fashion`·`logistics`·`construction`·`export_sme`는 **retail에 그대로 뒀다.**
유통·물류·건설은 제조와 또 다르며 각각 별도 판단이 필요하다.

### ② 단계 분할 (세션 용량 한계 대응)
| 단계 | 내용 | 커밋 |
|---|---|---|
| 1 | 그룹 등록 + 골격 | `47f4813` (push 보류) |
| 2 | D1~D3 15문항 | `c12a23f` (1단계와 함께 push) |
| 3 | D4~D7 16문항 | `425dc37` |
| 4 | `ACTION_PLAN` 35항목 + 경고 하드코딩 정리 | 이번 (완료) |

⚠ **1단계만 배포된 상태를 만들지 않았다.** 오버라이드가 비면 제조업 사장님이
구체성 없는 중립 문항만 받으므로 **2단계까지 끝낸 뒤 함께 push**했다.


### ②-2 1·2단계에서 내린 판단 (커밋 `47f4813` · `c12a23f`)
**1단계 — 골격만. 오버라이드는 비워 뒀다**
`INDUSTRY_GROUP_MAP` 2줄 · `GROUP_LABELS` · `INDUSTRY_WORDING.manufacturing = {}` ·
`WARN_WORDING` · `DOMAIN_DESC_BY_GROUP`(도메인 1·2·3·4 부분 지정).

⚠ **빈 오버라이드가 안전한지 먼저 확인하고 착수했다.**
`getSchema('nosuch')` → 예외 없이 기본 ITEMS 35개 반환 / `getGroup('unknown')` → `food` 폴백 /
`detectCrossWarnings(_, 'nosuch')` → `WARN_WORDING.food` 폴백. **미등록 그룹으로도 깨지지 않는다.**

⚠ **`WARN_WORDING.cost`를 `'제조원가'`가 아니라 `'재료비+노무비'`로 썼다.**
`1_3`이 실제로 재는 것은 prime cost(재료비+인건비)이고 **제조원가는 제조간접비까지 포함해
범위가 넓다.** retail의 `'상품원가+인건비'` 명명 방식과도 일치한다.

⚠ **`subj`를 `'이 공장은'`이 아니라 `'이 사업장은'`으로 썼다.**
소규모 식품 가공장에 "공장"은 과하다. **부품가공과 식품제조를 모두 포괄**해야 한다.

⚠ **`DOMAIN_DESC`는 1·2·3·4만 지정했다.** 1(원가 개념이 다름) · 2(공간 동선이 다름) ·
3(판로가 다름) · 4(재고가 3단계). `label`·`key`·`weight`는 **절대 분기하지 않는다** —
`calcScores`가 `domain.label`을 반환값에 실어 레이더차트·대시보드로 흘려보낸다.

**2단계 — D1~D3 15문항**
`3_4`를 **ROAS → 견적 대비 수주율**로 재정의했다. **축(투입 대비 성과)은 유지**하되
B2B 수주 산업에 맞게 바꾼 것이다. `2_1`은 파사드(보행자 시선)가 B2B에 무의미해
**`기업 신뢰도 노출`로 축을 옮겨** 거래처·감사 담당자에게 보유 설비·인증을 보이는가로 바꿨다.

⚠ **`manufacturing`이 파일에 두 번 선언된다**(`DOMAIN_DESC_BY_GROUP` 552행 /
`INDUSTRY_WORDING` 1689행). D4 때 경계를 넘은 것과 같은 조건이라
패치 스크립트는 반드시 **`INDUSTRY_WORDING` 앵커 + 그룹 경계 assert**를 써야 한다.

### ③ 제조업 개념 반영 — `매입 원가`를 쓰지 않는다
| 문항 | 내용 |
|---|---|
| `1_3` | **제조원가(재료비+노무비) 통제** — guide에 제조간접비는 별도 관리임을 명시 |
| `1_4` | **품목별 ACM** = 직접재료비 + 직접노무비 + **가공 시간(설비 점유 시간)**. scale5는 `저수익 수주를 선별 거절` |
| `2_4` | **로트별 품질 편차** — 규격 편차·불량률·공정능력(Cpk) |
| `2_5` | **설비 배치·공정 동선** — 자재 보관 → 가공 → 검사 → 완제품 보관 |
| `3_2` | **규격 포장·로트 관리** — 로트번호·생산일자·유통기한 역추적 |
| `3_1`·`3_3` | 거래처 다변화·수주 파이프라인 / 납품 단가에 **어음 만기·회수 기간 금융비용** 반영 |
| `3_4` | ROAS → **견적 대비 수주율**로 재정의(축은 투입 대비 성과로 유지) |
| `4_1` | 접수 지연 → **납기 준수율**. scale5는 수주 시점 능력 검토(CTP) |
| `4_2` | **원자재·재공품·완제품 3단계** 재고 + 바코드 |
| `7_3` | 영수증 리뷰 → **거래처 평판·추천 확보**(B2B에 리뷰는 성립하지 않는다) |
| `7_4`·`7_5` | 플레이스 저장·CTR → **카탈로그 다운로드·견적 문의 전환** |

⚠ **`2_1`은 파사드(보행자 시선)가 B2B에 무의미해 `기업 신뢰도 노출`로 축을 옮겼다** —
거래처·감사 담당자에게 취급 품목과 **보유 설비·인증(ISO·HACCP·KC)** 을 인식시키는가.
`3_2` scale5의 `거래처 품질 감사에 즉시 대응`과 함께 **인증·품질감사 요구를 여기서 흡수**했다.

### ④ ⚠ 예상 못 했던 발견 2건
1. **`5_4`가 요식·소매 전제였다** — `일회용 쓰레기`·`플라스틱`·**`친환경 용기 전환율 50%`**.
   제조업 폐기물은 **스크랩·폐수·부산물**이므로 재작성했다. 사전 분류에 없던 항목이다
2. **`6_1` scale3의 `권리금 회수 가치`는 임대공장에 성립하지 않는다** —
   제조업은 **설비 잔존가액·중고 매각 가치**가 그 자리다

### ⑤ ⚠ 오버라이드를 만들지 않은 4개 — 억지로 채우지 않았다
`5_3`(근로계약) · `6_2`(폐업 부가세 기한) · `6_3`(해고 예고·퇴직금) · `6_5`(희망리턴패키지)
→ **근로기준법·세법·소상공인 제도는 업종과 무관**하다. 기본 문항이 그대로 정확하다.
결과: **오버라이드 31개** (`food` 30 / `pro_service` 29와 같은 성격의 부분 커버리지)

⚠ **작업 중 죽은 오버라이드를 하나 만들었다가 잡았다.** `5_1` guide를 기본과
**글자까지 동일하게** 써서 병합해도 아무 변화가 없었다. 검증에서 "실효 오버라이드 개수"가
기대보다 1개 적게 나와 발견했고, `자재 대금·외주 가공비의 사업 카드 결제 비중`으로 실질화했다.
**오버라이드를 추가할 때는 실제로 기본을 덮는지 확인해야 한다.**

### ⑥ 양쪽 업종 포괄
기본 문항에 한쪽 전용어(`식재료`·`금형`)를 쓰지 않고 **guide에 병기**했다.
> `2_4` — `예: 부품가공은 로트별 치수 편차·공정능력(Cpk), 식품제조는 배치별 중량·규격 편차·불량률`

8문항(`1_1`·`1_2`·`1_3`·`1_4`·`2_4`·`3_2`·`4_2`·`5_4`)에 양쪽 예시가 함께 들어 있다.

### ⑦ 검증 (1단계 12/12 · 2단계 20/20 · 3단계 19/19)
| 항목 | 결과 |
|---|---|
| `getGroup` | `mfg_parts`·`food_mfg` → `manufacturing` ✓ |
| 매핑 변경 | **2건뿐** · 나머지 14개 불변 ✓ |
| manufacturing 본문 | 소매·외식 전용어 **0건** (렌더 HTML도 0) ✓ |
| 기존 5개 그룹 | 문항·DOMAINS **완전 불변** ✓ |
| `calcScores` | 1~5점 전부 동일 ✓ |
| **guide** | **6그룹 × 35문항 = 210건** ✓ |
| `scale`·`ai_trigger` | 5단계·`warning` 코드 전수 불변 ✓ |
| **6개 그룹 키 개수 검사** | food 30 / beauty 27 / retail 27 / edu 27 / pro 29 / **manufacturing 31** — 매 단계 PASS ✓ |

⚠ **`manufacturing`은 파일에 두 번 선언된다**(`DOMAIN_DESC_BY_GROUP` 552행 /
`INDUSTRY_WORDING` 1689행). D4 때 경계를 넘은 것과 같은 조건이라
패치 스크립트는 반드시 **`INDUSTRY_WORDING` 앵커 + 그룹 경계 assert**를 써야 한다.

### ⑧ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** — 1단계 `20260818m` · 2단계 `n` · 3단계 `20260818o`

### ⑧-2 4단계 — ACTION_PLAN 35항목 (가장 체감이 큰 부분이었다)
3단계까지는 **문항은 제조업인데 처방이 외식업**이었다. `manufacturing`에 그룹 계획이 없어
base(외식)로 폴백해 `Dog 메뉴 제거`·`파사드 시선 촬영`·`테이블에 QR 스탠드`가 나갔다.

`profit_ops` · `place_seo` · `multichannel` · `smart_dx` · `sns_ai` **5키 x 7일 = 35항목** 신설.

| 키 | Day1 -> Day7 요지 |
|---|---|
| profit_ops | 품목별 수주량 -> 재료비 -> 가공 시간 -> ACM 4분면 -> **Dog 품목 수주 중단·단가 재협상** -> Star 우선순위 -> 재료비+노무비 정산 |
| place_seo | 설비 목록 1장 -> **인증서 유효기간 점검** -> 회사소개서 갱신 -> 입고~출하 동선 실측 -> 통로 적치물 제거 -> 기업정보 일치 확인 -> 로트 편차 재측정 |
| multichannel | **1위 거래처 의존도** -> 결제 조건·회수 기간 -> 적자 거래처 식별 -> **견적 대비 수주율** -> 탈락 사유 유형화 -> 후보 10곳 -> 3곳 발송 |
| smart_dx | **납기 준수율 집계** -> 3단계 재고 실사 -> 장부·실물 차이 -> 바코드 견적 -> 현품표 시범 부착 -> 설비 이상 대응 1장 -> **견적서에 납기 확약란 추가** |
| sns_ai | 대표 이미지 최신성 -> 강점 문구 3개 -> 카탈로그 시안 -> **PDF 다운로드 링크** -> **견적 문의 폼 + 24시간 회신** -> 거래처 추천 요청 -> 문의 건수 추이 |

⚠ **7일 안에 실제로 할 수 있는 일로만 썼다.** `ERP 도입` 같은 것은 넣지 않고
`견적서 양식에 납기 확약란 추가`·`설비 이상 대응 절차 1장 매뉴얼` 수준으로 맞췄다.
⚠ 한쪽 전용 작업(`금형 점검`·`HACCP 기록지`)을 title에 쓰지 않고
`보유 인증서·시험성적서 유효기간 점검`처럼 **양쪽에 통하는 상위 개념**으로 썼다.

### ⑧-3 ⚠ 하드코딩 경고 — 지시된 1건 외에 1건을 더 찾았다
`detectCrossWarnings` 7규칙을 전수 검사한 결과 업종어가 박힌 것은 **2건**이었다.

| code | 문제 | 조치 |
|---|---|---|
| `digital_invisible` | `오프라인 간판 정보와 플레이스 정보` 하드코딩 | `WARN_WORDING.vis`로 분기 |
| **`reputation_backfire`** | **`SNS 콘텐츠와 리뷰 관리` 하드코딩** — B2B 제조에 리뷰는 성립하지 않는다 | `WARN_WORDING.cont`·`rev`로 분기 |

나머지 5규칙은 이전 작업에서 이미 분기됐거나 원래 중립이다.

⚠ **작업 중 기존 그룹 문구를 건드렸다가 되돌렸다.** 처음에 `CTR도 낮습니다`를
`노출 대비 반응도 낮습니다`로, `SNS 콘텐츠와`를 `콘텐츠와`로 바꿔 **food까지 변경**됐다.
회귀 검사에서 잡아 **변수화 지점을 `vis`·`cont`·`rev` 세 곳으로 좁혀** 원문을 복원했다.
**메시지를 그룹 분기할 때는 바뀌어야 하는 어절만 필드로 빼야 한다.**

⚠ **`pro_service`의 `플레이스`는 제거하지 않았다.** 지시에 "제조업·전문서비스에 플레이스는
성립하지 않는다"고 되어 있었으나, **의원·세무사는 네이버 플레이스가 실제 핵심 채널**이고
`pro_service`의 `2_2`·`7_5` 문항 자체가 플레이스를 쓴다. `간판` -> `사무소 안내`로만 순화했다.

### ⑧-4 4단계 검증 (19/19)
| 항목 | 결과 |
|---|---|
| manufacturing ACTION_PLAN | **5키 x 7일 = 35항목** · `{day,title}` 형식 ✓ |
| 7일 계획 외식·소매 용어 | **0건** ✓ |
| 기존 4개 그룹 ACTION_PLAN | **완전 불변** ✓ |
| base `ACTION_PLAN_7DAY` | 불변 · **food는 여전히 base 폴백** ✓ |
| `digital_invisible` | food·pro_service·manufacturing **3종으로 분기** ✓ |
| `reputation_backfire` | manufacturing `리뷰` 0건 / food `리뷰` 유지 ✓ |
| food·beauty·retail·edu 경고 | **완전 불변** ✓ (pro_service만 1곳) |
| 무인자 폴백 | 원문과 동일 ✓ |
| 기본 ITEMS·기존 5그룹 문항·calcScores | 전부 불변 ✓ |

### ⑨ 남은 이슈
1. ✅ **manufacturing 그룹 신설 완료 (1~4단계).** 문항 31개 오버라이드 + ACTION_PLAN 35항목 +
   경고 그룹 분기까지 갖췄다
2. `wholesale`·`fashion`·`logistics`·`construction`·`export_sme` **5개가 여전히 `retail`이다.**
   유통·물류·건설은 제조와 또 다르며 각각 별도 판단이 필요하다. retail이 아직 5개 업종을
   담고 있어 "기타" 성격이 완전히 해소되지는 않았다
3. **`DOMAIN_TO_ACTION_KEY`에 D5·D6이 없어 모든 그룹이 그 두 도메인은 base를 쓴다.**
   두 title(`사업 전용 계좌 분리…`·`월간 BEP 돌파율…`)은 중립이라 지금 문제는 없다
4. (기존) 죽은 필드 3종 — `ai_trigger.warning_msg` / base `ACTION_PLAN_7DAY`의
   `desc`·`tool`·`output`. 살리려면 그룹 계획 175항목에 `desc`를 새로 써야 한다

---

## 최근 수정 이력 (2026-09-04) — food_mfg(식품 제조·가공) 그룹 오매핑 수정

**`INDUSTRY_GROUP_MAP`에서 `food_mfg`가 `food`(외식)에 매핑돼 있었다.**
식품 제조는 B2B 납품·OEM 중심인데 "테이블 회전율"·"홀 동선"·"서빙 로봇"을 받고 있었다.
2026-09-03 `mfg_parts` 수정 때 함께 보고됐으나 당시 지시 범위 밖이라 남겨 뒀던 항목이다.

### ① ⚠ 옮기기 전에 19문항을 축별로 대조했다 — `mfg_parts` 때와 상황이 다르다
food와 retail이 갈리는 **19문항 전부**를 식품 제조 기준으로 판정했다.

| 축 | food | retail | 판정 |
|---|---|---|---|
| `1_3` 원가 | 프라임코스트(**식재료비**+인건비) | 상품**매입**원가+인건비 | **food** — 제조 원가는 원료비+노무비. retail의 '매입 원가'는 제조에 성립하지 않는다 |
| `1_4` ACM | 메뉴별 식재료 원가 + **조리** 시간 | 상품 **매입** 원가 + 진열 시간 | **food** |
| `2_4` 편차 | **조리 완료 시간** 편차 180초 | 결제 처리 시간 편차 3분 | **food** — 가공 공정 시간 편차는 실제 품질 지표 |
| `2_5` 동선 | 싱크·**화구**·준비대 | 창고·**진열대·계산대** | **food 근소** — 식품공장에 세척·가열·전처리는 실재, 진열대·계산대는 없다 |
| `3_2` 패키징 | **진공포장·급속냉동·위생검증·밀키트** | 기프트셋·묶음상품 | **food 압도적** — 식품가공의 핵심 공정 그 자체 |
| `1_2`·`3_1`·`3_5`·`4_2`·`4_5`·`7_1`·`7_3`·`7_5` | 테이블 단가·배달 앱·홀 퇴식·스마트플레이스 | 제품별 단가·**B2B 도매 납품**·입고 검수·바코드·스마트스토어 | **retail 8건** |
| `5_1`·`6_1`·`6_4` | **매장** | **사업장**(중립) | **retail 3건** |
| `1_5`·`2_3`·`4_1` | — | — | 무승부 3건 |

**집계: retail 11 · food 5 · 무승부 3 → retail로 이동.**

⚠ **`mfg_parts`(부품가공) 때와 결정적으로 다른 점**: 부품가공은 food에서 얻을 것이 없었지만
**식품 제조는 `3_2`(포장 공정)·`1_3`·`1_4`(제조원가)에서 실제로 손해를 본다.**
11:5이므로 이동이 순이득이지만 **손실이 있는 이동임을 명시해 둔다.**
이 5개 축이 곧 **제조업 전용 그룹이 필요한 구체적 근거**다.

### ② 검증 (10/10, 직전 커밋 기준)
| 항목 | 결과 |
|---|---|
| `getGroup('food_mfg')` | `'retail'` ✓ |
| 변경된 매핑 | **`food_mfg` 1건뿐** · 16개 항목 수 불변 ✓ |
| 5개 그룹 문항·guide·DOMAINS | **전부 불변** ✓ |
| `ACTION_PLAN` | 불변 ✓ (retail 계획이 자동 적용 — `상품별 매출 추출`·`매입원가 산출`) |
| `calcScores` | 1~5점 전부 동일 ✓ |
| **food_mfg 진단 본문 외식용어** | **102건 → 0건** ✓ |
| 렌더링 | 35문항·guide 35건·HTML 내 외식용어 0건 ✓ |
| **회귀** `restaurant` | `food` 그룹 유지 · 외식 문구 92건 **보존** ✓ |

⚠ 검증 중 **기준선 실수**를 한 번 했다 — 비교 대상을 D1 작업 이전 스냅샷으로 잡아
"그룹 스키마 불변" 검사가 실패로 나왔다. **직전 커밋(`git show HEAD:`) 기준으로 다시 잡아** 통과했다.
**여러 커밋에 걸친 작업에서는 회귀 기준선이 무엇인지 매번 확인해야 한다.**

### ③ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818l`

### ④ 남은 이슈
1. **⚠ 제조업 전용 그룹 부재가 유일하게 남은 근본 한계다.**
   `mfg_parts`·`food_mfg`·`wholesale`·`construction`·`logistics`·`export_sme` **6개 업종이
   전부 `retail`을 쓴다.** 소매 문구(진열대·계산대·매입원가)가 제조·건설에 완전히 맞지 않는다.
   특히 **`1_3`·`1_4`의 '매입 원가'는 제조업에 성립하지 않는 개념**이다.
   신설 시 필요한 것: `manufacturing` 그룹 + `INDUSTRY_WORDING.manufacturing` 35문항 +
   `ACTION_PLAN_7DAY_BY_GROUP` 5도메인×7일 + `DOMAIN_DESC_BY_GROUP`. **별도 작업 규모다**
2. `agri_food`(농림·식품원료)는 `food`에 남아 있다. 원물 생산·1차 가공이라 식품 제조와 다르며
   **이번 대조 범위 밖이라 건드리지 않았다.** 별도 판단이 필요하다

---

## 최근 수정 이력 (2026-09-04) — DiagCommon(sme 경로) 외식업 편향 수정 8문항

**sme 경로(직원 6명 이상)가 쓰는 `DiagCommon` 20문항 중 8개가 오프라인 매장·요식업 전제였다.**
제조업 중소기업 대표가 "오픈·마감·위생 검수", "클라우드 POS·CCTV", "식재료 로스"를 받고 있었다.

### ① ⚠ DiagMicro와 방식이 다르다 — 그룹 구조를 만들지 않았다
`DiagCommon`에는 `INDUSTRY_WORDING` 같은 **그룹별 오버라이드 구조가 없다.**
따라서 DiagMicro의 "기본 중립 + food 보존" 방식을 쓸 수 없다.
그룹 구조 신설은 범위 밖이므로 **8문항을 업종 중립으로 직접 수정**했다.

**구체성 손실은 `guide`가 메운다.** `guide`는 그룹별로 바뀌지 않으므로
**한 문장에 여러 업종 예시를 함께 담았다.**
> `5_2` guide — `예: 외식업은 식재료 로스, 제조업은 불량·스크랩, 유통은 파손·반품, 서비스업은 소모품·인쇄물`

### ② 재작성 5개
| 문항 | 수정 전 | 수정 후 |
|---|---|---|
| `2_1` | **로컬 플랫폼 NAP 정합성** — 네이버 스마트플레이스·카카오맵·구글 비즈니스 | **온라인 기업 정보 정합성** — 홈페이지·검색 포털·업종 디렉터리 |
| `3_2` | **오픈·마감·위생 검수** 등 시간대별 임무 | **업무 시작·종료 점검, 품질 검수** 등 |
| `3_3` | **매장**·현장 부재 / 클라우드 **POS·CCTV** | **사업장**·현장 부재 / 클라우드 기반 시스템 |
| `4_3` | **POS·배달 플랫폼·ERP** 데이터 통합 | **판매·생산·재고** 시스템 데이터 통합 |
| `5_2` | 폐기물·**식재료 로스** 단가 가치 | 폐기물과 **원부자재 손실**의 단가 가치 |

`4_3`은 scale 5단계가 통째로 요식업 DX 경로(`플랫폼 등록 → 리뷰 대응·예약 플랫폼`)여서
`개별 도입 → 부분 연계 → AI 활용 → 전사 통합`으로 재작성했다. guide의 5단계 요약도 함께 고쳤다
(`초기수동→탐색노출→구축관리…`의 '탐색노출'은 로컬 SEO 개념이라 제조업에 성립하지 않는다).

### ③ 문구만 손본 3개
`1_3` scale4 `POS·ERP와 연동` → `매출 집계 시스템과 연동` /
`3_1` scale3 `POS·화이트보드에 게시` → `사내 게시판·대시보드에 게시` /
`3_4` scale4 `QR코드 현장 부착` → `QR코드 작업 현장 부착`

### ④ 검증
| 항목 | 결과 |
|---|---|
| 수정 8문항 **본문**(label·question·scale) | 외식·오프라인 매장 전용어 **0건** ✓ |
| 업종 무관 **12문항** | **완전 불변**(JSON 동일) ✓ |
| `calcScores` | 1~5점 전부 수정 전과 **동일** ✓ |
| 문항 수 | **20 불변** ✓ |
| `guide` | **20건 전수 유지** · sme 경로 렌더링 20건 확인 ✓ |
| `ai_trigger`·`scale` | `warning` 코드·threshold·5단계 **전수 불변** ✓ |
| `DOMAINS`·`detectCrossWarnings` | 불변 ✓ |

⚠ **`guide`에는 `POS`·`식재료`·`플레이스` 같은 단어가 의도적으로 남아 있다.**
업종별 예시를 나열하는 필드이므로 정상이며, 전용어 검사는 **본문만** 대상으로 해야 한다.

### ⑤ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818k`

### ⑥ 남은 이슈
1. **`DiagCommon`에는 업종 그룹 구조가 없다.** 이번엔 중립화 + guide 다중 예시로 해결했으나,
   sme 경로도 업종별 분화가 필요해지면 `DiagMicro`의 `INDUSTRY_WORDING` 방식을 이식해야 한다.
   **별도 작업이며, 하려면 `getSchema(group)` 시그니처 변경과 호출부 정리가 선행된다**
2. `DiagSme` 18문항은 여전히 어떤 경로에서도 렌더링되지 않는다 (기존 이슈, 변동 없음)

---

## 최근 수정 이력 (2026-09-04) — DiagMicro 외식업 편향 제거 **35문항 전체 완료 (D1~D7)**

**35문항이 외식업 기준으로 작성돼 있고 `INDUSTRY_WORDING` 오버라이드가 13문항만 덮어,
미용실·소매·학원·전문서비스 사장님이 "테이블 회전율"·"메뉴 구성"·"주방 동선"을 그대로 받고 있었다.**

### ① 방식 — 새 구조를 만들지 않았다
`getSchema(group)`이 `ITEMS`에 `INDUSTRY_WORDING[group]`을 `Object.assign`으로 병합하는
**기존 메커니즘을 그대로 쓴다.** 문제는 두 가지였다.
1. 기본 `ITEMS` 자체가 외식업
2. 덮어쓴 것이 13문항뿐 — 게다가 **`label`·`question`만 덮고 `scale`은 외식 문구가 남아 있었다**

```
기본 ITEMS  → 업종 중립으로 교체
INDUSTRY_WORDING.food (신설) → 제거한 외식 표현을 원문 그대로 보존
나머지 4그룹 → 빠진 문항 신설 + scale 보강 + guide 추가
```
⚠ **food 그룹 결과는 `label`·`question`·`scale`·`ai_trigger`가 수정 전과 글자까지 동일하다.**
중립화하면서 외식업 품질을 희생시키지 않는 것이 이 작업의 제약이다.

### ② `guide` 필드 신설 — 질문은 중립, 예시는 업종별
질문을 중립화하면 구체성이 떨어진다. 이를 `guide`가 메운다.

| 그룹 | `1_2` guide |
|---|---|
| food | 예: 테이블 회전율, 객단가, 재방문 주기 |
| beauty | 예: 시술 단가, 예약 밀도, 재방문 주기 |
| retail | 예: 구매 단가, 재구매율, 장바구니 크기 |
| edu_service | 예: 수강료 단가, 재등록률, 수강 지속 기간 |
| pro_service | 예: 건당 수임료, 재의뢰율, 계약 지속 기간 |

⚠ **`guide`는 원래 렌더링되지 않는 죽은 필드였다.** `DiagCommon`에 20문항 전부 `guide`가
있었지만 [wizard.js `_renderItemHtml()`](js/wizard.js)이 `question`·`scale`·`memo`만 출력하고
`guide`를 무시해 **화면에 나온 적이 없었다.** 이번에 렌더링 코드(2줄) + `.diag-item-guide`
스타일을 추가해 **살렸다.**
→ **부수효과(의도한 것)**: `DiagCommon` 20문항의 guide도 함께 표시된다.
   `업종 기준치: 외식 60% / 제조 75% / IT 30%` 같은 내용이라 sme 사장님의 응답에 도움이 된다.

### ③ 커버리지 — 실제로 비어 있던 곳
| 문항 | 수정 전 | 조치 |
|---|---|---|
| `1_1` 상권 빅데이터 | **오버라이드 0건** — 4그룹 전부 "메뉴 공급" 수신 | 기본 중립화 + 4그룹 신설 |
| `1_2` 판매 데이터 | label·question만 덮음 → **scale이 외식** | 그룹별 scale 신설 |
| `1_3`·`1_4` | 완전 커버 | guide만 추가 |
| `1_5` OHI | question만 덮음 → **scale이 "주방 조리·홀"** | 그룹별 scale 신설 |
| `2_1`·`2_2` | **오버라이드 0건** | 기본 중립화 + 4그룹 신설 |
| `2_3`·`2_4`·`2_5` | label·question만 덮음 → **scale이 외식** | 그룹별 scale 신설 |
| `3_1`·`3_2`·`3_5` | label·question만 덮음 → **scale이 외식** | 그룹별 scale 신설 |
| `3_3`·`3_4` | **오버라이드 0건** | 기본 중립화 + 4그룹 신설 |
| `4_1`·`4_2` | label·question만 덮음 → **scale이 외식** | 그룹별 scale 신설 |
| `4_3`·`4_4`·`4_5` | **오버라이드 0건** | 기본 중립화 + 4그룹 신설 |
| `5_1` / `6_1`·`6_4` | **오버라이드 0건**, 외식용어 3건 / 3건 | 기본 중립화 + food 보존 |
| `5_2`~`5_5`·`6_2`·`6_3`·`6_5` | **깨끗함** (자금·세무·폐업은 업종 무관) | **본문 미변경, guide만 삽입** |
| `7_1`·`7_5` | question만 덮음 → **scale이 외식** | 그룹별 scale 신설 |
| `7_2`·`7_3`·`7_4` | **오버라이드 0건** | 기본 중립화 + 4그룹 신설 |

`2_5`는 편향이 가장 심했다 — `주방 3각 작업 삼각형(싱크대·화구·준비대)`.
기본을 `작업 공간 3각 동선(자재 보관·작업대·처리 설비)`으로 바꾸고 그룹별로
`보관장→준비대→시술석`(beauty) / `창고→진열대→계산대`(retail) /
`교구실→교실→출력공간`(edu) / `파일실→상담실→접수대`(pro)로 분화했다.

⚠ `1_1`의 `매장`은 **기본을 `사업장`(중립)으로 두고 food·beauty·retail·edu가 `매장`으로
덮어쓴다.** `pro_service`만 `사업장`을 유지한다 — 세무사·컨설팅 사무소에 `매장`은 부적합하다.
**기본은 중립, 각 그룹이 자기 표현을 덮어쓰는 구조**가 이 모듈의 원칙이다.

### ④ ⚠ 같은 패턴의 결함 3건 — 그룹을 무시하고 기본 `ITEMS`를 참조
전수 조사 결과 **"화면은 미용실인데 내부 로직은 외식"** 인 지점이 더 있었다.

| 지점 | 판정 | 조치 |
|---|---|---|
| `buildPromptSummary()` | **결함** — `Object.entries(ITEMS)`로 기본 label 사용. 미용실 진단인데 **AI 프롬프트에 `POS 데이터 분석`·`조정 기여 마진(ACM) 산출`이 나갔다** | `getSchema(group).items` 사용으로 수정 |
| `detectCrossWarnings()` | **결함** — 그룹 인자가 없어 `Dog 메뉴 정리`·`배달 단일 채널`·`사장이 매장에 묶여` 가 전 업종에 나갔다 | `detectCrossWarnings(scores, industryGroup)`로 확장 |
| `calcScores()` L1230 `Object.keys(ITEMS)` | **결함 아님** — 키만 쓴다. 키는 전 그룹 동일 | 미변경 |

**`detectCrossWarnings` 시그니처 확장 시 호출부를 전수 조사했다.**
`DiagMicro`의 것은 **2곳**뿐이다 — `wizard.js:3127`(`microGroup` 전달로 갱신) / 내부 `buildPromptSummary`.
`dashboard.js:698`의 `D.detectCrossWarnings`는 **사회적경제 모듈**이라 무관하다.
⚠ **인자 미지정 시 `food`로 폴백**한다(`WARN_WORDING[group] || WARN_WORDING.food`).
무인자·`undefined`·오타 그룹 모두 기존 동작과 동일함을 검증했다.

`WARN_WORDING` 테이블로 어휘만 분기한다(`cost`·`dog`·`subj`·`channel`·`owner`).
조사(은/는) 문제를 피하려 **어절 단위가 아니라 완성 구(`이 사무소는`·`대표가 사무소에`)로 보관**한다.

### ④-2 D3·D4에서 특히 컸던 편향
| 문항 | 수정 전 (전 그룹) | 수정 후 (기본) |
|---|---|---|
| `3_2` | **패키징 규격 생산력** — 진공 포장·급속 냉동·**밀키트** | 상품·서비스 패키지화 |
| `4_1` | 주문 접수 지연율 — **테이블**·벨 호출·**키오스크** | 접수 처리 지연율 |
| `4_2` | 단순 육체노동 단축 — **홀 퇴식·서빙 로봇·식기세척** | 단순 반복 노동 단축 |

그룹별로 실제 설비·업무에 맞춰 분화했다 — `세탁·소독 자동화`(beauty) / `바코드·자동 재고 집계`(retail) /
`출결 자동화·자동 정산`(edu) / `전자문서·전자계약·자동 청구`(pro).

⚠ `retail`의 기존 `3_1` 문구에 있던 **`배달 앱`을 `오픈마켓`으로 바꿨다.**
`mfg_parts`(부품가공)·`wholesale`·`construction`도 retail 그룹이라 배달 앱은 부적합하다.

### ④-3 `DOMAINS.desc` 그룹 분기 (신규)
`DOMAINS`는 그룹 인지가 없어 D2 `'파사드 시선 주목도'`·D3 `'오프라인·배달·이커머스 판로'`가
세무사·학원에도 그대로 나갔다. `DOMAIN_DESC_BY_GROUP`을 신설하고 `getSchema()`가 **`desc`만 덮은**
`domains`를 반환하게 했다.

⚠ **`label`·`key`·`weight`·`id`는 절대 분기하지 않는다.** `calcScores()`가 반환값에 `domain.label`을
실어 레이더차트·대시보드로 흘려보내므로 바꾸면 표시가 어긋난다. `food`는 기본 `DOMAINS`를
객체 동일성까지 유지한다.

### ④-4 ⚠ ACTION_PLAN 전수 조사 — 보고된 것보다 훨씬 작았다
"실행계획이 외식업 그대로"라는 지적을 전수 조사한 결과 **실제 잔존은 1건**이었다.

| 확인 항목 | 결과 |
|---|---|
| `ACTION_PLAN_7DAY_BY_GROUP` 140개(4그룹×5도메인×7일) | 외식 용어 **1건** — `beauty` `'Dog 시술 예약 메뉴에서 비노출'` → `'예약 목록에서'` 교정 |
| `pro_service` 5건 | **오탐** — `포스터`가 `포스`로 매칭된 것. 실제 계획은 `서비스별 수임 건수 추출`·`크몽·숨고 입점`·`파일실·상담실 동선 실측`으로 이미 업종에 맞다 |
| base `ACTION_PLAN_7DAY`(외식 기준) | 비외식 그룹에는 **D5·D6만 도달**한다(그룹 계획이 없는 2개 도메인). 두 title 모두 중립(`사업 전용 계좌 분리…`·`월간 BEP 돌파율…`) |

⚠ **base의 `desc`·`tool`·`output`은 어디에서도 소비되지 않는다.** `buildPromptSummary`가 `title`만
프롬프트에 넣는다(저장소 전체 소비처 0건). 인용된 `메뉴별 판매수·단가 정렬`·`테이블에 QR 스탠드`는
이 죽은 `desc` 필드의 내용이며 **사용자에게 전달된 적이 없다.**

### ④-5 ⚠ 작업 중 실제로 낸 사고 — 그룹 경계 침범
`DOMAIN_DESC_BY_GROUP`을 신설하면서 **`beauty:` 같은 그룹명 키가 파일에 두 벌**이 됐다
(`DOMAIN_DESC_BY_GROUP` 512행대 / `INDUSTRY_WORDING` 711행대, 둘 다 4칸 들여쓰기).
D4 패치 스크립트가 **앞의 것을 먼저 잡아** beauty 범위를 226줄로 계산했고 그룹 경계를 넘었다.
→ 커밋 전 그룹별 오버라이드 키 목록 검사에서 `beauty NOT FOUND`로 잡아내고 **직전 커밋으로 되돌린 뒤**
   스크립트에 `INDUSTRY_WORDING` 이후에서만 탐색 + 그룹 경계 초과 시 assert를 넣어 재작업했다.
**같은 이름의 테이블이 둘 이상 생기면 텍스트 기반 패치는 반드시 앵커를 명시해야 한다.**

### ④-6 D5~D7 — 편향이 D7에 몰려 있었다
착수 전 문항별로 세어 보니 **24건이 고르지 않게 분포**했다.

| 영역 | 오염 문항 | 건수 |
|---|---|---|
| D5 운영자금·ESG보증 | `5_1`만 | 3 |
| D6 사업정리·폐업세무 | `6_1`·`6_4` | 3 |
| **D7 SNS·생성형AI** | **5문항 전부** | **18** |

`5_2`~`5_5`·`6_2`·`6_3`·`6_5` **8문항은 실제로 깨끗했다** — 자금·세무·폐업은 업종 무관이라는
기존 기록이 맞았다. 이 8문항은 **본문을 건드리지 않고 `guide`만 삽입**했다
(원문을 다시 타이핑하면 전사 오류가 날 수 있어 앵커 삽입 방식을 썼다).

| 문항 | 수정 전 | 수정 후 (기본) |
|---|---|---|
| `5_1` | 가계·**매장** 계좌 분리 | 가계·**사업** 계좌 분리 |
| `7_2` | **메뉴판** 폰트 수정, 신**메뉴** 홍보 포스터 | 안내물 문구 수정, 신규 홍보 포스터 |
| `7_3` | 리뷰 QR 스탠드를 **테이블**에 부착 | 접수대에 부착 |
| `7_5` | **매장 외경이나 흐릿한 고기 사진** | 사업장 외경이나 흐릿한 사진 |

그룹별로 실제 접점에 맞춰 분화했다 — 리뷰 QR 위치가 `접수대`(beauty) / `계산대`(retail) /
`상담실`(edu) / `상담 후 요청 문자 템플릿`(pro)으로 갈린다.

⚠ **`시그니처`도 외식 유래어로 보고 정리했다.** 기존 그룹 문구에 `시그니처 시술`·`시그니처 강좌`가
남아 있었는데 어색하다 → `대표 시술`·`대표 강좌`·`주력 상품`·`주력 서비스`로 바꿨다(7건).

⚠ **`매장`은 그대로 뒀다.** beauty·retail·edu_service에는 자연스러운 표현이며 이미 승인된 결정이다.
다만 **`pro_service`의 `3_1`에 `오프라인 매장 상담`이 남아 있어 `사무소 상담`으로 고쳤다** —
"pro_service만 중립어를 유지한다"는 결정과 정면으로 어긋나는 한 곳이었다.
D1~D4는 건드리지 않는다는 제약의 예외이며, **단어 하나만 바꿨다.**

### ⑤ 검증 (D1 9/9 · D2 14/14 · D3 15/15 · D4 15/15 · **D5~D7 16/16**)
| 항목 | 결과 |
|---|---|
| **food 전 35문항** | label·question·scale·ai_trigger **원문과 완전 동일** ✓ |
| 비외식 4그룹 외식용어 | **전 35문항 0건** (착수 전 D5~D7만 24건) ✓ |
| guide | 5그룹 × 35문항 = **175건** 전수 · 렌더링 확인 ✓ |
| scale·ai_trigger | 5단계 전수 · `warning` 코드 전 그룹·전 문항 **불변** ✓ |
| **calcScores** | 1~5점 전부 수정 전과 **동일** (점수 로직 무변경) ✓ |
| 문항 수 | **35 불변** ✓ |
| `7_1`·`7_5` 오버라이드 | 보존 확인 (그룹 경계 사고 회귀 검사) ✓ |
| DOMAINS | `label`·`weight` 전 그룹 불변 · food `domains` 객체 동일 ✓ |
| `INDUSTRY_GROUP_MAP` | 무변경 ✓ |
| food AI 프롬프트 | 무변경 ✓ |
| `detectCrossWarnings` 폴백 | 무인자·undefined·오타 그룹 = 원문 ✓ |

### ⑥ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818j` (D1 `f` · D2 `g` · D3 `h` · D4 `i` · D5~D7 `j`)

### ⑦ 남은 이슈
1. ✅ **DiagMicro 35문항 중립화 완료.** D1~D7 전 영역, 5개 그룹 전부 외식용어 0건이며
   food는 원문과 완전 동일하다. **`guide` 175건이 새로 생겼다.**
   ⚠ 남은 근본 한계는 **제조업 전용 그룹이 없다**는 것이다 — `mfg_parts`·`wholesale`·`construction`·
   `logistics`·`export_sme`가 전부 `retail` 그룹을 쓴다. 소매 문구(진열대·계산대·창고)가
   부품가공·건설에 완전히 맞지는 않는다. **그룹 신설은 별도 작업이다**
2. ✅ **`DOMAINS.desc` 그룹 분기 — D3 단계에서 완료** (④-3 참조)
3. **⚠ 죽은 필드 3종의 처리 방침 결정 필요.**
   - `ai_trigger.warning_msg` (그룹별로 작성돼 있으나 미사용)
   - base `ACTION_PLAN_7DAY`의 `desc`·`tool`·`output` (내용이 풍부한데 미사용)
   - `ACTION_PLAN_7DAY_BY_GROUP`은 **`title`만 있고 `desc`가 아예 없다** —
     위 `desc`를 살리기로 하면 **140개 항목에 desc를 새로 써야 한다**(별도 단계 규모)
4. **`ai_trigger.warning_msg`는 소비처가 0건이다.** `buildPromptSummary`는 `item.label`만 쓰고
   `detectCrossWarnings`는 자체 msg를 쓴다. 패턴 일관성을 위해 채워 두었으나
   **활용할지(취약 문항의 경고 문구로 프롬프트에 주입) 제거할지 결정이 필요하다**
4. **`getSchema()`가 오버라이드 없는 그룹에 기본 `ITEMS` 객체 참조를 그대로 반환한다**
   (`var finalItems = ITEMS`). 현재 호출부가 변형하지 않아 문제없으나, 방어적으로 사본을
   반환하는 편이 안전하다

---

## 최근 수정 이력 (2026-09-03) — 창업 초기(STARTUP) 도메인 매핑 오류 수정

### ① 문제 — 빈 막대는 증상이고 진짜 문제는 점수 오집계였다
[wizard.js `calcDomainScores()`](js/wizard.js)의 `isStartup` 분기에서
```js
finance         ← _s1_, _s2_
differentiation ← (없음)
```
STARTUP은 **4영역**인데 도메인은 **5개**다. `differentiation`이 항상 빈 배열이 되어 막대가 비었고,
더 중요하게는 **`s1`(사업 검증) 점수가 `finance`(자금)로 집계**되어 재무 점수가 왜곡됐다.
창업 초기 라벨에서 `differentiation`은 **'사업 검증도'**다 — 라벨과 소스가 어긋나 있었다.

### ② 수정 전 STARTUP 영역 실제 내용 확인
| 영역 | 문항 | 성격 |
|---|---|---|
| s1 사업 검증 및 계획 수립 | 고객 검증(파일럿·첫 계약) / 사업계획서 | **사업 검증** |
| s2 현금 생존력 (Runway) | 런웨이 개월 수 / BEP 산출·달성 시점 | **자금** |
| s3 초기 고객 확보·매출 파이프라인 | 확정 계약 / 30~90일 잠재 고객 | **고객 확보** |
| s4 핵심 역량 및 운영 준비도 | 즉시 제공 가능 여부 / 파트너·인력 확보 | **운영 준비** |

→ 제안된 매핑이 **정확하다**. 그대로 적용했다.
```js
differentiation ← _s1_                          // 사업 검증도
finance         ← _s2_                          // 자금·사업계획
bm              ← _s3_                          // 고객 확보력
hr              ← _s4_                          // 운영 준비도
future          ← diag-industry-container_      // 업종 대응력
```
⚠ **한 가지 어긋남을 남겨 둔다**: `finance` 라벨이 `'자금·사업계획'`인데 사업계획서(`s1_2`)는 `s1`에 있어 `differentiation`으로 간다. 라벨을 `'현금 생존력'`으로 바꾸는 편이 정확하나 **이번 지시 범위(소스 매핑)를 벗어나므로 건드리지 않았다.**

### ③ 영향 확인 (4곳)
**1. 레이더차트** — 5축 모두 값이 생긴다. 수정 전 `differentiation` 축은 0으로 찍혔고 점수 바에 `미입력`이 떴다. 수정 후 5축·점수 바 5건·해설 카드 5건 모두 정상.

**2. PatternDB — 더 나빠지지 않고 오히려 개선된다.**
`domainScores.differentiation?.avg || 3` 구조라 수정 전에는 **기본값 3**으로 대체됐다. 수정 후 실제 값이 전달된다. (PatternDB의 나머지 결함 — micro/사회적경제 키 불일치, `diagData.industry` 공백 — 은 그대로다)

**3. `consultingType` — 판정이 달라질 수 있다.**
`classifyConsultingType()`은 `d.avg > 0`으로 거르므로 수정 전 `differentiation`은 **순위에서 아예 빠졌다**(오분류가 아니라 무시). 다만 `finance`가 s1+s2 혼합값이었으므로 **최약 도메인 판정과 `overallAvg`가 왜곡**돼 있었다. 수정 후 `finance`가 순수 런웨이 점수가 되어 자금 위험이 제대로 최약으로 잡힌다. 이 값은 `data.consultingType`으로 **AI 프롬프트에 전달**되므로 조언 방향이 바뀐다 — 의도한 교정이다.

**4. AI 프롬프트** — `domainScores` 자체는 프롬프트에 직접 들어가지 않고 `consultingType`을 거쳐 반영된다. 위 3번과 동일.

### ④ ⚠ HistoryTracker — 과거 기록과의 불일치 (처리하지 않음, 제안만)
`_flattenDomains()`가 `v.avg || 0`으로 저장하므로 **과거 스냅샷의 `differentiation`은 0**이고 `finance`는 s1+s2 혼합값이다.
`renderCompare()`는 `finance·hr·bm·differentiation` 4개를 과거와 델타 비교한다.

→ 창업 초기 기업이 **재진단하면**:
- `differentiation`: 과거 0 → 현재 실제값 ⇒ **허위 급상승(▲)** 표시
- `finance`: 혼합값 → s2 단독 ⇒ 실제 개선과 무관한 변동

**영향 범위는 좁다** — 창업 초기 모드로 **이전에 진단한 적이 있고**(같은 회사명) 다시 진단하는 경우에만 발생한다. 일반·소상공인·사회적경제 경로는 무관하다.

**제안(임의 처리하지 않음)**: 스냅샷에 `schemaVersion`을 추가하고, `renderCompare()`가 버전이 다른 과거 스냅샷과는 **도메인 델타 비교를 건너뛰고 안내 문구만 표시**하는 방식이 안전하다. 과거 데이터를 소급 변환하는 것은 원본 훼손이라 권하지 않는다. **결정 후 별도 작업으로 진행할 사안이다.**

### ⑤ 검증 (16/16 통과)
`s1=5 · s2=1 · s3=3 · s4=4 · 업종=2`로 서로 다른 값을 넣어 어느 도메인으로 가는지 식별했다.

| 항목 | 결과 |
|---|---|
| ① 5개 도메인 전부 산출 | `finance 1.0 · hr 4.0 · bm 3.0 · future 2.0 · differentiation 5.0` — **0점 도메인 없음** ✓ |
| ② 사업 검증도 = s1 반영 | `differentiation = 5.0` ✓ / `finance = 1.0`(s2 단독) ✓ / **구 매핑이면 finance가 3.0**이었다 ✓ |
| ③ 일반 경로 회귀 | sme 전항목 3점 → 5도메인 **전부 3.0** · 라벨 불변 · micro는 기존대로 `_calcMicroDomainScores` 사용 ✓ |
| 레이더차트 | 5축 전부 0 초과 · 점수 바 5건 · **`미입력` 0건** · 해설 카드 5건 ✓ |
| PatternDB | `differentiation`이 기본값 3이 아닌 실제 값 5.0 전달 ✓ |
| consultingType | 취약 순 `finance(1) < future(2) < bm(3) < hr(4) < differentiation(5)` → `finance_strategy` ✓ |

**`isStartup` 분기만 수정했다.** 일반 경로 매핑·진단 문항은 손대지 않았다.

### ⑥ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818e`

---

## 최근 수정 이력 (2026-09-03) — radarChart id 중복 · PPT 파일 손상 수정

### ① `radarChart` id 중복 — 대시보드 차트가 diag-reveal을 덮어쓰고 있었다
`index.html`에 `id="radarChart"`가 **두 개**였다(1550 `#diag-reveal` / 1694 `#sec-diag`). HTML 명세 위반이며 `getElementById`는 **첫 번째만** 반환한다.

⚠ **조사 결과 대시보드 캔버스는 죽은 요소가 아니었다.** [dashboard.js `renderRadar()`](js/dashboard.js)가 **Chart.js로 실제로 그리고 있다**(`new Chart(ctx, {type:'radar'})`).
그런데 `getElementById('radarChart')`가 diag-reveal 캔버스를 반환하므로:
1. **Chart.js가 diag-reveal의 수동 레이더차트를 덮어썼다**(`drawRadarChart`가 그린 것 위에)
2. `#sec-diag`의 캔버스는 **영영 비어 있었다**

**수정**: 1694행을 `id="radarChartDash"`로 바꾸고 `renderRadar()`가 새 id를 참조하게 했다. 두 차트가 각자의 캔버스에 그린다.
- `ppt-export.js`의 `querySelectorAll` 우회는 **유지하되 의미를 바꿨다** — `'canvas#radarChart, #radarChart'`(중복 회피용) → `'#radarChart, #radarChartDash'`(진입 경로에 따라 채워진 쪽을 고름). 알파 채널 검사는 그대로 둔다
- 문서 전체 중복 id **0건** 확인

### ② PPT 파일이 열리지 않던 원인 — **CDN URL이 404**
```
https://cdnjs.cloudflare.com/ajax/libs/pptxgenjs/3.12.0/pptxgen.bundle.js  → HTTP 404
```
**cdnjs는 pptxgenjs를 아예 호스팅하지 않는다.** API 조회 결과 `{"error":true,"status":404,"message":"Library not found"}`.
2026-09-03 PPT 신설 때 존재하지 않는 URL을 넣었다.

**수정**: 이 프로젝트가 이미 쓰는 jsDelivr로 교체.
```
https://cdn.jsdelivr.net/npm/pptxgenjs@3.12.0/dist/pptxgen.bundle.js   → 200 OK, 477,529 bytes
```

**검증 절차(추측하지 않고 실제로 확인한 것)**
1. Node에 PptxGenJS 3.12.0을 설치해 **현재 코드와 동일한 호출 형태**로 파일 생성 → 유효한 zip. `writeFile({fileName})`·`addShape('rect')`·`addTable`·`addImage(data URI)`·`undefined/NaN` 혼입까지 4케이스 전부 정상
2. **실제 `ppt-export.js`를 실제 라이브러리로 실행**해 6개 유형 파일 생성 → 전부 유효
3. 생성 파일을 **실제로 압축 해제**해 `ppt/presentation.xml`·`[Content_Types].xml` 파싱 확인
→ **코드에는 결함이 없었다.** 라이브러리가 로드되지 않은 것이 유일한 원인이다.

⚠ **다만 보고된 증상과 완전히 일치하지는 않는다.** 404면 `PptxGenJS`가 `undefined`이므로 버튼이 `disabled`되고 `download()`는 안내만 띄운다 — 파일이 생성되지 않아야 한다. "다운로드는 되는데 열리지 않는다"는 이 실패 모드로 설명되지 않는다.
가능성: 브라우저가 404 HTML을 캐시했거나, 이전 배포 시점의 페이지에서 시도했거나, 다른 파일을 열었을 수 있다. **URL 결함은 확실하므로 고쳤고, 재배포 후 동일 증상이 남으면 브라우저 콘솔 오류를 확인해야 한다.**

### ③ 관련 증상 — "차별화 미표시 · 사업검증도 빈 막대"는 **별개 문제** (이번에 고치지 않음)
id 중복과 무관하다. 원인은 [wizard.js `calcDomainScores()`](js/wizard.js)의 **창업 초기 분기에서 `differentiation` 도메인에 점수 소스가 없는 것**이다.

```js
if (isStartup) {
  finance         ← _s1_, _s2_
  hr              ← _s4_
  bm              ← _s3_
  future          ← diag-industry-container_
  differentiation ← (없음)          // ← 라벨이 '사업 검증도'
}
```
STARTUP은 4영역(s1~s4)인데 도메인은 5개다. `differentiation`이 항상 빈 배열이 되어 **빈 막대**로 나온다.
- 일반 라벨은 `차별화·경쟁우위역량`, 창업 초기 라벨은 `사업 검증도` — **같은 도메인**이다. 보고된 두 증상은 한 원인이다
- 의도는 `differentiation ← s1(사업 검증)` / `finance ← s2(런웨이)`였던 것으로 보이나 `s1`이 `finance`로 들어가 있다. **매핑 오류**
- ⚠ 지시대로 **원인만 확인하고 수정하지 않았다.** 도메인 매핑 변경은 레이더차트·PatternDB·HistoryTracker에 함께 영향을 준다

### ④ 검증 (지시된 3개 항목)
| 항목 | 결과 |
|---|---|
| ① `id="radarChart"` 유일성 | 1개 · `radarChartDash` 1개 · **문서 전체 중복 id 0건** ✓ |
| ② diag-reveal 레이더차트 회귀 | `drawRadarChart`가 `radarChart`에 그림 · 5대 역량 산출 · 점수 바 5건 ✓ |
| ③ PPT zip 유효성 | 6유형 전부 PK 서명·`[Content_Types].xml`·`presentation.xml` OK. 압축 해제 후 슬라이드 수 micro 8 / sme 12 / 사회적경제 9·8·9 / 정책자금 7 — **설계와 일치** ✓ |

### ⑤ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818d`

---

## 최근 수정 이력 (2026-09-03) — 진단 경로 오류 2건 수정 (mfg_parts 그룹 · micro 창업자 STARTUP)

전면 점검 조사 결과를 바탕으로 **저위험 2건만** 수정했다. 나머지 문제는 남은 이슈로 남긴다.

### ① `mfg_parts`가 food(외식) 그룹에 매핑되어 있던 문제
[diagnosis-micro.js](js/diagnosis/diagnosis-micro.js)의 `INDUSTRY_GROUP_MAP`에서 **뿌리 제조·부품가공(`mfg_parts`)이 `food`**였다.
`INDUSTRY_WORDING`은 food를 기준(변경 0건)으로 두므로, **제조업 소상공인이 외식업 문구를 35문항 100% 그대로** 받았다.

**`retail`을 택했다** — 제조업 전용 그룹은 만들지 않았다(범위 밖). 5개 그룹 중 가장 덜 부적합한 쪽을 실제 문구를 비교해 골랐다.

| 비교 축 | `retail` | `pro_service` | 판정 |
|---|---|---|---|
| 원가 구조(1_3) | 상품원가 + 인건비 | **외주비·소모품비** + 인건비 | retail — 제조는 재료비+노무비. pro_service는 **재료비 개념 자체가 없어 원가 진단이 성립하지 않는다** |
| ACM(1_4) | 매입 원가 + 처리·진열 노동시간 | 직접 원가 + **상담** 노동시간 | retail |
| 판로(3_1) | 스마트스토어 + **B2B 도매 납품** | 크몽·숨고 + **B2B 자문 계약** | retail — 부품가공은 B2B 납품이 본업 |
| 자동화(4_2) | 바코드·**자동 재고 집계** | 예약 문자·계약서·서류 정리 | retail |
| 동선(2_5) | **창고** · 진열대 · 계산대 | 파일실 · 상담실 · 접수대 | retail — 자재창고 개념 대응 |

⚠ **완전한 해결이 아니다.** retail 전환 후에도 `메뉴·테이블·조리·주방`이 남는다(그룹 분화 커버리지 13/35). 근본 해결은 제조업 전용 그룹 신설이며 남은 이슈로 둔다.
⚠ 다른 업종 매핑은 건드리지 않았다. **`food_mfg`(식품제조)도 `food`로 남아 있으며 부적합하다** — 식품제조는 B2B 납품·OEM 중심인데 "테이블 회전율"을 받는다. 이번 지시 범위가 `mfg_parts`뿐이라 수정하지 않았고 남은 이슈로 기록한다.

### ② micro 경로에서 `isStartupMode`가 검사되지 않던 문제
`loadDiagnosisUI()`의 분기가
```
if (isSocial) … else if (isMicro) … else { if (isStartupMode) … }
```
구조여서 **창업 초기 검사가 마지막 `else` 안에만** 있었다.
→ 개업 1년 미만 소상공인이 STARTUP(8문항) 대신 **실적 기반 DiagMicro(35+16=51문항)**를 받았다. 매출 이력이 없는데 "최근 3개월 재방문율", "테이블 회전율"을 묻는 상태였다.

**수정**: `isStartupMode`를 `isMicro`보다 **먼저** 구하고 `isMicro`에서 창업 초기를 제외한다.
```js
const isStartupMode = document.getElementById('aiIsStartup')?.value === 'true';
const isMicro = currentBizScale === 'micro' && !isStartupMode && typeof DiagMicro !== 'undefined';
```
micro + 창업 초기가 마지막 `else`로 흘러 STARTUP이 렌더링된다. **분기 구조 자체는 그대로 두고 조건만 좁혔다.**
- `isMicro`가 함수 전체에서 일관된 의미를 갖게 되어 **탭 라벨(`isMicro ? '소상공인 7대 분야' : isStartupMode ? '창업 초기 진단'`)도 자동으로 맞는다**
- `_activeContainers`·`DIAG_CONTAINERS`는 `else` 분기가 이미 `diag-common-container`를 등록하므로 **추가 수정 불필요**

⚠ **`isSocial`(사회적경제 3유형)은 건드리지 않았다.** 조사에서 "창업 초기 배제가 의도인지 확인 불가"로 나왔고, 확인되지 않은 것을 임의로 바꾸지 않는다.

### ③ 검증 (지시된 5개 항목 + 렌더 상태)
| 항목 | 결과 |
|---|---|
| ① micro + `mfg_parts` | `getGroup` = **retail** · food 대비 13문항 변경 적용 ✓ |
| ② micro + 창업 초기 | **STARTUP 8 + 업종 16 = 24문항** · 진행률 `0 / 24` · 탭 라벨 `🚀 창업 초기 진단 (8문항)` · 활성 컨테이너 `diag-common` + `diag-industry` ✓ |
| ③ micro + 일반 (회귀) | DiagMicro **51문항** · 탭 라벨 `🏪 소상공인 7대 분야 (35문항)` ✓ |
| ④ sme + 창업 초기 (회귀) | STARTUP **24문항** 그대로 ✓ |
| ⑤ 사회적경제 3유형 (회귀) | 사회적기업·협동조합·소셜벤처 각 **56문항** 그대로 ✓ |
| 추가 | sme 일반 **36문항** 불변 · 타 업종 그룹 매핑 7건 불변 ✓ |

**문항 내용은 하나도 수정하지 않았다.** 매핑 1줄과 분기 조건 1줄만 고쳤다.

### ④ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818c`

### ⑤ 이번에 수정하지 않은 문제 (조사에서 확인된 것)
심각도 순. 각 항목의 영향 범위와 근거는 조사 결과 참조.
1. **`bizmodel/` 12개 모듈 192문항이 전혀 렌더링되지 않는다** (`TAB_ORDER`에 bizmodel 없음, `renderDiagModule('diag-bizmodel-container')` 호출 0건). 그 결과 **CROSS_RULES 34개 중 21개가 `collect()`의 BM 프록시(공통 진단 도메인 평균 복사)로 발동**한다 — 사용자가 답한 적 없는 값이다. 192개 업종×BM 조합 중 경고가 뜨는 조합은 18개(9.4%). ★★★ **"BM 진단을 되살릴 것인가, CROSS_RULES에서 BM 축을 걷어낼 것인가" 방향 결정이 선행되어야 한다**
2. **DiagMicro 그룹 분화 커버리지 13~14/35** — `pro_service`로 전환해도 요식업 단어 39건 잔존. 제조업 전용 그룹 부재. ★★☆
3. **DiagCommon 20문항 중 8개가 오프라인 매장/요식업 전제** (재작성 5: `2_1` 로컬플랫폼 NAP · `3_2` 오픈·마감/위생검수 · `3_3` 매장·CCTV·POS · `4_3` POS·배달플랫폼 · `5_2` 식재료 로스 / 문구만 3: `1_3`·`3_1`·`3_4`). sme 일반기업 전원 영향 ★★☆
4. **`food_mfg`(식품제조)가 food(외식) 그룹** — B2B 납품·OEM 중심인데 외식 문구를 받는다 ★★☆
5. **DiagSme 18문항이 어떤 경로에서도 렌더링되지 않는다.** `diag-sme-container`는 **git 전체 이력에서 index.html에 존재한 적이 없다** — 끊긴 것이 아니라 처음부터 연결되지 않았다. 살리려면 문항 재작성이 선행되어야 한다(프로세스 마이닝·ACM 산식·n8n 등 난이도 편향이 크다) ★☆☆
6. PatternDB 고정값 노출 (기존 기록) ★☆☆
7. `common.js`·`diagnosis-micro.js`·`diagnosis-sme.js`에 `module.exports`가 없어 Node 테스트에서 `require()`가 빈 객체를 반환한다 ☆

---

## 최근 수정 이력 (2026-09-03) — PPT 다운로드 신설 (PDF는 유지·병행)

**리포트를 발표·제출용 `.pptx`로 내려받는 기능을 신설했다.** 기존 PDF 저장은 그대로 두고 병행한다.

### ① `js/ppt-export.js` 신설 — 브라우저 생성
- **PptxGenJS를 CDN에서 `defer`로 로드**하고 클라이언트에서 생성한다. ⚠ 서버(`api/`)에서 만들지 않는다 — Vercel 함수 부하와 배포 용량이 늘어난다
- 라이브러리 미로드 시 **버튼을 `disabled`로 두고 `title`로 안내**한다. `window load` 후 `initButton()`이 상태를 갱신한다. 로드 실패로 페이지가 깨지지 않는다
- 파일명 `{조직명}_{진단유형}_{YYYYMMDD}.pptx` (조직명 없으면 `BizNavi`, 파일명 금지문자 제거)

### ② 밝은 테마 — 화면(다크)과 분리
발표·인쇄·제출용이므로 어두운 배경은 부적합하다.
흰 배경 `FFFFFF` / 본문 `333333` / 제목 `1A2340`(진한 남색) / 강조 `8B6914`
경고 등급 **CRITICAL `C0392B`(빨강) · HIGH `D35400`(주황) · MEDIUM `7F8C8D`(회색)**

⚠ **폰트는 `맑은 고딕` 하나만 지정한다.** PptxGenJS는 폰트를 임베딩하지 않고 폴백도 지정할 수 없다. 수신자 대부분이 Windows·PowerPoint 조합이라 이 선택이 안전하다(macOS는 시스템 대체 렌더링 — 감수).

### ③ 유형별 슬라이드 구성 — 섹션 수만큼 만들지 않는다
⚠ 리포트 섹션을 1:1로 옮기면 글자만 빽빽한 못 쓸 PPT가 된다. **슬라이드당 핵심 3~5개로 압축**하고 초과분은 하단에 `※ … 상세는 전체 리포트를 참조하십시오`로 명시한다.

| 유형 | 장수 | 구성 |
|---|---|---|
| **micro** | 8 | 표지 · 한눈에 보기 · 7대 영역(레이더) · 생애주기 · 상권STP·시장 · 영역별 처방(취약 4) · 90일 · 정부지원 |
| **sme** | 12 | 표지 · 한눈에 보기 · 5대 역량(레이더) · SWOT(각 3) · **STP** · **4P** · 핵심전략(4) · KPI(5) · 로드맵 · **실행 체계+90일 통합** · 정부지원 · **리포트에서 확인할 내용** |
| **사회적경제 3유형** | 9 | 표지 · 한눈에 보기(총점·취약Top3·CRITICAL/HIGH) · 8대 영역(레이더) · mission · revenue · profit · org · **AI 실행계획+제도** · 정부지원 |
| **정책자금** | 7 | 표지 · 판정 요약 · 기관별 2장 · 실행 로드맵 · 준비 서류 · 정부지원 (**레이더차트 슬라이드 없음**) |

판단이 갈린 지점:
- **STP·4P는 분리했다.** 한 장에 합치면 7줄이 되어 압축 원칙에 어긋나고, 컨설팅에서 가장 많이 인용되는 부분이라 손실이 크다
- **6가지 시스템+90일은 통합했다.** 실행 체계와 그 일정이므로 함께 보는 편이 낫다
- **유형별 특화 분석·린 캔버스는 슬라이드로 만들지 않았다.** 블록이 5~9개라 압축하면 의미가 사라진다. 대신 **마지막에 '리포트에서 확인할 내용' 슬라이드를 추가해 제외 항목명을 나열**한다 — "상세는 리포트 참조"만 적으면 받는 사람이 무엇을 놓쳤는지 모른다. **sme에만 넣는다**(다른 유형은 제외 항목이 없다)
- **사회적경제의 제도(system) 영역은 AI 계획 슬라이드 하단에 2줄로 합쳤다.** 단독 슬라이드로 만들면 얇다
  ⚠ **AI 계획이 실패하면 그 슬라이드가 생략되어 제도 영역이 통째로 사라진다.** 수미쌍관 위반이므로 `_appendSystemNote()`를 **org 슬라이드 하단으로 옮겨 붙인다.** 어느 쪽이든 8영역이 모두 출력된다

### ④ 레이더차트 — canvas → 이미지
`drawRadarChart()`가 Canvas 2D로 직접 그리므로 `toDataURL()`로 바로 추출된다(SVG 변환 불필요).

⚠ **`toDataURL()`은 빈 캔버스에서도 정상 문자열을 반환하므로 길이로 판별할 수 없다.**
→ `getImageData()`로 **알파 채널이 0이 아닌 픽셀**이 있는지 검사한다. `try/catch`로 tainted canvas도 방어.
비었으면 **영역별 점수 표로 대체**하고 다운로드는 계속 진행한다 — 차트 때문에 전체가 실패하면 안 된다.

⚠ **`index.html`에 `id="radarChart"`가 두 개 있다**(`#diag-reveal` · `#sec-diag`). `getElementById`는 첫 번째만 반환하므로 **`#sec-diag`의 canvas는 그려지지 않는 죽은 요소**다. PPT는 `querySelectorAll`로 전부 훑어 내용이 있는 쪽을 고른다.
정책자금은 `diag-reveal`을 거치지 않아 차트가 없으므로 **슬라이드 자체를 만들지 않는다.**

### ⑤ 텍스트 넘침 처리
PptxGenJS는 자동 축소를 하지 않는다 → **글자 수 상한**(줄 110 / 문단 260 / 제목 42)으로 자르고 `…`을 붙인다. 잘림이 발생한 슬라이드는 하단에 `※ 일부 내용이 요약·생략되었습니다`를 붙여 **잘렸다는 사실을 남긴다.** `_clippedInSlide` 플래그를 슬라이드 생성 시 초기화한다.

### ⑥ 데이터 접근 — `render()`가 AI 결과를 버리고 있었다
`Dashboard.render(data, fd, isDemo)`는 AI 결과를 DOM에 밀어넣고 **모듈에 보관하지 않았다.** PPT가 원본을 필요로 하므로 `_lastData`·`_fundingRoadmap`을 추가했다.

⚠ **이전 회사 데이터가 PPT에 섞이는 것이 최악이다.** 세 진입점이 모두 타 유형 잔존 데이터를 지우고, `App.restart()`가 `Dashboard.resetReport()`를 호출한다.

| 진입점 | 처리 |
|---|---|
| `render()` | `_lastData = data` · `_fundingRoadmap = null` |
| `renderSocial()` | `_lastData = null` · `_fundingRoadmap = null` |
| `renderFunding()` | `_lastData = null` |
| `renderFundingRoadmap('done')` | `_fundingRoadmap = roadmap` (그 외 상태는 `null`) |
| `App.restart()` | `Dashboard.resetReport()` — 전부 초기화 |

**유형 판별에 새 분기를 만들지 않았다.** `Dashboard.reportKind()`가 기존 `_isSocialFd()`·`_orgKind()`·`bizScale`·`purpose`를 그대로 재사용하고, 섹션 라벨·영역 매핑·경고 매핑도 `getReportContext()`가 넘겨준다(`secLabel`·`secDomainIds`·`warnSection`) — PPT가 로직을 복제하지 않는다.

### ⑦ 검증 (Node + PptxGenJS 스텁, 49/49 통과)
| 항목 | 결과 |
|---|---|
| 장수 | micro **8** / sme **12** / 사회적기업·소셜벤처·협동조합 각 **9** / 정책자금 **7** ✓ |
| sme 구성 | STP·4P 분리 · 실행체계+90일 통합 · 제외 항목 안내(특화분석·린캔버스) ✓ |
| **AI 실패** | 사회적경제 8장(계획 슬라이드 생략) · **제도 영역이 org 슬라이드에 살아 있음** ✓ / 정책자금 6장 ✓ |
| 빈 슬라이드 | 전 케이스 **0건** ✓ |
| 레이더차트 | 이미지 삽입 ✓ / **빈 캔버스 → 표 대체 + 다운로드 성공** ✓ / 정책자금 이미지 0 ✓ |
| 파일명 | `{조직명}_{유형}_{YYYYMMDD}.pptx` 3유형 검증 ✓ |
| 라이브러리 미로드 | `isAvailable() false` · 안내 후 중단 · 버튼 `disabled` ✓ |
| **데이터 격리** | sme→사회적경제 전환 시 이전 AI 결과 제거 · `resetReport()` 전부 초기화 ✓ |
| 테마·상한 | 흰 배경 · 등급별 색 · 맑은 고딕 · 잘림 표시 ✓ |
| PDF 유지 | `Dashboard.print()` 버튼 그대로 ✓ |

⚠ 실제 PowerPoint로 열어 레이아웃을 확인하지는 않았다(브라우저 다운로드 필요). 좌표·폰트 크기는 16:9 기준으로 잡았으나 **실사용 확인이 필요하다.**

### ⑧ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818b` (신규 `js/ppt-export.js` 포함, 외부 CDN 제외)

### ⑨ 남은 이슈 — PDF 빈 페이지 (보류)
**재현 조건 미확보로 이번에 손대지 않았다.** 원인 후보 3가지:
1. `#sec-six-systems`가 **micro에서도 표시되는데** `break-before: page`가 걸려 있음([print.css:351](css/print.css#L351))
2. `.print-cover`의 `break-after: page`와 뒤 섹션 `break-before: page`가 **연속** — 2026-05-06 재무 리포트에서 동일 문제를 겪고 `break-after: avoid`로 해결한 전례가 있다
3. `display` 처리된 `.section-card`의 마진 잔존

⚠ **`display:none` 요소는 박스를 생성하지 않으므로 페이지 나눔의 직접 원인이 아니다.** 초기 진단이 이 점을 잘못 짚었다. 추정으로 고치면 오진이 기록에 남으므로, **어느 리포트 유형에서 몇 번째 페이지가 비는지** 확인한 뒤 진행한다.

---

## 최근 수정 이력 (2026-09-02) — 협동조합 전용 진단지 신설 (C1~C8 40문항) · 사회적경제 3유형 완성

**`orgType='cooperative'`가 `DiagSocial`(사회적기업)을 빌려 쓰고 있었다.**
이번 신설로 **사회적경제 3유형(사회적기업 S1~S8 / 소셜벤처 V1~V8 / 협동조합 C1~C8)** 전용 진단이 완성됐다.

### ① 근거 — 협동조합기본법
조합원 자격·출자 / **1인 1표 의결권(출자액 무관)** / 총회·이사회 운영 의무 / 임원 선출·임기 규정 /
**잉여금 배당 제한(법정적립금 우선·이용고 배당 원칙)** / 조합원 교육 의무 /
조합원 외 거래 비중 제한(사회적협동조합) / 결산보고서 제출 의무

⚠ **사회적기업은 '인증'을 통해 얻는 지위, 협동조합은 '설립 신고'로 성립하는 법인 형태다.**
→ C7에 **만료·갱신 경고를 넣지 않는다**(S7·V7과 동일 원칙, 활용도에 집중).

### ② `js/diagnosis/diagnosis-coop.js` 신설 — `DiagCoop`
**C1 조합원 기반 / C2 민주적 운영·거버넌스 / C3 사업 지속가능성 / C4 재정·원가구조 /
C5 회계·법규 준수 / C6 판로·마케팅 / C7 제도 활용 / C8 디지털·AX** — 8영역 × 5문항 = 40문항.
점수 키 접두어 `diag-coop-container_`. `detectCrossWarnings` 14규칙(CRITICAL 2 / HIGH 5 / MEDIUM 7).

**⚠ C4·C6이 S4·S3+S6과 겹치는 것은 정상이다.**
재무 구조(변동비·고정비·공헌이익·BEP)는 법인 형태와 무관하게 같은 것을 봐야 한다 — 협동조합이라고 손익분기점을 다르게 계산하지 않는다. 억지로 다르게 만들면 진단 품질이 떨어진다.
대신 **문항 서술을 협동조합 맥락으로 썼다** — 예: `c4_5` 잉여금 처리는 법정적립금 우선·이용고 배당 원칙을 반영.
**협동조합기본법 고유 축은 C1·C2·C3·C5·C7**이며 차별성은 이쪽에 실었다.

### ③ 구조 재사용 — 새 분기를 만들지 않았다
DiagVenture 때 정리한 구조를 그대로 썼고, **`_calcOrgDomainScores()`와 `collect()`는 수정이 필요 없었다**(모듈의 `KEY_PREFIX`·`DOMAINS`에서 파생하므로). 이것이 그때 정리한 구조의 성과다.

| 지점 | 조치 |
|---|---|
| `wizard.js` `_orgDiagModule()` | `cooperative → DiagCoop` **한 줄 추가** |
| `wizard.js` `DIAG_CONTAINERS` · `_countDiagItems` 호출부 | `diag-coop-container` 추가 (⚠ 빠뜨리면 진행률이 틀린다) |
| `wizard.js` `COOP_DOMAIN_EXPLAIN` + `ORG_DOMAIN_EXPLAIN` | 해설 맵 분기를 **테이블 한 곳**으로 모음 |
| `wizard.js` `ORG_ICON` | 탭 라벨·배너의 아이콘 삼항 분기를 테이블로 통합 |
| `dashboard.js` `_orgKind(fd)` | `_isVentureFd` 이진 분기 → **`'social'|'venture'|'coop'` 단일 함수** |
| `dashboard.js` `SEC_LABEL` | 목차·섹션 제목이 **같은 테이블**을 쓴다(분기 중복 제거) |
| `dashboard.js` `SEC_DOMAINS` · `SOCIAL_WARN_SECTION` | coop 매핑·경고 코드 17개 추가 |
| `ai-engine.js` | 모듈 조회 3분기 확장 |
| `index.html` | `#diag-coop-container` + 스크립트 태그 |

**`ORG_TYPE_PENDING = {}`** — 3유형 모두 전용 모듈을 갖췄으므로 협동조합의 "준비 중" confirm과 배너 안내가 사라졌다.

### ④ 리포트 — 9섹션 구조 유지, 라벨·내용만 조직 형태별
| 섹션 | 사회적기업 | 소셜벤처 | **협동조합** |
|---|---|---|---|
| mission | 미션과 사업이 맞물리나 (S1·S2) | 사회문제를 제대로 정의했나 (V1·V2) | **조합원 실익과 사업이 맞물리나** (C1·C3) |
| revenue | 어디서 돈이 들어오나 (S3·S6) | 자금을 어떻게 조달하나 (V5·V4) | 어디서 돈이 들어오나 (C6·C4) |
| profit | 수익 구조 점검 (S4) | 기술이 무기가 되고 있나 (V3) | **법과 정관을 지키고 있나** (C5) |
| org | 조직이 버틸 수 있나 (S5·S8) | 팀이 버틸 수 있나 (V6·V8) | **조합 운영이 건강한가** (C2·C8) |
| system | 제도를 제대로 쓰고 있나 (S7) | 판별과 제도를 활용하고 있나 (V7) | 제도를 제대로 쓰고 있나 (C7) |

⚠ **당초 지시에는 `profit` 섹션에 대응할 협동조합 영역이 지정되지 않아 빈 곳이 생겼다.**
C5(회계·법규 준수)를 그 자리에 두고 **'법과 정관을 지키고 있나'**로 명명해 8영역이 빠짐없이 대응된다.
협동조합에서 법정 의무 이행은 **지원사업 결격과 직결**되므로 독립 섹션 가치가 있다.
- 핵심 강조 경고도 분기: 사회적기업 `winning_but_losing` / 소셜벤처 `tech_social_disconnect` / **협동조합 `legal_duty_unmet`**
- ⑦ 제도 섹션은 협동조합 전용 내용 — **"설립 신고로 성립하므로 인증 만료·갱신을 걱정할 필요가 없다"**를 명시하고 사회적협동조합 전환 검토·활용 제도에 집중

### ⑤ 정부지원사업 — 변경 없음
협동조합 매칭이 이미 적절해 **사업을 추가하지 않았다.**
협동조합 활성화 지원사업 포함 ✓ / TIPS·핀테크 제외 ✓(`orgType !== 'social_venture'` 조건에 자연히 포함) / 소셜벤처 전용 2건 제외 ✓

### ⑥ 검증 (Node DOM 스텁, 123/123 통과 — 소셜벤처 69건 포함)
**수미쌍관을 코드로 확인** — `data-domain` 속성 역추적:

| ID | 영역 | 렌더링 섹션 |
|---|---|---|
| C1 | 조합원 기반 | status + mission |
| C2 | 민주적 운영·거버넌스 | status + org |
| C3 | 사업 지속가능성 | status + mission |
| C4 | 재정·원가구조 | status + revenue |
| C5 | 회계·법규 준수 | status + profit |
| C6 | 판로·마케팅 | status + revenue |
| C7 | 제도 활용 | status + system |
| C8 | 디지털·AX | status + org |

**누락 0. 40문항 라벨이 전부 리포트에 등장.**

| 항목 | 결과 |
|---|---|
| 모듈 | 8영역·40문항·weight 1.0 · scale 1~5 전수 · 1점→20 / 3점→60 / 5점→100 ✓ |
| 경고 | 3점 **0건** / 1점 **14건**(CRITICAL 2) ✓ |
| **⚠ 진행률** | 협동조합 40 + 업종 16 = **56** ✓ |
| **⚠ 레이더차트** | 키 `c1~c8` · **0점 영역 없음** · c5 부분 저점 2.8 정확 · 해설 카드 8건 본문 채워짐 ✓ |
| collect | `orgPrompt` = C1~C8 · **S1~S8 미적용** · `orgDiagKeyPrefix` 전달 · 구 필드 병행 ✓ |
| 프롬프트 지침 | 설립 신고 명시 · 1인1표·법정적립금·이용고 배당 포함 · 만료·갱신 문구 0 ✓ |
| ⑦ 섹션 | 만료·갱신 안내 0 · SVI·소셜벤처 판별 등 **타 유형 문구 혼입 0** ✓ |
| **회귀** 3유형 | `social_enterprise`→social / `social_venture`→venture / `cooperative`→coop 컨테이너, **나머지 0문항 · 진행률 전부 56** ✓ |
| **회귀** micro·일반기업·gov-support | 전부 불변 ✓ |
| 구조 | `ORG_TYPE_PENDING` 비어 있음 · `_orgKind` 단일 분기 · dashboard 접두어 하드코딩 0 ✓ |

### ⑦ 캐시버스팅
`index.html` 로컬 `?v=` **51곳** 전부 `20260818a`

### ⑧ 남은 이슈 (갱신)
1. ✅ **협동조합 전용 진단지 — 이번에 완료.** 사회적경제 3유형 전용 진단이 모두 갖춰졌다.
2. **다음 작업 — 구 필드 `socialPrompt`/`socialWarnings` 제거.**
   `orgPrompt`/`orgWarnings`로 대체됐고 회귀 방지를 위해 병행 유지해 왔다.
   **협동조합 작업 완료 후 제거하기로 한 약속이므로 다음 작업에서 정리한다.**
   현재 병행 지점: `wizard.js collect()`(대입) / `dashboard.js _socialWarnings()`(읽기) / `ai-engine.js _buildSocialPrompt`·`_buildSocialWarnBlock`(읽기).
3. **`gov-support`의 `social_economy` 태그가 사회적기업과 협동조합을 함께 묶고 있어,
   사회적기업 인증 기업 전용 사업(사회적기업 육성사업·재정지원사업)이 협동조합에도 상위 매칭된다.**
   실무상 중복 인증이 흔해 완전한 오정보는 아니나, 태그를 `social_enterprise_only` / `cooperative_only` / `social_all`로 세분화하는 것이 정확하다. **회귀 위험이 있어 별도 작업으로 분리.**
4. **`gov-support` `match()`의 동점 처리가 이름 순(`localeCompare`)** — 상위 6개가 사실상 가나다순으로 정해지는 구간이 있다. 2차 정렬 기준 설계 필요.
5. (기존) 사회적경제 AI 결과가 이력 스냅샷에 저장되지 않음 / `claude-analyze-funding.js`의 continuation 미적용 — 변동 없음

---

## 최근 수정 이력 (2026-09-02) — 소셜벤처 전용 진단지 신설 (V1~V8 40문항)

**소셜벤처를 선택하면 사회적기업 진단(S1~S8)이 적용되고 있었다.** 두 유형은 평가 축이 근본적으로 다르다.

### ① 근거 — 기술보증기금 소셜벤처 판별기준 (중소벤처기업부고시)
사회성 판별표 12개 + 혁신성장성 판별표 12개, 각각 70점 이상이어야 판별.
출처: https://sv.kibo.or.kr/Homepage/attach/guide.pdf

S1~S8을 그대로 쓸 수 없는 이유 3가지:
1. **특별법상 지속 자격이 없다.** 사회적기업·협동조합·마을기업과 달리 판별은 자격이 아니라 **지원사업 신청 기준**이다 → 만료·갱신 경고를 넣지 않는다
2. **혁신성장성(기술 혁신성·사업 성장성·R&D 역량·대표자 기술역량)이 평가의 절반**인데 S1~S8에는 이 축이 거의 없다 → V3·V4·V6로 확보
3. **상법상 영리법인이며 벤처투자·기술보증 트랙**을 탄다. 공공조달 중심인 사회적기업과 자금 조달 경로가 다르다 → S3(공공조달·판로) 자리에 **V5(투자유치·자금조달)**

⚠ **판별표 점수(사회성 70 / 혁신성장성 70)를 예측하지 않는다.** SVI를 흉내내지 않기로 한 것과 동일 원칙 — 실제 판별 결과와 달라지면 신뢰 문제가 생긴다. 8영역 균등 배점(0.125)이며 문항 해설은 "판별 신청 전 갖춰야 할 것"을 짚는다.

### ② `js/diagnosis/diagnosis-venture.js` 신설 — `DiagVenture`
**V1 사회문제 정의 / V2 임팩트 측정·검증 / V3 기술·아이디어 혁신성 / V4 사업 성장성·시장 / V5 투자유치·자금조달 / V6 팀·R&D 역량 / V7 판별·제도 활용 / V8 디지털·AX** — 8영역 × 5문항 = 40문항.
점수 키 접두어 `diag-venture-container_`. `detectCrossWarnings` 12규칙(CRITICAL 2 / HIGH 4 / MEDIUM 6).
- `v1_1`(정관 명시)은 판별표 최대 배점 항목이며 **정관 변경은 총회 의결·등기가 필요**해 준비 기간이 든다 — 경고 문구에 반영
- `v3_5`(기술↔사회성 연결)는 판별의 핵심축이라 별도 경고(`tech_social_disconnect`)로 둔다

### ③ ⚠ orgType 분기 단일화 — `_orgDiagModule()`
지금까지 orgType 분기가 여러 곳에 흩어져 반복적으로 사고를 냈다. 모듈이 하나 더 늘어나면서 **단일 진입점**으로 정리했다.
```js
_orgDiagModule(orgType) → DiagVenture | DiagSocial | null   // wizard.js 한 곳
_orgContainerId(mod)    → mod.KEY_PREFIX에서 컨테이너 id 파생 (하드코딩 금지)
```
파생 대상 — **하드코딩을 전부 제거했다**:

| 지점 | 이전 | 이후 |
|---|---|---|
| `loadDiagnosisUI()` | `renderDiagModule('diag-social-container', _diagSocialToAreas(DiagSocial))` | `renderDiagModule(orgContainerId, _diagOrgToAreas(orgMod))` |
| `_calcSocialDomainScores()` | 정규식 `/^diag-social-container_s(\d)_/` **하드코딩** | `_calcOrgDomainScores(scores, mod)` — `mod.KEY_PREFIX`·`mod.DOMAINS`에서 파생 |
| `dashboard.js` 3곳 | `flat['diag-social-container_' + k]` | `flat[PRE + k]`, `PRE = fd.orgDiagKeyPrefix` |
| `DIAG_CONTAINERS`·`_countDiagItems` | venture 누락 | `diag-venture-container` 추가 |

⚠ **뒤 두 곳을 빠뜨리면 소셜벤처에서 레이더차트가 비고 진행률이 틀린다.** 사회적기업 때 정확히 같은 문제를 겪었으므로 검증 항목에 명시적으로 넣었다([3]·[2] 케이스).

### ④ 필드명 일반화 — `orgPrompt` / `orgWarnings`
`socialPrompt`/`socialWarnings`가 이미 3개 파일 6곳에 퍼져 있었고, 세 번째 모듈이 같은 필드를 공유하면 "사회적기업 전용"으로 오독된다.
```js
data.orgPrompt   = data.socialPrompt   = mod.buildPromptSummary(allScores);
data.orgWarnings = data.socialWarnings = mod.detectCrossWarnings(allScores);
// 읽는 쪽: d.orgPrompt || d.socialPrompt
```
**구 필드를 같은 값으로 병행 유지**해 이번 커밋의 회귀 위험을 0으로 만들었다. 한 번에 갈아엎으면 변경이 큰 상태에서 회귀 원인 추적이 어려워진다.

### ⑤ 정부지원사업 — `orgType` 태그 세분화
**문제**: 소셜벤처로 진단해도 사회적경제 사업 5개가 각 +4점으로 상위를 독점해 TIPS 등 벤처 계열이 `slice(0,6)`에서 밀려났다. (TIPS 제외 조건은 이미 `orgType !== 'social_venture'`로 되어 있었으므로 "제외"가 아니라 **점수 독점**이 원인이었다.)

`'social' | 'all'` 2종 → **4종**으로 확장:
| 태그 | 대상 | 소셜벤처 | 사회적기업·협동조합 | 일반 기업 |
|---|---|---|---|---|
| `social_economy` | 사회적기업·협동조합 중심 | +1 | **+4** | 제외 |
| `social_all` | 사회적경제 전체 | **+4** | **+4** | 제외 |
| `venture` | 소셜벤처 적합 | **+4** | 제외 | 제외 |
| `all`/미지정 | 무관 | 0 | 0 | 0 |

신설 2건(`orgType:'venture'`, **금액 수치 없음**): `sv_growth`(소셜벤처 육성사업 — 중소벤처기업부) · `sv_kibo_guarantee`(기술보증기금 소셜벤처 보증)
- ⚠ **"임팩트 투자 매칭"은 추가하지 않았다.** 모태펀드 임팩트 계정은 연도별 출자사업 공고 형태에 가까워 상시 사업으로 실재하는지 확인되지 않았다. 정책자금에서 세운 **"확인되지 않은 것은 넣지 않는다"** 원칙과 같다
- 기존 `tips`·`fintech_support`까지 벤처 계열은 4건

### ⑥ 리포트 — 9섹션 구조 유지, 라벨·내용만 조직 형태별
| 섹션 | 사회적기업 | 소셜벤처 |
|---|---|---|
| revenue | 어디서 돈이 들어오나 (S3·S6) | **자금을 어떻게 조달하나** (V5·V4) |
| system | 제도를 제대로 쓰고 있나 (S7) | **판별과 제도를 활용하고 있나** (V7) |
| mission | 미션과 사업이 맞물리나 (S1·S2) | 사회문제를 제대로 정의했나 (V1·V2) |
| profit | 수익 구조 점검 (S4) | 기술이 무기가 되고 있나 (V3) |
| org | 조직이 버틸 수 있나 (S5·S8) | 팀이 버틸 수 있나 (V6·V8) |

- 핵심 강조 경고도 분기: 사회적기업 `winning_but_losing` / 소셜벤처 `tech_social_disconnect`
- ⑦ 제도 섹션은 소셜벤처 전용 내용 — **"판별은 지속 자격이 아니므로 만료·갱신을 걱정할 필요가 없다"**를 명시하고 신청 전 준비 항목에 집중
- `SOCIAL_WARN_SECTION`에 venture 코드 12개를 합쳤다(코드가 겹치지 않으므로 한 테이블)

### ⑦ 검증 (Node DOM 스텁, 69/69 통과)
**수미쌍관을 코드로 확인** — `data-domain` 속성 역추적:

| ID | 영역 | 렌더링 섹션 |
|---|---|---|
| V1 | 사회문제 정의 | status + mission |
| V2 | 임팩트 측정·검증 | status + mission |
| V3 | 기술·아이디어 혁신성 | status + profit |
| V4 | 사업 성장성·시장 | status + revenue |
| V5 | 투자유치·자금조달 | status + revenue |
| V6 | 팀·R&D 역량 | status + org |
| V7 | 판별·제도 활용 | status + system |
| V8 | 디지털·AX | status + org |

**누락 0. 40문항 라벨이 전부 리포트에 등장.**

| 항목 | 결과 |
|---|---|
| 모듈 | 8영역·40문항·영역당 5문항·weight 1.0 · scale 1~5 전수 · 1점→20 / 3점→60 / 5점→100 ✓ |
| 경고 | 3점 **0건** / 1점 **12건**(CRITICAL 2) ✓ |
| **레이더차트** | 키 `v1~v8` · **0점 영역 없음** · v3 부분 저점 2.8 정확 · 해설 카드 8건 본문 채워짐 ✓ |
| **진행률** | 소셜벤처 40 + 업종 16 = **56** · 탭 라벨 40문항 · 배너에 "준비 중" 없음 ✓ |
| collect | `orgPrompt`/`orgWarnings` 생성 · 구 필드 병행 · `orgDiagKeyPrefix` 전달 · `bizScale` micro 유지 ✓ |
| gov-support | 소셜벤처 상위에 벤처·보증 계열 포함 · 전용 2건 노출 · 사회적경제 독점 해소 · **소셜벤처 TIPS 포함** ✓ |
| **회귀** 사회적기업 | S1~S8 40문항 · 진행률 56 · 목차 라벨 기존 유지 · gov-support 사회적경제 상위 ✓ |
| **회귀** micro | 35문항 · 진행률 51 · 배너 숨김 · social/venture 컨테이너 0 ✓ |
| **회귀** 일반 기업 | gov-support 6건 · 외식업 스마트화 포함 · orgType 미지정도 동일 ✓ |
| 하드코딩 | 신규 2건 금액 표기 0 · 구 `orgType:'social'` 0 · dashboard 접두어 하드코딩 0 ✓ |

### ⑧ 기타
- `js/diagnosis/industry/social_venture.js`(41줄)는 **삭제하지 않고** `industryVarMap`에서만 제외했다
- biz-context 조직 형태 선택에서 소셜벤처의 "준비 중" confirm 제거 (`ORG_TYPE_PENDING`에 협동조합만 남음)
- `index.html`에 `diag-venture-container` + 스크립트 태그 추가, 로컬 `?v=` **50곳** 전부 `20260817c`

### ⑩ 후속 — `orgAffinity` 신설 (소셜벤처에서 TIPS가 상위에 들지 못하던 문제)

**증상**: 소셜벤처 · 관심분야 미선택 시 TIPS가 상위 6개에서 잘리고 "클라우드 서비스 바우처"보다 아래로 밀렸다.

**원인 — 태그가 아니라 점수 축의 소실이었다.**
TIPS는 `orgType` 태그가 없어(`'all'` 취급) 조직 형태 가점이 0이고,
관심분야 미선택이면 3점 축이 통째로 죽어 **업종(+3) + 규모(+1) = 4점**만 남는다.
4점 동점 그룹 3개가 이름 순(`localeCompare('ko')`)으로 갈리면서 TIPS가 마지막이 됐다.
```
글로벌 강소기업 육성사업 → 클라우드 서비스 바우처 → TIPS
```
관심분야를 선택하면(R&D·정책자금·창업성장 모두) TIPS는 **5위**로 정상 진입했다.

**⚠ `orgType: 'venture'`를 붙이면 안 된다 — 실측으로 확인했다.**
`match()`의 게이트가 `if (pOrg !== 'all') { if (!isSocial) return -1; }` 이므로
**일반 기업 결과에서 TIPS가 통째로 사라진다.** TIPS는 기술창업 전반이 대상이지 소셜벤처 전용이 아니다.

**해결 — 필드의 역할을 분리했다**

| 필드 | 의미 | 게이트 관여 |
|---|---|---|
| `orgType` | **자격 제한.** 해당 조직 형태만 신청 가능 | **O** (제외 판정) |
| `orgAffinity` | **적합도 가점.** 자격은 열려 있으나 특정 형태에 더 적합 | **X** (가점 전용, +2) |

`orgAffinity: ['venture']`를 **TIPS·핀테크 혁신펀드 2건에만** 부여했다(전 항목에 붙이면 변별력이 사라진다).
```js
// 게이트 블록 바깥. 여기서 return하지 않는다
if (isVenture && (p.orgAffinity || []).indexOf('venture') !== -1) score += 2;
```
⚠ 제외 판정은 계속 `VC_TRACK_IDS`로 한다. 두 목록이 지금은 같은 사업을 가리키지만 **의미가 다르므로 분리해 둔다** — 적합도가 높다고 해서 반드시 다른 형태에 부적합한 것은 아니다. `orgAffinity`를 게이트에 쓰면 같은 문제가 재발한다(코드에 주석으로 명시).

**검증 — 기준선 대조 (10케이스, 회귀 실패 0)**

| 케이스 | 결과 |
|---|---|
| 일반 기업 (R&D / 미선택 / 외식·디지털 / orgType 미지정 / sme) | **5건 전부 불변** ✓ |
| 사회적기업 (미선택 / R&D) | **불변** · TIPS 제외 유지(`VC_TRACK_IDS`) ✓ |
| 협동조합 | **불변** ✓ |
| 소셜벤처 미선택 | `핀테크 6점 4위` · `TIPS 6점 5위` **진입** · 기술보증기금 **1위 유지** ✓ |
| 소셜벤처 R&D | TIPS·핀테크 10점 상위 유지 ✓ |

소셜벤처 전용 진단 스모크도 69/69 그대로 통과.

### ⑨ 남은 이슈
1. ✅ **협동조합 전용 진단지 — 2026-09-02 완료(DiagCoop, C1~C8).** 예상대로 `_orgDiagModule()` 한 줄 추가로 끝났다.
2. **`socialPrompt`/`socialWarnings`는 `orgPrompt`/`orgWarnings`로 대체됨.** 회귀 방지를 위해 구 필드를 병행 유지 중이며, **협동조합 작업 완료 후 제거 예정.** 새 코드에서는 `orgPrompt`/`orgWarnings`만 사용할 것.
3. **`gov-support` `match()`의 동점 처리가 이름 순(`localeCompare`)이라 사용자에게 보여줄 상위 6개가 사실상 가나다순으로 정해지는 구간이 있다.** 특히 관심분야 미선택 시 3점 축이 통째로 죽어 동점 그룹이 커진다. 이번 `orgAffinity`는 소셜벤처만 보정했고 **모든 경로 공통 문제는 그대로다.** 2차 정렬 기준(예: 기관 신뢰도, 지원 규모, 신청 난이도)을 설계해야 한다.
4. (기존) 사회적경제 AI 결과가 이력 스냅샷에 저장되지 않음 / `claude-analyze-funding.js`의 continuation 미적용 — 변동 없음

---

## 최근 수정 이력 (2026-09-01) — 사회적경제 리포트 2차: 전용 AI 단일 호출 연결 + 속도 개선

### ① ⚠ 원인 정정 — "sme 경로·web_search 사용"이 아니다
초기 진단은 "사회적기업이 sme 경로를 타서 web_search까지 쓴다"였으나 **사실이 아니다.**
`callClaude()`의 분기는 `_isMicro = formData.bizScale === 'micro'`인데
**사회적기업은 `bizScale`이 `micro`**이므로 실제로는 **micro 경로**(`noSearch: true`)를 탔다.
정확한 서술: **micro 경로 · noSearch · 3회 순차 호출(1/2/3차) · 각 `max_tokens` 16000.**
→ web_search는 쓰지 않았지만 **3회 호출·최대 48000토큰**과
**생성 결과(SWOT·STP·4P·D1~D7 처방)를 `renderSocial()`이 전부 버리는** 낭비 구조는 그대로였다.
잘못된 원인이 기록에 남으면 나중에 오판을 부르므로 명시해 둔다.

### ② `api/claude-analyze-social.js` 신설 — 단일 호출
- `runWithContinuation` 적용(절단 시 assistant prefill 이어쓰기 최대 2회). **정적 `require('../lib/claude-stream')`** 여야 `@vercel/nft` 의존성 추적에 포함된다
- `max_tokens: 6000`(경영진단 16000 대비 축소) · **web_search 미사용** · 3분할 없음
- `vercel.json`: `{ "regions": ["icn1"], "maxDuration": 120 }`
- ⚠ `api/claude-analyze-1/2/3.js`는 **미변경**(경영진단이 사용 중)

**프롬프트 입력**: 조직 기본정보 + `d.socialPrompt`(`DiagSocial.buildPromptSummary()` 원문) + `detectCrossWarnings()` 전체 + `GovSupport.buildPromptBlock()`
- ⚠ 점수 블록을 다시 작성하지 않고 **`socialPrompt`를 그대로 쓴다.** 그쪽이 해석 지침("SVI 예상 점수를 추정하지 마라" 등)까지 담고 있어 여기서 다시 쓰면 중복이 된다. `_buildSocialScoreBlock()`은 fallback 전용
- ⚠ 경고 `msg`는 **완성된 판정 문장**이다. "재해석·재판정 금지"를 프롬프트에 명시하고, AI에게는 "어떤 순서로 어떻게 해소할 것인가"만 요구한다

**출력**: `priority[3~5]` (order·action·why·how) / `plan90[3]` (month·focus·tasks) / `cautions[2~4]`

**시스템 프롬프트 제약**: 진단에 없는 문제 생성 금지 · 미응답 정보 서술 금지 · 금액/한도/비율 수치 금지 · **SWOT·STP·4P·린캔버스 등 프레임워크 이름 금지**(사회적경제 조직은 규모상 소기업 수준이라 그 틀이 맞지 않는다) · 단정 표현 금지
+ 사회적기업 고유 맥락 5종: 미션과 수익의 균형 / 공공 발주 의존 위험 / 보조금 종료 대비 / 조직 지속성 / 제도 활용도

### ③ 속도 개선 — diag-reveal 유지 + 백그라운드 호출 (지시 정정)
⚠ **당초 지시는 "renderSocial로 즉시 전환"이었으나 그러면 `diag-reveal`을 건너뛰게 된다.**
거기엔 8영역 레이더차트·도메인 해설·진단유형 카드·KOSIS 생존율이 있고 전부 AI 없이 즉시 계산된다.

채택한 흐름:
```
진단 완료 → (AI 대기 없음) showDiagReveal → show('diag-reveal')
          ↘ 동시에 백그라운드로 callSocialPlan() 시작
[사용자가 레이더차트를 읽는 시간 = AI 생성 시간]
[클릭] → renderSocial → sec-social-action은 이미 done 이거나 로딩 중
```
→ **사용자가 화면을 읽는 시간이 곧 대기 시간**이 되어 체감 대기가 사실상 사라진다.

**경쟁 조건 방어** — `_socialReqId` 카운터:
- 요청마다 `++_socialReqId`로 id를 발급하고, 응답 도착 시 `reqId !== _socialReqId`면 **결과를 버린다**
- `restart()`(새 분석)에서도 `_socialReqId++` → 진행 중이던 응답이 새 화면에 꽂히지 않는다
- **백그라운드 실패를 `diag-reveal`에 띄우지 않는다.** 그 시점의 사용자는 레이더차트를 보고 있다. 에러는 대시보드의 `sec-social-action`에서만 표시한다
- 사용자가 즉시 클릭하든 오래 머물든 동작한다 — 상태(`loading`/`done`/`error`)를 `Dashboard` 모듈에 저장하고 `renderSocialAction()`이 읽는 구조라 진입 시점과 무관하다

### ④ `sec-social-action` 렌더링 — 자리표시자 교체
`Dashboard.renderSocialPlan(state, plan)` 신설. 진단 경고 나열(즉시)은 그대로 두고 하단만 3상태로 바뀐다.
- `loading` → 스피너 + "위 진단 결과는 지금 바로 확인하실 수 있습니다"
- `done` → **먼저 해야 할 일**(번호 카드 · action 굵게 / 왜 / 어떻게) + **90일 실행 계획**(1·2·3개월차 3열 그리드, 골드 강조) + 주의 박스
- `error` → 에러 박스 + `[다시 시도]`(`App.retrySocialPlan()` — **AI만 재호출**, 진단 재계산 없음)
- ⚠ **`fakeAnalysis` 계열 가짜 데이터를 쓰지 않는다.** 진단 결과는 이미 확보돼 있으므로 AI 섹션만 생략한다

### ⑤ 기존 경로 차단
- `App.runAnalysis()`에서 `_isSocialData(data)`면 **`callClaude()`를 호출하지 않고** 전용 경로로 분기 후 `return`
- 판정은 `fd.isSocialOrg` 파생 플래그 사용(문자열 배열 재정의 금지). 플래그가 없는 레거시 데이터는 `orgType !== 'general'` fallback
- `proceedToSolution()`의 `if (!_pendingResult) return` 가드를 사회적경제 경로에서 우회 — AI 결과 없이도 9섹션이 렌더링되어야 한다
- **`ai-engine.js` `buildPrompt1()`의 socialPrompt 주입 분기 제거** — `buildPrompt1`의 유일한 호출자가 `callClaude()`이고 그 경로를 차단했으므로 도달 불가능한 죽은 코드가 됐다. **`d.socialPrompt` 자체는 `_buildSocialPrompt()`의 입력으로 계속 쓴다**

### ⑥ 예상 응답 시간
| 항목 | 이전 (micro 3회) | 이번 (단일) |
|---|---|---|
| API 호출 | 3회 순차 | **1회** |
| max_tokens 상한 | 16000 × 3 = 48000 | **6000** |
| 사용자가 기다리는 구간 | AI 전체 완료까지 | **없음**(진단 화면 먼저) |
- 출력 토큰이 대략 1/8 수준이고 순차 호출이 사라지므로 **AI 자체 소요는 이전의 1/5~1/8 수준**으로 추정된다
- 다만 체감상 더 중요한 것은 ③이다 — **사용자는 AI를 기다리지 않는다.** 레이더차트를 읽고 넘어올 즈음이면 대개 완료돼 있다
- ⚠ 실제 API로 측정하지는 않았다(토큰 소모). 위는 호출 횟수·토큰 상한 기반 추정치다

### ⑦ 검증 (Node DOM + fetch 스텁, 42/42 통과)
| 항목 | 결과 |
|---|---|
| 엔드포인트 | `/api/claude-analyze-social` 호출 · **`claude-analyze-1/2/3` 미호출** ✓ |
| 프롬프트 | 제약 4종·맥락 5종 포함 · 경고 원문 포함 · `socialPrompt` 포함 · 지원사업 블록 포함 · **금액 수치 0건** ✓ |
| 선행 렌더 | `runAnalysis`가 AI를 기다리지 않고 반환 · diag-reveal 활성 · dashboard 숨김 · 레이더차트 렌더됨 ✓ |
| 로딩 중 진입 | 스피너 표시 · **나머지 8섹션은 정상 렌더** ✓ |
| 완료 후 | 자동 갱신 · priority 카드 · 90일 3개월 · cautions · 자리표시자 문구 소멸 ✓ |
| AI 실패 | `sec-social-action`만 에러 + 다시 시도 · **가짜 데이터 없음** · 나머지 7섹션 정상 · **`analysis-error` 전체 화면으로 안 넘어감** ✓ |
| 경쟁 조건 | 낡은 응답이 새 화면에 꽂히지 않음(로딩 상태 유지) ✓ |
| **회귀** | sme·micro 모두 `claude-analyze-1/2` 호출 유지 · `claude-analyze-social` 미호출 ✓ |
| 설정 | `vercel.json` icn1·120초 · **1/2/3차 maxDuration 300 불변** · `claude-analyze-1.js` 미변경 ✓ |

### ⑧ 캐시버스팅
`index.html` 로컬 `?v=` 49곳 전부 `20260817b`

### ⑨ 남은 이슈
1. **사회적경제 AI 결과가 이력에 저장되지 않는다.** `HistoryTracker.save(data, null)`로 호출하므로 `priority`·`plan90`이 스냅샷에 남지 않는다.
   현재 `HistoryTracker`는 **도메인 점수 비교·이슈 태그만 렌더링**하고 전체 리포트를 복원하는 기능이 없으므로 **지금 당장 깨지는 것은 없다.**
   다만 향후 "지난번 진단 결과 다시 보기"를 만들면 사회적경제만 AI 부분이 비게 된다. 그때 스냅샷 스키마에 필드를 추가해야 한다(이번엔 구조 미변경).
2. **`api/claude-analyze-funding.js`도 절단 시 그냥 실패한다.** `runWithContinuation`을 쓰지 않고 `stop_reason === 'max_tokens'`면 에러를 반환할 뿐이다.
   `max_tokens: 6000`이라 정책자금 출력량 기준으로 절단 가능성은 낮지만, 신설 social API와 동일하게 `runWithContinuation`으로 교체하는 것이 일관적이다. **이번에는 수정하지 않았다**(정책자금 경로 무영향 제약).

---

## 최근 수정 이력 (2026-09-01) — 진단 진행률·탭 라벨 문항 수 오류 수정

**`.diag-item`을 DOM 전역에서 세어 미사용 컨테이너(다른 경로용)의 문항까지 합산되던 문제.**
sme 경로에서 실제 36문항이 **76**으로 표시됐다.

### ① 재현 — 76의 정체를 정확히 특정했다
```
사회적기업 진단 → (새 분석) → sme 재진단
  diag-common-container   20문항
  diag-social-container   40문항  ← 이전 경로 잔존
  diag-industry-container 16문항
  ─────────────────────────────
  전역 합계 76  ← 보고값과 일치
```
- `loadDiagnosisUI()`는 미사용 컨테이너를 `classList.add('hidden')`만 하고 **`innerHTML`을 비우지 않았다.** `reset()`도 `diagScores`만 비우고 컨테이너는 손대지 않았다
- **단일 경로만 실행한 세션에서는 원래도 정확했다.** 경로 전환 시에만 터지는 버그이며, 컨테이너가 3개(common/micro/social)로 늘면서 오차가 커졌다
- ⚠ `diagTab-common` 안에 **common / micro / social 3개가 형제로 공존**하는 구조가 원인이다

### ② 수정 — 활성 컨테이너 기준으로 통일
`_activeContainers`(모듈 스코프) 신설. `loadDiagnosisUI()`가 매번 갱신한다.

| 헬퍼 | 역할 |
|---|---|
| `_countDiagItems(ids)` | `#컨테이너 .diag-item:not([data-signal-only])` 스코프 카운트 |
| `_countDoneScores()` | `diagScores` 키를 **활성 접두어로 한정**해 카운트 |
| `_clearInactiveContainers()` | 이번 경로에 없는 컨테이너 `innerHTML=''` |
| `_clearAllDiagContainers()` | `reset()`용 전체 비우기 |

- **분모(DOM)와 분자(diagScores)의 범위를 일치**시켰다. 어긋나면 전 문항을 채워도 100%가 되지 않는다
- `updateDiagProgress()` · `validate(2)` · 탭 라벨 · 배너가 **전부 같은 함수**를 쓴다
- `industryData`가 없는 업종(매핑 미지원)에서도 이전 렌더가 남던 문제를 함께 처리 — 활성 목록에 넣지 않아 자동으로 비워지고 탭 버튼도 숨긴다
- `loadDiagnosisUI()` 말미의 진행률 표시를 `'0 / N'` 고정에서 **`updateDiagProgress()` 호출**로 교체(`restoreScores()` 뒤). 저장된 점수를 갖고 step2에 재진입해도 0으로 보이던 문제가 함께 해소된다

### ③ 하드코딩 문항 수 제거 — 실제 렌더 결과에서 파생
| 위치 | 기존(하드코딩) | 실제 |
|---|---|---|
| 업종 탭 라벨 | `특화 진단 (5문항)` | **16문항** (18개 모듈 전수: 4영역×4문항) |
| 공통 탭 라벨 | `기본 경영 진단 (8문항)` | **20문항** |
| 사회적기업 배너 | `8대 영역 40문항 + 업종 특화 5문항` | 40 + **16** |

- 2026-04-27에 업종 진단을 5문항으로 줄였다가 2026-05-17 v2.0 재작성에서 16문항으로 되돌아갔는데 **라벨만 `(5문항)`으로 남아 있었다**
- `_cntSuffix(n)` — **n이 0이면 숫자를 붙이지 않는다.** 잘못된 숫자를 보여주느니 빼는 편이 낫다

### ④ ⚠ 기준 통일 — signal-only 제외 쪽을 택했다
공통 진단은 화면에 **21개**가 보이지만 그중 **DX 탐지 1문항은 `_signalOnly`**로 점수에 반영되지 않는다(2026-04-30 도입).
**진행률 분모·탭 라벨 모두 `:not([data-signal-only])` 기준(20)으로 통일했다.**
필수가 아닌 문항을 분모에 넣으면 100% 달성이 구조적으로 불가능해지기 때문이다.
→ **sme = 20 + 16 = 36**, micro = 35 + 16 = 51, 사회적기업 = 40 + 16 = 56

### ⑤ 검증 (Node DOM 스텁, 35/35 통과)
| 항목 | 결과 |
|---|---|
| 단일 경로 | sme **36** / micro **51** / 사회적기업 **56** ✓ |
| 경로 전환 (reset 경유) | 사회적기업 56 → reset(컨테이너 전부 빔) → sme **36** (76 아님) ✓ |
| 경로 전환 (reset 없이) | `loadDiagnosisUI` 재호출만으로도 36 · social 컨테이너 자동 정리 ✓ |
| 전 문항 응답 | 세 경로 모두 **정확히 100%** + `validate(2)` 통과 ✓ |
| 탭 라벨 | 공통 20/35/40 · 업종 16 · **라벨 합 = 진행률 분모** ✓ |
| 배너 | 40문항 + 16문항, `5문항` 표기 소멸 ✓ |
| 미지원 업종 | 이전 업종 문항 잔존 0 · 탭 숨김 · 진행률 20 ✓ |
| 잔존 점수 | 사회적기업 10개 응답 후 sme 전환 → **done 0** (미합산) ✓ |
| **회귀** | micro 35문항 `D1_1→D7_5` · social 40문항 `s1_1→s8_5` · 점수 키 접두어 전부 동일 ✓ |

### ⑥ 캐시버스팅
`index.html` 로컬 `?v=` 49곳 전부 `20260817a`

### ⑦ 남은 이슈 — 잔존 `diagScores`가 AI로 전달될 수 있다 (이번엔 손대지 않음)
`done` 계산은 활성 접두어로 한정했지만 **`diagScores` 자체는 정리하지 않았다.**
- 경로가 바뀌는데 `reset()`을 거치지 않는 흐름이 있다: **step1에서 직원 수를 고친 뒤 `analyzeBiz()` 재실행** → biz-context → `startDiagnosis()` → `loadDiagnosisUI()`. 이때 `diagScores`에 이전 경로 점수가 남는다
- `DiagMicro/DiagSocial.calcScores()`·`_calcMicroDomainScores()`는 **접두어로 필터**하므로 안전하다. 그러나 **`CrossContext`는 접두어 무관으로 전 키를 훑으므로** 이전 경로 점수가 교차 경고를 오발동시킬 수 있다
- **판단: 이번엔 삭제하지 않았다.** `loadDiagnosisUI()`는 호출 시점에 DOM 값이 아직 확정되지 않았을 수 있고, 경로 판정이 한 번이라도 어긋나면 **사용자 응답이 되돌릴 수 없이 삭제된다.** 진행률 오류는 표시 문제지만 응답 삭제는 데이터 손실이다
- **다음 수정 시 권장**: `diagScores`를 지우지 말고 **`collectAllScores()`가 활성 접두어만 반환**하도록 좁히는 쪽이 안전하다(데이터는 보존, AI 전달만 차단)

---

## 최근 수정 이력 (2026-08-31) — 사회적경제 전용 리포트 화면 신설 (1차: 9섹션, AI 미연결)

**진단은 S1~S8으로 바뀌었는데 결과 리포트는 중소기업용 13섹션(SWOT·STP·4P·린캔버스·6가지시스템)이 그대로 나왔다.**
40문항을 묻고 **S5(조직·거버넌스)·S7(인증·제도)·S8(디지털) 15문항 결과는 리포트에서 아예 다뤄지지 않았다.**

### ① 설계 원칙 — 수미쌍관
**진단에서 물은 8개 영역은 리포트에서 모두 다뤄져야 한다.** 묻고 안 쓰는 문항은 응답자의 시간을 낭비시킨다.
SWOT·STP·4P·린캔버스는 쓰지 않는다 — 사회적경제 조직은 규모상 소기업 수준이라 그 틀이 맞지 않는다.

### ② 9섹션 — 목차 라벨은 프레임워크 이름 대신 사장님이 읽는 말로
| 섹션 id | 목차 라벨 | 대응 진단 영역 |
|---|---|---|
| `sec-social-summary` | 한눈에 보기 | 총점 + 취약 Top3 + CRITICAL·HIGH 경고 |
| `sec-social-status` | 우리 조직 현재 상태 | **S1~S8 전 영역** |
| `sec-social-mission` | 미션과 사업이 맞물리나 | S1 + S2 |
| `sec-social-revenue` | 어디서 돈이 들어오나 | S3 + S6 |
| `sec-social-profit` | 수익 구조 점검 | S4 |
| `sec-social-org` | 조직이 버틸 수 있나 | S5 + S8 |
| `sec-social-system` | 제도를 제대로 쓰고 있나 | S7 |
| `sec-social-action` | 무엇부터 할 것인가 | 경고 전체 (레벨순) |
| `sec-gov` | 정부지원사업 | **기존 섹션 재사용 — 미변경** |

### ③ 키 체계 — 앱에 8영역 키가 두 벌 있다
- `wizard.js _calcSocialDomainScores()` → **`s1~s8`** (diag-reveal 레이더차트 전용)
- `DiagSocial.calcScores()` → **`mission`·`value_biz`·…** (`domain.key`, `pct`·`total` 포함)

**리포트는 `DiagSocial.calcScores()`(`fd.scaleScores`)를 정본으로 쓴다.** 양쪽 다 고치지 않았다.
⚠ 매핑 테이블을 dashboard.js에 **하드코딩하지 않는다** — `DiagSocial.DOMAINS`가 `{id:'s1', key:'mission'}`을 둘 다 갖고 있으므로 `_socialDomainList()`가 **런타임에 파생**한다. 하드코딩하면 협동조합 전용 모듈(DiagCoop) 추가 시 또 복제해야 하고, 영역이 바뀌면 한쪽만 고쳐 조용히 깨진다. DiagSocial 미로드 시 빈 배열을 반환해 예외는 나지 않는다.

### ④ `isSocialOrg` 파생 플래그 신설 — 배열 3중 복제 방지
`_isSocialOrg()`는 wizard.js 클로저 내부라 dashboard.js에서 접근할 수 없다.
`SOCIAL_ORG_TYPES` 배열을 세 번째로 복제하는 대신 **`collect()`가 `data.isSocialOrg`(boolean)를 실어 보낸다.**
판정 기준은 wizard.js 한 곳에만 둔다.
- dashboard.js `_isSocialFd(fd)`: 플래그가 boolean이면 그대로 사용, 없으면(레거시·이력 스냅샷) **배열 복제 없이 `orgType !== 'general'`** 로 판단
- `SOCIAL_DOMAIN_EXPLAIN`도 복제하지 않고 **`Wizard.SOCIAL_DOMAIN_EXPLAIN`을 export해 읽는다**

### ⑤ 렌더 진입점 — `renderSocial(fd)` 신설
- ⚠ **`render(data, fd, isDemo)`를 재사용하지 않는다.** 그쪽은 `data.executiveSummary`·`swot`·`stp`를 DOM에 직접 밀어넣는 단일 거대 함수라 AI 데이터가 없는 1차에서는 깨진다
- `renderFunding()`과 동일한 **keep 화이트리스트** 패턴 — `SOCIAL_ONLY_SECTIONS + ['sec-gov']`만 표시
- `render()` 진입부에서 `_isSocialFd(fd)`면 `renderSocial(fd)` 호출 후 **early return**
- `render()`에 `SOCIAL_ONLY_SECTIONS` 숨김 블록 추가 (social → general 재진입 시 잔상 방지)
- `buildNav(isMicro, isFunding)` → **`(isMicro, isFunding, isSocial)`** 로 확장, `isSocial`을 맨 앞 분기로. 기존 호출 2곳은 무영향

### ⑥ 섹션별 처리에서 판단이 갈린 지점
- **⑤ 수익 구조** — `winning_but_losing`("수주는 하는데 남는 게 없는" 구조)이 이 화면의 핵심이므로 문항표보다 **앞에 별도 강조 박스**(`.soc-headline-warn`)로 배치
- **⑦ 제도** — ⚠ **인증 만료 경고·갱신 절차 안내를 넣지 않았다.** 실무 확인 결과 갱신을 놓쳐 자격을 잃는 사례가 거의 없어 값이 낮다. 대신 **활용도**에 집중: SVI 측정 여부(`s7_2`) / 자격은 있는데 못 쓰는 제도(`s3_4` 우선구매·`s3_3` 조달등록·`s7_3` 중간지원조직·`s7_5` 지배구조) / 다음 단계 인증 검토(`s7_1` 기준)
- **⑧ 실행** — 1차는 경고를 `CRITICAL → HIGH → MEDIUM` 순 나열만. `AI 실행 계획은 준비 중` 자리표시자를 두었다. AI 우선순위·90일 플랜은 2차
- 경고 code→섹션 매핑(`SOCIAL_WARN_SECTION`)에 없는 code도 **⑧에는 전부 나온다** → 새 규칙이 추가돼도 화면에서 사라지지 않는다

### ⑦ D. 소상공인 목차 라벨 정리 (함께 처리)
`Executive Summary → 한눈에 보기` / `생애주기 진단 → 우리 가게 지금 단계` / `상권 STP · 시장규모 → 우리 동네 손님과 시장` / `7대 영역 처방 → 영역별 처방` / `90일 실행 로드맵 → 90일 실행 계획`.
⚠ **`href`(섹션 id)·순서·내용은 미변경.** sme 목차 13개는 손대지 않았다.

### ⑧ 검증 (Node DOM 스텁, 39/39 통과)
**수미쌍관을 코드로 확인** — `DiagSocial.DOMAINS` 8개가 실제로 어느 섹션에 렌더링되는지 `data-domain` 속성으로 출력:

| ID | 영역 | 렌더링 섹션 |
|---|---|---|
| S1 | 미션·사회적 성과 | status + mission |
| S2 | 사업의 사회가치 지향성 | status + mission |
| S3 | 공공조달·판로 | status + revenue |
| S4 | 재정·원가구조 | status + profit |
| S5 | 조직·거버넌스 | status + org |
| S6 | 마케팅·브랜딩·품질 | status + revenue |
| S7 | 인증·제도·ESG | status + system |
| S8 | 디지털·AX | status + org |

**누락 0. 진단 40문항 라벨이 전부 리포트 HTML에 등장하는 것도 확인했다.**

| 케이스 | 결과 |
|---|---|
| `social_enterprise` | 목차 9개·섹션 9개 · 8섹션 본문 전부 채워짐 · SWOT/린캔버스 숨김 ✓ |
| 전 문항 1점 | 경고 12건 발동 · CRITICAL 2건이 ①과 지정 섹션에 표시 · **msg 원문 그대로** · ①에 MEDIUM 미표시 · ⑧ 레벨순 정렬 ✓ |
| S7 문구 검사 | **만료·갱신 절차 표현 0건** ✓ |
| **회귀** micro | 목차 7개 href·순서 동일, 라벨만 변경 · 사회적경제 8섹션 숨김 · micro 전용 섹션 표시 유지 ✓ |
| **회귀** sme | 목차 13개 **완전 동일** · 라벨 미변경 ✓ |
| **회귀** 정책자금 | 목차 5개 동일 ✓ |
| `isSocialOrg` 부재 | `orgType` fallback 진입 ✓ / **협동조합도 자동 적용** ✓ |
| 중복 검사 | s1↔key 하드코딩 없음 · `SOCIAL_ORG_TYPES` 3중 복제 없음 · `SOCIAL_DOMAIN_EXPLAIN` 복제 없음 ✓ |

### ⑨ 캐시버스팅
`index.html` 로컬 `?v=` 49곳 전부 `20260808b`

### ⑩ 2차 예정 (이번에 만들지 않음)
- `sec-social-action`에 AI 우선순위 과제 + 90일 실행 캘린더 연결
- 전용 프롬프트는 `ai-engine.js`에 별도 분기로 (이번에 `ai-engine.js`·`api/`는 건드리지 않았다)

---

## 최근 수정 이력 (2026-08-16) — 사회적기업 결과 화면이 일반 기업 것으로 나오던 문제 수정

**진단 문항은 S1~S8으로 정상 교체됐으나 결과 화면 3곳이 `orgType`을 전혀 모르고 있었다.**
문항만 바꾸고 결과 화면을 확인하지 않아 생긴 누락이다.

### ① 원인 — 분기 축이 `bizScale` 하나뿐이었다
`showDiagReveal()`의 유일한 분기가 `isMicro = (data.bizScale === 'micro')`인데,
**사회적기업도 `bizScale`은 `micro`**이므로 micro 분기로 빠진다.
`_calcMicroDomainScores()`는 `/^diag-micro-container_(\d)_/`로 찾는데 실제 점수 키는
`diag-social-container_s1_1` 형태라 **하나도 매칭되지 않아 전 영역 0점** → 차트가 비고 해설 카드가 사라진다.

### ② A. 레이더차트 — `_calcSocialDomainScores()` 신설
- 정규식 `/^diag-social-container_s(\d)_/`, 8영역, 반환 형식은 `_calcMicroDomainScores`와 동일(`{key:{label,avg,color}}`)
- **분기 순서: `_isSocialOrg(orgType)` → `bizScale === 'micro'` → 그 외.** ⚠ orgType을 먼저 두지 않으면 영원히 micro로 빠진다. `isMicro`도 `!isSocial &&`로 막았다
- `SOCIAL_DOMAIN_EXPLAIN` 신설 — **키를 `s1`~`s8`로 맞춰야 한다.** `explainMap[key]` 조회 방식이라 키가 어긋나면 해설 카드가 조용히 빈다
- 섹션 타이틀 `📊 사회적기업 8대 영역 프로파일` + "8개 영역은 균등 배점이며 **SVI 예상 점수가 아니다**" 명시
- 진단유형 카드 `🤝 사회적기업 8대 영역 진단` + preview 6건(S1·S3·S4·S5·S6·S7). 보조 유형은 비움(micro와 동일)
- micro D1 미입력 진행 잠금은 `isMicro` 분기 안이라 사회적기업에 적용되지 않음 — 의도된 동작

### ③ B. 정부지원사업 — `orgType` 태그 + 사회적경제 사업 5건 신설
- **기존 PROGRAMS 29개 중 사회적경제 관련 0건**이었다(전수 확인). 409~410행의 `social_enterprise`/`social_venture`는 `INDUSTRY_LABEL` 역매핑일 뿐 사업 항목이 아니다
- 신설 5건(전부 `orgType: 'social'`, **금액 수치 없음**): `se_growth`(사회적기업 육성사업) · `se_fiscal`(재정지원 — 일자리창출·사업개발비) · `coop_support`(협동조합 활성화) · `svf_loan`(사회가치연대기금) · `se_market`(판로지원 — 공공기관 우선구매·e-store36.5)
- `match()` 로직:
  | 조건 | 처리 |
  |---|---|
  | `p.orgType === 'social'` + 사회적경제 기업 | **+4 (최우선)** |
  | `p.orgType === 'social'` + 일반 기업 | **제외**(감점 아님) — 신청 자격이 없는 사업 노출은 오정보다 |
  | 사회적경제 기업 + `VC_TRACK_IDS`(`tips`·`fintech_support`) | 제외. ⚠ **`social_venture`는 제외하지 않는다**(실제 대상이 될 수 있음) |
- `orgType` 미지정(레거시 호출)은 `'general'`로 취급 → 기존 동작 그대로
- 판정 목록(`SOCIAL_ORG_TYPES`)은 wizard.js와 같은 값을 **gov-support.js 내부에 복제**했다 — 모듈 간 의존을 만들지 않기 위함

### ④ C. 동종업계 비교 — 지시와 실제 구조가 달라 분리 처리
"동종업계 비교"에 해당하는 박스가 **2개**이고 성격이 다르다.
- **`#drSurvivalBox`(KOSIS 생존율)** — `data.industryKey` 기준이라 실제로 IT 업종 생존율이 나온다. **(b) 채택**: 유지하되 `ℹ️ 일반 {업종} 기준이며 사회적기업 특성은 반영되지 않았습니다` 안내를 박스 상단에 명시. 통계청 실측치이고 사회적기업도 같은 시장에서 폐업하므로 정보 가치가 있다
- **`#drPatternBox`(PatternDB)** — 지적된 것과 달리 **IT 지표와 비교되고 있지 않았다.** 이미 모든 경로에서 고정값을 표시 중(아래 남은 이슈). 이번엔 **사회적기업 경로에서만 `display:none`** 처리하고 소상공인 경로는 손대지 않았다

### ⑤ 검증 (Node DOM 스텁, 33/33 통과)
| 케이스 | 결과 |
|---|---|
| `social_enterprise` + S1~S8 점수 | 8영역 산출 · 키 `s1~s8` · **0점 영역 없음** · s3 부분 저점 2.8 정확 반영 ✓ |
| 화면 요소 | 타이틀 8대 영역 · 진단유형 사회적기업 · **해설 카드 8건 전부 본문 채워짐** · 점수 바 8건 · PatternDB 숨김 ✓ |
| gov-support (사회적기업) | 상위 5개가 전부 사회적경제 사업 · **TIPS·핀테크 제외** ✓ |
| gov-support (소셜벤처) | **TIPS 포함**(제외 안 함) ✓ |
| **회귀** `general`+`restaurant` | 사회적경제 사업 미노출 · 6건 유지 · 외식업 스마트화 포함 · orgType 미지정도 동일 ✓ |
| **회귀** micro+general | 키 `d1~d7` · 타이틀 7대 영역 · **PatternDB 숨기지 않음** ✓ |
| **회귀** sme | 5대 역량 · 보조 유형 표시 · 5축 ✓ |
| 금액 하드코딩 | 신설 5건에 억원·만원·% 표기 **0건** ✓ |

### ⑥ 캐시버스팅
`index.html` 로컬 `?v=` **49곳** 전부 `20260808a`.
⚠ `js/gov-support.js`는 `?v=` 자체가 없던 파일이라 **이번에 새로 부여**했다(48 → 49곳). `?v=` 없는 파일을 수정하면 반드시 새로 붙여야 캐시가 갱신된다

### ⑦ 남은 이슈 — PatternDB(`#drPatternBox`)가 모든 경로에서 무의미한 값 표시 중
**사회적기업만의 문제가 아니라 소상공인 경로에서도 이미 깨져 있다.** 원인 2가지:
1. `pattern-db.js`가 `domainScores.finance / hr / bm / differentiation` 키를 읽는데
   **micro는 `d1~d7`, 사회적기업은 `s1~s8`** 키를 쓴다. 전부 `undefined`가 되어
   `?.avg || 3` 으로 **기본값 3점(중립) 고정**이 된다
2. `diagData.industry`는 `#industry` select 제거(2026-04-17) 이후 **항상 빈 문자열**이므로
   `INDUSTRY_CONTEXT`가 **`local_service`로 고정**된다. `industryKey`를 써야 한다
→ 결과적으로 **어떤 사용자든 동일한 고정값이 표시된다.**
수정 시 **PatternDB 4축과 D1~D7 / S1~S8의 매핑 설계가 선행**되어야 하며,
리포트 구조 개편 작업과 함께 다루는 것이 적절하다. 임시 매핑은 나중에 다시 뜯어야 한다
- 협동조합·소셜벤처 **전용 진단지**는 여전히 미완. 이번 수정은 전부 `_isSocialOrg()` 기준이므로 전용 모듈이 생기면 자동 적용된다

---

## 최근 수정 이력 (2026-08-16) — 사회적기업 진단이 발동하지 않던 문제 수정 (orgType 분리)

**사회적기업 진단지를 만들어 놓고도 실제로는 한 번도 발동하지 않고 있었다.**
실사용 테스트에서 사회적경제기업이 `knowledge_it`(지식서비스·IT)으로 판별돼 일반 진단(8+5문항)이 그대로 나왔다.

### ① 원인 — `analyze-biz.js`가 `social_enterprise`를 반환할 수 없다
- `_detectOrgType()`은 `industryKey === 'social_enterprise'`일 때만 발동하는데, **`api/analyze-biz.js`에 `social_enterprise`라는 값 자체가 존재하지 않는다**(전수 확인). AI 업종분석이 이 값을 반환할 방법이 없으므로 판별 로직이 **구조적으로 무효**였다
- 근본 판단: **사회적기업은 업종이 아니라 조직 형태다.** 도로시앤컴퍼니는 업종상 컨설팅업이 맞고 사회적기업은 그 위에 얹힌 법적 지위다 → AI가 판별할 것이 아니라 사용자가 직접 선택해야 한다

### ② `orgTypeSelect` 신설 — biz-context 화면
- 위치: `#biz-context-content`(업종 판별 결과) **바로 아래**. `.biz-ctx-orgtype` 블록 + `#orgTypeSelect`
- 선택지 4종: `general`(기본) / `social_enterprise` / `cooperative` / `social_venture`
- ⚠ **step1이 아니라 biz-context에 둔 이유**: step1 필드는 사업자등록증 기재사항이고 OCR 자동입력 대상이다. 법적 지위를 여기 섞으면 OCR 흐름과 충돌한다. 업종 판별 결과를 본 뒤 "그 위에 사회적기업"을 얹는 순서가 자연스럽다
- ⚠ **정책자금 경로에서는 숨긴다** — `showBizContext()` 말미에서 `_purpose === 'funding'`이면 `#orgTypeBlock`을 `display:none`. **biz-context는 정책자금 경로도 거치므로**(startFundingDiagnosis → step1 → analyzeBiz → biz-context → step5) 숨기지 않으면 step5 인증 체크박스와 중복 질문이 된다
- `cooperative`·`social_venture`는 전용 모듈이 없어 **선택 시 confirm** — 확인하면 S1~S8을 적용하되 **orgType 값은 선택값 그대로 유지**(전용 모듈 신설 시 자동 전환), 취소하면 `general`로 원복
- 이벤트는 HTML `onchange`가 아니라 **`DOMContentLoaded` 리스너**로 등록 (2026-06-05 c317c03 캐시 문제 재발 방지)

### ③ `_detectOrgType()` — 사용자 선택 우선
```js
const sel = document.getElementById('orgTypeSelect')?.value || '';
if (sel) return sel;
return industryKey === 'social_enterprise' ? 'social_enterprise' : 'general';  // 레거시 fallback
```
- **`industryKey`는 건드리지 않았다.** 컨설팅업 사회적기업 = `industryKey:'knowledge_it'` + `orgType:'social_enterprise'`가 정확한 상태다. `INDUSTRY_MAP`·업종 판별 로직 미변경
- `SOCIAL_ORG_TYPES = ['social_enterprise','cooperative','social_venture']` + `_isSocialOrg()` 헬퍼 신설

### ④ ⚠ 지시에 없던 충돌 3건 — 함께 수정 (이게 없으면 화면에서만 동작한다)
| 지점 | 문제 | 조치 |
|---|---|---|
| `ai-engine.js:1286` | `d.orgType === 'social_enterprise'` 단일 비교 → **협동조합 선택 시 socialPrompt가 AI에 전달되지 않고 micro 분기로 빠짐** | 3종 포함 조건으로 확장 |
| `wizard.js collect()` | 동일 단일 비교 → `scaleScores`가 DiagSocial이 아닌 **DiagMicro로 계산됨** | `_isSocialOrg()`로 교체 |
| `wizard.js reset()` | `_orgType` 미초기화 → "새 분석" 시 잔상 | `_orgType='general'` + select·배너 초기화 추가 |

### ⑤ 업종 특화 탭 — **a-1(숨김) 폐기, b안(유지) 채택**
- **기존 a-1의 근거가 이번 수정으로 소멸했다.** 과거엔 `industryKey === 'social_enterprise'`였으므로 업종 탭이 렌더링하던 것이 `INDUSTRY_SOCIAL_ENTERPRISE`(9문항, S1~S8과 내용 중복)였다. 이제 industryKey가 실제 업종이므로 업종 탭은 **컨설팅업 특화 5문항**을 렌더링하고 중복이 0이다
- `industryData = isSocial ? null : ...` → 조건 제거, 탭 버튼 `display:none` → 항상 노출
- `_tabOrder()`는 **함수를 남기되 항상 `TAB_ORDER` 반환**(사용처 4곳 미변경). 점수 키가 `diag-industry-container_*`로 분리되어 `DiagSocial.calcScores()`(`diag-social-container_` 접두어만 수집)에 섞이지 않음 — 확인 완료
- 결과: 사회적기업 = **40 + 5 = 45문항**

### ⑥ 진단 유형 배너 — `#diag-type-banner` (step2 상단)
- `🤝 사회적기업 전용 진단 (8대 영역 40문항) + 업종 특화 5문항`
- 협동조합·소셜벤처는 `전용 진단 준비 중 — 사회적기업 진단(공통 항목)으로 진행합니다` 부기
- **일반 기업은 배너를 표시하지 않는다**(기존 화면 그대로)

### ⑦ 검증 (Node DOM 스텁, 32/32 통과)
| 케이스 | 결과 |
|---|---|
| 사회적기업 + `knowledge_it` | `orgType:'social_enterprise'` · **`industryKey:'knowledge_it'` 유지** · `bizScale:'micro'` 유지 · socialPrompt 생성 · scaleScores 8영역 ✓ |
| 협동조합 | `orgType:'cooperative'` **선택값 유지** + socialPrompt 생성(S1~S8 차용) · microPrompt 미생성 ✓ |
| 소셜벤처 | 동일 ✓ |
| **일반 기업 회귀** (`restaurant`/micro) | `orgType:'general'` · socialPrompt 미생성 · **microPrompt 생성** · 배너 숨김 · 탭 레이블 기존 유지 ✓ |
| `orgTypeSelect` DOM 부재 | industryKey fallback 정상 동작 ✓ |
| `loadDiagnosisUI` 렌더 | 예외 없음 · **업종 탭 노출 유지** · 업종 5문항 + S1~S8 동시 렌더 ✓ |
| ai-engine 분기 | cooperative → 주입 / general → 미주입 ✓ |

### ⑧ 캐시버스팅
`index.html` 로컬 `?v=` 48곳 전부 `20260807c`

### ⑨ 남은 이슈
- 협동조합·소셜벤처 **전용 진단지(40문항 수준) 신설**은 여전히 미완. 신설 시 `_isSocialOrg()` 분기에서 `orgType`별로 모듈만 갈아끼우면 된다 (선택값을 보존해 둔 이유)
- `js/diagnosis/industry/social_enterprise.js`(9문항)·`social_venture.js`(41줄)는 `INDUSTRY_MAP['사회적기업']` 경로로만 도달 가능한 상태로 남아 있음

---

## 최근 수정 이력 (2026-08-07) — 사회적기업 전용 진단지 신설 (S1~S8 40문항)

**기존 `social_enterprise.js`는 3영역 9문항뿐이고, 공통 진단(D1~D7 35문항)이 그대로 붙어
사회적기업에 "테이블 회전율"·"재방문율" 같은 무관한 문항이 나오고 있었다.** (실사용 리포트에서 확인)

### ① `js/diagnosis/diagnosis-social.js` 신설 — `DiagSocial`
- **S1 미션·사회적 성과 / S2 사업의 사회가치 지향성 / S3 공공조달·판로 / S4 재정·원가구조 / S5 조직·거버넌스 / S6 마케팅·브랜딩·품질 / S7 인증·제도·ESG / S8 디지털·AX** — 8영역 × 5문항 = **40문항**
- 근거: 한국사회적기업진흥원 **SVI(사회적가치지표) 14개 지표** 프레임 (사회적/경제적/혁신 성과)
- ⚠ **SVI 실제 배점(사회적 60 / 경제적 30 / 혁신 10)을 따르지 않는다.** BizNavi는 SVI 점수를 예측하는 도구가 아니라 **준비도를 진단하는 도구**다. SVI를 흉내내면 진흥원 실제 결과와 달라 신뢰 문제가 생긴다 → **8영역 균등 배점 `weight: 0.125`**
- ⚠ **S3(공공조달·판로)는 SVI에 없는 축**이다. 지역기반 사회적기업 매출의 실체이며, **SVI는 가치 창출은 보되 수익 구조는 보지 않기 때문에** 별도로 둔다
- `buildPromptSummary()`에 해석 지침 명시: "SVI 예상 점수·등급을 추정하지 마라 / 8영역은 균등 배점이므로 SVI 배점에 맞춰 재가중하지 마라"

### ② 스키마는 `diagnosis-micro.js`와 완전 동일
- `DOMAINS: { id, key, label, icon, desc, weight }` / `ITEMS: { label, question, scale[5], ai_trigger:{threshold, warning} }`
- ⚠ **`scale`은 1~5 다섯 단계 전부 작성**했다. `_scaleToAnchors()`가 `anchors[s.score] = s.desc`로 그대로 매핑하므로 **1/3/5만 주면 2점·4점 앵커가 비어 렌더링이 성글어진다**
- 서술은 추상어 대신 구체적 행동으로 작성 (예: `'연 1회 결산 때만 사업별로 나눠보고 개별 수주 판단에는 쓰지 않음'`)
- 점수 키 접두어 **`diag-social-container_`** — micro와 키 충돌 방지. `collectAllScores()`는 `diagScores`의 모든 키를 접두어 무관하게 수집하므로 **수정 불필요**(확인 완료)
- `window` + `module.exports` **듀얼 익스포트**(CrossContext 패턴) — Node 스모크 테스트 가능
- `detectCrossWarnings()` 교차 규칙 **11개** (CRITICAL 2 / HIGH 5 / MEDIUM 4)
  - `public_lock_in`(공공의존+민간판로 없음) · `winning_but_losing`(공헌이익 미산출+공공수주 높음) · `subsidy_cliff`(지원금 의존+BEP 모름) · `mission_drift_risk` · `formal_shell` 등

### ③ `orgType` 플래그 신설 — **`bizScale`은 건드리지 않았다**
- ⚠ `bizScale`에 `'social'`을 추가하지 않았다. **`FundingRules`가 `bizScale === 'micro'` 기준으로 중진공 소상공인 제외(융자제한 9호)를 판정**하므로 값을 바꾸면 그 로직이 깨진다
- `wizard.js` 모듈 스코프에 `let _orgType`, `_detectOrgType(industryKey)` 추가
- `collect()`: **후반부에서 최종 `data.industryKey`(정책자금 override 반영) 기준으로 `data.orgType` 확정**
- 사회적기업이어도 `bizScale`은 `micro`/`sme` 그대로 유지됨 (검증 완료)

### ④ 진단 UI 분기 — C안 **a-1** 채택
- `social_enterprise.js`(9문항)는 **로드하지 않는다.** S1~S8과 내용이 크게 중복(`se_1_1`↔`s1_1`, `se_1_2`↔`s1_3`, `se_1_3`↔`s1_4` 등)
- ⚠ 두 모듈은 **물리적으로 다른 탭**(`diag-industry-container` vs 공통 탭)이라 "중복 문항 제거"(b안)는 두 파일 교차 편집이 필요하고 남는 문항이 3개뿐
- **업종 탭 자체를 숨긴다** — `industryData = null` 처리 + `diagTabBtn-industry`를 `display:none`. 그냥 로드만 끊으면 **탭 버튼은 있는데 내용이 없는 빈 탭**이 된다
- `TAB_ORDER`(const)는 그대로 두고 **`_tabOrder()` 런타임 분기**로 처리 — 사회적기업이면 `['common']`, 그 외는 기존 `['common','industry']`. 사용처 4곳(`goStep`·`prevDiagTab`·`updateDiagTabUI`) 교체
- 탭 레이블: `🤝 사회적기업 8대 영역 (40문항)`

### ⑤ 점수·AI 전달
- `collect()` 후반부: `orgType === 'social_enterprise'`면 `DiagSocial.calcScores()` → **`data.scaleScores`**(micro/sme와 동일한 자리) + `data.socialWarnings` + `data.socialPrompt`
- `ai-engine.js buildPrompt1()`: `d.orgType === 'social_enterprise' && d.socialPrompt`를 **micro/sme 분기보다 앞에** 배치 (규모와 무관하게 S1~S8 사용)
- 대시보드는 신규 렌더링을 만들지 않았다. `scaleScores` 구조가 micro와 동일해 기존 표시 로직이 그대로 동작한다

### ⑥ 검증 (Node, 28/28 통과)
| 항목 | 결과 |
|---|---|
| 모듈 구조 | 8영역 · 40문항 · 영역당 5문항 · weight 0.125 균등(합계 1.0) ✓ |
| scale 전수 검사 | **40문항 전부 1~5 다섯 단계 + 서술 10자 이상** ✓ |
| `calcScores` | 전 문항 1점→20 / 3점→60 / 5점→100. 영역 원점수 5·15·25점(25점 만점) ✓ |
| 교차경고 | 3점 → **0건** / 1점 → **11건**(CRITICAL 2 포함) ✓ |
| 지시 예시 3건 | `public_lock_in`·`winning_but_losing`·`svi_gap` 전부 정확 발동 ✓ |
| `orgType` 판별 | 사회적기업 → `social_enterprise`, **`bizScale`은 `micro` 유지** ✓ |
| **일반 업종 회귀** | `restaurant` → `orgType: 'general'`, `microPrompt` 생성, `scaleScores` **7영역**, `socialPrompt` 미생성 ✓ |

### ⑦ 캐시버스팅
`index.html` 로컬 `?v=` **48곳**(신규 스크립트 포함) 전부 `20260807b`

---

## 최근 수정 이력 (2026-08-07) — 경영진단 1차 max_tokens 절단 수정 + fakeAnalysis 호출 차단

### ⚠ 진단 정정 — 관찰된 에러는 `max_tokens` 감지 경로에서 나온 것이 아니다
사용자가 본 문구는 **"1차 분석 JSON 파싱 실패: {"executiveSummary":"…"**, 즉 API가 **200 OK로 성공 반환**한 뒤 `extractJSON`이 실패한 경우다.
세 API 파일 모두 `stop_reason === 'max_tokens'`를 이미 감지해 에러를 반환하고 있었으므로, **절단이 감지되지 않은 채 잘린 텍스트가 성공으로 반환**된 것이다. 원인 후보 3가지:
1. **스트리밍 중 `stop_reason` 미수신** — `stopReason`은 SSE `message_delta`에서만 채워진다. 그 전에 스트림이 끊기면 `null` → 검사 통과 → 부분 텍스트가 성공 반환 (가장 유력)
2. **SSE `error` 이벤트 무시** — 스트림 중간 오류를 처리하는 코드가 없었다
3. **sme 루프의 마지막 `break`** — `stop_reason`이 4종(end_turn/max_tokens/pause_turn/tool_use) 중 어느 것도 아니면 부분 텍스트를 성공 반환

→ **continuation만으로는 이 증상이 재발한다.** 완성 판정을 `stop_reason`이 아니라 **결과물(JSON 파싱 가능 여부)** 기준으로 바꾼 것이 실질적 수정이다.

### ① `lib/claude-stream.js` 신설 (공용 모듈)
- ⚠ **`api/` 하위에 두지 않는다** — Vercel이 `api/*.js`를 엔드포인트로 인식한다. `vercel.json`의 `functions`가 파일을 명시 열거하는 방식이라 `lib/`는 함수로 잡히지 않으며, 각 함수에서 **정적 `require('../lib/claude-stream')`** 로 참조해야 `@vercel/nft` 의존성 추적에 포함된다(동적 import·경로 조합 금지)
- `diagnoseJson(text)` → `'ok'` / `'truncated'` / `'malformed'` / `'empty'` 4상태
  - **파싱 실패에는 두 종류가 있다**: ① 잘려서 실패 ② 애초에 JSON이 아니라서 실패. ①은 이어쓰기가 답이지만 **②에 이어쓰기를 돌리면 엉뚱한 내용이 덧붙어 상황이 악화된다**
  - 판별: 코드펜스·앞뒤 설명문 제거 후 파싱 → 실패 시 **문자열·이스케이프를 존중하며 괄호 균형**을 센다. 미닫힘/문자열 미종료 → `truncated`, 균형인데 실패 → `malformed`
- `runWithContinuation()` — 스트리밍 호출 + **최대 2회** 이어쓰기. `stop_reason === 'max_tokens'`면 판별 없이 바로 continuation
- SSE `error` 이벤트 처리 추가

### ② 이어쓰기는 assistant prefill 방식
- 누적 텍스트를 **마지막 assistant 메시지**로 넣으면 Claude가 그 메시지를 이어서 쓰므로 **앞부분 반복이 구조적으로 발생하지 않는다**
- (지시받은 "assistant + user 메시지" 방식은 새 assistant 턴을 시작시켜 앞부분을 반복할 수 있어 채택하지 않음. 중복 방지 요구를 더 확실히 만족하는 쪽을 택함)
- 연속 continuation 시 assistant 메시지가 두 번 연달아 들어가지 않도록 **교체**한다
- Anthropic은 prefill 끝 공백을 허용하지 않으므로 `trimEnd`

### ③ ⚠ 검증에서 잡힌 실제 버그 — `stripOverlap` 무조건 적용 금지
- 초기 구현은 겹침을 무조건 제거했는데, **반복되는 정상 텍스트**(같은 문자·문구의 연속)를 겹침으로 오인해 **정상 내용을 최대 200자 잘라내 JSON을 깨뜨렸다** (스모크 케이스 2·3에서 발견)
- **수정**: `joinContinuation()` — 기본은 그냥 잇고(prefill이므로 중복이 원칙적으로 없음), **그 결과가 파싱되지 않을 때만** 겹침 제거본을 후보로 시도
- 불변식은 "바이트 동일"이 아니라 **"유효한 JSON"** 이다. 파싱되는 한 직접 연결을 채택한다(과잉 제거로 인한 조용한 내용 손실이 더 나쁘다)

### ④ 적용 범위
| 경로 | 처리 |
|---|---|
| 1차 micro / 2차 micro / 3차 | `runWithContinuation()` 사용 |
| **1차 sme (web_search tool_use 루프)** | 루프 내 개별 적용. **continuation은 tool_use turn과 별도로 최대 2회**, 전체 turn 합계 상한 `MAX_TOTAL_TURNS = 8` |
| 2차 sme (단일 요청) | 단일 요청 + 최대 2회 이어쓰기로 재작성 |
| **정책자금(`claude-analyze-funding.js`)** | **미변경** — 이미 실패 처리가 되어 있음 |
- 세 경로 모두 **반환 직전 `diagnoseJson()` 최종 검사** 추가 → `stop_reason`을 못 받아도 절단이면 에러로 잡힌다
- `max_tokens` 값 자체는 올리지 않았다 (비용·응답시간 증가, 근본 해결 아님)

### ⑤ fakeAnalysis 호출 경로 차단 (수정 2 — 더 중요)
- **실제 분석이 실패했는데 그럴듯한 가짜 보고서가 나오면 사용자가 자기 회사 분석으로 오인한다.** 정책자금에서 이미 제거한 방식을 경영진단에도 적용
- `app.js` catch: `alert` + `AIEngine.fakeAnalysis()` 제거 → **`analysis-error` 전용 화면**
  - 원인별 문구 변환: `max_tokens` 계열 → "분석 내용이 길어 생성이 중단되었습니다" / 네트워크·타임아웃 → "서버 응답이 지연되었습니다" / 그 외 → "일시적인 오류가 발생했습니다"
  - **원본 에러 메시지(JSON 조각 포함)는 `console.error`로만** — 화면에 노출하지 않는다
  - `[다시 시도]`(같은 입력으로 재분석) + `[입력 수정]`(위저드 STEP 4 복귀)
- ⚠ `fakeAnalysis` 함수 자체와 `isDemo` 배지 로직은 **삭제하지 않았다** (호출 경로만 차단)

### ⑥ 검증 (Node, Anthropic SSE 모의 — 16/16 통과)
`max_tokens`를 문자 수로 강제 제한하는 가짜 Claude로 절단을 재현했다.

| continuation 7건 | 결과 |
|---|---|
| 절단 없음 | 1회로 완성 ✓ |
| 1회 절단 | continuation 1회로 복구, **중복 0자** ✓ |
| 2회 절단 | continuation 2회로 복구, **중복 0자** ✓ |
| 3회 필요 | 상한(2회) 초과 → 실패 반환 + `[ERROR]` 로그 ✓ |
| 꼬리 반복 모델 | 결과가 유효한 JSON ✓ |
| 형식 오류(표 반환) | **continuation 생략**, 즉시 실패 ✓ |
| JSON 없음 | **continuation 생략** ✓ |

| `diagnoseJson` 9건 | 결과 |
|---|---|
| 완전한 JSON / 코드펜스 / 앞 설명문 / trailing comma | 전부 `ok` ✓ |
| 중괄호 미닫힘 / 문자열 미종료 / 배열 미닫힘 | 전부 `truncated` ✓ |
| 괄호 균형인데 파싱 실패 | `malformed` ✓ |
| JSON 아님 | `empty` ✓ |

### ⑦ 남은 이슈
- **협동조합·소셜벤처 전용 진단지도 동일 수준(40문항)으로 신설 예정.** `social_venture.js`는 현재 **41줄 부실 모듈** 상태이며, 협동조합은 전용 모듈 자체가 없다. 사회적기업과 마찬가지로 `orgType` 분기를 확장하는 방식이 될 것
- **`fakeAnalysis` / `_fakeByConsultingType` / `_fakeSpecialized` 약 1,530줄이 호출 경로 차단 후 죽은 코드가 됨.** `isDemo`는 `render(data, fd, isDemo)` 시그니처에 남아 있어 함께 정리 필요. **별도 커밋으로 진행 예정** (지금 섞으면 문제 발생 시 continuation 버그인지 삭제 부작용인지 구분이 안 됨)
- 실제 Anthropic API로 절단을 재현하는 테스트는 하지 않았다(토큰 소모). 재현하려면 `MAX_TOKENS_DEFAULT`를 임시로 500 등으로 낮추고 `vercel dev`로 1회 실행하면 된다

### ⑧ 캐시버스팅
`index.html` 로컬 `?v=` 47곳 전부 `20260807a`

---

## 최근 수정 이력 (2026-08-06) — ⚠ 제조업 영위 문항 오진 수정 (실사용 오진 발생)

**실제 산출물에서 음식점업 사업자가 '직접 영위함'을 선택해 중진공이 "제조업 예외로 대상 포함"으로 잘못 판정됐다.**
신청 자격이 없는 사업자에게 중진공 신청을 안내하는 오진.

### ① 원인 — 문항이 주관적 해석을 유발
- 기존 문구 "제조·가공 공정을 직접 영위하십니까?" → 음식점은 조리·가공을 하므로 '예'가 자연스러운 답
- 그러나 정책자금의 제조업은 **표준산업분류 C(10~34)** 이며, 음식점업은 **I(56) 숙박·음식점업**으로 제조업이 아님

### ② 수정 1 — 판단 기준을 '행위'에서 '서류 기재'로 변경
- 라벨: **"사업자등록증의 업태에 '제조업'이 기재되어 있습니까?"**
- hint: 표준산업분류 C(10~34) 기준임을 명시 + "음식점·카페의 조리, 미용실 시술, 건설 시공 등은 제조업에 해당하지 않습니다"
- 선택지: `업태에 제조업 기재됨` / `기재되어 있지 않음` / `모름` (value는 `yes`/`no`/`unknown` 유지 — 판정 로직 호환)
- **핵심**: '공정을 하느냐'는 주관적 해석이 개입하지만, '서류에 뭐라고 적혀 있느냐'는 확인 가능한 사실이다

### ③ 수정 2 — 명백한 비제조 업종의 '제조업' 응답에 재확인 경고
- `KOSMES_NON_MFG_INDUSTRIES` 신설 (9개 업종 + 한국어 라벨):
  `restaurant` · `local_service` · `wholesale` · `medical` · `finance` · `education` · `media` · `logistics` · `construction`
  - ⚠ `gov-support.js`의 `INDUSTRY_LABEL`에 의존하지 않도록 **이 코드 경로가 닿는 9개 라벨만 모듈 내부에 보유**(자립성 유지)
- 해당 업종 + `isManufacturing === 'yes'` → 통과시키되 재확인 유도:
  ```
  eligible: true, eligibilityUncertain: true,
  exceptionBy: '제조업 영위 (재확인 필요)', exceptionLabel: '제조업 예외 (확인 필요)',
  warning: '업종이 {라벨}(으)로 판별되었는데 제조업 영위로 응답하셨습니다. …'
  ```
- **`eligible`을 `false`로 바꾸지 않는다** — 실제로 음식점을 하면서 별도로 제조업을 등록한 경우가 있을 수 있으므로 단정하지 않고 재확인을 강하게 유도
- `evaluate()`의 agency 객체에 `warning` 필드 추가

### ④ 화면 — 경고 박스 + 주황 배지
- `dashboard.js`: `a.warning`이 있으면 기관 카드 **상단에 `.fa-warning` 경고 박스**
- 배지는 `.fv-exception`(파랑) 대신 **`.fv-exception-warn`(주황)** 으로 구분

### ⑤ AI 프롬프트 반영
- `_SYSTEM_FUNDING` 제약 추가: "기관에 `[⚠ 재확인 필요]` 경고가 붙어 있으면 그 확인 과제를 `priority`의 1번으로 배치하고, 확인이 끝나지 않은 상태에서 그 기관 신청을 전제한 계획을 세우지 마라"
- `_buildVerdictBlock()`이 `warning`을 `[⚠ 재확인 필요]` 라인으로 프롬프트에 주입

### ⑥ 검증 (Node, 지시 3케이스 + 회귀 6건)
| 케이스 | 결과 |
|---|---|
| `restaurant` + `yes` | `eligible: true` · `uncertain: true` · **warning 있음** ✓ |
| `mfg_parts` + `yes` | `eligible: true` · `uncertain: false` · warning 없음 ✓ |
| `restaurant` + `no` | `eligible: false` (기존과 동일) ✓ |
| `fashion` + `yes` | 비제조 목록에 없어 정상 예외 통과 ✓ |
| `fashion` / `mfg_parts` + `unknown` | 기존 업종 기반 로직 그대로 ✓ |
| `construction` + `yes` | 경고 발동 ✓ |
| `sme` 규모 | 게이트 자체를 안 타므로 경고 없음 ✓ |

### ⑦ 후속 수정 (해결됨) — `certs` 검사를 `isManufacturing` 분기보다 앞으로
- **문제**: `restaurant` + `제조업 기재됨` + 사회적경제 인증 조합에서 불필요한 제조업 재확인 경고가 뜨고, `exceptionLabel`도 `'제조업 예외'`로 잘못 표시됐다
- **판단 근거**: 비제조 경고의 목적은 *"자격이 없는데 있다고 오해해 헛수고하는 것"* 을 막는 것이다. **사회적경제 인증으로 이미 자격이 확정된 기업에는 그 위험이 존재하지 않는다.** 경고는 불안만 준다
- **수정**: `_kosmesEligibility` 분기 순서를 `bizScale → **certs** → isManufacturing('yes'/'no') → unknown` 으로 변경. ②③의 순서만 교체하고 나머지 로직은 미변경
  - certs 분기는 `isManufacturing` 값과 무관하게 **확정 반환**하며 `warning`을 붙이지 않는다
- **검증 (11/11 통과)**

  | 케이스 | 결과 |
  |---|---|
  | `restaurant`+`yes`+협동조합 | `사회적경제 예외` · uncertain false · **warning 없음** ✓ |
  | `restaurant`+`yes`+인증 없음 | `제조업 예외 (확인 필요)` · warning 있음 (기존 유지) ✓ |
  | `mfg_parts`+`yes`+인증 없음 | `제조업 예외` · warning 없음 (기존 유지) ✓ |
  | **`restaurant`+`no`+협동조합** | `사회적경제 예외` — **인증 예외가 먼저 걸림** ✓ |
  | `restaurant`+`no`+인증 없음 | `eligible: false` (기존 유지) ✓ |
  | 회귀 6건 (`unknown` 경로·업종 충돌 안내·`sme` 게이트 미진입) | 전부 기존 동작 유지 ✓ |

### ⑧ 캐시버스팅
`index.html` 로컬 `?v=` 47곳 전부 `20260806a` → ⑦ 수정으로 `20260806b`

---

## 최근 수정 이력 (2026-08-06) — 정책자금 진단 5단계 2차: AI 실행 로드맵 연결

**판정 결과를 먼저 렌더링한 뒤 AI를 호출하므로 AI 실패 시에도 판정은 온전히 표시된다.**
`exceptionBy` 배지 추가로 중진공 예외 적용 사유를 화면에 노출.

### ① `api/claude-analyze-funding.js` 신설 (단일 호출)
- 정책자금은 출력량이 적어 `claude-analyze-1/2/3` 3분할이 불필요 → **단일 호출**
- `stream: true`, `max_tokens: 6000`, `res.writeHead(200)` 즉시 전송(CDN TTFB 타임아웃 방지, b681af2 패턴 동일)
- `vercel.json`: `{ "regions": ["icn1"], "maxDuration": 120 }`
- **기존 `claude-analyze-1/2/3.js`는 미변경**

### ② `api/claude-analyze.js` 삭제 — 죽은 파일 정리
- 저장소 전수 검사 결과 **런타임 참조 0건** (자기 파일 헤더 · `vercel.json` · CLAUDE.md/스킬 문서 기록뿐, JS·HTML 참조 없음)
- 파일 삭제 + `vercel.json`의 `maxDuration: 300` 항목 제거
- ⚠ 이 파일을 재활용하지 않고 신규 생성한 이유: `max_tokens: 8000`, 경영진단용 프롬프트 구조라 용도 불일치

### ③ ai-engine.js — 별도 함수 `callFundingRoadmap()` 신설 (기존 로직 미변경)
**내부 분기가 아니라 별도 함수로 간 이유 3가지**:
1. `callClaude()`는 1차→2차(→3차) 순차 호출 + `FIRST_PASS_KEYS` 보호 병합이 한 몸이라, 내부 분기 시 early-return이 흐름 앞에 끼어 함수가 두 관심사를 갖게 됨
2. `apiCall()`의 엔드포인트가 `_callLabel` 삼항이라 라벨 추가 시 **기존 삼항을 수정**해야 함
3. 호출부(`runAnalysis` vs `checkFundingInput`)와 실패 처리 요구가 이미 완전히 분리돼 있음
- `_SYSTEM_FUNDING` — 제약 6종 명시: 단정 표현 금지 / 예상 승인액·금리·한도 제시 금지 / 판정에 없는 결격 사유 생성 금지 / `unknownItems`는 "확인 필요"로만 / 신보·기보 언급 금지 / 미응답 정보를 있는 것처럼 서술 금지
- `_buildVerdictBlock()` — 판정 결과를 텍스트로 직렬화. **`findings`의 `message`는 이미 근거 조항이 포함된 완성 문장이므로 AI가 재해석·재판정하지 않도록 "재판정 금지"를 프롬프트에 명시**
- `_buildFundingPrompt()` — 기업 기본 정보 + 판정 결과 + `GovSupport.buildPromptBlock()`
  - **업종은 한국어 라벨로 전달** (영문 키를 그대로 넣으면 AI가 업종을 인식하지 못함) → `gov-support.js`의 `INDUSTRY_LABEL`을 export해 사용
- `_extractJSONFunding()` — `callClaude` 내부 `extractJSON`과 동일 로직을 복제(기존 중첩 함수는 그대로 둠)
- 출력: `{ situation, priority[3~5], prepare90[3], cautions[] }`

### ④ 화면 — `sec-funding-roadmap` 신설 + 섹션 순서 정정
- ⚠ **1차의 실수 수정**: 정책자금 섹션을 `sec-gov` **뒤**에 넣어 화면 순서가 `지원사업 매칭 → 판정 요약 → …` 로 목차와 어긋나 있었음
- `sec-gov` **앞**으로 이동 + 로드맵 섹션 추가 → 최종 순서 **판정 요약 → 기관별 상세 → 실행 로드맵 → 준비 서류 → 지원사업 매칭**
- `sec-gov` 자체는 이동하지 않아 **경영진단 경로 섹션 순서 불변**(정책자금 4섹션은 general 모드에서 `display:none`)
- `renderFundingRoadmap(state, roadmap)` — `'loading'` / `'error'` / `'done'` 3상태
- `buildNav` funding 분기에 로드맵 링크 추가, `FUNDING_ONLY_SECTIONS`·`allSecIds` 확장

### ⑤ `exceptionBy` 배지화 (1차 보완)
- ⚠ **정정**: `exceptionBy`는 이미 `.fa-exception` 초록 한 줄로 **렌더링되고 있었다.** "계산은 되지만 표시되지 않는다"는 전제는 사실이 아니었음 → 실제 작업은 위치·형태 변경
- `_kosmesEligibility`에 **`exceptionLabel`** 추가 (배지용 짧은 라벨). `exceptionBy` 전문은 유지 — 프롬프트·로그에서 정확한 표현이 필요
  | `exceptionBy` | `exceptionLabel` |
  |---|---|
  | `제조업 영위` / `제조업 영위 예외 (업종 기준)` | `제조업 예외` |
  | `(예비)사회적기업·협동조합·마을기업·소셜벤처 예외` | `사회적경제 예외` |
- 기관명 옆 배지로 `"제조업 예외로 대상 포함"` 표시. **정보성이므로 파랑 계열**(`.fv-exception`)로 verdict 배지와 시각적 위계 구분, `title` 속성에 전문 노출
- 기존 `.fa-exception` 줄 제거

### ⑥ 실패 처리 — 이번 작업의 핵심
1. `validate(5)` → `collect()` → **판정 화면 먼저 렌더링**(AI 응답을 기다리지 않음) → `show('dashboard')`
2. 그 다음 `_loadFundingRoadmap(data)` 호출, 로드맵 섹션은 로딩 스피너
3. 성공 → `renderFundingRoadmap('done', roadmap)`
4. **실패 → 로드맵 섹션에만** "AI 실행 로드맵 생성에 실패했습니다. 위 판정 결과는 정상적으로 확인하실 수 있습니다." + **[다시 시도]** 버튼(`App.retryFundingRoadmap()` — AI만 재호출, 판정 재계산 없음)
- **가짜 데이터(fakeAnalysis 계열)를 사용하지 않는다.** 전체 화면이 에러로 대체되지 않는다

### ⑦ 검증 (Node, fetch 스텁)
| 항목 | 결과 |
|---|---|
| 섹션 DOM 순서 | 정책자금 4섹션이 `sec-gov` 앞 ✓ / 전부 기본 숨김 ✓ / **경영진단 경로 15섹션 순서 불변** ✓ |
| `exceptionLabel` 3종 | 제조업 응답·사회적경제 인증·업종 기준 전부 정확 매핑 ✓ |
| 엔드포인트 | `/api/claude-analyze-funding` ✓ |
| JSON 파싱 | 코드펜스 + trailing comma 응답도 파싱 ✓ |
| userPrompt | 판정 findings·재판정 금지·GovSupport 블록·부채비율·규모 전부 포함 ✓ / **업종 = "외식 및 휴게음식업"(한국어)** ✓ |
| systemPrompt | 제약 5종 문구 전부 포함 ✓ |
| API 실패 | `throw` 전파 확인 → 호출부에서 로드맵 섹션만 error 처리 ✓ |

### ⑧ 캐시버스팅
`index.html` 로컬 `?v=` 47곳 전부 `20260802a`

---

## 최근 수정 이력 (2026-08-02) — 정책자금 진단 5단계 1차: 전용 결과 화면

**판정 결과를 화면에 출력.** AI 프롬프트(B)는 화면 검증 후 2차로 분리 진행.
`clear` 항목을 접힘 상태로 함께 표시해 남은 과제 수를 명확히 한다.

### ① `_INCLUDE_CLEAR_FINDINGS = true` 전환 + clear 문구 8건
- **전환 이유**: 빨간 경고만 잔뜩 보이면 사용자가 "나는 안 되나 보다" 하고 포기한다. 통과 항목을 함께 보여주면 남은 과제가 몇 개인지 명확해져 행동으로 이어진다
- **판정 로직 영향 없음(사전 확인 완료)**: `_verdictOf()`는 `some(blocked)`/`some(conditional||unknown)` 기반, `blockedCount` 등은 status 필터, `unknownItems`는 unknown만 수집 → 전부 무영향. 유일한 변화는 `findings` 배열에 clear 항목이 추가되는 것
- `message.clear`가 없던 8개 규칙에 문구 추가. **작성 원칙: 사용자 응답을 그대로 확인해주는 형태**
  - 좋은 예 `'국세·지방세 체납이 없다고 응답하셨습니다.'` / 나쁜 예 `'체납 사실이 확인되지 않았습니다.'`(앱이 조회한 것처럼 읽힘)
  - clear여도 `source`·`sourceUrl`은 동일하게 표시
- 휴폐업 규칙 2건의 `severity`에 `clear → 'low'` 분기 추가 (통과 항목이 노란 막대로 보이던 문제)

### ② `kind: 'verdict' | 'reference'` 필드 도입
- `semas_closure_history`만 `'reference'`. **통과/결격 집계에서 제외** — 결격 판정 항목이 아닌 참고 항목이 섞이면 "점검 N개"의 의미가 흐려진다
- 화면에서는 기관 카드 하단 **"참고 사항"** 영역으로 분리 표시
- 결과: 소진공 findings 7건 = **점검 6건(verdict) + 참고 1건(reference)**

### ③ `[앱에서 자동 판정 불가]` 표기 + severity `low` 고정
- 대상 3개: `semas_industry` · `kosmes_industry` · `kosmes_debt_ratio` (구조상 절대 `clear`를 반환하지 않는 규칙)
- 사용자가 "왜 항상 확인 필요지?"라고 느끼는 원인은 **앱이 판정을 못 하는 항목과 실제로 문제가 있는 항목이 구분되지 않아서**다 → 메시지 문두에 명시
- 실제 결격 가능성이 아니라 확인 안내이므로 `severity: 'low'` **고정** (`restrictedBiz === 'yes'`여도 low 유지 — 이전에는 high로 올렸음)

### ④ 화면 — 전용 3섹션 + sec-gov 재사용
- `index.html`: `#sec-funding-summary` · `#sec-funding-agency` · `#sec-funding-docs` 추가 (기본 `display:none`)
- `buildNav(isMicro, isFunding)`: **세 번째 분기만 추가**, 기존 micro/sme 삼항 미변경. funding은 4링크(판정 요약·기관별 상세·준비 서류·지원사업 매칭)
- **`Dashboard.renderFunding(fd)` 별도 진입점 신설** — `render(data, fd, isDemo)`는 첫 인자로 AI 결과를 기대하므로 재사용하지 않는다. 억지 재사용으로 경영진단 경로를 깨뜨리지 않기 위함
- `render()`에 정책자금 3섹션을 항상 숨기는 코드 추가 (funding → general 재진입 시 잔상 방지)
- `initScrollReveal()`의 `allSecIds`에 3섹션 추가 (display 필터가 있어 general 모드 무영향)
- **판정 요약**: 한 줄 헤드라인(신청 가능/어려움/해당 없음) + 기관별 `점검 6개 · 통과 5 · 확인 필요 1` **분모 명시** + `unknownItems` 강조 박스
- **기관별 상세**: verdict 배지(결격/확인 필요/결격 없음/대상 아님 + "확인 시 대상 가능"), severity별 좌측 색 막대(high 빨강·medium 노랑·low 회색), **항목마다 근거 `source`+`sourceUrl` 표시(필수)**, `remedy` 별도 줄, clear는 `<details>` 접힘("통과한 항목 N개 보기"), reference는 "참고 사항" 영역
  - `eligible: false`면 `notEligibleReason`만 표시하고 findings 미렌더
  - 카드 하단 제도상 한도 안내 — **구체 금액 수치 없음**, 기관 URL 링크
- **준비 서류**: 기관 공통 6종 + 응답 기반 추가(체납 yes → 징수유예 서류 / certs 보유 → 인증서 강조 / isManufacturing yes → 공장등록증) + 하단 고지

### ⑤ 버튼 연결
`App.checkFundingInput()` → `validate(5)` → `collect()` → **`Dashboard.renderFunding(data)` + `show('dashboard')`**.
정책자금은 진단 점수·레이더차트가 없으므로 **diag-reveal을 거치지 않는다.** `fundingVerdict`가 없으면 alert 후 중단(가짜 데이터 미사용).

### ⑥ 검증 (Node)
| 항목 | 결과 |
|---|---|
| 소진공 집계 | findings 7건 = verdict 6 + reference 1 → "점검 6개 · 통과 5 · 확인 필요 1" ✓ |
| 빈 메시지 전수 검사 | micro/sme × none/yes/unknown 전 조합 → **빈 메시지 0건** ✓ |
| 자동 판정 불가 3규칙 | 전부 `severity: low` + 메시지 문두에 표기 ✓ |
| `restrictedBiz: 'yes'` | `semas_industry`·`kosmes_industry` 여전히 `low` 고정 ✓ |
| verdict·카운트 회귀 | clear 포함 전후 `verdict`/`blockedCount`/`unknownItems`/`recommendedOrder` 전부 동일 ✓ |

### ⑦ 캐시버스팅
`index.html` 로컬 `?v=` 47곳 전부 `20260801d`

### 2차 예정 (B — AI 실행 로드맵)
- `ai-engine.js`에 `d.purpose === 'funding'` 전용 프롬프트 분기 (단일 호출, 3분할 API 미사용)
- 출력: `situation` / `priority[]` / `prepare90[]` / `cautions[]`
- 시스템 프롬프트 제약: 단정 표현 금지, 예상 승인액·금리 제시 금지, 판정에 없는 결격 사유 생성 금지, unknownItems는 "확인 필요"로만 언급, 지원 한도 금액 언급 금지
- **AI 실패 시 판정 결과는 정상 표시하고 AI 섹션만 생략** + "AI 실행 로드맵 생성에 실패했습니다. 판정 결과는 아래에서 확인하실 수 있습니다. [다시 시도]"

---

## 최근 수정 이력 (2026-08-01) — 정책자금 진단 2단계-보완: step5 정밀 입력 필드 추가

**데이터로 확정 가능한 항목을 '확인 필요(conditional)'로 넘기지 않는다.**
4단계 FundingRules 구현 결과 데이터 부족으로 확정 판정이 불가능했던 3건을, 사용자에게 직접 물어 확정 판정으로 전환.
`employees` 구간 문자열의 5인/10인 경계 문제와 `fashion`·`agri_food`의 제조업 판별 모호성을 사용자 직접 응답으로 해소.

### ① index.html — step5 최상단에 '🧭 기본 정보 확인' 섹션 신설 (전부 선택 입력)
- **업종 확인·수정**: `#fundIndustryDetected`(AI 판별 결과 한국어 표시) + `#fundIndustryOverride` select
  - **19개 라벨** 전부 제공 (`INDUSTRY_MAP` 기본 17개 + 사회적기업·소셜벤처). `'소셜벤쳐'` 오타 alias만 제외
  - 사회적기업·협동조합·소셜벤처는 **중진공 융자제한 9호 예외와 직결**되므로 선택지에서 빠지면 예외 적용 기회를 놓친다 → hint로 명시
- **`fundEmployeeCount`**: 상시근로자 수 숫자 직접 입력. **`0`은 유효값(대표자 1인 사업장), 미입력은 `null`** — 반드시 구분
- **`fundIsManufacturing`**: 제조·가공 공정 직접 영위 여부 (`yes`/`no`/`unknown`)
- **`fundCurrentStatus`**: 현재 정상 영업 여부 (`active`/`closed`/`unknown`)
- 기존 `fundClosureHist`는 **삭제하지 않고** 라벨을 "(참고용)"으로 변경 + "현재 상태는 '기본 정보 확인' 응답이 판정에 사용됩니다" 안내 추가
- ⚠ step1의 `employees`(구간 문자열)는 **미변경** — 경영진단 경로가 계속 사용

### ② wizard.js
- `INDUSTRY_LABEL_BY_KEY` — `INDUSTRY_MAP` 역매핑 (먼저 등록된 라벨 우선 → alias 자동 제외)
- `updateFundIndustryDisplay()` — `goStep(5)` 진입 시 호출. `aiIndustryKey`가 비면 "업종이 판별되지 않았습니다. 직접 선택해 주세요."
- `collect().fundingData` += `employeeCount`(number|null) · `isManufacturing` · `currentStatus`
- **`industryKey` override**: `_purpose === 'funding'` 일 때만 `fundIndustryOverride` 우선 적용. **경영진단 경로 업종 판별에는 영향 없음**
- 신규 3필드는 전부 선택 입력 — `validate(5)` 필수 검사(결격요건 7문항)는 그대로

### ③ funding-rules.js — 3개 규칙을 확정 판정으로 전환
- **`semas_scale`**: `employeeCount`가 숫자면 **확정 판정**(`clear`/`blocked`), 없으면 기존 구간 문자열 로직으로 fallback
  - 기준: 제조·건설·운수·광업 10인 미만 / 그 외 5인 미만 (`_semasLimit()`)
  - 메시지에 "직전 사업연도 평균 인원 기준이며, 신청 시점 4대보험 가입자 수도 함께 검토됩니다" 참고 문구 병기
- **`_kosmesEligibility`**: 판정 우선순위 재정렬
  1. `isManufacturing === 'yes'` → **industryKey와 무관하게** `eligible: true` (`exceptionBy: '제조업 영위'`)
  2. 사회적경제 인증 → `eligible: true`
  3. `isManufacturing === 'no'` → 제조업 예외 미적용. `industryKey`가 제조업이면 "업종은 제조업으로 판별되었으나 직접 영위하지 않는다고 응답 — 응답이 정확한지 확인" 덧붙임
  4. `'unknown'` → 기존 업종 기반 로직 (`mfg_parts`·`food_mfg` eligible / `fashion`·`agri_food` uncertain)
- **`semas_closure`·`kosmes_closure`**: 판정 기준을 `closureHist`(과거 이력) → **`currentStatus`(현재 상태)** 로 변경
  - `closed` → `blocked`/`high` / `active` → `clear` / `unknown` → `unknown`
- **`semas_closure_history` 신설** (소진공에만 1건): 과거 이력을 `conditional`/**`severity: 'low'`** 참고 항목으로 분리
  - 중진공은 소상공인이면 대부분 not_eligible로 걸러져 findings가 비므로, 참고 항목만 덩그러니 붙으면 혼란 → 소진공에만 배치
- **`_INCLUDE_CLEAR_FINDINGS = false`** 상수 도입: `clear` 항목은 현재 findings에서 제외하되, **각 규칙의 `message.clear`/`detail(clear)` 문자열은 미리 작성해 둠** → 5단계에서 상수만 `true`로 바꾸면 "요건 충족" 표시가 바로 동작

### ④ 검증 (Node 스모크 — 5개 지정 케이스 + 회귀 4건 전부 통과)
| 케이스 | 결과 |
|---|---|
| restaurant / 4명 / active | `semas_scale` **clear**, `semas_closure` **clear** ✓ |
| restaurant / **5명** | `semas_scale` **blocked/high** ("5명으로 5인 미만 초과") ✓ |
| mfg_parts / 8명 | `semas_scale` **clear** (제조업 10인 미만 기준) ✓ |
| fashion / `isManufacturing:'yes'` | 중진공 **`eligible: true`**, `exceptionBy: '제조업 영위'` ✓ |
| restaurant / `employeeCount: null` | 구간 fallback → **conditional/low** ✓ |
| `employeeCount: 0` | clear (0과 null 구분 확인) ✓ |
| `closed` + `closureHist:'yes'` | `semas_closure` blocked/high + `semas_closure_history` conditional/low 공존 ✓ |
| mfg_parts / `isManufacturing:'no'` | not_eligible + 업종-응답 충돌 안내 문구 출력 ✓ |

### ⑤ 캐시버스팅
`index.html` 로컬 `?v=` 47곳 전부 `20260801c`

---

## 최근 수정 이력 (2026-08-01) — 정책자금 진단 4단계: FundingRules 모듈 신설 (기관 선별 + 결격 판정)

`js/diagnosis/funding-rules.js` 신설. **기관 선별(소진공·중진공) 후 기관별 결격 판정.**
판정은 `blocked` / `conditional` / `clear` / `unknown` **4상태**이며 **`unknown`을 `clear`로 처리하지 않는다.**
근거: 소진공 `ols.semas.or.kr` 지원 제외업종 표, 중진공 2026 융자제한기업 15개 항목.
**대시보드 출력·AI 프롬프트는 5단계이므로 미구현.** 신보·기보는 범위 밖.

### ① 모듈 구조 (cross-context.js와 동일한 IIFE + 듀얼 익스포트)
- `AGENCIES` — semas(소진공) / kosmes(중진공) 2곳
- `EXCLUDED_INDUSTRIES_SEMAS` — 소진공 제외업종 38항목 `{ code, name, exceptions[] }`
- `INDUSTRY_WATCH` — industryKey → 해당 업종에서 확인이 필요한 제외업종 코드
- `RULES` — 소진공 6개 + 중진공 8개 규칙
- `evaluate(fundingData, context)` → `{ agencies[], unknownItems[], recommendedOrder[] }`
- `window.FundingRules` + `module.exports` 듀얼 익스포트 (Node 스모크 테스트 가능)

### ② ⚠ industryKey로 제외업종을 자동 판정하지 않는다
BizNavi의 `industryKey`(16종)는 **표준산업분류 코드가 아니다.** 제외업종 표는 자동 결격 판정에 쓰지 않고
`semas_industry`·`kosmes_industry` 규칙에서 **항상 `conditional`** 을 반환하며 "확인 필요" 안내 + 예외 조항 제시용으로만 사용한다.
`_watchText()`가 업종별 주의 항목과 예외를 함께 출력한다 (예: 안마시술소 → 시각장애인 운영 안마원은 예외).

### ③ 소진공 규모 요건 — 구간 문자열의 경계값 문제
`employees`는 숫자가 아니라 **구간 문자열**(`'1~5명'`, `'6~10명'` …)이라 5인/10인 경계를 확정할 수 없다.
- `'1~5명'` → `conditional` + **severity `low`** ("정확히 5인이면 제외되므로 확인 필요" — 확인 요청 수준의 문구, 경고성 표현 없음)
- `'6~10명'` → 제조·건설·운수·광업(`mfg_parts`·`food_mfg`·`construction`·`logistics`)이면 `conditional`, 그 외 `blocked`
- `'11~50명'` 이상 → `blocked` (severity `high`) / 값 없음 → `unknown`
- `findings`에 **`severity: 'high'|'medium'|'low'`** 필드 추가 — 5단계 화면에서 표시 강도 구분용

### ④ 중진공 기관 자격 게이트 (융자제한 9호) — `eligible` + `eligibilityUncertain` 2단 구분
소상공인은 원칙 제외. 예외 인정 범위를 **보수적으로** 잡았다.
- `certs`에 (예비)사회적기업·협동조합·마을기업·소셜벤처 판별기업 중 하나 → `eligible: true`
- `mfg_parts`·`food_mfg`만 '제조업 영위' 예외 인정 → `eligible: true`
- `fashion`·`agri_food`는 제조 영위 여부가 모호 → `eligible: false, eligibilityUncertain: **true**` + "제조 공정을 직접 영위하면 예외 대상일 수 있음"
- 그 외 소상공인 → `eligible: false, eligibilityUncertain: false`
- 모든 not_eligible 사유 끝에 공통 문구: "혁신성장·초격차·신산업 분야 영위 기업, 소상공인 유예기업도 예외 대상이므로 해당 여부를 중진공에 확인할 것"
- **`eligibilityUncertain: true`여도 중진공 개별 규칙은 평가하지 않는다** — 자격 미확정 상태에서 부채비율 경고를 띄우는 것은 오진

### ⑤ 단정 금지 원칙이 규칙에 박혀 있는 지점
- `kosmes_marginal`(11호): 수집 데이터로 **'2년 연속 적자'·'3년 연속 이자보상배율'을 알 수 없으므로 절대 `blocked`로 단정하지 않고 `conditional`**. 자본잠식 단독·영업적자 단독도 각각 `conditional`
- `kosmes_debt_ratio`(10호): **별표5의 업종별 수치가 앱에 없으므로 임의 기준을 만들지 않는다.** 산출된 부채비율만 표시하고 대조는 중진공 공고로 넘김. `foundedYear`가 있으면 업력을 계산해 "업력 7년 미만 적용 예외" 안내에 반영(자동 적용은 하지 않음)
- `semas_tax_arrears`: 체납이어도 `blocked`가 아니라 `conditional` (국세징수법 압류·매각 유예 등 예외 조항 존재). 반면 `kosmes_tax_arrears`는 예외 조항이 없어 `blocked`
- `semas_closure`·`kosmes_closure`: 현행 문항이 '최근 5년 내 이력'을 묻고 있어 **현재 상태와 다르므로** `blocked` 처리하지 않음
- `verdict`: blocked 1개 이상 → `blocked` / conditional·unknown 있으면 → `review` / 전부 clear → `clear`

### ⑥ 연결
- `index.html` — `js/diagnosis/funding-rules.js?v=20260801b` 추가 (cross-context.js 다음)
- `wizard.js collect()` — `_purpose === 'funding'` 일 때만 `data.fundingVerdict = FundingRules.evaluate(...)`. `typeof FundingRules === 'undefined'` + try/catch 이중 방어
- `App.checkFundingInput()` — `console.log(fundingData / fundingVerdict)` + `alert('판정 완료. 결과 화면은 5단계에서 구현 예정입니다.')`

### ⑦ 검증 (Node 스모크 테스트 4케이스 — 전부 기대와 일치)
| 케이스 | 결과 |
|---|---|
| restaurant/micro/1~5명/체납 yes | 소진공 `review`(체납 `conditional` + 예외 안내) / 중진공 `eligible:false, uncertain:false` ✓ |
| restaurant/micro/`certs:['협동조합·마을기업']` | 중진공 **`eligible: true`** (`exceptionBy` 표시) ✓ |
| fashion/micro/인증 없음 | 중진공 `eligible:false` + **`uncertain: true`** ✓ |
| mfg_parts/sme/11~50명/전부 unknown/자본 250억 | 소진공 `blocked`(규모, severity high) / 중진공 `kosmes_prime` **`blocked`**(200억 초과) / `unknownItems` 10건 수집 ✓ |
| `evaluate(undefined, undefined)` | 예외 없이 agencies 2건 반환, unknownItems 12건 ✓ |

### 개선 대기 항목
- **`employees`가 구간 문자열이라 소상공인 5인/10인 경계를 확정할 수 없다. 정책자금 경로에서는 정확한 상시근로자 수를 숫자로 입력받는 필드 추가 검토.**
- `recommendedOrder`는 `eligible` 기관만 담되 `verdict: 'blocked'` 기관도 포함된다(순위만 뒤로). 5단계 화면에서 blocked 기관을 어떻게 표시할지 판단 필요
- `unknownItems`는 label 기준 dedup이라 소진공·중진공의 동일 쟁점이 각각 별도 항목으로 잡힌다(라벨이 다르기 때문)

---

## 최근 수정 이력 (2026-08-01) — 정책자금 진단 3단계: 정부지원사업 데이터 소스 3중 중복 정리

### ① 정부지원사업 데이터 소스를 `js/gov-support.js` 단일 마스터로 통합
- 기존: `api/bizinfo.js` FALLBACK_PROGRAMS 10개 + `js/gov-support.js` PROGRAMS 26개가 **7개 중복**, 금액 필드명도 불일치(`amount` vs `support`)
- 중복 7건: 창업도약패키지·중소기업 정책자금·수출 바우처·R&D 과제·스마트 제조혁신·온라인 판로·고용창출 장려금
- **고유 3건만 gov-support.js에 병합** (`soho_smart` 소상공인 스마트화 / `soho_consulting` 소상공인 경영컨설팅 / `untact_voucher` 비대면 서비스 바우처) → PROGRAMS 26 → **29개**

### ② 구체 금액·마감일 표기 전면 제거 — `support` → `supportType`
- **매년 바뀌는 수치를 하드코딩하면 그 자체가 오정보다.** 앱은 '어떤 사업이 맞는지'만 판별하고 구체 수치는 주관기관 공고로 넘긴다
- 29개 전 항목 `support` → `supportType`으로 필드명 변경 + 금액·비율 삭제
  - `'최대 1억원 (정부 50% 매칭)'` → `'설비 도입비 매칭 지원'`
  - `'최대 3,000만원 바우처'` → `'바우처 형태 지원'`
  - `'무료 (전문가 5~10회 파견)'` → `'전문가 파견 컨설팅'`
- `period`는 병합 3건에만 `'수시'`/`'정기 공고'` 수준으로 부여 (기존 26개는 period 필드 자체가 없었음)
- `GovSupport.DISCLAIMER` 상수 export — **섹션 상단 1회 + AI 프롬프트 1회만** 표시 (카드마다 반복은 노이즈)
  - 문구가 **상시 지원사업 전용**임이 드러나게 작성. 실시간 공고 영역에는 붙이지 않는다
- `buildPromptBlock()`에 "구체적인 금액·비율·마감일을 지어내지 말 것" 지침 추가
- 소비처 수정: `dashboard.js:339` `💰 ${p.support}` → `🎁 ${p.supportType}`

### ③ ⚠ `#industry` select 제거 이후 GovSupport 업종 매칭이 항상 실패하던 문제 수정
- `#industry` select 제거(2026-04-17) 이후 **`collect()`의 `d.industry`는 항상 빈 문자열**
- `GovSupport.match()`가 `d.industry`만 읽어 업종 점수(+2)가 `'all'` 태그 사업에만 부여됨
  → **HACCP·외식업 스마트화·스마트 건설 등 업종 특화 사업이 아무에게도 매칭되지 않고 있었다** (기능이 죽어 있던 상태)
- **수정**: `INDUSTRY_LABEL` 역매핑(영문 키 → 한국어 라벨) 추가 후 `d.industry || INDUSTRY_LABEL[d.industryKey] || ''` 로 읽음
  - 역매핑 19개 키는 `wizard.js INDUSTRY_MAP`을 그대로 반전한 값. 대조 결과 **불일치·누락 0건** (gov-support가 쓰는 라벨 12개 + `all` 전부 INDUSTRY_MAP에 존재)
  - PROGRAMS의 `industry` 태그(한국어)는 미변경 — 최소 변경 원칙
- `industryKey`도 비어 있으면 **업종 필터를 적용하지 않고 전 항목에 동점 부여** (빈 결과 방지)

### ④ step4 govSupport 체크박스 → 매칭 필터 연결 (`getInterestTags` 재작성)
- 기존은 **if/else 조기 return 체인**이라 복수 선택 시 첫 카테고리만 반영됐고, 체크박스 6개 중 **'창업·성장 지원'·'정책 자금 융자' 2개는 매핑이 아예 없어 `[]` 반환**. 코드의 `'전반'` 분기는 어떤 체크박스에도 없는 사문화 값
- **수정**: `Set` 누적 방식으로 재작성, 배열·문자열 입력 모두 허용
  - `'창업'|'성장'` → `fund`,`marketing` / `'정책 자금'|'정책자금'|'융자'` → `fund` 추가, `'전반'` 분기 제거
- 미선택 시 `[]` 반환 → 관심분야 필터 미적용(전체 반환)은 현행 유지

### ⑤ `api/bizinfo.js` — 실시간 조회 전용으로 축소
- `FALLBACK_PROGRAMS`(10개) + `scoreProgram()` 삭제
- 키 없음 → `{ ok: false, reason: 'no_api_key', programs: [] }` / 조회 실패·0건 → `{ ok: false, reason: 'api_error', programs: [] }`
- 성공 시 `{ ok: true, status: 'api', programs, total }`
- **실시간 API가 안 되는데 오래된 하드코딩 데이터를 실시간인 것처럼 보여주지 않는다** (fakeAnalysis와 동일한 문제)
- 실시간 공고의 `amount`(`item.sprtLmt`)는 **조회 시점 기관 공고값이므로 유지** — 하드코딩 수치와 성격이 다름

### ⑥ `js/app.js` — 소스 전환
- `GovSupport.match(data)`를 **먼저 확보**(항상 동작하는 기반) → `data.govPrograms` / `window._govPrograms`
- `/api/bizinfo` 병행 호출, `ok === true` 일 때만 `data.bizinfoPrograms` 주입. `ok:false`·예외는 **조용히 생략**(사용자에게 에러 미노출)
- `typeof GovSupport === 'undefined'` 및 `try/catch` 이중 방어

### ⑦ `#drBizinfoBox` 재사용 — 새 섹션 신설 없이 박스가 사라지지 않게
- 실시간 있으면 `실시간` 배지, 없으면 `GovSupport.match()` 결과를 `상시 지원사업 · 공고 확인 필요` 배지로 표시 + DISCLAIMER
- 기존 카드 마크업 재사용을 위해 `supportType` → `amount` 슬롯에 정규화 (금액이 아니라 **지원 형태**가 들어감 — 주석 명기)
- fallback 삭제로 박스가 통째로 사라지던 회귀를 방지

### ⑧ 검증 (Node 스모크 테스트)
| 항목 | 결과 |
|---|---|
| 업종 역매핑 (restaurant + 디지털 관심) | '외식업 스마트화 지원사업'이 매칭 결과에 등장 ✓ (수정 전에는 불가능) |
| 업종 정보 전무 | 결과 6건 — 빈 배열 안 됨 ✓ |
| 복수 선택 (디지털+수출) | 스마트공장 7점 / 수출 바우처·KOTRA 6점 — 양쪽 태그 모두 반영 ✓ |
| '창업·성장 지원' | 창업도약패키지·정책자금·컨설팅 등 6건이 관심분야 가점(3점) 획득 ✓ |
| '정책 자금 융자' | 정책자금 융자·소상공인 정책자금 등 4건 가점 ✓ |
| 금액 표기 잔존 | `supportType` 내 억원·만원·% 표기 **0건** / 남은 `support:` 필드 **0건** ✓ |
| 병합 3건 | 3개 모두 존재, 실매칭 결과에도 등장 ✓ |

### ⑨ 캐시버스팅
`index.html` 로컬 `?v=` 46곳 전부 `20260731d` → 이후 ⑩ 수정으로 `20260801a`

### ⑩ `match()` 업종 점수 차등화 (배포 완료) — 후속 수정
- **문제**: 업종 점수가 `정확 매칭 == 범용('all') == +2`로 동일해서, 외식업으로 진단해도 '외식업 스마트화 지원사업'(6점)과 범용 사업들(6점)이 **동점**이 되고 `slice(0, 6)`에서 **배열 앞쪽의 범용 사업만 남아** 업종 특화 사업이 노출되지 않았다. 실제 화면에서 6개 전부 범용 사업이었음
  - ③에서 역매핑으로 업종 값을 살렸지만, 점수 배분이 평평해 **효과가 화면에 드러나지 않던 상태**
- **수정**: 정확 매칭 **+3** / 범용 `'all'` **+1** / 불일치 **0** 으로 차등화
  - 업종 미상(`industry === ''`)이면 기존대로 전 항목 동점(**+1**) → 특정 사업 탈락 방지
- **동점 정렬 보완**: `점수 내림차순 → 업종 정확 매칭 우선(_industryExact) → 이름 순(localeCompare 'ko')`
  → **PROGRAMS 배열 선언 순서에 결과가 좌우되지 않는다**
- PROGRAMS의 `industry` 태그 자체는 미변경
- **검증**

  | 케이스 | 결과 |
  |---|---|
  | `restaurant` / micro / 디지털 | 상위 4개가 전부 업종 정확 매칭. **외식업 스마트화 3위(7점)** ✓ (수정 전 미노출) |
  | `food_mfg` / micro / 관심분야 미선택 | 상위 6개 전부 업종 정확 매칭. **식품 HACCP 4위(4점)** ✓ |
  | 업종 미상 | 6건 반환, 빈 배열 아님 ✓ |
  | `mfg_parts` / sme / 디지털+수출 | 스마트공장 8점 1위, 상위 5개가 업종 정확 매칭 ✓ |

### 남은 과제
- `#drBizinfoBox`의 `.bizinfo-amount` 슬롯이 상시 사업에서는 '지원 형태'를 담는 의미 불일치 — 클래스 분리는 4~5단계 대시보드 작업 시 정리
- `ai-engine.js` 섹션11은 실시간 공고만 참조. 실시간이 없으면 `(지원사업 데이터 없음 — 웹 검색으로 보완)`이 출력되지만, `GovSupport.buildPromptBlock()`이 별도 섹션으로 이미 주입하므로 프롬프트 정보 손실은 없음

---

## 최근 수정 이력 (2026-07-31) — 정책자금 진단 2단계: step5 진단 문항 13개 구성

1단계에서 만든 `step5` 스켈레톤에 문항을 채웠다. **판정 로직·트랙 라우팅·대시보드·AI 프롬프트는 4~5단계로 미구현.**

### ① index.html — step5 문항 3섹션
- 카드 상단 안내문(결격 사유 반려 경고 + 미저장 고지 + "모르면 '모름' 선택") 추가
- **섹션1 결격 요건 7문항 (필수)** — 전부 3지선다 라디오 `해당 없음 / 해당됨 / 모름`
  `fundTaxArrears` `fundCapitalImpair` `fundCreditIssue` `fundClosureHist` `fundRestrictedBiz` `fundPriorSupport` `fundOverdue`
  - **id는 감싸는 `.form-group`에, `name`은 라디오 그룹에** 부여 (라디오 3개에 같은 id를 줄 수 없으므로) → validate 실패 시 id로 스크롤·하이라이트
  - value를 `none`/`yes`/`unknown` 영문으로 직접 지정해 collect() 정규화를 불필요하게 함
- **섹션2 보유 자산 2문항 (선택)** — 체크박스 복수선택 `fundCerts`(12개) / `fundIP`(6개), 기존 `.gov-check-group`·`.gov-check-item`·`.gov-check-label` 재사용
- **섹션3 재무 정보 4문항 (선택)** — `fundAssetTotal` `fundDebtTotal` `fundEquityTotal` `fundOpProfit` (`type="number"`, 단위 백만원, required 없음)
  - `#fundDebtRatio` 표시 전용 div 신설 — 부채·자본 모두 입력 시 부채비율 실시간 표시
- 버튼 → `App.checkFundingInput()` (runAnalysis 미연결)
- **연매출은 step1 `#revenue`를 재사용**하고 step5에 중복 생성하지 않음

### ② js/wizard.js — fundingData / validate(5) / 인터랙션
- `collect()` 리터럴 안에 **`fundingData` 하위 객체** 추가. **미입력을 기본값으로 채우지 않는 것이 핵심 원칙**:
  | 구분 | 미응답 | 명시적 응답 |
  |---|---|---|
  | 라디오 7개 | `'unknown'` (절대 `'none'` 아님) | `'none'` / `'yes'` / `'unknown'` |
  | 체크박스 2개 | `[]` (응답하지 않음) | `['해당 없음']` (명시적 미보유) |
  | 숫자 4개 | `null` | `0`도 유효값 (0 ≠ null) |
  - `debtRatio`: 부채·자본 모두 있고 자본 > 0일 때만 숫자, 그 외 `null`
  - `revenue`: step1 원문 문자열 그대로, 빈 값이면 `null`. **파싱·정규화하지 않음**
- `_fundNum(id)` / `_fundDebtRatio(debt, equity)` / `_onFundCheckToggle()` / `updateFundDebtRatio()` 헬퍼 + `FUND_ELIG_ITEMS` 상수 추가
- **`validate(5)` 신설** — 결격요건 7문항 선택 여부만 검사('모름'도 정상 입력). 미선택 시 `.fund-invalid` 하이라이트 + 첫 항목으로 `scrollIntoView` + 누락 항목명 나열 alert
- **'해당 없음' 배타 처리** — '해당 없음' 체크 시 같은 그룹 전부 해제, 다른 항목 체크 시 '해당 없음' 해제. `fundCerts`·`fundIP` 양쪽 적용
  - 모순 입력(`['벤처기업','해당 없음']`)을 입력 단계에서 차단해 4단계 판정 로직의 예외 처리 부담을 없앰
- 이벤트는 **HTML `oninput` 속성이 아니라 `DOMContentLoaded` 리스너로 등록** (2026-06-05 c317c03 캐시 문제 재발 방지)

### ③ js/app.js — `checkFundingInput()` 신설 (export 포함)
`validate(5)` → `console.table(collect().fundingData)` → alert. **의도적으로 `runAnalysis()` 미연결** (빈 진단점수로 Claude API 호출 시 토큰만 소모)

### ④ css/style.css — 신규 클래스 5개, 파일 끝에 추가만
`.fund-radio-row` `.fund-radio-opt` `.fund-ratio-box` `.form-group.fund-invalid` + 모바일 1열 미디어쿼리. **기존 선택자 미변경**

### ⑤ 검증 (Node DOM 스텁)
| 시나리오 | 결과 |
|---|---|
| 미입력 상태 | 라디오 7개 전부 `unknown`, `certs`/`ip` `[]`, 숫자 4개 `null`, `validate(5)` false |
| 결격 7선택 + 부채700·자본500 | `debtRatio: 140`, `opProfit: -80`, `revenue: '3억'`(원문), `validate(5)` true |
| 자본 -100 / 0 | `debtRatio: null`, 표시 "자본잠식 상태로 부채비율 산출 불가" |
| 영업이익 0 입력 | `opProfit: 0` 유지 (미입력 `null`과 구분됨) |
| 결격 2개 누락 | `validate(5)` false + 누락 항목명 나열 alert |
| '해당 없음' 배타 | 일반 2개 → '해당 없음' 체크 시 2개 해제 / 일반 재체크 시 '해당 없음' 해제 ✓ |

### ⑥ 캐시버스팅
`index.html` 로컬 `?v=` **46곳 전부 `20260731c`** (외부 CDN 제외)

---

## 최근 수정 이력 (2026-07-31) — collect() 조기 return으로 진단 데이터가 AI에 미전달되던 버그 수정 ⚠ 중대

정책자금 진단 2단계 작업 중 발견. **CROSS_RULES 33개 + micro/sme 규모별 진단 결과가 AI 분석에 전혀 반영되지 않고 있었음.**

### ① 근본 원인 — `collect()` 조기 return
- `js/wizard.js collect()`가 `return { … };` 로 **객체 리터럴을 즉시 반환**하고 종료
- 그 뒤 53줄(`scaleScores` 계산 / `microWarnings`·`microPrompt` / `smeWarnings`·`smePrompt` / `CrossContext` 블록 / `return data;`)은 **도달 불가(unreachable)** 였고, `data` 변수는 선언조차 없어 도달해도 ReferenceError
- 결과: `ai-engine.js:1286~1292`가 참조하는 `d.microPrompt` / `d.smePrompt` / `d.crossPrompt` 가 **항상 undefined** → 프롬프트에 아무것도 주입되지 않음
- **수정**: `return {` → `const data = {` 로 변경, 맨 끝 `return data;` 가 최종 반환이 되도록 복구

### ② `collectAllScores()` 신설 — DOM 수집 금지
- 죽은 코드가 호출하던 `collectAllScores`는 **저장소 어디에도 존재하지 않는 미선언 식별자**였음
  (`collectAllScores ? … : {}` 는 undefined 체크가 아니라 **ReferenceError를 던짐** — 살리는 순간 collect() 전체가 예외로 중단됐을 것)
- ⚠ **진단 점수는 DOM이 아니라 `diagScores` 객체에만 존재한다.** `setScore()`가 `diagScores[key] = {score, memo}` 로만 저장하고, `type="hidden"` 입력은 하나도 만들지 않는다
- 따라서 기존 CrossContext 블록의 `document.querySelectorAll('[id^="diag-"]')` 수집은 **항상 빈 객체 `{}`** 를 반환했음 (`diag-item-*`·컨테이너는 div라 `.value`가 없음) → 이 방식도 함께 폐기
- 신설 `collectAllScores()`: `diagScores` → `{ 'diag-micro-container_1_3': 3, … }` **평면 숫자 맵** 반환 (0점·미입력 제외)
  - `DiagMicro.calcScores` / `DiagSme.calcScores` / `CrossContext.buildScoreMap`(`Number(val)`) 모두 평면 숫자를 기대하므로 `{score,memo}` 객체를 넘기면 NaN으로 전부 탈락함
- micro / sme / CrossContext **세 곳이 이 함수를 공유**. CrossContext는 BM 프록시 주입으로 맵을 변형하므로 `Object.assign({}, allScores)` 사본(`crossScores`)을 사용

### ③ 빈 스코어의 실제 증상 — 허위 발동이 아니라 '전면 무력화' (기록 정확성 중요)
`cross-context.js:600`의 룰 평가부가
```js
return score !== null && score <= trigger.threshold;
```
이므로 **키가 없으면 `null` → 조건 자체가 false → 규칙이 아예 발동하지 않는다.**
빈 스코어가 CRITICAL 경고를 무더기로 **허위 발동시키는 것이 아니라**, CROSS_RULES 전체가 조용히 무력화되는 것이 정확한 증상이다. (초기 진단 중 "허위 발동" 추정이 있었으나 사실이 아님 — 잘못된 원인을 기록하면 나중에 오판을 부르므로 명시해 둠)

### ④ `bizScale` 후반부 덮어쓰기 삭제 — 2026-05-15 버그 재발 방지
- 죽은 코드에 `const bizScale = g('bizScale') || g('bizScaleSelect') || 'micro';` 재계산 + `data.bizScale = bizScale;` 덮어쓰기가 있었음
- 리터럴 내부 계산은 explicit 없을 때 `employees`로 추론(`'1~5명'`·공백 → micro, **그 외 sme**)하는데, 후반부는 **무조건 `'micro'`** → 직원 6명 이상 기업이 micro로 오분류
- 이는 CLAUDE.md 2026-05-15 ①에서 고친 *"bizScale 미추론 → micro 모드 출력 → keyStrategies 누락"* 버그의 **정확한 재발**
- **수정**: 후반부 재계산·덮어쓰기 2줄 모두 삭제, `const bizScale = data.bizScale || 'micro'` 로 리터럴 계산값을 그대로 사용

### ⑤ DiagSme는 현재 비활성 — 별도 작업 필요
- `diagnosis-sme.js`는 `scores['diag-sme-container_*']` 키를 기대하지만, **`diag-sme-container`는 diagnosis-sme.js 안에만 존재**하고 index.html·`loadDiagnosisUI()` 어디에서도 렌더링되지 않음
- 그대로 살리면 `smePrompt`가 **전 항목 0점("0점 (위험)")** 인 허위 요약이 되므로 가드 적용:
  ```js
  else if (bizScale === 'sme' && window.DiagSme &&
           Object.keys(allScores).some(k => k.startsWith('diag-sme-container_')))
  ```
- **미해결 과제**: `diag-sme-container` 렌더링 연결 (별도 작업으로 분리)

### ⑥ 검증 (Node DOM 스텁 스모크 테스트)
micro 35문항을 동일 점수로 채워 `Wizard.collect()` 실행:

| 입력 | scaleScores.total | microPrompt | crossPrompt | microWarnings | crossWarnings |
|---|---|---|---|---|---|
| 전 항목 3점 | 60 | 353자 | 생성됨 | 0 | 0 |
| 전 항목 1점 | 20 | 1794자 | 생성됨 | 6 | 12 |

점수에 따라 경고 수가 실제로 변동 → 파이프라인 정상 연결 확인 (수정 전에는 전부 undefined)

### ⑦ 캐시버스팅 일괄 갱신
- `index.html`의 로컬 `?v=` **46곳 전부 `20260731b`로 통일** (외부 CDN 제외)
- 참고: `js/gov-support.js`·`js/industry-trends.js`·`js/diagnosis/cross-context.js`·`js/diagnosis/diagnosis-micro.js`·`js/diagnosis/diagnosis-sme.js`·`js/diagnosis/startup.js`·`js/ticker.js` 7개는 **`?v=` 자체가 없음** — 이 파일들을 수정할 때는 `?v=`를 새로 붙여야 캐시가 갱신됨

### 검증 방법 (배포 후 브라우저 콘솔 — 소상공인 모드로 진단 완료 상태)
```js
const d = Wizard.collect();
console.log({ bizScale: d.bizScale, hasScaleScores: !!d.scaleScores,
              hasMicroPrompt: !!d.microPrompt, hasCrossPrompt: !!d.crossPrompt,
              warnCount: (d.crossWarnings||[]).length });
console.log(d.crossPrompt);
```

---

## 최근 수정 이력 (2026-07-30) — 정책자금 진단 1단계: 진입점·purpose 플래그·step5 스켈레톤 추가

세 번째 진입 경로(재무분석 / 경영전략 진단 / **정책자금 진단**) 중 정책자금 진단의 **뼈대만** 구축.
문항·판정 로직·대시보드 섹션·AI 프롬프트는 이번 단계에서 **의도적으로 미구현**.

### ① js/wizard.js — purpose 플래그 + step5 라우팅
- 모듈 스코프 `let _purpose = 'general';` 추가 (`'general'` | `'funding'`)
- `setPurpose(p)` / `getPurpose()` 추가 → `return { ... }` export 목록에 등록
- `reset()`: `_purpose = 'general'` 초기화 + 숨김 배열에 `'step5'` 추가
- `collect()`: 반환 객체 최상단에 `purpose: _purpose || 'general'` 필드 추가
- `goStep(n)`: 진입부에 `if (_purpose === 'funding' && n === 2) n = 5;` — step2·3·4를 건너뛰고 step5로 리다이렉트 (`loadDiagnosisUI()` 미호출)
- `updateStepUI(n)`: `_restoreStepIndicator()` 헬퍼 신설 → **목적과 무관하게 먼저 general 기준(3단계)으로 무조건 원복**한 뒤, `_purpose === 'funding'`일 때만 2단계 표시로 덮어씀
  - funding: `l2` → '정책자금 진단', `c3`의 부모 `.step-ind`·`ln2` → `display:none`, `n===5`를 2번째 인디케이터 active로 매핑, 진행률 `n===1?50:100`
  - general: 기존 루프(`i<=4`) + `33/66/100` **완전 무수정** (시각 동작 100% 동일)

### ② js/app.js — 진입점
- `startFundingDiagnosis()` 신설: `Wizard.reset()` → `Wizard.setPurpose('funding')` → `Wizard.goStep(1)`(인디케이터 재도색) → `show('wizard')`
  - `reset()`이 `_purpose='general'`로 초기화하므로 **setPurpose는 반드시 reset 이후** 호출해야 함
- `startDiagnosis()`: biz-context 숨긴 직후 `Wizard.getPurpose() === 'funding'`이면 `Wizard.goStep(5)` 후 early return — `loadDiagnosisUI`·`updateRiskPlaceholder`·`goToStep2FromBm` 미실행. general 경로는 기존 코드 그대로
- `return { ... }` export 목록에 `startFundingDiagnosis` 추가

### ③ index.html
- `step4` 다음에 `<div id="step5" class="wiz-card hidden">` 신설 — 제목 + "2단계에서 문항 추가 예정" + `← 이전`(`App.backToStep1()`) + `분석 시작 → (준비중)`(alert placeholder)
  - **`App.runAnalysis()`에 연결하지 않음** — `validate(4)`(problems·goals)를 통과시키면 빈 diagScores로 실제 Claude API가 호출되어 토큰만 소모됨
- 히어로 CTA(74~78줄)에 `💰 정책자금 진단 시작하기` 버튼 1개 추가 (155·354줄 랜딩 영역은 미접촉)

### 핵심 제약 (id 명명 규칙)
`goStep(n)`은 `document.getElementById('step' + n)`으로 화면을 찾으므로 **새 단계 id는 숫자 형식 필수** → `step-funding`이 아니라 `step5`

### 1단계 알려진 한계 (2단계에서 처리)
- step5의 `← 이전`은 `App.backToStep1()` → `Wizard.reset()`을 호출하므로 **`_purpose`가 `'general'`로 리셋**됨. 정책자금 진단으로 되돌아오려면 랜딩 버튼을 다시 눌러야 함
- `updateStepUI()` general 분기의 `i<=4` 루프·`ln3` 참조는 DOM에 `c4`·`ln3`이 없어 죽은 코드지만, 기존 동작 보존 최우선으로 **의도적으로 미정리**

---

## 최근 수정 이력 (2026-06-04~05) — 업종별 placeholder 동적 업데이트 + inferIndustryFromType 버그 수정

### ① 업종별 주요제품·핵심강점·고객문제 placeholder 동적 업데이트 (배포 완료) — 커밋 6c42f25
- `_BIZ_PLACEHOLDERS`: 16개 업종 × 3개 필드(products·coreStrength·customerProblem) 예시 맵 추가
- `updateBizPlaceholders(industryKey)`: 업종 추론 완료 시 해당 업종 맞춤 예시로 placeholder 즉시 교체
- `onIndustryChange()`: `updateBizPlaceholders` + `updateRiskPlaceholder` 함께 호출

### ② inferIndustryFromType 연쇄 버그 수정 4건 (배포 완료) — 커밋 ba37c3e~c317c03
- **1차** (ba37c3e): `id="industry"` SELECT 미존재로 early return — `industrySelect` 의존 제거, `aiIndustryKey` 직접 세팅
- **2차** (6827fb5): `id="bizInferResult"` 미존재로 early return — `resultEl` optional 처리
- **3차** (20c897a): OCR 자동입력 후 `inferIndustryFromType()` 미호출 — `handleOcrUpload()` fill 완료 후 명시적 호출 추가
- **4차** (c4248a0): `BIZ_TYPE_MAP.industry`가 한국어 레이블(`'외식 및 휴게음식업'`)인데 `_BIZ_PLACEHOLDERS` 키는 영문(`restaurant`) — `INDUSTRY_MAP[topIndustry]`로 변환 후 호출
- **5차** (c317c03): HTML `oninput` 캐시 문제 + `!elProducts.value` 가드 차단 — `DOMContentLoaded`에서 직접 이벤트 리스너 등록, 가드 제거하여 항상 갱신

### 핵심 동작 흐름 (완성)
```
업태/종목 입력(직접·OCR) → inferIndustryFromType()
  → BIZ_TYPE_MAP 키워드 스코어링
  → topIndustry(한국어) → INDUSTRY_MAP → topKey(영문)
  → aiIndustryKey 저장
  → updateBizPlaceholders(topKey) → 3개 필드 placeholder 교체
  → updateRiskPlaceholder(topKey) → 외부리스크 예시 교체
```

---

## 업종별 맞춤 진단 v2.0 완성 (2026-05-27)

### 업종 그룹 5개
- food: 외식업 (기본값)
- beauty: 미용·뷰티
- retail: 소매·판매
- edu_service: 서비스·학원
- pro_service: 전문서비스

### 완료 작업
1차: 질문 텍스트·용어 치환 (D2_5·D3_2 완전 교체 포함)
2차: BARS 1~5점 척도 + ai_trigger 경고 메시지 업종별 분기
3차·4차: D1~D4·D7 7일 액션 플랜 업종별 분기 (총 140개)

### 불변 항목
- D5·D6 전체 (자금·세무·폐업 — 업종 무관 공통)
- food 그룹 기존 텍스트 100% 유지
- 키값 D1_1~D7_5 변경 없음

### 핵심 함수
- DiagMicro.getSchema(industryGroup)
- DiagMicro.getActionPlan(domainKey, industryGroup)
- DiagMicro.buildPromptSummary(allScores, industryGroup)

---

## 최근 수정 이력 (2026-05-27) — 소상공인 진단 업종 그룹별 용어 치환 + BARS scale 업종 분기

### ① /fix-js-edit 스킬 신규 생성 (배포 완료) — 커밋 37278a2
- `.claude/commands/fix-js-edit.md` 신규: JS 파일 편집 반복 실패(old_string not found) 진단·수정 스킬
- 증상별 원인: Read 미실행 / anchor 불일치(공백·한글 인코딩) / 중복 문자열
- 대형 파일 패턴: Grep → offset Read → Edit (wizard.js·ai-engine.js 2000줄+)
- `fix-ai-error.md`, `fix-pdf.md`에서 `/fix-micro`와 중복된 CDN TTFB·PDF 공란 섹션 제거

### ② diagnosis-micro.js 업종 그룹별 용어 치환 맵 (1차 — label·question) (배포 완료) — 커밋 d590d60
- `INDUSTRY_GROUP_MAP`: 16개 업종 키 → 5그룹 매핑 (food/beauty/retail/edu_service/pro_service)
- `GROUP_LABELS`: 그룹별 한국어 레이블
- `INDUSTRY_WORDING`: 4그룹 × ~14항목 label·question 오버라이드
  - food-전용 항목 완전 교체: `2_5` (Work Triangle → 그룹별 작업 동선), `3_2` (밀키트 → 그룹별 패키지)
  - D5·D6 절대 미접촉
- `getGroup(industryKey)`, `getSchema(industryGroup)` API 추가
- `buildPromptSummary()`: `[업종 그룹]` 라인 AI 프롬프트 주입

### ③ wizard.js micro 진단 업종 그룹 연동 (배포 완료) — 커밋 27a2b3b
- `loadDiagnosisUI()`: `DiagMicro.getGroup(industryKey)` → `_diagMicroToAreas(DiagMicro, microGroup)` 전달
- `_diagMicroToAreas(diagMicro, industryGroup)`: 파라미터 추가 → `getSchema(industryGroup)` 호출
- `collect()`: `DiagMicro.getGroup(data.industryKey)` → `buildPromptSummary(allScores, microGroup)` 전달

### ④ diagnosis-micro.js BARS scale 1~5 + ai_trigger warning_msg 업종별 분기 (배포 완료) — 커밋 6014eee
- **적용 항목**: 4그룹 × 3항목 = scale 12개 배열 / 4그룹 × 2항목 = warning_msg 8개
  - `1_3` (Prime Cost): beauty·retail·edu_service·pro_service 각 scale 5단계 + warning_msg
  - `1_4` (ACM): beauty·retail·edu_service·pro_service 각 scale 5단계 + warning_msg
  - `2_4` (편차 통제): beauty·retail·edu_service·pro_service 각 scale 5단계
- **getSchema() deep-merge**: `ai_trigger` 오버라이드 시 기존 `threshold·warning` 보존, `warning_msg` 추가
  - `merged.ai_trigger = Object.assign({}, ITEMS[key].ai_trigger, overrides[key].ai_trigger)`
- food 그룹 기존 텍스트 100% 유지, D5·D6 미접촉

### ⑤ diagnosis-micro.js ACTION_PLAN_7DAY 업종 그룹별 분기 3차 (배포 완료) — 커밋 120be37
- `ACTION_PLAN_7DAY_BY_GROUP` 상수 추가: 2개 도메인 × 4그룹 × 7일 = 56개 액션 항목
  - `profit_ops` (D1 경영진단): beauty·retail·edu_service·pro_service 각 7일 플랜
  - `place_seo` (D2 점포환경): beauty·retail·edu_service·pro_service 각 7일 플랜
- `DOMAIN_TO_ACTION_KEY`: 도메인 ID `'1'`→`profit_ops`, `'2'`→`place_seo` 매핑
- `getActionPlan(domainKey, industryGroup)`: 그룹별 플랜 반환, 없으면 `null` (food → base fallback)
- `buildPromptSummary()` 수정: 취약 도메인 D1·D2는 그룹 전용 7일 플랜, D3~D7은 `ACTION_PLAN_7DAY` 단일 액션 fallback
- food 그룹 `ACTION_PLAN_7DAY` 100% 유지, D5·D6 미접촉

### ⑥ diagnosis-micro.js ACTION_PLAN_7DAY D3·D4·D7 업종 그룹별 분기 4차 (배포 완료) — 커밋 c85f4a3
- `ACTION_PLAN_7DAY_BY_GROUP`에 3개 도메인 추가: 3개 도메인 × 4그룹 × 7일 = 84개 액션 항목
  - `multichannel` (D3 다채널 판로): beauty·retail·edu_service·pro_service 각 7일 플랜
  - `smart_dx` (D4 스마트DX): beauty·retail·edu_service·pro_service 각 7일 플랜
  - `sns_ai` (D7 SNS·생성형AI): beauty·retail·edu_service·pro_service 각 7일 플랜
- `DOMAIN_TO_ACTION_KEY` 확장: `'3'`→`multichannel`, `'4'`→`smart_dx`, `'7'`→`sns_ai` 추가
- D1~D4·D7 총 5개 도메인 × 4그룹 × 7일 = 140개 그룹별 액션 플랜 완성
- food 그룹 `ACTION_PLAN_7DAY` 100% 유지, D5·D6 미접촉

---

## 최근 수정 이력 (2026-05-26) — micro 역량 프로파일 D1~D7 교체 + Failed to fetch 근본 수정

### ① 3차-micro 504 타임아웃 수정 (배포 완료) — 커밋 105aa7f
- **원인**: `claude-analyze-3.js` maxDuration=60초, D5~D7+plan90days 스트리밍이 60초 초과
- **수정**: `vercel.json` claude-analyze-3.js maxDuration 60 → 300
- **수정**: `claude-analyze-3.js` MAX_TOKENS 16000 → 8000 (이후 다시 16000으로 상향)

### ② micro 대시보드 신규 섹션 추가 (배포 완료) — 커밋 5fb2459
- `sec-lifecycle` (생애주기 진단 — 창업기/생존기/성장기/성숙기/전환기 시각화) 추가
- `sec-market-micro` (상권 STP + TAM/SAM/SOM 3카드) 추가
- `buildNav(isMicro)` micro 전용 7링크로 교체
- `sec-six-systems` micro에서 "7대 영역 처방 (D1~D7)"으로 제목 동적 변경
- `sec-lean-canvas` micro에서 smeOnly에 포함 (leanCanvas 미생성)
- CSS: lifecycle-stage-banner/steps + tsm-grid (TAM=골드·SAM=블루·SOM=그린) 추가

### ③ micro 역량 프로파일 D1~D7 레이더차트 교체 (배포 완료) — 커밋 dadad52
- `MICRO_DOMAIN_EXPLAIN`: D1~D7 해설 카드 7개 추가
- `_calcMicroDomainScores()`: `diag-micro-container_X_Y` 키 파싱 → D1~D7 점수 계산
- `showDiagReveal()`: `isMicro` 분기 추가
  - micro: 7개 축 레이더차트 + D1~D7 점수 바 + "소상공인 7대 영역 진단" 고정
  - SME: 기존 5대 역량 도메인 유지

### ④ claude-analyze-3.js MAX_TOKENS 8000 → 16000 (배포 완료) — 커밋 5885672
- 소상공인 내용 대폭 수정 시 D5~D7+plan90days JSON 절단 방지

### ⑤ micro 스트리밍 CDN TTFB 타임아웃 방지 — Failed to fetch 근본 수정 (배포 완료) — 커밋 b681af2
- **근본 원인**: Claude SSE 누적(60~120초) 동안 브라우저에 아무것도 안 보내다가 한꺼번에 응답 → Cloudflare CDN이 TTFB 기준으로 연결 끊음 → "Failed to fetch"
- Vercel 로그에서는 1~3차 모두 성공으로 표시되지만 응답이 클라이언트에 미전달
- **수정**: `claudeRes.ok` 확인 직후 `res.writeHead(200, {'Content-Type':'application/json'})` 즉시 전송
- 이후 SSE 누적 완료 시 `res.end(JSON.stringify({text: fullText}))` 전송
- `claude-analyze-1.js` / `claude-analyze-2.js` / `claude-analyze-3.js` 모두 적용

---

## 최근 수정 이력 (2026-05-25) — micro 전 구간 스트리밍 전환 + SYSTEM 지침 경량화

### ① micro 1차 스트리밍 전환 (배포 완료) — 커밋 c18d83d
- **원인**: max_tokens=2000으로도 SYSTEM의 executiveSummary 5-label 지침만으로 토큰이 부족 → max_tokens 초과 반복
- **수정**: `claude-analyze-1.js` micro 분기(`noSearch=true`) → `stream:true`, max_tokens:16000, SSE 청크 누적 방식으로 전환
- 스트리밍 경로: `response.body.getReader()` + `TextDecoder` → `content_block_delta.text_delta` 누적 → 완성 후 반환

### ② micro 2차·3차도 동일 스트리밍 전환 (배포 완료) — 커밋 08e9baa
- `claude-analyze-2.js`: `noSearch=true` 분기에 스트리밍 추가 (micro 전용 라우팅 신호로 재활용)
- `claude-analyze-3.js`: micro 전용 함수이므로 항상 스트리밍, max_tokens:16000
- `ai-engine.js`: `_opts2: { noSearch: true }` (streaming 트리거), 3차 opts `{}` (서버에서 항상 스트리밍)

### ③ SYSTEM 상수 micro 1차 과잉 생성 유발 지침 제거 (배포 완료) — 커밋 22ff99e
- **원인**: 스트리밍 16000으로도 `stop_reason=max_tokens, output_tokens=16000` → SYSTEM에 sixSystems·plan90days JSON 템플릿이 있어 Claude가 1차에서도 전부 생성
- **제거 항목 3개**:
  - `[인과사슬 3축 진단]` 블록 17줄 — executiveSummary·SWOT 상세 서술 강제
  - `[executiveSummary 출력 포맷 지침]` 12줄 — 5-label 구조 강제
  - `sixSystems` 7항목 + `plan90days` 3항목 JSON 템플릿 — 1차 호출인데도 생성 유도
- `[사업 규모별 모드 분기]` micro 7단계 상세 설명 → 1줄 요약으로 축소
- `buildPrompt1()` micro 분기 맨 앞에 강제 규칙 추가:
  `[1차 호출 절대 규칙] 너는 지금 1차 호출이다. 아래 7개 필드만 JSON으로 출력하고 절대 다른 내용을 추가하지 마라: executiveSummary, lifecycleStage, swot, stp, tam, sam, som`

---

## 최근 수정 이력 (2026-05-24) — micro 3차 구조 재설계 + 1차 JSON 절단 수정 완결

### ⑥ micro 1차 SWOT/STP 키 불일치 수정 (배포 완료) — 커밋 c38cc6a
- **원인**: 1차 JSON 템플릿이 `S/W/O/T` 키, `segment` 키를 사용했으나 dashboard.js는 `strengths/weaknesses/opportunities/threats`, `segmentation` 키를 기대함 → 대시보드 SWOT·STP 렌더링 공백
- **수정**: `buildPrompt1()` micro 분기 JSON 예시 → `strengths/weaknesses/opportunities/threats` + `segmentation`
- **수정**: `_buildPrompt2Micro()` SWOT 참조 → `r1.swot.S[0]` → `r1.swot.strengths[0]` / `r1.swot.W[0]` → `r1.swot.weaknesses[0]`

### ⑤ micro 1차 JSON 절단 수정 (배포 완료) — 커밋 3b6e4be
- **원인**: 1차가 executiveSummary·SWOT(6개씩)·STP·fourP·keyStrategies·specializedAnalysis 전부 요청 → 3000 토큰 초과로 JSON 중간 절단
- **수정**: 1차 출력을 7개 최소 필드만으로 축소 (executiveSummary 3줄, lifecycleStage 1줄, swot 각 1개, stp 1줄씩, tam/sam/som)
- **수정**: `claude-analyze-1.js` `MAX_TOKENS_MICRO` 8000 → 2000
- **수정**: keyStrategies·fourP·specializedAnalysis 전부 2차로 이동

### ④ micro 모드 3차 호출 구조 재설계 (배포 완료) — 커밋 99f824c
- `api/claude-analyze-3.js` 신규: D5~D7 처방 + plan90days (maxDuration:60)
- `vercel.json`: claude-analyze-3.js 등록 (icn1, 60초)
- `_SYSTEM_EXEC_MICRO_2` → D1~D4+KPI+로드맵+전략, `_SYSTEM_EXEC_MICRO_3` → D5~D7+plan90days
- `callClaude()`: micro 3차 호출 + mergedSixSystems(D1~D4+D5~D7) 7개 병합

---

## 최근 수정 이력 (2026-05-24) — micro 모드 504 타임아웃 수정 + 3차 호출 재설계 (이전 기록)

### ① micro 모드 1차 호출 504 타임아웃 근본 수정 (배포 완료) — 커밋 a46eab4
- **원인**: `claude-analyze-1`의 `web_search` tool_use → 검색 → pause_turn/continue 루프 3턴 × ~100초 = 300초 초과
- **수정**: `noSearch: true` 파라미터 → micro 1차에서 web_search 완전 비활성화 → 단일 Anthropic 턴으로 완료
- `api/claude-analyze-1.js`: `noSearch`/`maxTokens` 파라미터 수신 구조 추가
- `ai-engine.js callClaude()`: micro 1차 `{ noSearch: true, maxTokens: 8000 }` 전달

### ② micro 모드 3차 호출 구조 재설계 (배포 완료) — 커밋 99f824c
- **설계**: 1차(전략) + 2차(D1~D4+KPI+로드맵) + 3차(D5~D7+plan90days) 3분할
- `api/claude-analyze-3.js` 신규: micro 전용 3차 함수 (maxDuration:60, max_tokens:4000)
- `vercel.json`: `claude-analyze-3.js` 등록 (icn1, 60초)
- `ai-engine.js`:
  - `_SYSTEM_EXEC_MICRO` → `_SYSTEM_EXEC_MICRO_2`(D1~D4+KPI+로드맵) + `_SYSTEM_EXEC_MICRO_3`(D5~D7+plan90days) 분리
  - `_buildPrompt2Micro()`: D1~D4 집중 프롬프트로 교체
  - `_buildPrompt3Micro()` 신규: D5~D7 처방 + 90일 캘린더 + govSupport
  - `apiCall()`: `'3차'` → `/api/claude-analyze-3` 엔드포인트 추가
  - `callClaude()`: micro 3차 호출 추가 + `mergedSixSystems = [...D1~D4, ...D5~D7]` 7개 병합
  - micro 1차 maxTokens: 8000 → 3000 (응답 시간 단축)
- `wizard.js animateLoading(isMicro)`: micro 모드 로딩 레이블 4단계 동적 변경
  - `생애주기·전략 분석 (1차)` → `D1~D4 경영진단 처방 (2차)` → `D5~D7·정부지원 처방 (3차)` → `보고서 통합 완성 중`
- `app.js`: `animateLoading(data.bizScale === 'micro')` 전달

---

## 최근 수정 이력 (2026-05-23) — 소상공인 전용 AI 7단계 프롬프트 교체

### ① ai-engine.js 소상공인 전용 AI 2차 프롬프트 7단계 구조 (배포 완료) — 커밋 fc09ceb
- `_SYSTEM_EXEC_MICRO` 신규: 소상공인 2차 시스템 프롬프트 (leanCanvas 생략, D1~D7 7항목)
  - `lifecycleStage` + `kpi` + `roadmap(3단계)` + `sixSystems(D1~D7)` + `plan90days` 5개 필드
  - 각 D영역별 action은 무료·저비용 도구 + 소상공인진흥공단 지원사업 중심
- `buildPrompt1` 소상공인 모드 1차 추가 지침: `lifecycleStage` 필드, STP 포지셔닝 맵, executiveSummary 레이블
- `_buildPrompt2Micro()` 신규: 소상공인 전용 2차 사용자 프롬프트
  - D1~D7 처방 지침, 90일 액션 무료도구 우선, govSupport 소상공인진흥공단·지자체 포함
- `buildPrompt2`: micro 시 `_buildPrompt2Micro()` 분기
- `callClaude`: `bizScale === 'micro'` 시 `_SYSTEM_EXEC_MICRO` 사용 (기존 `_SYSTEM_EXEC` 대체)

---

## 최근 수정 이력 (2026-05-22) — 소상공인 7대 영역 진단 v2.0 완성

### ① diagnosis-micro.js 7대 영역 35항목 전면 교체 (배포 완료) — 커밋 ba7d54b
- 기존 3영역 15항목 → 7대 영역 35항목 전면 재설계
- D1 경영진단·손익분석 / D2 점포환경·PLACE SEO / D3 다채널 판로
  D4 스마트DX / D5 운영자금·ESG보증 / D6 사업정리·폐업세무 / D7 SNS·생성형AI
- 출처: knowledge_base/소상공인_오퍼레이션_고도화_및_디지털_전환_DX_을_위한_7대.txt
- 5점 BARS 척도 + ai_trigger + ACTION_PLAN_7DAY 7일 린 액션 연결

### ② cross-context.js 소상공인 교차 트리거 13개 추가 (배포 완료) — 커밋 a533921
- CRITICAL 5개: 프라임코스트+ACM, 계좌혼용+현금위기, BEP미관리, 외식+플레이스, 생활서비스+리뷰
- HIGH 6개: 프랜차이즈+OHI+D2C, DX+오너의존, 디지털고립, 노동법+세무, DX투자낭비, 조리편차+PrimeCost
- MEDIUM 2개: ACM+AI마케팅, 패키징+단일채널
- bizScale 필터 + diag-micro-container_ 키 패턴 추가

### ③ wizard.js bizScale 주입 패치 (배포 완료) — 커밋 dc4e92e
- CrossContext 호출 시 bizScale 4번째 인수로 전달
- detectCrossWarnings(): rule.bizScale 불일치 규칙 skip → micro 규칙 오발동 방지

### ④ CrossContext micro 추가 규칙 4개 (배포 완료) — 커밋 a533921
- HIGH: micro_dx_investment_wasted (DX ROI없음+데이터미활용), micro_ops_margin_collapse (테이블회전율+PrimeCost)
- MEDIUM: micro_acm_marketing_mismatch (ACM분류+AI콘텐츠), micro_packaging_channel_gap (패키징+단일채널)
- 총 CROSS_RULES: 34개, micro 규칙 13개

### ⑤ DiagMicro 7대 분야 진단 탭 렌더링 연결 (배포 완료, 테스트 완료) — 커밋 720b3de
- index.html: diagTab-common 안에 diag-micro-container 추가 (hidden 기본)
- wizard.js loadDiagnosisUI(): bizScale 감지 → isMicro=true 시 diag-micro-container 렌더, diag-common-container 숨김
- wizard.js _diagMicroToAreas(): DiagMicro.getSchema() → renderDiagModule 호환 포맷 변환
- 탭 레이블 → '🏪 소상공인 7대 분야 (35문항)', score key = diag-micro-container_X_Y → calcScores() 정상 연동

---

## 최근 수정 이력 (2026-05-19 오후) — XBRL 재무분석 개선 4건

### ① XBRL 계정명 매칭 강화 + 분기보고서 rcept_no 수정 (배포 완료) — 커밋 f8d498f
- `_parseXbrl()` 계정명 후보 추가: `Cash`, `FinishedGoods`, `Merchandise`, `BorrowingsFromFinancialInstitutions`, `LongTermBorrowings`, `CostOfRevenues`
- `_supplementWithXbrl()` 분기/반기 키워드 분기 추가 — 기존에 항상 `사업보고서` rcept_no만 사용하던 버그 수정
- 분기: `분기보고서` 키워드로 해당 연도 제출 파일 우선 탐색, 없으면 `사업보고서` fallback

### ② XBRL 파일명 기준 연도 자동 보정 (배포 완료) — 커밋 255087a
- 근본 원인: 분기→사업보고서 fallback 시 `_parseXbrl(xml, 2026)` 호출하지만 실제 XBRL은 `entity_2025-12-31.xbrl` → 연도 불일치 → 전부 null
- 수정: `xbrlEntry.entryName.match(/(\d{4})-\d{2}-\d{2}/)` 로 파일명에서 연도 추출 → `_parseXbrl(xml, xbrlYear)` 사용

### ③ 당좌자산 파생계산 + 전년도매출액 자동입력 (배포 완료) — 커밋 846473d
- 당좌자산: IFRS 기업은 별도 항목 없음 → `유동자산 - 재고자산` 파생 계산 자동 주입
- 전년도매출액: `getPrev()` 함수 추가 (`frmtrm_amount` 전용) + `prevRevenue` 필드 → `finance-wizard.js` `_setField('fin_prev_revenue')` 연결

### ④ /fix-dart 스킬 업데이트 — 커밋 ea487eb
- 오늘 발견된 4개 패턴 추가: XBRL year 불일치 / 분기 rcept_no 버그 / 당좌자산 파생 / 전년도매출액 매핑

---

## 최근 수정 이력 (2026-05-19 오전) — CrossContext 한글-영문 ID 매핑 버그 수정

### ① cross-context.js — detectCrossWarnings() ID 정규화 추가 (배포 완료)
- `data.bizModel`은 한국어 레이블(`'프랜차이즈'`)을 저장하지만 CROSS_RULES는 영문 ID(`'franchise'`) 사용 → 항상 매칭 실패하던 근본 버그 수정
- `BM_ID_MAP` (12개 BM), `INDUSTRY_ID_MAP` (16개 업종) 룩업 테이블을 `detectCrossWarnings()` 진입부에 추가
- 함수 시작에서 `industryId = INDUSTRY_ID_MAP[industryId] || industryId` 정규화 → `buildPromptSummary()`도 내부 호출이므로 자동 적용
- 커밋: `9f67067`

### ② wizard.js — CrossContext 블록 fallback 체인 확장 + BM 프록시 주입 (배포 완료)
- `industryId` 추출: `data.industryKey || data.industry || data.industryName || ''`
- `bmId` 추출: `data.bizModel || data.bm || data.bizModelName || ''`
- `TAB_ORDER = ['common', 'industry']`로 BM 탭 제거 → `diag-bm-container_*` DOM 요소 없음 → BM 트리거 키 점수 수집 불가 버그 수정
- BM 탭 없을 때 common 도메인 평균값으로 BM 프록시 주입: area1·2 → domain3(BM역량), area3 → domain2(인력운영), area4 → domain4(미래역량)
- 커밋: `9f67067`

---

## 최근 수정 이력 (2026-05-18) — BM 진단 시스템 v2.0 완성 + 렌더링 버그 수정

### ① 나머지 BM 진단 파일 9개 v2.0 전면 교체 (배포 완료)
- 교체 파일: `b2b_saas.js` · `b2c_sub.js` · `b2b_solution.js` · `b2c_commerce.js` · `platform.js` · `franchise.js` · `mfg_dist.js` · `service.js` · `etc.js`
- 신규 포맷: `BM_XXX` 변수명, 4 areas × 4 items, `scale:[{score:1~5,desc}]`, `ai_trigger`, `ai_analysis` 배열
- 커밋: `998087b`

### ② ai-engine.js BM 변수명 통일 (배포 완료)
- `bizModelVarMap` 내 `BIZMODEL_XXX` → `BM_XXX` 12개 전체 교체
- 커밋: `233726b`

### ③ wizard.js v2.0 포맷 렌더링 5개 버그 수정 (배포 완료)
- `_diagCommonToAreas()` 헬퍼 추가 — DiagCommon.getSchema() 결과를 renderDiagModule 호환 포맷으로 변환
- `loadDiagnosisUI()` — COMMON_DIAGNOSIS → DiagCommon 우선 분기 추가
- `renderDiagModule` — `data.label||data.title`, `area.label||area.title` fallback 추가
- `_renderItemHtml` — `item.question||item.label||item.text` fallback 추가
- `_scaleToAnchors()` 헬퍼 추가 — `scale:[{score,desc}]` → `anchors:{1:'...',5:'...'}` 변환
- 커밋: `04f1749`

### ④ cross-context.js v2.0 전면 교체 (배포 완료)
- CrossContext v2.0 IIFE 모듈 — 20개 CROSS_RULES (업종×BM 핵심 조합)
- CRITICAL 9개 규칙: restaurant×franchise, export_sme×usage_based, medical×advertising 등
- HIGH 11개 규칙: knowledge_it×b2b_saas, fashion×b2c_commerce, logistics×platform 등
- Public API: `detectCrossWarnings(industryId, bmId, diagScores)` · `buildPromptSummary()` · `getRulesFor()` · `getStats()`
- 커밋: `1050f44`

---

## 최근 수정 이력 (2026-05-17) — 진단 모듈 v2.0 전면 재설계

### ① 규모별 전용 진단 모듈 신규 구축 (배포 완료)
- `js/diagnosis/common.js` v2.0 전면 개편 — DiagCommon IIFE, 5 domains × 4 items, `buildPromptSummary()` 추가
- `js/diagnosis/diagnosis-micro.js` 신규 — DiagMicro IIFE, 소상공인(bizScale='micro') 전용
  - 3 도메인: 로컬SEO(0.35) · 메뉴엔지니어링(0.35) · 위임시스템(0.30)
  - ACTION_PLAN_7DAY, calcScores, detectCrossWarnings, buildPromptSummary, getSchema
- `js/diagnosis/diagnosis-sme.js` 신규 — DiagSme IIFE, 소기업(bizScale='sme') 전용
  - 4 도메인: process_cost(0.28) · backoffice_dx(0.25) · org_talent(0.22) · scaleup_radar(0.25)
  - G_1~G_6 Scale-up 6대 축 radar (S_SEO, S_AI, S_CF, S_PCR, S_SD, S_ESG)

### ② 연동 수정 3파일 (배포 완료)
- `index.html` — script 태그에 diagnosis-micro.js · diagnosis-sme.js 추가
- `wizard.js collect()` 말미 — bizScale 감지 후 DiagMicro/DiagSme 호출, `data.microPrompt`/`data.smePrompt` 주입
- `ai-engine.js buildPrompt1()` — `return \`` → `let prompt = \`` 구조 변경 후 common/micro/sme 프롬프트 append

### ③ 업종 진단 파일 v2.0 전면 교체 (배포 완료)
- 교체 파일: `logistics.js` · `energy.js` · `agri_food.js` · `export_sme.js`
- 신규 포맷: top-level `label/icon/description` + 4 areas × 4 items + `ai_analysis` 배열
- `scale:[{score:1~5, desc}]` 구조, `ai_trigger:{threshold, warning}` 항목별 AI 경보 설계
- `window.INDUSTRY_XXX` + `module.exports` 듀얼 익스포트

### ④ BM 진단 파일 v2.0 전면 교체 (배포 완료)
- 교체 파일: `usage_based.js` · `advertising.js` · `deeptech.js`
- 변수명 `BIZMODEL_XXX` → `BM_XXX` 통일, 신규 포맷 동일 적용
  - `BM_USAGE_BASED`: 종량제 (과금 설계·사용자 확장·Margin·이탈관리 4영역)
  - `BM_ADVERTISING`: 광고 기반 (오디언스·인벤토리·광고주·UX균형 4영역)
  - `BM_DEEPTECH`: 딥테크·바이오 (IP·R&D·자금조달·사업화 4영역)

---

## 다음 세션 예정 작업

### 1순위: 소상공인 AI 분석 end-to-end 실전 테스트 (⚠ 테스트 대기 중)
- biznavi.vercel.app에서 소상공인 모드로 실제 분석 1회 실행
- 1차: 7개 필드만 응답 확인 (executiveSummary·lifecycleStage·swot·stp·tam·sam·som), max_tokens 초과 없어야 함
- 2차: keyStrategies·fourP·D1~D4 + KPI(5개) + 로드맵(3단계) 정상 출력 확인
- 3차: D5~D7 + plan90days(3개월, govSupport 포함) 정상 출력 확인
- 대시보드: renderSixSystems 7항목(D1~D7) 카드 정상 표시 확인
- SWOT strengths/weaknesses/opportunities/threats 렌더링 확인
- 로딩 스텝 레이블 micro 4단계 정상 표시 확인

### 2순위: reference-db.js 데이터 업그레이드
- `knowledge_base/industry_full_benchmarks.csv` 기준 수치 갱신
- `small_biz_cost_guide.csv` 소상공인 비용 구조 프롬프트 반영

### 3순위: 진단 시스템 end-to-end 실전 테스트
- biznavi.vercel.app에서 업종×BM 조합 3종 이상 실제 진단 실행
- v2.0 렌더링(scale → BARS) 정상 출력, cross-context 경보 카드 표시 확인

---

## 최근 수정 이력 (2026-05-15) — DART 분기보고서 탐색 + 대시보드 버그 수정

### ① 핵심전략 섹션 누락 버그 수정 (배포 완료)
- **원인**: `bizScale` 숨김 input이 OCR 콜백에서만 세팅 → 일반 입력 시 항상 `''` → AI 프롬프트에 `bizScale: '미입력'` 전달 → micro 모드 출력(sixSystems 중심), keyStrategies 누락
- **수정**: `wizard.js collect()` — `bizScale` 자동 추론 추가
  ```js
  const explicit = g('bizScale') || g('bizScaleSelect');
  if (explicit) return explicit;
  const emp = g('employees');
  return (!emp || emp === '1~5명') ? 'micro' : 'sme';
  ```

### ② BM역량 항상 빈칸 버그 수정 (배포 완료)
- **원인**: `TAB_ORDER = ['common', 'industry']`로 bizmodel 탭 제거 후 `diag-bizmodel-container_*` 키가 존재하지 않아 `domains.bm.scores = []` → 0점 표시
- **수정**: `wizard.js calcDomainScores()` — `3_2`(차별화 요소) 점수를 BM역량 proxy로 공유
  ```js
  } else if (key === 'diag-common-container_3_2' || key.startsWith('diag-common-container_5_')) {
    domains.differentiation.scores.push(s);
    domains.bm.scores.push(s); // bizmodel 탭 제거 보완
  ```

---

## 오늘 작업 (2026-07-18)

- 작업 요약:
  - `js/diagnosis/industry/social_enterprise.js` 추가 — 사회적기업 진단 모듈
  - `js/diagnosis/industry/social_venture.js` 추가 — 소셜벤처 진단 모듈
  - `js/wizard.js`에 `INDUSTRY_MAP`, `INDUSTRY_BM_MAP`, `BIZ_TYPE_MAP` 항목 및 `industryVarMap` 등록 추가
  - `index.html`에 두-컬럼 레이아웃(`aside#chatPane` / `main.wizard-content`) 및 스크립트 태그 추가
  - `css/style.css`에 채팅 패널 스타일 추가
- 배포:
  - GitHub에 커밋 및 `main` 브랜치로 푸시됨
  - Vercel로 자동 배포 트리거됨 — https://biznavi.vercel.app 에서 파일 서빙 확인됨
- 확인 방법:
  - 브라우저 DevTools Console에서 아래 한 줄 실행:
    console.log(!!window.INDUSTRY_SOCIAL_ENTERPRISE, !!window.INDUSTRY_SOCIAL_VENTURE, document.getElementById('aiIndustryKey')?.value);
- 비고:
  - 채팅은 현재 플레이스홀더이며, 백엔드 AI 연동은 별도 작업 필요


### ③ DART 최신 보고서 자동 탐색 구현 (배포 완료)
- `api/dart-lookup.js` — `REPRT_CODE_MAP` 추가, 명시적 6단계 탐색 순서로 교체
- `finData`에 `reprtCode`, `reprtName` 저장 → result 객체에 포함
- `_supplementWithXbrl()` — `reprtCode` 파라미터 추가, XBRL URL에 적용
- `js/finance-wizard.js` — DART 결과 상단에 `📋 2025년 사업보고서 기준` 초록 배지 표시
- `css/style.css` — `.dart-reprt-badge` 스타일 추가 (초록 계열)

### ④ DART 탐색 순서 재설계 (배포 완료)
- **배경**: 이전 구조는 `bsns_year=2026` 전체(8호출) 실패 후 2025 연간만 fallback → "최신 분기 우선" 의도가 이전 연도에 미적용
- **DART `bsns_year` 의미**: 보고 대상 회계연도 (제출 연도 아님) — 2026 Q1은 오늘(5/15) 마감, 나머지 2026 보고서는 미래
- **새 탐색 순서**:
  1. `bsns_year=2026, 11013` — 오늘 마감 Q1, 있으면 최신
  2. `bsns_year=2025, 11011` — 완성도 최우선 연간 (대부분 여기서 결정)
  3. `bsns_year=2025, 11014` — 3분기
  4. `bsns_year=2025, 11012` — 반기
  5. `bsns_year=2025, 11013` — 1분기
  6. `bsns_year=2024, 11011` — 최후 fallback
- **효과**: 불필요 API 호출 8개 → 최대 2개(CFS/OFS)로 감소

---

## 최근 수정 이력 (2026-05-13) — AI 버그 수정 + knowledge_base 구축

### ① AI 분석 end-to-end 버그 수정 (배포 완료)
- C-3: `api/claude-analyze-1.js` `stop_reason === 'max_tokens'` 시 500 에러 반환
- W-4: `api/claude-analyze-2.js` 동일 처리 (로그만 찍던 것 → 500 에러 반환)
- C-2: `dashboard.js:491` `(data.executiveSummary || '').replace(...)` null 가드
- W-1: `ai-engine.js:1497` `digital_strategy` 템플릿 리터럴 백틱 수정 (`'..${co}..'` → `` `..${co}..` ``)

### ② pause_turn 핸들링 추가 (배포 완료)
- `api/claude-analyze-1.js`: `stop_reason === 'pause_turn'` 시 `'continue'` 전송 후 루프 재진행
- stop_reason 처리 체계 완성: `end_turn` / `max_tokens` / `pause_turn` / `tool_use`

### ③ W-2·W-3·W-5·S-2 수정 (배포 완료)
- W-2: `js/ai-engine.js` growth/structure/innovation/cx_strategy kpi 8개 → 10개 확장
- W-3: `js/wizard.js:1079` validate(2) `|| 13` fallback 제거 + DOM 0개 시 alert 가드
- W-5: `js/wizard.js:1930` dDay 음수 → "마감" 뱃지, 0 → "D-Day" 뱃지 처리
- W-5: `css/style.css` `.bizinfo-dday.expired` 회색 뱃지 스타일 추가
- S-2: `api/claude-analyze-1.js`, `claude-analyze-2.js` `[TIMING]` console.log + `T_START` 전부 제거

### ④ knowledge_base 폴더 구축 (배포 완료)
- `knowledge_base/12개사업모델진단.pdf` — 12개 BM 진단 설계 원본
- `knowledge_base/16개업종진단.pdf` — 16개 업종 진단 설계 원본
- `knowledge_base/industry_full_benchmarks.csv` — 18개 업종 재무 벤치마크 (영업이익률·부채비율 등)
- `knowledge_base/small_biz_cost_guide.csv` — 10개 업종 소상공인 비용 구조 (임대료·인건비·원가 비중)
- CLAUDE.md 작업 규칙: "AI 진단 로직 수정 시 → knowledge_base PDF 우선 참조" 추가

---

## 최근 수정 이력 (2026-05-13 오후) — 실전 테스트 후 UI/UX 개선

### ① 대시보드 상단 헤더 가려짐 수정 (배포 완료)
- `css/dashboard.css:7` `#dashboard` `padding-top: 40px` → `80px`
- 원인: ID 선택자 특이도가 `.has-nav` 클래스 선택자보다 높아 네비(60px) 덮어쓰기 → 20px 겹침
- 수정: 네비 60px + 여백 20px = 80px 확보

### ② Executive Summary 가독성 개선 (배포 완료)
- `js/ai-engine.js` SYSTEM 프롬프트 `[executiveSummary 출력 포맷 지침]` 섹션 추가
  - 전문 경영 용어 단독 사용 금지 → 쉬운 설명으로 직접 서술
  - 5개 레이블 구조 강제: `[운영현황]` `[핵심위험]` `[차별화포인트]` `[시장기회]` `[즉시과제]`
  - 각 항목에 구체적 수치·근거 포함 의무화
- `js/dashboard.js` execSummary 렌더링에 `[레이블]` 패턴 → `.es-label` bold 강조 추가
  - `.replace(/\[([^\]]+)\]/g, '<strong class="es-label">[$1]</strong>')`
- `css/dashboard.css` `.es-label { color: var(--gold); font-weight: 700; }` 추가

---

## 다음 세션 예정 작업

### 1순위: Executive Summary 개선 결과 실전 테스트
- 5개 레이블 구조 정상 출력, 골드 bold 강조 확인

### 2순위: reference-db.js 데이터 업그레이드
- `js/reference-db.js` 벤치마크 수치를 `knowledge_base/industry_full_benchmarks.csv` 기준으로 갱신
- 소상공인 비용 구조 데이터 (`small_biz_cost_guide.csv`) 프롬프트 반영 검토

### 3순위: CORS 보안 강화
- API 키 노출 방지 추가 검토

---

## 최근 수정 이력 (2026-05-12 추가) — QA·CSS 정리

### ① print.css .rpt-cover 정리 (배포 완료)
- `break-after: page` → `break-after: avoid` (JS 주입과 일치)
- `height: 257mm; box-sizing: border-box` CSS에 명시 추가
- `print.css?v=20260506a` → `?v=20260512a` 버전 갱신

### ② --teal CSS 변수 제거 (배포 완료)
- `style.css:18` `--teal: #F5C030` 정의 삭제
- `dashboard.css` 6곳 `var(--teal)` → `var(--gold)` 치환 (시각 변화 없음 — 동일 색상)

### ③ QA CRITICAL 수정 — bizinfo industryKey 버그 (배포 완료)
- `app.js:190` `industryKey: data.industry || ''` → `data.industryKey || data.industry || ''`
- 근본 원인: `id="industry"` select 제거 후 `data.industry`가 항상 `''` → 기업마당 업종 매칭 무력화
- QA WARNING 2건(CORS 와일드카드·tool_result 빈 배열)은 즉시 수정 보류

---

## 최근 수정 이력 (2026-05-12) — 생존율 인사이트 구현 (4순위 D)

### ① INDUSTRY_CLOSURE_CAUSES 상수 추가
- 16개 업종 × 3도메인(재무·인력·BM) 폐업 원인 상수 추가
- js/ai-engine.js 또는 js/pattern-db.js에 위치

### ② _buildSurvivalInsights() 헬퍼 함수 추가 (+97줄)
- 시나리오 1·2·3·6 통합 처리
- 인사이트 예시 (외식업·창업 8년·재무 2.1점):
  - 3년 생존율 31.2% — 전체 평균(39.6%) 대비 21% 낮은 고난이도 업종
  - 업력 8년 — 5년 생존율 21% 기준 상위 생존 코호트 진입 확인
  - ⚠ 생존 역설 경고: 7년 이상 운영 + 고위험 업종 조합
  - 재무역량(2.1점) — "식재료비·임대료 현금흐름 압박" 폐업 원인과 직접 일치
  - ⚠ 긴급 생존 모드 강제 적용

### ③ 섹션13 교체 (js/ai-engine.js — buildPrompt1())
- 기존 22줄 KOSIS 블록 → _buildSurvivalInsights() 1줄 호출로 단순화
- 기존 긴급 모드 블록 내부 통합 완료

### ④ app.js
- 수정 불필요 (167줄에 data.domainScores = _domScores 이미 존재)

---

## 최근 수정 이력 (2026-05-09) — extractJSON 개선 + 스킬 시스템 구축

### ① extractJSON 4단계 파싱 + trailing comma 수리 (배포 완료)
- `repairJSON()`: Claude가 자주 생성하는 trailing comma 자동 제거
- `extractJSON()`: 직접파싱 → 수리 → 코드블록그리디 → 코드블록수리 4단계
- `claude-analyze-2.js`: stop_reason + output_tokens 로깅 추가

### ② 2차 JSON 파싱 실패 해결 (배포 완료)
- 원인: 대규모 기업(중소기업급) 분석 시 kpi+roadmap+sixSystems 합산 토큰 초과
- claude-analyze-2 max_tokens: 12000 → 16000

### ③ BizNavi 반복 오류 스킬 시스템 구축 (배포 완료)
- `.claude/commands/` 폴더 신규 생성
- 7개 스킬 파일 생성 (47개 과거 버그 패턴 수록)

| 스킬 | 대상 오류 |
|------|----------|
| `/fix-ai-error` | AI 분석 504·JSON 파싱 실패 |
| `/fix-ai-engine` | AI 엔진 로직 버그 (키 충돌·consultingType 등) |
| `/fix-wizard` | 위저드 화면전환·버튼 버그 |
| `/fix-dart` | DART 조회·재무분석 버그 |
| `/fix-pdf` | PDF 표지·출력 버그 |
| `/qa` | qa-reviewer 에이전트 검증 호출 |
| `/wrap-up` | 세션 마무리 일괄 처리 |

---

## 최근 수정 이력 (2026-05-08) — 504 타임아웃 근본 해결 + 시스템 프롬프트 QA

### ① _SYSTEM_EXEC 2차 호출 동기화 (배포 완료)
- `const _SYSTEM_EXEC` 페르소나: "맥킨지·BCG 20년" → "30년 카드/금융 경영지도사" (SYSTEM과 일치)
- `_SYSTEM_EXEC` keyMetrics 슬롯: 재방문율(인과사슬 1축)·ROAS(인과사슬 2축) 필수 포함 지시 추가
- qa-reviewer 검증 결과 WARNING 2건 수정

### ② vercel.json maxDuration 전체 정비 (배포 완료)
- `bok-avg`, `biz-lookup`, `analyze-biz`, `kosis-survival`, `bizinfo`: 기본 10초 → 30초
- `claude-analyze` 계열: Pro 플랜 확인 후 **60 → 300초**로 상향

### ③ claude-analyze 1차·2차 함수 분리 — 504 근본 해결 (배포 완료)

#### api/claude-analyze-1.js (신규)
- 1차 호출 전용: executiveSummary·SWOT·STP·4P·keyStrategies·specializedAnalysis
- web_search 1회 포함 (실시간 업종 트렌드·정부지원사업 조회)
- maxDuration: 300초, max_tokens: 16000

#### api/claude-analyze-2.js (신규)
- 2차 호출 전용: kpi·roadmap·sixSystems·plan90days·leanCanvas
- web_search 없음 (실행플랜은 1차 전략 기반 내부 생성)
- maxDuration: 300초, max_tokens: 12000

#### js/ai-engine.js
- `apiCall(_callLabel)`: `'1차'` → `/api/claude-analyze-1`, `'2차'` → `/api/claude-analyze-2`
- 각 호출 왕복 시간 `[TIMING]` 로그 추가

#### 근본 원인 요약
- 기존 `claude-analyze.js` 단일 함수에서 1차+2차 순차 실행 → 합산 60초 초과
- Vercel Pro 플랜임에도 `maxDuration: 60` 고정이어서 Pro 한도(300초) 미적용 상태였음
- 함수 분리 + maxDuration 300 적용으로 완전 해결

### ④ 1차 JSON 파싱 실패 해결 (배포 완료)
- 원인: 새 시스템 프롬프트(30년 경력 노하우 5개 섹션) 추가로 응답 생성 공간 부족 → JSON 중간 절단
- claude-analyze-1: max_tokens 8000 → 16000
- claude-analyze-2: max_tokens 8000 → 12000

---

## 최근 수정 이력 (2026-05-07) — QA 버그 수정 + fakeAnalysis 완성

### ① apiKey UI 완전 제거 — 서버 환경변수 전환 후속 정리 (배포 완료)

#### js/app.js
- `mode`, `apiKey` 상태 변수 제거 (localStorage 의존 제거)
- 제거된 함수: `showModal()`, `closeModal()`, `setMode()`, `confirmKey()`, `saveApiKey()`, `showApiModal()`, `fillSavedKey()`
- `runAnalysis()`: 항상 `AIEngine.callClaude(data)` 직접 호출 (`mode === 'demo'` 분기 제거)
- `_pendingIsDemo`: 에러 fallback 시에만 `true` (이전: demo 모드 시 true)
- 진단 이력 저장: demo 여부 무관하게 항상 `HistoryTracker.save()` 호출

#### index.html
- `apiModal` div 전체 제거 (API 키 입력 모달)
- STEP 4 `wiz-api-box` 제거 (API 키 입력란 + 확인 버튼)
- 위저드 nav "⚙ API 설정" 버튼 + `nav-actions` wrapper 제거

#### js/wizard.js
- `App.fillSavedKey` 호출 제거 (goStep 내)

---

### ② industryVarMap 키 영문화 + 4개 업종 추가 (배포 완료)

#### js/ai-engine.js — buildInsightsSummary()
- **근본 버그**: 기존 12개 항목이 한국어 키('제조업', '식품/음료')로 등록되어 있었으나 함수 호출 시 영문 키(aiIndustryKey: 'mfg_parts', 'logistics')가 전달 → 항상 미매칭 (dead code)
- **수정**: 전체 16개 항목을 영문 industryKey로 교체
  ```
  'mfg_parts', 'food_mfg', 'local_service', 'wholesale', 'restaurant',
  'knowledge_it', 'construction', 'medical', 'finance', 'education',
  'fashion', 'media', 'logistics', 'energy', 'agri_food', 'export_sme'
  ```
- bizModelVarMap은 Korean 키 그대로 유지 (d.bizModel이 한국어 레이블이므로 정상)

---

### ③ Object.assign 키 충돌 방지 (배포 완료)

#### js/ai-engine.js — callClaude() 2차 호출 병합
- **문제**: `Object.assign({}, result1, result2)` → 2차 응답이 1차 핵심 키(SWOT·전략 등)를 덮어씀
- **수정**: 1차 전용 키 보호 후 병합
  ```js
  const FIRST_PASS_KEYS = ['executiveSummary', 'swot', 'stp', 'fourP', 'keyStrategies', 'specializedAnalysis'];
  const r2Clean = Object.fromEntries(
    Object.entries(result2).filter(([k]) => !FIRST_PASS_KEYS.includes(k))
  );
  return Object.assign({}, result1, r2Clean);
  ```

---

### ④ fakeAnalysis 4개 컨설팅 유형 데이터 완성 (배포 완료)

#### js/ai-engine.js — _fakeByConsultingType()
기존 `return {}` 상태였던 4개 유형 완전 구현:

| 유형 | 핵심 전략 방향 |
|------|---------------|
| `growth_strategy` | LTV 극대화, 신규 채널 개척, 단가 상향, 반복 매출 모델 |
| `structure_strategy` | SOP 매뉴얼화, R&R 위임, 채용·온보딩, 대표 의존도 감소 |
| `innovation_strategy` | 고객 불만 기반 혁신, 린 MVP 검증, 파트너십, 정부 R&D 연계 |
| `cx_strategy` | 고객 여정 맵, NPS 측정, 온보딩, 컴플레인 기준, 재구매 루틴 |

각 유형 구조: keyStrategies×6(`[진단][방침][행동]`) + kpi×8 + sixSystems×6(상태 배지+액션 3+지원사업) + plan90days×3

---

### ⑤ 에이전트 도구 추가 (배포 완료)

#### .claude/agents/fullstack-engineer.md
- tools에 `WebSearch`, `WebFetch` 추가
- 이유: Vercel API 변경사항·npm 패키지 문서·Node.js 버전 호환성 등 외부 레퍼런스 조회 대응

---

## 최근 수정 이력 (2026-05-06) — 재무분석 PDF 리포트 완성

### ① NICE BizINFO 수준 재무분석 보고서 출력 구현 (배포 완료)

#### js/finance-wizard.js — renderReport() 전면 재작성
- 기존 단순 테이블 → NICE BizINFO 스타일 10개 섹션 구조
- `sHdr(num, title, sub)`: 섹션 헤더 헬퍼 (번호 배지 + 제목 + 영문 부제)
- `aBlock(areaKey)`: 평가(현황) → 진단(원인) → 처방(대안) 3단 서술 블록
- `_buildOpinionPara(areaKey, r, d)`: 5개 비율군별 자동 서술 문단 생성
  - 유동성·안전성·수익성·활동성·성장성 각각 비율값·산업평균 비교 → 진단 텍스트 자동 생성
- `_buildFinalOpinion(r, d, sc, score, gradeLbl)`: 종합 의견 문단 (등급·핵심 취약점·개선 방향)
- `_buildImprovements(r, d)`: 우선순위별 개선 권고 항목 리스트
- 10개 섹션: 표지·재무현황요약·재무상태표·손익계산서·유동성·안전성·수익성·활동성·성장성·종합의견

#### css/style.css — nice-* 클래스 ~180줄 추가
- `.nice-sec-hdr`, `.nice-sec-num`: 섹션 헤더 (네이비 번호 배지)
- `.nice-kv-grid`, `.nice-kv-card`: 재무현황 카드 그리드
- `.nice-score-bar`: 종합 점수 바
- `.nice-def-table`, `.nice-ratio-table`: 비율 정의·비교 테이블
- `.nice-analysis`, `.nice-analysis-row`: 평가/진단/처방 서술 블록

---

### ② PDF 표지 2페이지 넘침 완전 수정 (배포 완료) — 5개 원인 동시 해결

#### 원인 분석
1. **print.css 캐시** — 버전 파라미터 없어 브라우저가 구버전 서빙 → `?v=20260506a` 추가
2. **height 수학 버그** — `height:255mm + padding:8mm×2 = 271mm > 259mm(A4)` → `box-sizing:border-box + height:257mm`
3. **style.css 충돌** — `@media print .rpt-cover { background:#1A2340 !important }` 잔재 → 삭제
4. **캐시 근본 해결** — `printPdf()`에서 `window.print()` 직전 `<style>` 태그 DOM 직접 주입 (캐시 완전 무력화)
5. **이중 페이지브레이크** — `.rpt-cover { break-after:page }` + `.rpt-page { break-before:page }` 동시 적용 → `break-after:avoid`로 변경

#### 표지 HTML inline 스타일 → CSS 클래스 전환
- 기존: `<div style="color:#F5C030;...">` 형태 (print에서 색상 오버라이드 불가)
- 수정: `.rpt-cover-logo`, `.rpt-cover-tagline`, `.rpt-cover-subtitle`, `.rpt-cover-company`, `.rpt-cover-ind` 클래스 사용
- print.css에 각 클래스 인쇄용 색상 명시 (흰 배경 대응)

---

## 최근 수정 이력 (2026-04-30) — 5대 외부 데이터 연동 완료

### ① 통계청 KOSIS 업종 생존율 연동 (배포 완료)

#### api/kosis-survival.js (신규)
- 기업생멸행정통계 2022 확정치 16개 업종 fallback 내장
- KOSIS_API_KEY 선택 (없으면 fallback 자동 동작)
- 반환: `{ y1, y3, y5, risk('폐업 고위험'/'주의 필요'/'상대적 안정'), name, source }`
- INDUSTRY_TO_KSIC: BizNavi 16개 업종 키 → KSIC 대분류 코드 매핑

#### 연동 흐름
- `app.js` runAnalysis()에서 `Promise.allSettled` 병렬 선조회 (AI 호출 전)
- `data.survivalData` → `buildPrompt1()` 섹션13에 KOSIS 블록 주입 → SWOT 위협·전략에 반영
- `wizard.js` showDiagReveal() → `#drSurvivalBox` 카드 렌더링 (diag-reveal 화면)

---

### ② 기업마당 정부지원사업 실시간 매칭 (배포 완료)

#### api/bizinfo.js (신규)
- 10개 curated fallback 프로그램 내장 (소상공인스마트화·경영컨설팅·창업도약패키지·정책자금 등)
- BIZINFO_API_KEY 선택 (없으면 fallback 자동 동작)
- scoreProgram(): 업종 키워드 + 규모 매칭 → 관련도 점수 → 상위 5개 반환
- 반환: `{ programs[{ name, amount, dday, url, summary, score }] }`

#### 연동 흐름
- runAnalysis()에서 KOSIS와 동시 선조회 (Promise.allSettled)
- `data.bizinfoPrograms` → `buildPrompt1()` 섹션11에 기업마당 블록 주입 → 로드맵 1단계에 반영
- `wizard.js` showDiagReveal() → `#drBizinfoBox` 지원사업 카드 렌더링

---

### ③ 재무 시뮬레이션 엔진 — BEP·현금흐름 계산기 (배포 완료)

#### js/bep-simulator.js (신규)
- `BepSim.init(d)`: 재무분석 결과로 초기값 자동 설정
  - monthlyRev = revenue/12, varRatio = costOfSales/revenue (0.3~0.9 클램핑)
  - fixedCost = laborCost/12 + revenue/12×0.15 (DART 미제공 항목 추정)
- 5개 range 슬라이더 실시간 연동 (월매출·변동비율·고정비·초기투자·현금잔액)
- 4개 결과 카드: BEP 금액·달성률·런웨이·월순이익 (수준별 색상 bep-pos/warn/neg)
- 6개월 현금흐름: 테이블 + Canvas 바 차트 (양=초록, 음=빨강)
- `finance-wizard.js`에서 대시보드 렌더 후 `BepSim.init(d)` 호출

---

### ④ 진단 패턴 DB — 소상공인 실태조사 기반 (배포 완료)

#### js/pattern-db.js (신규)
- PatternDB 모듈: `match()`, `renderDiagReveal()`, `buildPromptBlock()`
- 4대 아키타입 (도메인 점수 조건 기반):
  - `finance_crisis`: finance < 2.5 → 34.2% 비중, 52.8% 3년 폐업률
  - `market_weak`: (bm+diff)/2 < 2.5 → 28.7% 비중, 44.1% 폐업률
  - `growth_stall`: 평균 2.5~3.5 → 29.4% 비중, 28.3% 폐업률
  - `strength_build`: 평균 ≥ 3.5 → 7.7% 비중, 12.1% 폐업률
- INDUSTRY_CONTEXT: 16개 업종별 평균월매출·BEP범위·주요폐업트리거·디지털효과·성공시그널
- AGE_CONTEXT: 창업기(0~2년)·성장기(3~5년)·안정기(6~10년)·확장기(10+년)
- `renderDiagReveal()`: `#drPatternBox`에 아키타입 배지 + 동종업 통계 + 상위3 액션 렌더링
- `buildPromptBlock()`: AI 프롬프트 섹션12 주입

---

### ⑤ 분기별 진단 이력 추적 — localStorage (Firebase 마이그레이션 대비) (배포 완료)

#### js/history-tracker.js (신규)
- HistoryTracker 모듈: `save()`, `loadPrev()`, `loadAll()`, `renderCompare()`, `renderPanel()`
- `_storage` 인터페이스 추상화 → Firebase 교체 시 이 객체만 Firestore 호출로 교체
- 스냅샷 구조: `{ id, savedAt, quarter(YYYY-Qx), company, domainScores(flattened avg), consultingType, topIssues, execSummary(200자) }`
- 최대 20개, 같은 분기+회사명 → 덮어쓰기
- `renderCompare()`: `#drHistoryBox` 이전 분기 대비 도메인 ▲▼ 비교 바 + 컨설팅 유형 변화
- `renderPanel()`: 우상단 "📋 이력" 버튼 → 슬라이드 드로어 (회사별 그룹, 미니 바, 이슈 태그)

#### app.js 추가
- 실제 AI 분석 완료 후 자동 저장: `HistoryTracker.save(data, result)` (데모 제외)
- `openHistory()` / `closeHistory()`: 드로어 오버레이 토글

#### 공통 인프라 수정
- `js/ai-engine.js`: buildPrompt1()에 섹션 11(기업마당)·12(패턴DB)·13(KOSIS) 추가
- `vercel.json`: kosis-survival.js·bizinfo.js 함수 등록 (icn1 리전)
- `css/style.css`: surv-*·bizinfo-*·bep-*·pat-*·hist-* 스타일 일괄 추가

---

## 최근 수정 이력 (2026-04-30) — 히어로·UI

### 히어로 가독성 강화 — 마스터카드 코리아 스타일 (배포 완료)

#### js/hero-bg.js 오버레이 강화
- 방향: `160deg` 평면 → `125deg` 대각선 (좌하단→우상단)
- 강도: 텍스트 영역(좌) .94 완전 불투명 → 우측 .50 사진 노출
- 효과: 텍스트는 완전 보호, 우측에 사진 배경 일부 시각적 노출

#### css/landing.css 히어로 텍스트 개선
- `.lp-hero-h1`: `#F0F2F8` → `#FFFFFF` + `text-shadow:0 2px 8px rgba(0,0,0,.65),0 6px 32px rgba(0,0,0,.45)`
- `.lp-hero-sub`: opacity `.82`→`.92`, weight `500`→`600`, text-shadow 2중 강화
- `.lp-hero-badge`: `backdrop-filter:blur(8px)` + 테두리/배경 강화
- `.lp-hero-stats`: 배경 `rgba(5,8,20,.55)` (이전 `.04`), `blur(16px)`
- `.lp-stat-label`: opacity `.35`→`.55`

### 히어로 배경 시간대별 사진 슬라이드쇼 (배포 완료)

#### js/hero-bg.js (신규)
- 낮(06~17시) / 밤(18~05시) 이미지 세트 각 3장 (Unsplash 무료)
  - day: 소규모 팀 미팅·카페 비즈니스·스마트폰 비즈니스 여성
  - night: 다크 모던 오피스·화면 앞 집중 업무·야간 미팅
- A/B 두 레이어 크로스페이드 (1.8s ease), 8초 간격 자동 전환
- 랜덤 시작 인덱스, 이미지 로드 실패 시 다음 이미지 fallback
- 탭 숨김 시 인터벌 중지, 복귀 시 재개 (visibility API)
- 초기화 시 `#hero-canvas` opacity:0 → display:none (이미지로 대체)

#### index.html hero 섹션 재편
- `#hero-bg-a`, `#hero-bg-b`: 배경 이미지 레이어 (z-index:0, opacity 전환)
- `#hero-overlay`: 오버레이 그라디언트 레이어 (z-index:1)
- `#lp-hero::before/::after`: 격자 패턴·골드 글로우 (z-index:2)
- `.hero-content-wrap`: 콘텐츠 래퍼 (position:relative; z-index:3) — 배경 위에 표시 보장

#### css/landing.css 추가
- `.hero-bg-layer`: absolute·inset:0·opacity:0·background-size:cover
- `.hero-overlay-layer`: absolute·inset:0·pointer-events:none·z-index:1
- `.hero-content-wrap`: position:relative·z-index:3·flex column center

---

### Dynamic Common Core — 업종별 공통 질문 문구 오버라이드 (배포 완료)

### Dynamic Common Core — 업종별 공통 질문 문구 오버라이드 (배포 완료)

#### js/wizard.js 추가
- `COMMON_WORDING_MAP`: 11개 업종 × `1_3` 재구매 항목 업종별 호칭 교체
  - 제조(mfg_parts/food_mfg/wholesale/export_sme/logistics): 재발주율·재주문율
  - 외식(restaurant)/생활서비스(local_service): 재방문율
  - IT(knowledge_it): 갱신율/Retention Rate
  - 의료(medical)/교육(education): 재등록율
  - 건설(construction): 재계약율·수의계약율
  - 각 업종 `1_1`·`1_2`에 `benchRef` (업종 평균 + 양호 기준) 주입
- `NUMERIC_BENCH_REF_DEFAULT`: 업종 매핑 없는 경우 중소기업 평균 fallback
- `DX_DETECT_ITEM`: 공통 진단 끝에 1문항 추가 (점수 미반영, 전략 시그널 수집)
  - `_signalOnly: true` → `data-signal-only` 속성 → 진행률·검증에서 자동 제외
- `_applyIndustryWording(diagData, industryKey)`: 데이터 오버라이드 함수
- `_injectDxDetect(diagData)`: DX 탐지 영역 주입 함수
- `loadDiagnosisUI()`: 창업 초기 제외하고 두 함수 적용
- `_renderNumeric()`: `item.benchRef` 있으면 골드 박스(평균/양호 기준/출처) 렌더링
- `updateDiagProgress()` + `validateCurrentTab()`: signal-only 항목 자동 제외
- `collect()`: `dxSignal` ('analog'|'digital_ready'|'') + `ceoDependencySignal` (boolean) 추가
  - `ceoDependencySignal`: `3_1점수 ≤ 2 AND 직원수 > 1` 복합 조건

#### js/ai-engine.js 수정
- `buildPrompt1()`: DX·대표의존도 시그널 블록 추가
  - `dxSignal === 'analog'`: SWOT 위협 + sixSystems 자동 주입 지시
  - `ceoDependencySignal`: sixSystems[operations] 1순위 + SOP 위임 체계 액션

#### css/style.css 추가
- `.diag-signal-badge`: DX 탐지 인라인 뱃지 (연파랑)
- `.diag-signal-item`: DX 탐지 카드 좌측 연파랑 테두리
- `.diag-bench-ref`: 업종 평균 참고값 박스 (골드 테두리)
- `.bench-avg` / `.bench-good` / `.bench-src`: 참고값 내부 강조 스타일

#### 현재 미완성
- `growth_strategy`, `structure_strategy`, `innovation_strategy`, `cx_strategy` — `_fakeByConsultingType()`에서 여전히 `return {}` (기본 fakeAnalysis 데이터 표시)

---

## 최근 수정 이력 (2026-04-29)

### _fakeByConsultingType 전체 유형 특화 완료 (배포 완료)

#### js/ai-engine.js 수정
- `finance_strategy`: kpi(10개)·sixSystems(6개)·plan90days(3개) 추가 (기존 keyStrategies만 있었음)
- `differentiation_strategy` (신규): USP·해자(Moat)·ROI증명·니치시장 전략 + 완전한 실행 데이터
- `marketing_strategy` (신규): StoryBrand·콘텐츠 루틴·리드육성·디지털채널 최적화 + 완전한 실행 데이터
- `hr_strategy` (신규): R&R 명확화·매뉴얼화·성과체계·채용·eNPS + 완전한 실행 데이터
- `pivot_strategy` (신규): 현황진단·잔존역량·런웨이 확보·MVP검증·재피벗 + 완전한 실행 데이터
- 모든 유형: keyStrategies(6개)·kpi(10개)·sixSystems(6개)·plan90days(3개) 완전 구성

#### 현재 미완성 (growth_strategy, structure_strategy, innovation_strategy, cx_strategy)
- 위 4개 유형은 여전히 `return {}` — 기본 fakeAnalysis 데이터로 표시됨

---

### Phase 3: AI 분석 2회 호출 분할 (배포 완료)

#### js/ai-engine.js 구조 변경
- `buildPrompt()` → `buildPrompt1()`: 1차 호출 사용자 프롬프트 (SWOT·전략 6개 섹션)
  - 끝에 "★ 1차 호출 응답 범위" 지시어 추가 → kpi/roadmap/sixSystems 제외 강제
- `buildPrompt2(d, r1)` 신규: 2차 호출 사용자 프롬프트
  - r1.keyStrategies를 참조하여 KPI·로드맵이 전략과 일관성 유지
  - _ctGuidance 포함, 웹검색 지시어 (정부지원사업 특화)
- `_SYSTEM_EXEC` 신규: 2차 전용 시스템 프롬프트 (KPI·로드맵·6시스템·90일플랜·린캔버스 5개 JSON 템플릿)
- `callClaude()` 재작성: 2회 순차 호출 + `Object.assign(result1, result2)` 병합
  - `apiCall()` 헬퍼: /api/claude-analyze 공통 호출
  - `extractJSON()` 헬퍼: JSON 파싱 로직 공통화

#### 로딩 화면 업데이트 (index.html + wizard.js)
- 스텝 텍스트: "SWOT 분석 중" → "1차 진단·전략 분석" / "KPI·실행플랜 작성 (2차)"
- `animateLoading()`: 고정 700ms → 가변 딜레이 (3000/5000/5000ms — 실제 2회 API 호출 시간 반영)

#### 효과
- 8000 토큰을 전체 섹션에 분산 → 1차 4000+α (전략 깊이) + 2차 4000+α (실행 깊이)
- 2차 호출이 1차 keyStrategies를 참고 → KPI·로드맵·6시스템이 전략과 일관성 유지

---

### Phase 2: Claude web_search 실시간 데이터 연동 (배포 완료)

#### api/claude-analyze.js (신규) — Vercel 서버사이드 Claude API 프록시
- `web_search_20250305` 도구 포함 — Anthropic이 실시간 웹 검색을 서버에서 실행
- 모델 `claude-sonnet-4-6` 으로 업그레이드 (knowledge cutoff Aug 2025, 기존 Oct 2024 → +10개월)
- 다중 턴 처리 루프 (stop_reason: 'tool_use' 시 tool_result로 응답 반복, 최대 10턴)
- `maxDuration: 60초` (웹 검색 포함 시 응답 지연 대비)

#### js/ai-engine.js callClaude() 전환
- 기존: 브라우저 → Anthropic API 직접 (CORS 우회 헤더 의존)
- 변경: 브라우저 → `/api/claude-analyze` → Anthropic API (서버사이드 프록시)

#### js/ai-engine.js buildPrompt() 섹션10 추가 — 웹 검색 지시어
- 업종 2025~2026 시장 동향 검색 → SWOT 기회·위협에 직접 반영
- 경쟁사 현황 검색 → 포지셔닝·차별화 전략에 반영
- 정부지원사업 2025 검색 → 로드맵에 지원사업 신청 일정 포함

---

### Reference DB + Chain of Consulting 통합 (배포 완료)

#### js/reference-db.js 신규 (ReferenceDB 모듈)
- 16개 업종 벤치마크 준거 데이터 (한국은행 기업경영분석 2023, 소상공인진흥공단 2023, 통계청 기업생멸통계 2023)
- 업종별 핵심 재무 지표: 영업이익률·부채비율·매출성장률·3/5년 생존율·디지털화율
- 업종별 특화 KPI: 건설(수주성공률·원가초과율·외주비율), IT(이탈률·가동률·RFP수주율), 외식(식재료비율·좌석회전율·1인건비), 제조(설비가동률·불량률·납기준수율), 의료(재방문율·비급여비율·예약완료율) 등
- `buildPromptBlock(industryKey)`: AI 프롬프트용 구조화 텍스트 자동 생성
- `getBench(industryKey)`: 원시 벤치마크 객체 반환
- `evalMargin(industryKey, actual)`: 실제값 vs 벤치마크 수준 평가 (high/ok/warn/danger)

#### js/ai-engine.js buildPrompt() 통합
- 섹션9 추가: `ReferenceDB.buildPromptBlock()` 호출 → AI가 실제 업종 평균 수치와 비교하여 진단·전략 제시
- IndustryTrends(8-1) → GovSupport(8-2) → **ReferenceDB(9)** → buildCausalChain → _ctGuidance 순서

#### buildCausalChain() — Chain of Consulting (이전 세션 완료)
- 5개 도메인 점수 + 업종×BM 패턴으로 인과관계 내러티브 자동 생성
- 건설(흑자도산 구조), IT/SaaS(ARR 누적 실패), 외식(박마진×운영취약), 물류(고정비×공차율), 의료·교육·생활서비스(재방문율 붕괴) 등 특화 시나리오

---

### consultingType 사전계산 + 진단유형별 솔루션 특화

#### 핵심 버그 수정: consultingType 순서 역전 문제
- **문제**: `showDiagReveal(data)` (AI 호출 후)에서 consultingType을 결정 → AI 프롬프트에 항상 `'미확인'`으로 전달
- **수정 위치**: `app.js runAnalysis()` — `Wizard.collect()` 직후, AI 호출 전에 삽입
  ```js
  const _domScores = Wizard.calcDomainScores(data.diagScores || {});
  const _ctResult  = Wizard.classifyConsultingType(_domScores);
  data.consultingType = _ctResult?.primary || '';
  data.consultingTypeSecondary = _ctResult?.secondary || '';
  ```
- **효과**: 진단 유형이 AI 프롬프트에 실제로 반영됨

#### buildPrompt 강화: _ctGuidance() 헬퍼 추가 (ai-engine.js)
- 컨설팅 유형별 상세 지침을 전체 프롬프트에 주입 (SWOT·keyStrategies·KPI·6시스템·90일플랜 전체 반영 지시)
- 구현된 유형: `digital_strategy`, `finance_strategy`, `growth_strategy`, `differentiation_strategy`, `hr_strategy`, `marketing_strategy`

#### fakeAnalysis 유형별 특화: _fakeByConsultingType() 추가 (ai-engine.js)
- `digital_strategy`: keyStrategies(6개)·KPI(10개)·sixSystems(6개)·plan90days(3개) 전면 디지털 전환 특화
  - CRM 도입·온라인 마케팅·업무 자동화·데이터 대시보드·디지털 판매 채널·디지털 역량 강화
  - 각 sixSystem: 현재 아날로그 문제 → 디지털 도구명·비용 포함 즉시 실행 액션
- `finance_strategy`: keyStrategies(6개) 재무 특화 (BEP·현금흐름·수익성·비용·자금조달·대시보드)
- 구조: `Object.assign(base, _fakeByConsultingType(...))` — 반드시 마지막에 오버라이드

#### 기타
- `.diag-item-text` font-size: 14px → 16px (진단 질문 가독성 향상)

---

## 최근 수정 이력 (2026-04-28)

### 위저드 흐름 버그 수정 + Step4 개선

#### 버그 수정
- **OCR 이미지 압축**: Canvas 기반 압축(max 1200px, JPEG 85%) → Vercel 4.5MB 제한 해결
- **OCR maxDuration 45초** + AbortController 40초 타임아웃 추가
- **개업연도 파싱**: 날짜형(20101116) → 앞 4자리만 사용 (wizard.js + analyze-biz.js)
- **Step2 "이전" 버튼**: `goStep(1)` → biz-context 확인 화면으로 복귀 (`prevDiagTab()` 수정)
- **"정보 수정" 빈화면**: `Wizard.goStep(1)` → `Wizard.reset()` 호출로 수정 (`app.js backToStep1()`)
- **validate 오류**: `validate(3)` → `validate(4)` (Step3 제거된 새 흐름 반영)
- **Step4 "이전" 버튼**: `App.goStep(3)` → `App.goStep(2)` 수정

#### Step4 개선
- **업종별 리스크 placeholder**: `Wizard.updateRiskPlaceholder(industryKey)` — 16개 업종 각각 맞춤 예시 자동 적용
- **기타참고사항**: 소상공인 실정에 맞는 예시 텍스트 (원맨구조, 핵심직원 퇴사 등)
- **notes 필드**: placeholder → 라벨 아래 p.hint로 항상 표시

### UI 고도화 6개 항목

#### 1. diag-reveal 세로 레이아웃 전환
- 2열 가로(레이더+유형) → 3섹션 세로 스크롤
- 섹션1: 📊 5대 역량 프로파일 (레이더 차트 + 점수 바 + **도메인별 해설 카드**)
  - 각 도메인: 무엇을 측정하는지 + 현재 점수에 따른 구체적 조언
  - `DOMAIN_EXPLAIN` 상수 (wizard.js) + `#drDomainGuide` DOM 주입
- 섹션2: 🎯 진단 컨설팅 유형 (설명 텍스트 포함)
- 섹션3: 💡 제안 솔루션 미리보기 (설명 텍스트 포함)

#### 2. 경영진단 분석 강화 (dashboard.js)
- 기존: 점수 pill + 취약 경고 배지
- 추가: **영역별 상세 분석 카드** (`AREA_INSIGHTS` + `diag-area-cards`)
  - 재무건전성/조직·인력/고객·매출/경영역량/업종특화/사업모델 각각
  - 점수 구간(high/ok/low)별 구체적 유지·개선 방향 제안 문장

#### 3. fakeAnalysis 요약 개선 (ai-engine.js)
- 기존: 입력값 단순 삽입
- 개선: 진단 점수 분석 → 취약 영역·강점 영역 명시 + 3단계 로드맵 방향 제시

#### 4. 기타 UI 개선
- step-badge 폰트: 11px → 13px (모든 스텝 통일)
- 기타참고사항: placeholder → p.hint 라벨 아래 항상 표시

#### 다음 세션 예정 작업
- 테스트 후 버그 수정
- AI 분석 2회 호출 분할 (1차: 진단·전략방향, 2차: 실행플랜)
- 소상공인/소기업 출력 분리 강화
- OCR 추가 안정화 테스트

---

## 최근 수정 이력 (2026-04-27)

### 경영전략 진단 문항 전면 리설계 — 65문항 → 13문항

#### 핵심 변경
- **common.js**: 20문항(5영역×4개) → 8문항(3영역: 사업성과·운영역량·경쟁력)
  - 수치 기반 3개(매출성장률·영업이익률·재구매율) + BARS 4개 + mixed 1개
  - 진단 시간 18분 → 약 5분으로 단축
- **16개 업종 파일 전면 재작성**: 각 16~20문항 → 각 5문항(업종 핵심만)
  - 수치 기반 1~2개 포함(업종별 핵심 KPI)
  - 쉬운 한국어, 영어 약어 풀네임 병기 유지
- **BM 진단 탭 제거**: `TAB_ORDER = ['common', 'industry']` 2탭으로 단순화
  - BM은 추론으로만 사용(UI에서 탭 제거)
- **총 진단 문항**: 65문항 → 13문항 (공통 8 + 업종 5)

---

## 최근 수정 이력 (2026-04-26)

### XBRL 파싱 완전 수정 + DART 확인화면 전 계정 표시

#### XBRL getVal 핵심 버그 수정 (api/dart-lookup.js)
- `new RegExp('[\\w-]*')` → `[\w-]*` 백슬래시 손실 버그 → 네임스페이스 prefix 매칭 실패
- **수정**: `indexOf` 기반 문자열 탐색으로 완전 교체 (`new RegExp` 제거)
- **K-GAAP 날짜 컨텍스트 대응**: `CFY{year}` 외에 `String(year)` 패턴 추가
  → K-GAAP 날짜 기반 contextRef (`D20240101T20241231` 등) 지원
- `xsi:nil` 자기닫힘 태그(`/>`) 건너뜀 처리 추가
- XBRL 추출 항목 확대: `payable`(매입채무), `borrowings`(차입금) 추가
  → `TradeAndOtherCurrentPayables`, `Borrowings`, `ShortTermBorrowings` 등

#### XBRL 보완 파이프라인 실제 작동 확인
- 빛과전자(KOSDAQ): cash/inventory/tangibleAssets/receivable/payable/borrowings/costOfSales/grossProfit/interestExpense 모두 추출 ✓
- 대한광통신(KOSPI): cash/inventory/tangibleAssets/borrowings/costOfSales/grossProfit/interestExpense 추출 ✓
- `laborCost`: IFRS 기능별 표시 상장사는 손익계산서에 인건비 항목 없음 — 구조적 한계 (null 정상)

#### DART 확인 화면 전 계정 표시 (js/finance-wizard.js)
- Step1 DART 조회 결과: 매출액·자산총계 2개 → 재무상태표·손익계산서 **전 계정 2열 그리드**
- 금액 형식: `1,394억원` (천단위 콤마)
- 미조회 항목: `N/A` 흐린 이탤릭 표시
- 합계 행(자산·부채·자본·매출·순이익): 굵은 초록 강조

#### Step2 재무입력 필드 개선 (index.html + js/finance-wizard.js)
- `type="number"` → `type="text" inputmode="numeric"` (22개 필드 전환)
- `_setField()`: null → `el.value=''` + `el.placeholder='N/A'`, 값 있으면 천단위 콤마 적용
- `_fmtComma()`: regex 기반 천단위 포맷 (`toLocaleString` 의존 제거)
- blur/focus 이벤트: 편집 시 콤마 제거, 이탈 시 콤마 재적용
- `_collectData()`: `.replace(/,/g,'')` 후 parseFloat — 콤마 있어도 정상 계산
- **미완료**: Step2 입력 필드 콤마/N/A 표시 — 배포는 됐으나 브라우저 캐시 등 이슈로 다음 세션에 재확인

#### 다음 세션 예정 작업
- Step2 천단위 콤마 + N/A placeholder 표시 재확인 (Ctrl+Shift+R 후 테스트)
- 사업자등록증 OCR 수정 (api/ocr-scan.js 확인)

---

## 최근 수정 이력 (2026-04-25)

### XBRL 재무데이터 보완 + 금융업 계정명 fallback + 섹션별 레이더 차트

#### XBRL 구현 (api/dart-lookup.js)
- `_parseXbrl(xml, year)`: XBRL ZIP 파싱 → 현금·재고·유형자산·매출채권·매출총이익·이자비용·인건비 추출
- `_supplementWithXbrl()`: fnlttSinglAcnt 누락값을 XBRL로 보완 (null만 채움, 기존값 덮어쓰지 않음)
- XBRL context 매칭: 3월·6월 결산법인 대응 (year, year+1 허용)
- 방어 코드: AbortController 타임아웃(list.json 15초, XBRL 20초), ZIP 5MB 초과 시 스킵
- `vercel.json`: `maxDuration: 45` 설정

#### 금융업(카드·은행·캐피탈) 계정명 fallback
- 하나카드 등 금융회사는 IAS 1.54 예외로 유동/비유동 구분 없음 — 아래 대체 계정 추가
  - `revenue`: 이자수익·순영업수익·영업수익합계
  - `grossProfit`: 순이자손익
  - `cash`: 현금및예치금
  - `receivable`: 상각후원가측정금융자산(카드채권)
  - `borrowings`: 차입부채

#### 재무분석 섹션별 레이더 차트 (js/finance-wizard.js)
- 유동성·안전성·수익성·활동성·생산성·성장성 각 섹션 카드 안에 개별 레이더 차트 삽입
- 차트(왼쪽 230px) + 테이블(오른쪽) 2열 레이아웃, 모바일은 1열
- `_getSectionAxes(data)`: 섹션 데이터 → 0~100 정규화 (50=산업평균)
  - LOW_IS_GOOD 항목(부채비율 등): 역방향 정규화 → 레이더 위 = 항상 좋음
- `_drawRadarAxes(canvasId, axes)`: Canvas API 기반, 외부 라이브러리 없음
- 성장성(항목 1개)은 레이더 미표시, 테이블만 표시
- 요약 레이더 차트 제거, 섹션별 상세 차트로 대체

#### 다음 세션 예정 작업
- 사업자등록증 OCR 수정 (api/ocr-scan.js 확인)
- K-GAAP 중소기업 DART 조회 추가 테스트
- 인건비(판+제) XBRL 계정명 추가 발굴

---

## 최근 수정 이력 (2026-04-23)

### vercel dev 로컬 테스트 환경 구축

#### 변경 내용
- **finance-wizard.js**: localhost 분기 3곳 제거 (`location.hostname === 'localhost'` 체크 삭제)
  - DART 조회, DART 버튼, ECOS API 모두 localhost/vercel 구분 없이 실제 API 호출
  - `_mockDartData()` 함수는 코드에 남겨두되 더 이상 자동 호출되지 않음
- **vercel.json**: `api/ocr-scan.js` 함수 목록에 추가 (누락 보완)
- **.env.local** (gitignore됨, 로컬 전용): 환경변수 파일 생성
  - `DART_API_KEY` 설정 완료
  - `ECOS_API_KEY`, `NTS_SERVICE_KEY`, `GOOGLE_VISION_API_KEY` → 본인 키 입력 필요

#### 로컬 테스트 방법 (vercel dev)
```bash
# 1. 프로젝트 루트에서 Vercel 로그인 (최초 1회)
vercel login

# 2. 프로젝트 연결 (최초 1회) — biznavi 선택
vercel link

# 3. 로컬 서버 실행
vercel dev
# → http://localhost:3000 으로 접속
# → api/dart-lookup.js, api/bok-avg.js 등 serverless 함수 포함 전체 작동
```

#### 주의사항
- `.env.local`에 `ECOS_API_KEY`, `NTS_SERVICE_KEY` 미입력 시 해당 기능(ECOS 업종평균, 사업자조회) fallback 모드로 동작
- `vercel login` 및 `vercel link`는 터미널(PowerShell/CMD)에서 대화형으로 실행 필요

---

## 최근 수정 이력 (2026-04-22) — 오후 2차

### 재무분석 직접입력 + 이전 버튼 네비게이션 수정

#### 문제 및 해결
- **Step2 직접입력 필드 불가**: `.fin-input-item input`이 `.form-group` 밖에 있어 다크테마 CSS 미적용 → 흰색 기본 input처럼 보여 비활성처럼 오인
  - `style.css`에 `.fin-input-item input, .fin-input-item select` 다크테마 스타일 추가
- **DART 실패 시 수동입력 전환**: DART 조회 실패(`nextStep()` else 분기)에서 `_inputMode = 'manual'` 자동 설정 → Step2 진입 즉시 입력 가능
- **재무 대시보드 이전 버튼 추가**: 기존 "← 다시 분석"(Step1 초기화)과 분리
  - `FinWizard.backToStep2()`: 대시보드 → Step2(재무입력)로 복귀
  - `App.showFinanceWizard()`: finance-wizard 화면만 표시 (Step1 리셋 없음)
  - 대시보드 nav: **← 이전**(Step2 복귀) + **↺ 처음부터**(Step1 초기화) 분리
- **finance-wizard Step1 이전 버튼**: `App.showModeSelect()` → `App.showLanding()` 변경 (랜딩에서 직접 진입 시 뒤로가기가 서비스선택 화면으로 가던 문제 해결)

---

### 랜딩페이지 전면 리뉴얼 — 재무분석 모듈 반영

#### 변경 내용
- **Hero**: "재무상태 진단부터 경영전략 실행까지" + CTA 2개(재무분석/경영전략 분리), stat에 109,030개 기업 수치 추가
- **섹션3(Value)**: 두 서비스를 카드 형태로 나란히 소개 (재무분석/경영전략 각 기능 4가지 명시)
- **섹션4(Features)**: 재무분석 4개 카드 + 경영전략 6개 카드로 재구성
  - 재무분석: DART 자동조회, 6대 비율, 한국은행 ECOS 벤치마크, PDF 출력
  - 경영전략: 16개 업종 BARS 진단, 10대 컨설팅 유형, 규모별 맞춤 보고서, 90일 플랜, 경영서적 10권, 5대 역량 레이더 차트
- **섹션6(Demo)**: 재무분석 대시보드 목업 추가 (6대 비율 카드 양호/주의/위험 수준 표시 + SWOT 통합)
- **섹션5(Trust)**: 후기 업데이트 (경영지도사 재무진단 후기 포함), 통계 수치 현실화
- **섹션7(Pricing)**: 각 플랜별 재무분석 포함 기능 업데이트
- **섹션8(FAQ)**: DART 설명, 한국은행 업종평균 연동 방식, 16개 업종 지원 FAQ 추가 (총 6개)
- **섹션9(CTA)**: 두 서비스 버튼 분리
- **Footer**: © 2026, 이메일 dodson108@gmail.com 업데이트

---

## 다음 세션 예정 작업

### 1순위: AI 분석 정상 작동 최종 확인
- biznavi.vercel.app에서 실제 분석 1회 end-to-end 테스트
- 1차·2차 모두 정상 JSON 반환 확인, 대시보드 렌더링 확인

### 2순위: JSON 파싱 실패 근본 해결
- max_tokens 증가 방식 revert됨 — 다른 접근 필요
- 후보: 시스템 프롬프트 토큰 축소 / buildPrompt1 섹션 경량화

### 3순위: [TIMING] 타이머 로그 제거
- api/claude-analyze-1.js, claude-analyze-2.js의 console.log 제거
- ai-engine.js 클라이언트 타이머 로그 제거

### 4순위: vercel dev 로컬 실행 방법
```powershell
$env:DART_API_KEY="<DART_API_KEY 값 — .env.local 또는 Vercel 환경변수 참조>"
vercel dev
# → http://localhost:3000
```

### 5순위: PDF 표지 확인
- 실제 인쇄/PDF 저장 테스트 → 표지 1페이지 확인

### 6순위: --teal CSS 변수 정리
- 미사용 CSS 변수 정리 및 다크테마 일관성 점검

### 마지막: 통합 테스트 1회
- 전체 흐름 (위저드 → AI 분석 → 대시보드 → PDF) end-to-end 검증

---

## 최근 수정 이력 (2026-04-22) — 오전

### DART 기업목록 안정화 — corp-list.json 로컬 파일 방식으로 전환

#### 문제 원인 분석 및 해결
- **Vercel 인스턴스 간 캐시 불공유**: 각 콜드스타트 인스턴스가 corpCode.xml을 별도 다운로드 시도 → 실패 시 error 반환
- **해결**: `scripts/build-corp-list.js`로 109,030개 기업목록을 `api/corp-list.json`으로 로컬 저장
  - 로컬에서 `DART_API_KEY=... node scripts/build-corp-list.js` 실행 후 커밋
  - dart-lookup.js가 JSON 파일 우선 읽기 → 실패 시만 DART 다운로드 fallback
- **`vercel.json`**: `buildCommand` + `outputDirectory: "."` 추가 (빌드 시 스크립트 자동 실행)

#### 상태별 메시지 개선 (finance-wizard.js)
- `no_financial`: "DART에 등록된 기업이지만 공시된 재무제표가 없습니다. 직접 입력해주세요."
- `not_found`: "DART 미등록 기업입니다. 직접 입력해주세요."
- `error`: "조회 실패 — 직접 입력해주세요."

#### 삼성전자 DART 조회 결과 (IFRS 대형기업 한계)
- 반환 항목: 유동자산, 비유동자산, 자산총계, 유동부채, 비유동부채, 부채총계, 자본총계, 매출액, 영업이익, 당기순이익
- **현금, 재고자산, 유형자산, 매출총이익, 이자비용, 인건비 = DART가 미제공** (IFRS 대형기업 특성)
- 중소기업(K-GAAP)은 더 상세한 항목 반환 예상

#### 다음 세션에서 확인 필요
- 벽산건설 등 K-GAAP 중소기업 DART 조회 정상 작동 여부 테스트
- ECOS 업종별 산업평균 연동 최종 확인
- _debugAccounts 임시 필드 제거 (테스트 완료 후)

---

## 최근 수정 이력 (2026-04-21)

### 재무분석 모듈 완성 — DART + ECOS + 리포트 연동

#### DART 회사명 자동변형 검색 (api/dart-lookup.js)
- `(주)`, `㈜`, `주식회사` 등 접두어 자동 제거 후 재검색
- 원본명 → 핵심명 → `주식회사 XXX` → `㈜XXX` → `(주)XXX` 순서로 시도
- 어떤 형식으로 입력해도 DART 검색 성공률 대폭 향상

#### 한국은행 ECOS API 업종별 산업평균 동적 연동
- `api/bok-avg.js` (신규): Vercel Serverless Function
  - KSIC 대분류(A~S) → ECOS 기업경영분석 업종코드 매핑 (18개 업종)
  - 통계표 008Y003 호출 → 재무비율 30여 항목 파싱
  - 환경변수: `ECOS_API_KEY` (ecos.bok.or.kr에서 발급)
- `js/finance-wizard.js` 수정:
  - `_BOK_AVG_DEFAULT`: 제조업 기준 하드코딩 기본값 (fallback)
  - `_bokAvg`: 분석 실행 시 ECOS에서 해당 업종 실제값으로 동적 교체
  - `_fetchBokAvg()`: `/api/bok-avg` 호출, 실패 시 기본값 유지
  - `_bokAvgSource`: 대시보드·리포트에 산업평균 출처 자동 표시
  - `analyze()` 비동기화, 버튼 disabled 처리

#### 재무분석 흐름 (완성)
1. 회사명 입력 → DART 조회 (회사명 변형 자동시도)
2. DART 성공: 업종코드 + 전체 재무데이터(B/S+I/S) 자동입력
3. DART 실패: 직접 입력 모드 전환 + 업종 수동 선택
4. 분석 실행 → ECOS API로 해당 업종 산업평균 조회
5. 6대 재무비율 분석 대시보드 렌더링
6. 재무분석 리포트 생성 (PDF 인쇄 가능)

#### Vercel 환경변수 (필수)
```
DART_API_KEY  = [opendart.fss.or.kr 발급]
ECOS_API_KEY  = [ecos.bok.or.kr 발급]
```

---

## 최근 수정 이력 (2026-04-20)

### 사업자등록번호 자동조회 → 업종 자동세팅 기능 추가

#### 개요
- Step 1 상단에 사업자등록번호 조회 블록 추가 (선택사항, 건너뛰기 가능)
- 사업자번호 체크섬 검증 (클라이언트, 국세청 알고리즘)
- Vercel Serverless Function (`/api/biz-lookup.js`) → 국세청 사업자 상태 조회
- 업태/종목 텍스트 입력 → 16개 업종 키워드 매핑 → 업종 드롭다운 자동선택

#### 구현 흐름
1. 사업자번호 입력 → `###-##-#####` 자동 포맷 + 체크섬 실시간 유효성 표시
2. 대표자명 입력 → [자동조회] 버튼 → `/api/biz-lookup` 호출
3. 국세청 API 응답: 정상(active) / 휴업(suspended) / 폐업(closed)
4. 업태·종목 입력란 표시 → 키워드 매핑으로 16개 업종 자동선택
5. "✓ 업종 자동 설정: ○○" 배지 표시 + 수동 변경 항상 허용

#### 신규/수정 파일
- `api/biz-lookup.js` (신규): Vercel Serverless Function
  - `NTS_SERVICE_KEY` 환경변수 필요 (Vercel Dashboard → Environment Variables)
  - 미설정 시 `manual` 모드 fallback (업태/종목 직접 입력 유도)
  - 공공데이터포털 API: https://www.data.go.kr/data/15081808/openapi.do
- `index.html`: Step 1 상단 `.biz-lookup-block` 추가
- `js/wizard.js`: `formatBizNo`, `validateBizNo`, `lookupBiz`, `inferIndustryFromType`, `skipBizLookup` 추가
- `css/style.css`: `.biz-lookup-block`, `.biz-status-*`, `.biz-infer-*` 스타일 추가

#### 향후 계획
- 예비창업자 전용 플로우 추가 (사업자번호 없이 진행)
- DART API 연동 → 재무제표 자동 입력 (Phase 9)
- OCR 연동 → 사업자등록증 스캔으로 자동입력 (Phase 9)

#### Vercel 환경변수 설정 방법
```
Vercel Dashboard → biznavi 프로젝트 → Settings → Environment Variables
  NTS_SERVICE_KEY = [공공데이터포털에서 발급받은 서비스 키]
```

---

### 전 업종 진단 문항 BARS 5단계 앵커 + 구체적 현장 문항 전면 재작성

#### 개요
- `common.js` 구문 오류 수정 + 5개 역량 도메인 전면 재작성 (BARS 형식)
- 13개 업종 진단 파일 전면 재작성 — 모든 문항 구체적 현장 상황 묘사로 교체
- 전문용어 풀네이밍 + 쉬운 설명 삽입 (HACCP, MTBF, TMS, FDS, 공차율 등)
- `app.js`: API 키 오류(`invalid x-api-key`) 시 localStorage 초기화 + `fillSavedKey()` 함수 추가
- `wizard.js`: `bizScale` 필드 수집 + `fillSavedKey()` 연동

#### 진단 문항 형식 통일 (BARS: Behaviorally Anchored Rating Scales)
- 모든 문항: `type: "bars"` + 5단계 `anchors` (각 레벨별 구체적 현장 상황 묘사)
- 질문 텍스트 구조: "왜 중요한지 설명 — 구체적 질문 내용"
- 수치 입력 항목은 `type: "numeric"` + `scoreRanges` 구간별 점수 자동 계산

#### 재작성된 파일 목록 (13개 업종)
- `local_service.js`: 노쇼 방지·재방문율·네이버플레이스·BEP 등 생활밀착 서비스 특화
- `wholesale.js`: 채널 집중도·CCC(현금전환사이클)·ROAS·재고회전율 등 유통 특화
- `restaurant.js`: 식재료 원가율·임대료 비중·HACCP·배달 플랫폼 수익률 등 외식 특화
- `construction.js`: 기성금·흑자 도산·나라장터·중대재해처벌법 등 건설 특화
- `medical.js`: EMR·PACS·비급여·의료광고법 등 의료 특화
- `education.js`: LMS·OJT·상담 전환율·스타 강사 의존도 등 교육 특화
- `fashion.js`: SKU·OEM/ODM·ROAS·ER·D2C·QC 등 패션 특화
- `media.js`: IP·OSMU·FDS·흑자 도산 등 미디어 특화
- `logistics.js`: 공차율·TMS·WMS·DTG·km당 운송 원가 등 물류 특화
- `energy.js`: Backlog·REC·VCM·탄소크레딧·RE100·ISMS 등 에너지 특화
- `agri_food.js`: HACCP·GAP·CSA·수율·OEM 등 농림식품 특화
- `export_sme.js`: CE·FDA·HS코드·FTA·환헤지(선물환)·CAC/LTV 등 수출중소기업 특화
- `finance.js`: FDS·금소법·ISMS·BCP/DR·LTV/CAC·네팅 등 금융 특화

---

## 최근 수정 이력 (2026-04-17)

### 소상공인/소기업 이분법 대시보드 모드 분리 구현

#### 개요
- Step 1에 `bizScale` 선택 필드 추가 (소상공인 / 소기업·중소기업)
- 선택한 규모에 따라 대시보드 목차·섹션 완전히 다르게 렌더링
- **소상공인 모드**: 비즈니스 캔버스 + 도널드 밀러 6가지 시스템 + 90일 즉시 실행 플랜 중심
- **소기업·중소기업 모드**: 기존 SWOT/STP/4P/핵심전략/KPI/실행로드맵 + 6가지 시스템/90일 플랜 추가

#### index.html
- Step 1 `employees`/`revenue` 아래 `bizScale` select 추가 (소상공인 / 소기업·중소기업, 필수)
- `#reportNav` 내 정적 링크 제거 → JS 동적 생성으로 전환
- `#sec-six-systems` 섹션 추가 (도널드 밀러 6가지 비즈니스 시스템)
- `#sec-plan90` 섹션 추가 (90일 즉시 실행 플랜)

#### wizard.js
- `validate(1)`: `bizScale` 미선택 시 alert 추가
- `collect()`: `bizScale: g('bizScale')` 수집 추가

#### ai-engine.js
- SYSTEM 프롬프트: `bizScale` 기준 모드 분기 지침 추가
  - `micro` (소상공인): `sixSystems`·`plan90days` 최우선 집중 작성
  - `sme` (소기업): 기존 전략 프레임워크 풍부하게 작성
- JSON 템플릿에 `sixSystems` 6개 (리더십·마케팅·판매·제품·운영·재무) + `plan90days` 3개 (월별 액션 플랜) 구조 추가
- `buildPrompt()`: 사업 규모 항목 추가
- `fakeAnalysis()`: 두 모드 모두 풍부한 `sixSystems`·`plan90days` 데모 데이터 생성

#### dashboard.js
- `buildNav(isMicro)`: 모드별 동적 목차 생성 함수 추가
- `renderSixSystems(data)`: 6가지 시스템 카드 렌더링 (상태 배지 + issue + 액션 3개 + 추천 자원)
- `renderPlan90(data)`: 90일 플랜 타임라인 렌더링 (월별 목표·액션·기대효과·지원사업)
- `render()`: `fd.bizScale` 기반 섹션 표시/숨김 제어, 소상공인 모드 시 린 캔버스 → 비즈니스 캔버스로 타이틀 변경
- `initScrollReveal()`: 표시된 섹션만 스크롤 스파이에 포함

#### css/dashboard.css
- `.mode-badge-inline`: 모드 표시 초록 뱃지
- `.six-sys-grid`, `.sys-card`, `.sys-status-*`: 6가지 시스템 카드 2열 그리드
- `.plan90-timeline`, `.plan90-month`, `.plan90-num`: 타임라인 레이아웃 (세로 골드 라인 + 번호 원형)
- 모바일(768px): `.six-sys-grid` 1열, `.plan90-timeline` 컴팩트 축소

---

### AI API 모델 ID 수정 + 버그 수정 + 진단 문항 품질 개선

#### css/style.css
- `.gov-check-group .gov-check-item` 선택자 특이도 강화 (`display:flex !important`) — `.form-group label { display:block }` 과 충돌로 정부지원 체크박스 레이아웃 깨지던 버그 수정
- 체크박스 선택 시 골드 테두리·배경 시각 피드백 추가 (`:has(input:checked)`)

#### js/wizard.js
- `loadDiagnosisUI()` 내 `COMMON_DIAGNOSIS` 직접 참조 → `typeof` 가드로 변경 (ReferenceError 방어)

#### js/app.js
- `devJump` 함수 및 localhost 개발 플로팅 패널 제거
- `startWizard()`에 `Wizard.reset()` 추가 (재진입 시 Step1 미표시 버그 수정)
- `confirmBm()` → `Wizard.goToStep2FromBm()` 전용 함수로 교체 (BM 확인 후 빈 화면 버그 수정)

#### js/diagnosis/industry/mfg_parts.js — 전면 재작성
- COPQ (Cost of Poor Quality: 불량 손실비), MTBF (Mean Time Between Failures: 평균 고장 간격), SMED (Single Minute Exchange of Die: 신속 교체법), 치공구(Jig), 가치사슬 위치(Tier) 등 **모든 전문용어 풀네이밍 + 쉬운 설명**으로 교체
- 전 항목에 `type: "bars"` + 5단계 `anchors` (구체적 현장 상황 설명) 추가

#### js/diagnosis/industry/food_mfg.js — 전면 재작성
- HACCP (식품안전관리인증기준), CCP (핵심 관리 지점), FIFO (선입선출), 콜드체인 등 전문용어 쉬운 설명으로 교체
- 전 항목에 `type: "bars"` + 5단계 `anchors` 추가

#### js/diagnosis/industry/knowledge_it.js — 전면 재작성
- M/M (공수: 1명이 1개월 일하는 업무량), Scope Creep (계약 외 추가 업무 범위 확대), MRR (월 반복 매출: Monthly Recurring Revenue), SOP (표준 업무 절차서) 등 전문용어 풀네이밍 + 쉬운 설명
- 전 항목에 `type: "bars"` + 5단계 `anchors` 추가

---

## 최근 수정 이력 (2026-04-17)

### 16개 업종 + 12개 사업모델 자동추론 시스템

#### index.html (Step 1 재설계)
- 업종 드롭다운: 12개 → 16개 (수출중소기업·물류운송·환경에너지·농림식품원료 추가)
- 비즈니스모델 드롭다운 **제거** → 추론 결과 표시 div (`#inferredBmDisplay`) + hidden input (`#bizModel`) 로 전환
- `onchange="Wizard.onIndustryChange()"` 업종 선택 시 즉시 BM 추론·표시
- 중복 질문 4개 → 3개로 정리:
  - 삭제: `bizStrengths` (핵심 경쟁력 textarea — coreStrength와 중복)
  - 유지/재명명: `coreStrength` / `customerProblem` / `unfairAdvantage`
- script 태그 7개 추가 (신규 진단 파일)

#### js/wizard.js
- `INDUSTRY_MAP`: 16개 업종 키 매핑 (export_sme / logistics / energy / agri_food 추가)
- `BIZMODEL_MAP` **삭제** (미사용 — BM_LABELS로 대체)
- `INDUSTRY_BM_MAP`: 16개 업종 → 현실적 BM 후보 목록 (불가능한 조합 자동 제외)
- `BM_LABELS`: BM 키 → 한국어 표시 레이블
- `inferBizModel(industryKey, formData)`: 키워드 매칭 + 우선순위 기반 BM 추론
- `onIndustryChange()`: 업종 변경 시 BM 추론 실행 + UI 업데이트
- `validate(1)`: `bizModel` 필수 검사 제거, `customerProblem` 필수 추가, 이동 전 `onIndustryChange()` 실행
- `collect()`: `bizStrengths` 제거, `bizModelKey` 추가 (추론된 BM 키)
- `loadDiagnosisUI()`: `_inferredBmKey` 기반 BM 진단 로드, 4개 신규 업종 변수 추가
- 신규 BM 변수 3개 (`BIZMODEL_USAGE_BASED` / `BIZMODEL_ADVERTISING` / `BIZMODEL_DEEPTECH`) 추가
- 공개 API: `onIndustryChange` 추가

#### 신규 진단 파일 (4 업종 + 3 BM = 7개)
- `js/diagnosis/industry/export_sme.js` — 수출 중소기업 (바이어 다각화·인증·환율·수출지원)
- `js/diagnosis/industry/logistics.js` — 물류·운송 (가동률·네트워크·안전·TMS)
- `js/diagnosis/industry/energy.js` — 환경·에너지 (수주·인허가·정책금융·Backlog)
- `js/diagnosis/industry/agri_food.js` — 농림·식품원료 (원물조달·HACCP·가공·판로)
- `js/diagnosis/bizmodel/usage_based.js` — 종량제·사용량기반 (미터링·NRR·가격구조)
- `js/diagnosis/bizmodel/advertising.js` — 광고기반 (MAU·CPM·콘텐츠·수익다각화)
- `js/diagnosis/bizmodel/deeptech.js` — 딥테크·바이오 (특허·TRL·런웨이·사업화)

#### css/style.css
- `.inferred-bm-display`: 추론 결과 표시 컨테이너 (골드 테두리 배경)
- `.bm-tag` / `.bm-tag.primary`: BM 후보 태그 (1순위 골드 강조)
- `.bm-infer-hint`: 안내 텍스트

---

## 최근 수정 이력 (2026-04-14)

### B-3: 단계별 실행 가이드 구조화 — 로드맵 프레임워크 배지 + 린 캔버스 시각화

#### js/ai-engine.js
- SYSTEM 프롬프트 roadmap에 `framework` 필드 추가 (린 스타트업 / 플라이휠 / 6대 시스템)
- JSON 구조에 `leanCanvas` 필드 추가: 9블록 (problem, customerSegments, uniqueValueProposition, solution, channels, revenueStreams, costStructure, keyMetrics, unfairAdvantage)
- fakeAnalysis roadmap 각 단계에 `framework` 추가
- fakeAnalysis에 `leanCanvas` 객체 추가 (form 입력값 customerProblem·unfairAdvantage 자동 반영)

#### index.html
- 대시보드 목차에 `린 캔버스` 항목 추가 (`sec-lean-canvas`)
- 실행 로드맵 아래 `#sec-lean-canvas` 섹션 추가 (9블록 그리드)

#### js/dashboard.js
- `renderLeanCanvas(data, fd)` 함수 추가 (9블록 lean canvas 렌더링)
- 로드맵 렌더링에 `.rm-framework` 배지 표시 추가
- `render()` 함수에서 `renderLeanCanvas()` 호출 추가
- 스크롤 스파이 `secIds`에 `sec-lean-canvas` 추가

#### css/dashboard.css
- `.lc-grid`: 3열 반응형 그리드 (모바일 2열 → 1열)
- `.lc-block`: 9블록 카드 기본 스타일
- `.lc-uvp`: 핵심 가치 제안 골드 강조 (특별 border + 배경)
- `.lc-problem`: 문제 블록 붉은 계열 테두리
- `.lc-revenue`: 수익 블록 그린 계열 테두리
- `.rm-framework`: 로드맵 단계별 프레임워크 배지 스타일 (골드 배경)

---

### B-2: AI 솔루션 출력 구조 개선 — 컨설팅 유형별 특화 분석 섹션 추가

#### js/ai-engine.js
- SYSTEM 프롬프트에 컨설팅 유형별 specializedAnalysis 프레임워크 지침 추가
  - finance_strategy → BEP·현금흐름 분석 (4블록)
  - growth_strategy → 비즈니스 모델 캔버스(BMC) 9블록
  - differentiation_strategy → VRIO 경쟁우위 분석 (5블록)
  - hr_strategy / structure_strategy → 맥킨지 7S 프레임워크 (7블록)
  - digital_strategy → 디지털 전환 MVP 로드맵 (5블록)
  - 그 외 유형 → 맞춤형 특화 처방 분석 (4~5블록)
- JSON 구조에 `specializedAnalysis` 필드 추가: `{type, framework, summary, blocks[]}`
- buildPrompt에 컨설팅 유형 specializedAnalysis 작성 지침 추가
- `_fakeSpecialized(d)` 헬퍼 함수 추가 (6개 유형별 완전한 데모 데이터)
- `fakeAnalysis()` 리턴에 `specializedAnalysis: _fakeSpecialized(d)` 추가

#### index.html
- 대시보드 목차에 `유형별 특화 분석` 항목 추가 (`sec-consulting`)
- 경영 진단 섹션 바로 아래 `#sec-consulting` 섹션 추가
  - `#specFrameworkBadge`: 프레임워크명 뱃지
  - `#specSummary`: 컨설팅 유형 아이콘 + 요약 텍스트
  - `#specBlocks`: 유형별 분석 블록 그리드

#### js/dashboard.js
- `renderSpecializedSection(data, fd)` 함수 추가
  - `data.specializedAnalysis` 존재 여부 확인 후 `sec-consulting` 표시/숨김
  - 컨설팅 유형 아이콘 매핑 (10종)
  - 블록 그리드 렌더링 (번호 뱃지 + 레이블 + 내용)
- `render()` 함수에서 `renderSpecializedSection()` 호출 추가
- 스크롤 스파이 `secIds`에 `sec-consulting` 추가

#### css/dashboard.css
- `.spec-type-row`, `.spec-type-icon`, `.spec-type-label`: 상단 유형 표시
- `.spec-summary-text`: 요약 텍스트 (골드 좌측 테두리)
- `.spec-blocks`: 자동 반응형 그리드 (`auto-fit, minmax(270px, 1fr)`)
- `.spec-block`: 개별 분석 블록 카드 (hover 골드 테두리)
- `.spec-block-label`, `.spec-block-num`, `.spec-block-content`: 블록 내부 스타일
- 모바일(768px): `.spec-blocks` 1열 전환

---

## 최근 수정 이력 (2026-04-13)

### Phase 1: 진단고도화 — 5대 역량 도메인 + 10대 컨설팅 유형 + 진단유형 확인 화면

#### js/diagnosis/common.js
- 5번째 진단 영역 `differentiation` 추가: 차별화·경쟁우위 역량 (4문항)
  - 5_1: 경쟁사 대비 핵심 차별화 요소, 5_2: 고객 선택 이유, 5_3: 모방 난이도, 5_4: 비가격 경쟁 수단

#### index.html (STEP 1 + 신규 섹션)
- STEP 1에 `customerProblem` 텍스트영역 추가 (린 캔버스 Problem 블록, 필수)
- STEP 1에 `unfairAdvantage` 입력란 추가 (모방 불가 경쟁 우위, 선택)
- `#diag-reveal` 섹션 신규 추가 (loading과 dashboard 사이)
  - 레이더 차트 canvas, 5대 역량 점수 바, 진단 유형 박스, 솔루션 미리보기 목록
  - 하단 네비: "진단 수정하기" ← → "솔루션 전체 보고서 보기"

#### js/wizard.js (v3.2)
- `CONSULTING_TYPES` 상수: 10대 컨설팅 유형 정의 (label, icon, desc, preview 4항목)
  - 경영재무전략 / 사업화·성장전략 / 차별화·경쟁우위전략 / 기업구조·시스템전략 / 혁신·신사업전략
  - 마케팅·브랜드전략 / 조직·인력운영전략 / 디지털전환전략 / 사업재편·피벗전략 / 고객경험·서비스전략
- `calcDomainScores(diagScores)`: diagScores 키 패턴으로 5대 역량 도메인 점수 계산
  - finance(1_*+4_*) / hr(2_*) / bm(3_*+bizmodel) / future(industry) / differentiation(5_*)
- `classifyConsultingType(domainScores)`: 규칙 기반 분류기 → primary + secondary 컨설팅 유형 반환
  - 전체 평균 < 2.0이면 피벗전략, 최약 도메인 → 유형 매핑, 특수 케이스(finance+hr 동시 낮음)
- `showDiagReveal(data)`: diag-reveal 화면 DOM 채우기 + drawRadarChart 호출
- `drawRadarChart(canvasId, domainScores)`: Canvas API로 5각형 레이더 차트 렌더링
- `collect()`: `customerProblem`, `unfairAdvantage` 수집 추가
- 공개 API: `showDiagReveal`, `calcDomainScores`, `classifyConsultingType`, `drawRadarChart` 추가

#### css/style.css
- `#diag-reveal`, `.dr-wrap`, `.dr-header`, `.dr-body`, `.dr-radar-wrap`, `.dr-score-*`
- `.dr-type-box`, `.dr-type-primary/secondary/desc`, `.dr-preview`, `.dr-nav` 스타일 추가
- 모바일 반응형: dr-body 1열, dr-nav 세로 정렬

#### js/app.js
- `screens` 배열에 `'diag-reveal'` 추가
- `_pendingResult`, `_pendingData`, `_pendingIsDemo` 상태 변수 추가
- `runAnalysis()`: AI 호출 완료 후 dashboard 대신 diag-reveal로 이동
- `proceedToSolution()`: diag-reveal → dashboard 이동 (보관된 결과 렌더링)
- `goBackToDiag()`: diag-reveal → wizard STEP4로 복귀
- 공개 API: `proceedToSolution`, `goBackToDiag` 추가

#### js/ai-engine.js (buildPrompt)
- `customerProblem` (린 캔버스 Problem), `unfairAdvantage`, `consultingType` 프롬프트 반영

---

## 최근 수정 이력 (2026-04-10)

### 업종×사업모델 유기적 통합 진단 시스템 (js/diagnosis/cross-context.js 신규)
- `cross-context.js`: 13개 업종 × 9개 사업모델 핵심 조합 **31개** 특화 교차 진단 4문항 정의
  - 건설×B2B솔루션, IT×SaaS, 제조×유통, 식품×구독, 외식×프랜차이즈 등 핵심 조합 모두 커버
  - 미정의 조합은 IND_CTX / BIZ_CTX 컨텍스트 키워드 기반 자동 fallback 생성
  - `CrossContext.buildCrossArea(industryKey, bizModelKey, label, label)` Public API
- `wizard.js` 수정:
  - 사업모델 탭 하단에 `🔗 통합 진단: [업종] × [사업모델]` 영역 자동 추가 (4문항)
  - 탭 버튼 레이블 동적 변경: "🏭 건설/부동산 특화 진단" / "💼 B2B솔루션 × 통합 진단"
  - 진행률 카운터 DOM 실제 항목 수 기준으로 동적 계산 (기존 48 → 52항목)
  - `updateDiagTabUI()`: id 기반 선택자로 안정화
- `index.html`: 탭 버튼에 id 추가 (diagTabBtn-common/industry/bizmodel)
- `css/style.css`: `.diag-cross-area` 골드 테두리·배경 강조 스타일 추가

---

## 최근 수정 이력 (2026-04-09)

### AI 출력 품질 고도화 v3.0 (js/ai-engine.js)
- SYSTEM 프롬프트 전면 재작성
  - 10대 경영 프레임워크 → 특정 JSON 필드에 1:1 직접 매핑 (체크리스트 형태로 강제)
  - 필수 반영 원칙 6가지 추가:
    1. 5 Forces → SWOT 기회/위협 직접 문장 인용
    2. TAM/SAM/SOM → STP 세분화 + KPI 목표 수치 연계
    3. 경쟁사 약점 → SWOT 기회 + 포지셔닝 전략 직접 활용
    4. 진단 점수 등급별 우선순위 처방 (🔴위험 즉각 1순위)
    5. 업종 트렌드 → SWOT 기회 최소 2개 직접 인용
    6. 정부지원사업 → 로드맵 1단계 신청 일정 포함
  - keyStrategies: 루멜트 [진단→방침→행동] 3단 구조 강제 (희망 목록 나열 금지)
  - fourP.promotion: StoryBrand 7단계 구조 강제 ([고객이 원하는 것]→[문제]→[가이드]→...)
  - 로드맵 1단계: 린 MVP Build→Measure→Learn 사이클 명시
  - 로드맵 3단계: 6가지 시스템 취약 영역 강화 순서 구조
  - 로드맵 전체: 플라이휠 가속 구조 (초기 성공 → 다음 단계 가속)
  - 60자 문자열 제한 완전 제거 → 상세 서술 허용
- app.js: saveApiKey() 함수 추가 (STEP 4 API 박스 확인 버튼 핸들러) + public API 등록
- index.html: STEP 4 wiz-api-box를 wiz-nav 버튼 아래로 이동 + saveApiKey 확인 버튼 추가

---

## 최근 수정 이력 (2026-04-10)

### Phase 8 완료 — Vercel 배포 + 디자인·UX 대규모 개선

#### Vercel 배포 설정
- `vercel.json` 추가 (보안 헤더·캐시 설정, builds 블록 제거로 정적 파일 전체 서빙)
- `.gitignore` 추가 (hwpx·env 파일 제외)
- `index.html` SEO meta + Open Graph / Twitter Card 태그 추가

#### 랜딩페이지 디자인 — Option 2 다크+화이트 투톤
- 히어로·가치·후기·CTA: 다크 `#0A0E1A` 유지
- Pain·Features·DEMO·Pricing·FAQ: 라이트 `#F4F6FB` (밝은 배경)
- 라이트 섹션 텍스트: `#1A2340`, 카드: 흰색+그림자

#### 히어로 Canvas 배경 애니메이션 (js/hero-canvas.js 신규)
- 우상향 차트 라인, 22개 데이터 노드+연결선, 플로팅 전략 카드 8종
- 골드 파티클 상승, 바 차트 그라데이션 애니메이션
- 탭 숨김 시 RAF 자동 중지, 리사이즈 대응

#### Finnhub API 연동 (ticker.js)
- 다우(^DJI)·나스닥(^IXIC)·닛케이(^N225) 실시간 + 등락률 표시
- API 키: `d7c610hr01quh9fcl1d0d7c610hr01quh9fcl1dg` (무료, 60회/분)
- 5분마다 환율+지수 동시 갱신

#### UX 개선
- "무료로 시작하기" → "진단 시작하기"로 전체 통일
- "데모 체험" 버튼 완전 제거
- 우상단 "진단 시작하기" 버튼 제거
- API 모달 제거 → startWizard() 직접 위저드로 이동
- STEP 4 하단 API 키 입력란 추가 (선택사항, 미입력 시 샘플 데이터)
- 모든 화면의 BizNavi 로고 클릭 → 랜딩 홈으로 이동

---

## 최근 수정 이력 (2026-04-09)

### Phase 7 착수 — AI 엔진 경영 프레임워크 10권 반영 (js/ai-engine.js)
- `SYSTEM` 프롬프트에 `[경영 프레임워크 10권 적용 지침]` 섹션 추가
  - ① 블루오션 전략 (김위찬): SWOT 기회에 ERRC 관점 새 시장 포함
  - ② 경쟁우위 (포터): 5 Forces → SWOT 위협·기회 직접 연결
  - ③ 좋은 전략 나쁜 전략 (루멜트): keyStrategies '진단→방침→행동' 3요소 구조
  - ④ OKR (존 도어): KPI를 Key Result 형태 + 분기별 체크포인트
  - ⑤ 린 스타트업 (에릭 리스): 로드맵 1단계 MVP 관점 + Build-Measure-Learn
  - ⑥ 제로 투 원 (피터 틸): SWOT 강점에 독점적 우위·비밀(Secret) 반영
  - ⑦ 하이 아웃풋 매니지먼트 (앤디 그로브): 핵심전략 레버리지 우선 배치
  - ⑧ 좋은 기업을 넘어 위대한 기업으로 (짐 콜린스): 헤지호그+플라이휠 → 전략·로드맵
  - ⑨ **무기가 되는 스토리 7단계** (도널드 밀러): 4P 촉진·STP 포지셔닝에 StoryBrand 7단계 적용 (고객=주인공, 브랜드=가이드)
  - ⑩ **무기가 되는 6가지 시스템** (도널드 밀러): 로드맵 3단계를 6대 시스템(리더십·마케팅·판매·제품·운영·재무) 취약순 강화 구조로 구성
- `buildPrompt()` 분석 지침 하단에 프레임워크 자가 체크리스트 6항목 추가 (AI 응답 누락 방지)

### 다음 작업 (Phase 7 잔여)
- 7-1: 정부지원사업 자동 매칭
- 7-3: 웹서치 연동 (실시간 업종 데이터)

---

## 최근 수정 이력 (2026-03-30)

### 위저드 1단계 필드 추가 (index.html + wizard.js)
- `bizModel` 선택박스 추가 — 업종 바로 아래 위치, 9가지 옵션 (B2B SaaS, B2C 구독, 플랫폼·마켓플레이스 등), 필수 필드(`*`)
- `coreStrength` 한 줄 입력란 추가 — 주요 제품/서비스 아래 위치, `maxlength="60"`, 필수 필드(`*`)
- `wizard.js` `validate(1)` 에 `bizModel`·`coreStrength` 미입력 시 alert 추가
- `wizard.js` `collect()` 에 `bizModel`·`coreStrength` 수집 추가

### 위저드 2단계 필수 필드 변경 (index.html + wizard.js)
- `competitors`: 선택 → 필수 필드로 변경 (`*` 표시, `field-check` ✓ 아이콘 추가)
- `targetCustomer`: 이미 필수였으나 placeholder 업데이트 (제조기업 예시로 구체화)
- `wizard.js` `validate(2)` 에 `targetCustomer`·`competitors` 미입력 시 alert 추가

### AI 엔진 고도화 v2.0 (js/ai-engine.js)
- 시스템 프롬프트 강화: "맥킨지 출신 시니어 컨설턴트" 페르소나 + 5대 핵심 원칙 (일반론 금지, 업종 특화, 근거 포함 등)
- SWOT 구조 변경: `string[]` → `{item, evidence}[]` (각 항목에 근거/활용방안 포함, 6개씩)
- `keyStrategies` 구조 변경: `owner`·`timeline` 필드 추가, 5개 → 6개로 확장
- `kpi` 구조 변경: `method`·`owner` 필드 추가, 6개 → 10개로 확장
- `roadmap` 구조 변경: 각 phase에 `budget` 필드 추가, task 4개 → 6개로 확장
- `buildPrompt()`: `bizModel`·`coreStrength`·`competitors` 반영, 분석 지침 섹션 추가
- `max_tokens`: 4096 → 8000으로 증가
- `fakeAnalysis()`: `bizModel`·`comp`·`tl`·`cs` 변수 활용한 동적 데모 데이터 생성

### 대시보드 CSS 보완 (css/dashboard.css)
- `.swot-evidence`: SWOT 각 항목의 근거 텍스트 스타일 (10px, `var(--txt3)`, 좌측 indent)
- `.strat-meta`: 핵심전략 카드 하단 담당자·기간 표시 영역 (flex, 10px, `var(--txt3)`)
- `.kpi-meta`: KPI 카드 하단 측정방법·담당자 표시 영역 (flex, 10px, `cursor:help`)
- `.rm-budget`: 로드맵 phase 헤더 우측 예산 표시 (10px, `var(--txt3)`, `margin-left:auto`)

---

## 최근 수정 이력 (2026-03-27)

### Hero 섹션 텍스트 변경 (index.html)
- 배지 텍스트: `Claude AI 기반 전략 분석 엔진` → `진단을 통한 실제전략 분석엔진`
- h1 헤드라인: 두 줄 구성 — `경영전략 수립과` / `실행계획을 한번에!` (두 번째 줄 골드 강조)
- 부제목: `<br>` 제거하여 한 줄로 — `SWOT · STP · 4P 분석과 실행 로드맵을 30분 안에 자동 생성`

### 글로벌 시장 실시간 롤링 배너 추가 (index.html + landing.css + js/ticker.js)
- TradingView 위젯 제거 → 자체 제작 롤링 배너로 교체
- 위치: `position:fixed; bottom:0` — 모든 화면에서 항상 하단 고정
- 높이: `48px` / 배경: `#050810` / 상단 테두리: `2px solid rgba(245,192,48,0.35)`
- `z-index: 9999` / `body { padding-bottom: 48px }` 로 콘텐츠 가림 방지

#### 표시 항목 (10종)
| 항목 | 표시 방식 | 데이터 |
|------|----------|--------|
| 코스피·코스닥·다우·나스닥·닛케이 | `장중` (이탤릭·뮤트) | CORS 제한으로 실시간 불가 |
| 달러/원·엔/원·위안/원 | 실시간 숫자 + 5분 갱신 | `open.er-api.com` (CORS 지원, 무료) |
| WTI·금 | 고정값 (`$78.00` / `$2,300`) | 참고용 고정 |

#### 구성 요소
- 좌측: `글로벌 시장 실시간` 레이블 (골드, 0.85rem) + 실시간 시계 HH:MM:SS
- 우측: `주가: 장중 업데이트` 안내 (데스크톱만 표시)
- 롤링: CSS `@keyframes lp-ticker-move` (translateX 0 → -50%), 콘텐츠 2× 복제로 끊김 없는 루프
- hover 시 일시정지, 속도 동적 조정 (~40px/s 기준)

#### ticker.js 구조
- IIFE 패턴, DOMContentLoaded 이후 실행
- `fetchFX()`: `open.er-api.com/v6/latest/USD` → KRW/JPY/CNY 환산
  - 엔/원은 100엔 기준으로 표시 (`rates.KRW / rates.JPY × 100`)
- `buildTrack()`: DOM 최초 1회 생성 (fallback 텍스트로 즉시 표시)
- `updateInPlace()`: 데이터 갱신 시 DOM 텍스트만 교체 (애니메이션 유지)
- `adjustSpeed()`: `requestAnimationFrame`으로 실제 너비 측정 후 duration 계산
- 5분 간격 자동 갱신 (`setInterval`)

#### CSS 클래스 (ticker 전용)
| 클래스 | 역할 |
|--------|------|
| `.lp-ticker-left` | 레이블·시계 영역 (flex-shrink:0, border-right) |
| `.lp-ticker-scroll` | 롤링 영역 (overflow:hidden) |
| `.lp-ticker-track` | 애니메이션 대상 (inline-flex, 2× 복제) |
| `.lp-ticker-note` | 우측 안내 텍스트 (데스크톱만) |
| `.lp-t-item` | 개별 종목 래퍼 (`data-id` 속성으로 JS 타겟팅) |
| `.lp-t-name` | 지수명 (#9BAAC8, 0.78rem) |
| `.lp-t-val` | 실시간 값 (#E8EDF5, 0.85rem, 600) |
| `.lp-t-fallback` | 장중 텍스트 (이탤릭, 뮤트, 0.7rem) |
| `.lp-t-fixed` | WTI·금 고정값 (반투명, 0.82rem) |
| `.lp-t-chg.up/.dn` | 등락률 (#4ADE80 / #F87171) |
| `.lp-t-sep` | 구분선 `|` (rgba(42,63,117,0.6)) |

---

## 최근 작업 현황 (2026-04-08 업데이트)

### Phase 2 완료: 진단 모듈 시스템 구축
- js/diagnosis/common.js: 공통 모듈 (4영역 16항목)
- js/diagnosis/industry/: 업종 특화 7개 파일 (뿌리제조·식품·서비스·유통·외식·IT·건설)
- js/diagnosis/bizmodel/: 사업모델 특화 9개 파일 (B2B SaaS·B2C구독·솔루션·커머스·플랫폼·프랜차이즈·제조유통·서비스·기타)

### Phase 3 완료 - 4단계 위저드 구현
- index.html: 4단계 스텝 인디케이터 완성
- STEP 2 업종별 맞춤 진단 화면 추가
  - 탭 구조: [기본 경영] [업종 특화] [사업모델]
  - 5점 척도 + 메모 입력 UI
  - 탭 순서대로 진행 (기본경영→업종특화→사업모델→STEP3)
  - 이전 버튼 탭 역순 이동
  - 미체크 항목 경고 + 스크롤 기능
  - 입력값 유지 (diagScores 보존)
  - 진행률 대시보드 정상 작동
- 업종 12개 + 사업모델 9개 드롭다운 반영
- wizard.js INDUSTRY_MAP / BIZMODEL_MAP 완성

### Phase 4 완료 - ai-engine.js 진단 결과 연동
- 진단 점수 계산 함수 추가 (calcDiagScores)
- 점수 등급 판별 함수 추가 (getScoreLabel)
- 진단 요약 생성 함수 추가 (buildDiagSummary)
- buildPrompt에 진단 결과 섹션 추가
- 시스템 프롬프트 고도화 (0~8번 지침)
  - 0번: 쉬운 한국어 언어 원칙
  - 5번: 진단 점수 등급별 활용 지침
  - 6번: 취약 영역 구체적 처방
  - 7번: 실행 가능한 액션 플랜
  - 8번: 업종별 인사이트 활용
- 업종별 insights 프롬프트 자동 반영 (buildInsightsSummary)
- 새분석 버튼 → STEP 1 초기화 수정
- STEP 3 이전 버튼 → STEP 2 진단 화면으로 수정

### 5개 업종 전용 JS 파일 완성
- medical.js / finance.js / education.js / fashion.js / media.js
- wizard.js INDUSTRY_MAP 전용 파일로 교체 완료
- index.html script 태그 추가 완료
- 총 진단 파일: 22개 (common 1 + industry 12 + bizmodel 9)

### 다음 작업
- Phase 7 잔여: 솔루션 품질 강화
  - 7-1: 정부지원사업 자동 매칭
  - 7-3: 웹서치 연동 (실시간 업종 데이터)
  - ~~7-2: 경영 서적 10권 프롬프트 반영~~ ✅ 완료 (2026-04-09)
- Phase 8: Vercel 배포
- Phase 9: DART 재무분석 모듈 별도 개발 (추후)

---

## 완성 상태 (2026-03-30 기준)
- 랜딩페이지 10개 섹션 완성 (Version B 신뢰구축형, 다크네이비 테마)
- 멀티파일 구조 분리 완료 (HTML/CSS/JS 완전 분리)
- 위저드 → AI 분석 → 대시보드 전체 흐름 완성
- 위저드 1단계: `bizModel` 선택박스 + `coreStrength` 입력란 추가 (필수 필드)
- 위저드 2단계: `targetCustomer`·`competitors` 모두 필수 필드로 확정
- 위저드 3단계 입력 화면 다크테마 통일 완료 (딥네이비 배경, 골드 포인트)
- AI 엔진 v2.0: SWOT evidence 구조, KPI 10개+측정방법, 전략 owner/timeline, 로드맵 budget 포함
- 모바일 반응형 완료 (햄버거 메뉴, 2×2 통계 그리드, 1열 레이아웃)
- Hero 격자 패턴 + 골드 후광 애니메이션 완료
- 섹션 스크롤 fade-in 애니메이션 완료 (IntersectionObserver)
- 네비게이션 스크롤 blur 효과 완료
- 결과 대시보드 개선 완료
  (좌측 목차 사이드바, KPI 카드 다크테마, DEMO DATA 뱃지 위치 수정, 섹션 간격 개선)
- 글로벌 시장 실시간 롤링 배너 완료 (자체 제작, position fixed 하단 고정)
- GitHub 업로드 완료 → Vercel 배포 진행 중 (미커밋 변경 존재)

---

---

## 부록 — 2026-09-16 문서 분리 시점에 CLAUDE.md에서 교체된 구버전 기술

CLAUDE.md를 6,410줄에서 427줄로 줄이면서 **사실과 달라진 구조 기술을 갱신**했다.
갱신 전 원문을 여기 보존한다(삭제가 아니라 이동이다).

### 갱신 사유
- `배포 상태` — "최신 커밋 `c317c03`"은 2026-06-05 시점 값이라 3개월째 낡아 있었다. 고정 정보만 남겼다
- `파일 구조` — **업종 진단 7개 / 사업모델 9개**로 적혀 있었으나 실제는 **19개 / 12개**다.
  `api/`·`lib/`·`print.css`·`ppt-export.js` 등 이후 추가된 파일도 누락돼 있었다
- `JS 로드 순서` — 위와 같은 이유로 실제 `index.html`과 달랐다(현재 53개 스크립트)
- 커밋 규칙 — 이력 기록 대상을 `CLAUDE.md` → `HISTORY.md`로 바꿨다

### 구 `## 배포 상태 (2026-06-05 최신)`

## 배포 상태 (2026-06-05 최신)

- **GitHub**: `https://github.com/dodson108-sudo/biznavi.git` — 최신 커밋: fix: 직접입력 시 placeholder 미갱신 2가지 수정 (c317c03)
- **Vercel**: GitHub 연동 자동 배포 중 (main 브랜치 push 시 자동 빌드), 서울 리전(icn1) 적용, **Pro 플랜** 운영 중
- **브랜치**: `main` (단일 브랜치 운영)


### 구 `## 파일 구조 및 역할` (JS 로드 순서 포함)

## 파일 구조 및 역할

```
biznavi/
├── knowledge_base/
│   ├── 12개사업모델진단.pdf          # 12개 BM 진단 설계 원본
│   ├── 16개업종진단.pdf              # 16개 업종 진단 설계 원본
│   ├── industry_full_benchmarks.csv  # 18개 업종 재무 벤치마크 (영업이익률·부채비율 등)
│   └── small_biz_cost_guide.csv      # 10개 업종 소상공인 비용 구조 (임대료·인건비·원가 비중)
├── index.html          HTML 뼈대, 인라인 CSS/JS 없음
├── css/
│   ├── style.css       공통 스타일 (위저드·로딩·모달·네비·진단 UI — 다크테마)
│   ├── landing.css     랜딩페이지 전용 (lp-* 클래스, 다크테마, 모바일 반응형)
│   └── dashboard.css   결과 대시보드 전용
└── js/
    ├── app.js          메인 코디네이터 (화면전환, 모달, 분석 실행, 햄버거 메뉴)
    ├── wizard.js       4단계 입력 위저드 로직 (v3.1)
    ├── ai-engine.js    Claude API 호출 및 데모 데이터 생성
    ├── dashboard.js    결과 렌더링 (SWOT/STP/4P/KPI/로드맵)
    ├── ticker.js       글로벌 시장 롤링 배너 (환율 실시간, 주가 장중 표시)
    └── diagnosis/
        ├── common.js               공통 경영 진단 (4영역 16항목)
        ├── industry/               업종 특화 진단 (7개)
        │   ├── mfg_parts.js        뿌리 제조 및 부품가공업
        │   ├── food_mfg.js         식품 제조 및 가공업
        │   ├── local_service.js    생활밀착형 서비스업
        │   ├── wholesale.js        전문 유통 및 도소매업
        │   ├── restaurant.js       외식 및 휴게음식업
        │   ├── knowledge_it.js     지식 서비스 및 IT개발
        │   └── construction.js     소규모 건설 및 인테리어
        └── bizmodel/               사업모델 특화 진단 (9개)
            ├── b2b_saas.js
            ├── b2c_sub.js
            ├── b2b_solution.js
            ├── b2c_commerce.js
            ├── platform.js
            ├── franchise.js
            ├── mfg_dist.js
            ├── service.js
            └── etc.js
```

### CSS 로드 순서 (index.html head)
```html
<link rel="stylesheet" href="css/style.css" />
<link rel="stylesheet" href="css/landing.css" />
<link rel="stylesheet" href="css/dashboard.css" />
```

### JS 로드 순서 (body 하단, 의존성 순)
```html
<script src="js/ai-engine.js"></script>               <!-- 의존성 없음 -->
<script src="js/dashboard.js"></script>               <!-- 의존성 없음 -->
<script src="js/wizard.js"></script>                  <!-- 의존성 없음 -->
<script src="js/diagnosis/common.js"></script>        <!-- 진단 공통 -->
<script src="js/diagnosis/industry/mfg_parts.js"></script>
<script src="js/diagnosis/industry/food_mfg.js"></script>
<script src="js/diagnosis/industry/local_service.js"></script>
<script src="js/diagnosis/industry/wholesale.js"></script>
<script src="js/diagnosis/industry/restaurant.js"></script>
<script src="js/diagnosis/industry/knowledge_it.js"></script>
<script src="js/diagnosis/industry/construction.js"></script>
<script src="js/diagnosis/bizmodel/b2b_saas.js"></script>
<script src="js/diagnosis/bizmodel/b2c_sub.js"></script>
<script src="js/diagnosis/bizmodel/b2b_solution.js"></script>
<script src="js/diagnosis/bizmodel/b2c_commerce.js"></script>
<script src="js/diagnosis/bizmodel/platform.js"></script>
<script src="js/diagnosis/bizmodel/franchise.js"></script>
<script src="js/diagnosis/bizmodel/mfg_dist.js"></script>
<script src="js/diagnosis/bizmodel/service.js"></script>
<script src="js/diagnosis/bizmodel/etc.js"></script>
<script src="js/app.js"></script>                     <!-- 위 모두 참조 -->
<script src="js/ticker.js"></script>                  <!-- 독립 실행, 의존성 없음 -->
```


### 구 커밋 규칙

- **모든 커밋에 CLAUDE.md 이력 기록을 포함한다.** 지시에 명시되지 않아도 기록한다. **"무엇을 했다"가 아니라 "왜 그렇게 했는지"를 남긴다** — 근거가 없으면 나중에 되돌려진다. 검토해서 채택하지 않은 대안과 그 이유도 함께 적는다

