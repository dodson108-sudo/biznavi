...
### ⑦ 남은 이슈 (미결)
- **`restaurant` + `yes` + 사회적경제 인증** 조합에서 제조업 경고가 뜬다. 이 기업은 인증만으로 이미 예외 대상이라 제조업 응답이 자격에 영향을 주지 않으므로 경고가 불필요한 노이즈다.
  → `_kosmesEligibility`에서 **인증(certs) 검사를 비제조 경고 분기보다 먼저** 두면 해소된다. 판단 대기 중.

### ⑧ 캐시버스팅
`index.html` 로컬 `?v=` 47곳 전부 `20260806a`
