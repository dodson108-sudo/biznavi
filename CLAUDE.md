...
## 최근 수정 이력 (2026-09-04) — DiagMicro 외식업 편향 제거 (D1·D2 완료, D3~D7 진행 예정)
...
### ⑤ 검증 (D1: 9/9 · D2 누적: 14/14)
| 항목 | 결과 |
|---|---|
| food D1+D2 10문항 | label·question·scale·ai_trigger **원문과 완전 동일** ✓ |
| 비외식 4그룹 외식용어 | D1 12건 → **0건** / D2 포함 **0건** ✓ |
| guide | 5그룹 × 10문항 = **50건** 전수 · 렌더링 확인 ✓ |
| scale·ai_trigger | 5단계 전수 · `warning` 코드 전 그룹·전 문항 **불변** ✓ |
| **calcScores** | 1~5점 전부 수정 전과 **동일** (점수 로직 무변경) ✓ |
| 문항 수 | **35 불변** ✓ |
| D3~D7 | 25문항 × 5그룹 **완전 무변경** ✓ |
| `INDUSTRY_GROUP_MAP` | 무변경 ✓ |
| food AI 프롬프트 | 무변경 ✓ |
| `detectCrossWarnings` 폴백 | 무인자·undefined·오타 그룹 = 원문 ✓ |

### ⑥ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818g` (D1 단계는 `20260818f`)

### ⑦ 남은 이슈
1. **D3~D7 30문항 미완.** D3(`3_2` 밀키트·`3_1` 배달), D4, D5~D7이 남았다. 같은 원칙으로 진행한다
2. **⚠ `DOMAINS`의 `desc`가 전 그룹 공통이다 (신규 발견, 미수정).**
   D2 `'파사드 시선 주목도와 로컬 SEO 연동'`·D3 `'오프라인·배달·이커머스 판로'`가
   세무사·학원에도 그대로 나간다. `_diagMicroToAreas`가 `domain.desc`를 영역 설명으로 렌더링한다.
   ⚠ `calcScores`가 `domain.label`을 반환값에 쓰므로 **`label`은 건드리면 안 된다**(레이더차트·대시보드 영향).
   `desc`만 그룹별로 분기하는 것이 안전하다. **D3~D7 작업과 함께 처리 예정**
3. **`ai_trigger.warning_msg`는 소비처가 0건이다.** `buildPromptSummary`는 `item.label`만 쓰고
