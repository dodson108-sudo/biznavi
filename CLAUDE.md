...
### ⑨ 남은 이슈
1. **4단계 미완** — `ACTION_PLAN_7DAY_BY_GROUP.manufacturing` 5도메인 × 7일 = 35항목.
   현재 `manufacturing`은 그룹 계획이 없어 **base(외식 기준)로 폴백**한다
   (`Dog 메뉴 제거`·`파사드 시선 촬영`·`테이블에 QR 스탠드`). **다음 작업에서 반드시 채워야 한다**
2. `wholesale`·`fashion`·`logistics`·`construction`·`export_sme` 5개가 여전히 `retail`이다.
   유통·물류·건설 그룹 분화는 별도 판단이 필요하다
