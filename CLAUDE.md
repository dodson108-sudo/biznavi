...
## 최근 수정 이력 (2026-09-04) — DiagCommon(sme 경로) 외식업 편향 수정 8문항

**sme 경로(직원 6명 이상)가 쓰는 `DiagCommon` 20문항 중 8개가 오프라인 매장·요식업 전제였다.**
제조업 중소기업 대표가 "오픈·마감·위생 검수", "클라우드 POS·CCTV", "식재료 로스"를 받고 있었다.

### ① ⚠ DiagMicro와 방식이 다르다 — 그룹 구조를 만들지 않았다
`DiagCommon`에는 `INDUSTRY_WORDING` 같은 **그룹별 오버라이드 구조가 없다.**
따라서 DiagMicro의 "기본 중립 + food 보존" 방식을 쓸 수 없다.
그룹 구조 신설은 범위 밖이므로 **8문항을 업종 중립으로 직접 수정**했다.

**구체성 손실은 `guide`가 메운다.** `guide`는 그룹별로 바뀌지 않으므로
**한 문장에 여러 업종 예시를 함께 담았다.**
> `5_2` guide — `예: 외식업은 식재료 로스, 제조업은 불량·스크랩, 유통은 파손·반품, 서비스업은 소모품·인쇄물`

### ② 재작성 5개
| 문항 | 수정 전 | 수정 후 |
|---|---|---|
| `2_1` | **로컬 플랫폼 NAP 정합성** — 네이버 스마트플레이스·카카오맵·구글 비즈니스 | **온라인 기업 정보 정합성** — 홈페이지·검색 포털·업종 디렉터리 |
| `3_2` | **오픈·마감·위생 검수** 등 시간대별 임무 | **업무 시작·종료 점검, 품질 검수** 등 |
| `3_3` | **매장**·현장 부재 / 클라우드 **POS·CCTV** | **사업장**·현장 부재 / 클라우드 기반 시스템 |
| `4_3` | **POS·배달 플랫폼·ERP** 데이터 통합 | **판매·생산·재고** 시스템 데이터 통합 |
| `5_2` | 폐기물·**식재료 로스** 단가 가치 | 폐기물과 **원부자재 손실**의 단가 가치 |

`4_3`은 scale 5단계가 통째로 요식업 DX 경로(`플랫폼 등록 → 리뷰 대응·예약 플랫폼`)여서
`개별 도입 → 부분 연계 → AI 활용 → 전사 통합`으로 재작성했다. guide의 5단계 요약도 함께 고쳤다
(`초기수동→탐색노출→구축관리…`의 '탐색노출'은 로컬 SEO 개념이라 제조업에 성립하지 않는다).

### ③ 문구만 손본 3개
`1_3` scale4 `POS·ERP와 연동` → `매출 집계 시스템과 연동` /
`3_1` scale3 `POS·화이트보드에 게시` → `사내 게시판·대시보드에 게시` /
`3_4` scale4 `QR코드 현장 부착` → `QR코드 작업 현장 부착`

### ④ 검증
| 항목 | 결과 |
|---|---|
| 수정 8문항 **본문**(label·question·scale) | 외식·오프라인 매장 전용어 **0건** ✓ |
| 업종 무관 **12문항** | **완전 불변**(JSON 동일) ✓ |
| `calcScores` | 1~5점 전부 수정 전과 **동일** ✓ |
| 문항 수 | **20 불변** ✓ |
| `guide` | **20건 전수 유지** · sme 경로 렌더링 20건 확인 ✓ |
| `ai_trigger`·`scale` | `warning` 코드·threshold·5단계 **전수 불변** ✓ |
| `DOMAINS`·`detectCrossWarnings` | 불변 ✓ |

⚠ **`guide`에는 `POS`·`식재료`·`플레이스` 같은 단어가 의도적으로 남아 있다.**
업종별 예시를 나열하는 필드이므로 정상이며, 전용어 검사는 **본문만** 대상으로 해야 한다.

### ⑤ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818k`

### ⑥ 남은 이슈
1. **`DiagCommon`에는 업종 그룹 구조가 없다.** 이번엔 중립화 + guide 다중 예시로 해결했으나,
   sme 경로도 업종별 분화가 필요해지면 `DiagMicro`의 `INDUSTRY_WORDING` 방식을 이식해야 한다.
   **별도 작업이며, 하려면 `getSchema(group)` 시그니처 변경과 호출부 정리가 선행된다**
2. `DiagSme` 18문항은 여전히 어떤 경로에서도 렌더링되지 않는다 (기존 이슈, 변동 없음)

---

## 최근 수정 이력 (2026-09-04) — DiagMicro 외식업 편향 제거 **35문항 전체 완료 (D1~D7)**

**35문항이 외식업 기준으로 작성돼 있고 `INDUSTRY_WORDING` 오버라이드가 13문항만 덮어,
미용실·소매·학원·전문서비스 사장님이 "테이블 회전율"·"메뉴 구성"·"주방 동선"을 그대로 받고 있었다.**

### ① 방식 — 새 구조를 만들지 않았다
`getSchema(group)`이 `ITEMS`에 `INDUSTRY_WORDING[group]`을 `Object.assign`으로 병합하는
**기존 메커니즘을 그대로 쓴다.** 문제는 두 가지였다.
1. 기본 `ITEMS` 자체가 외식업
2. 덮어쓴 것이 13문항뿐 — 게다가 **`label`·`question`만 덮고 `scale`은 외식 문구가 남아 있었다**

```
기본 ITEMS  → 업종 중립으로 교체
INDUSTRY_WORDING.food (신설) → 제거한 외식 표현을 원문 그대로 보존
나머지 4그룹 → 빠진 문항 신설 + scale 보강 + guide 추가
```
⚠ **food 그룹 결과는 `label`·`question`·`scale`·`ai_trigger`가 수정 전과 글자까지 동일하다.**
중립화하면서 외식업 품질을 희생시키지 않는 것이 이 작업의 제약이다.

### ② `guide` 필드 신설 — 질문은 중립, 예시는 업종별
질문을 중립화하면 구체성이 떨어진다. 이를 `guide`가 메운다.

| 그룹 | `1_2` guide |
|---|---|
| food | 예: 테이블 회전율, 객단가, 재방문 주기 |
| beauty | 예: 시술 단가, 예약 밀도, 재방문 주기 |
| retail | 예: 구매 단가, 재구매율, 장바구니 크기 |
| edu_service | 예: 수강료 단가, 재등록률, 수강 지속 기간 |
| pro_service | 예: 건당 수임료, 재의뢰율, 계약 지속 기간 |

⚠ **`guide`는 원래 렌더링되지 않는 죽은 필드였다.** `DiagCommon`에 20문항 전부 `guide`가
있었지만 [wizard.js `_renderItemHtml()`](js/wizard.js)이 `question`·`scale`·`memo`만 출력하고
`guide`를 무시해 **화면에 나온 적이 없었다.** 이번에 렌더링 코드(2줄) + `.diag-item-guide`
스타일을 추가해 **살렸다.**
→ **부수효과(의도한 것)**: `DiagCommon` 20문항의 guide도 함께 표시된다.
   `업종 기준치: 외식 60% / 제조 75% / IT 30%` 같은 내용이라 sme 사장님의 응답에 도움이 된다.

### ③ 커버리지 — 실제로 비어 있던 곳
| 문항 | 수정 전 | 조치 |
|---|---|---|
| `1_1` 상권 빅데이터 | **오버라이드 0건** — 4그룹 전부 "메뉴 공급" 수신 | 기본 중립화 + 4그룹 신설 |
| `1_2` 판매 데이터 | label·question만 덮음 → **scale이 외식** | 그룹별 scale 신설 |
| `1_3`·`1_4` | 완전 커버 | guide만 추가 |
| `1_5` OHI | question만 덮음 → **scale이 "주방 조리·홀"** | 그룹별 scale 신설 |
| `2_1`·`2_2` | **오버라이드 0건** | 기본 중립화 + 4그룹 신설 |
| `2_3`·`2_4`·`2_5` | label·question만 덮음 → **scale이 외식** | 그룹별 scale 신설 |
| `3_1`·`3_2`·`3_5` | label·question만 덮음 → **scale이 외식** | 그룹별 scale 신설 |
| `3_3`·`3_4` | **오버라이드 0건** | 기본 중립화 + 4그룹 신설 |
| `4_1`·`4_2` | label·question만 덮음 → **scale이 외식** | 그룹별 scale 신설 |
| `4_3`·`4_4`·`4_5` | **오버라이드 0건** | 기본 중립화 + 4그룹 신설 |
| `5_1` / `6_1`·`6_4` | **오버라이드 0건**, 외식용어 3건 / 3건 | 기본 중립화 + food 보존 |
| `5_2`~`5_5`·`6_2`·`6_3`·`6_5` | **깨끗함** (자금·세무·폐업은 업종 무관) | **본문 미변경, guide만 삽입** |
| `7_1`·`7_5` | question만 덮음 → **scale이 외식** | 그룹별 scale 신설 |
| `7_2`·`7_3`·`7_4` | **오버라이드 0건** | 기본 중립화 + 4그룹 신설 |

`2_5`는 편향이 가장 심했다 — `주방 3각 작업 삼각형(싱크대·화구·준비대)`.
기본을 `작업 공간 3각 동선(자재 보관·작업대·처리 설비)`으로 바꾸고 그룹별로
`보관장→준비대→시술석`(beauty) / `창고→진열대→계산대`(retail) /
`교구실→교실→출력공간`(edu) / `파일실→상담실→접수대`(pro)로 분화했다.

⚠ `1_1`의 `매장`은 **기본을 `사업장`(중립)으로 두고 food·beauty·retail·edu가 `매장`으로
덮어쓴다.** `pro_service`만 `사업장`을 유지한다 — 세무사·컨설팅 사무소에 `매장`은 부적합하다.
**기본은 중립, 각 그룹이 자기 표현을 덮어쓰는 구조**가 이 모듈의 원칙이다.

### ④ ⚠ 같은 패턴의 결함 3건 — 그룹을 무시하고 기본 `ITEMS`를 참조
전수 조사 결과 **"화면은 미용실인데 내부 로직은 외식"** 인 지점이 더 있었다.

| 지점 | 판정 | 조치 |
|---|---|---|
| `buildPromptSummary()` | **결함** — `Object.entries(ITEMS)`로 기본 label 사용. 미용실 진단인데 **AI 프롬프트에 `POS 데이터 분석`·`조정 기여 마진(ACM) 산출`이 나갔다** | `getSchema(group).items` 사용으로 수정 |
| `detectCrossWarnings()` | **결함** — 그룹 인자가 없어 `Dog 메뉴 정리`·`배달 단일 채널`·`사장이 매장에 묶여` 가 전 업종에 나갔다 | `detectCrossWarnings(scores, industryGroup)`로 확장 |
| `calcScores()` L1230 `Object.keys(ITEMS)` | **결함 아님** — 키만 쓴다. 키는 전 그룹 동일 | 미변경 |

**`detectCrossWarnings` 시그니처 확장 시 호출부를 전수 조사했다.**
`DiagMicro`의 것은 **2곳**뿐이다 — `wizard.js:3127`(`microGroup` 전달로 갱신) / 내부 `buildPromptSummary`.
`dashboard.js:698`의 `D.detectCrossWarnings`는 **사회적경제 모듈**이라 무관하다.
⚠ **인자 미지정 시 `food`로 폴백**한다(`WARN_WORDING[group] || WARN_WORDING.food`).
무인자·`undefined`·오타 그룹 모두 기존 동작과 동일함을 검증했다.

`WARN_WORDING` 테이블로 어휘만 분기한다(`cost`·`dog`·`subj`·`channel`·`owner`).
조사(은/는) 문제를 피하려 **어절 단위가 아니라 완성 구(`이 사무소는`·`대표가 사무소에`)로 보관**한다.

### ④-2 D3·D4에서 특히 컸던 편향
| 문항 | 수정 전 (전 그룹) | 수정 후 (기본) |
|---|---|---|
| `3_2` | **패키징 규격 생산력** — 진공 포장·급속 냉동·**밀키트** | 상품·서비스 패키지화 |
| `4_1` | 주문 접수 지연율 — **테이블**·벨 호출·**키오스크** | 접수 처리 지연율 |
| `4_2` | 단순 육체노동 단축 — **홀 퇴식·서빙 로봇·식기세척** | 단순 반복 노동 단축 |

그룹별로 실제 설비·업무에 맞춰 분화했다 — `세탁·소독 자동화`(beauty) / `바코드·자동 재고 집계`(retail) /
`출결 자동화·자동 정산`(edu) / `전자문서·전자계약·자동 청구`(pro).

⚠ `retail`의 기존 `3_1` 문구에 있던 **`배달 앱`을 `오픈마켓`으로 바꿨다.**
`mfg_parts`(부품가공)·`wholesale`·`construction`도 retail 그룹이라 배달 앱은 부적합하다.

### ④-3 `DOMAINS.desc` 그룹 분기 (신규)
`DOMAINS`는 그룹 인지가 없어 D2 `'파사드 시선 주목도'`·D3 `'오프라인·배달·이커머스 판로'`가
세무사·학원에도 그대로 나갔다. `DOMAIN_DESC_BY_GROUP`을 신설하고 `getSchema()`가 **`desc`만 덮은**
`domains`를 반환하게 했다.

⚠ **`label`·`key`·`weight`·`id`는 절대 분기하지 않는다.** `calcScores()`가 반환값에 `domain.label`을
실어 레이더차트·대시보드로 흘려보내므로 바꾸면 표시가 어긋난다. `food`는 기본 `DOMAINS`를
객체 동일성까지 유지한다.

### ④-4 ⚠ ACTION_PLAN 전수 조사 — 보고된 것보다 훨씬 작았다
"실행계획이 외식업 그대로"라는 지적을 전수 조사한 결과 **실제 잔존은 1건**이었다.

| 확인 항목 | 결과 |
|---|---|
| `ACTION_PLAN_7DAY_BY_GROUP` 140개(4그룹×5도메인×7일) | 외식 용어 **1건** — `beauty` `'Dog 시술 예약 메뉴에서 비노출'` → `'예약 목록에서'` 교정 |
| `pro_service` 5건 | **오탐** — `포스터`가 `포스`로 매칭된 것. 실제 계획은 `서비스별 수임 건수 추출`·`크몽·숨고 입점`·`파일실·상담실 동선 실측`으로 이미 업종에 맞다 |
| base `ACTION_PLAN_7DAY`(외식 기준) | 비외식 그룹에는 **D5·D6만 도달**한다(그룹 계획이 없는 2개 도메인). 두 title 모두 중립(`사업 전용 계좌 분리…`·`월간 BEP 돌파율…`) |

⚠ **base의 `desc`·`tool`·`output`은 어디에서도 소비되지 않는다.** `buildPromptSummary`가 `title`만
프롬프트에 넣는다(저장소 전체 소비처 0건). 인용된 `메뉴별 판매수·단가 정렬`·`테이블에 QR 스탠드`는
이 죽은 `desc` 필드의 내용이며 **사용자에게 전달된 적이 없다.**

### ④-5 ⚠ 작업 중 실제로 낸 사고 — 그룹 경계 침범
`DOMAIN_DESC_BY_GROUP`을 신설하면서 **`beauty:` 같은 그룹명 키가 파일에 두 벌**이 됐다
(`DOMAIN_DESC_BY_GROUP` 512행대 / `INDUSTRY_WORDING` 711행대, 둘 다 4칸 들여쓰기).
D4 패치 스크립트가 **앞의 것을 먼저 잡아** beauty 범위를 226줄로 계산했고 그룹 경계를 넘었다.
→ 커밋 전 그룹별 오버라이드 키 목록 검사에서 `beauty NOT FOUND`로 잡아내고 **직전 커밋으로 되돌린 뒤**
   스크립트에 `INDUSTRY_WORDING` 이후에서만 탐색 + 그룹 경계 초과 시 assert를 넣어 재작업했다.
**같은 이름의 테이블이 둘 이상 생기면 텍스트 기반 패치는 반드시 앵커를 명시해야 한다.**

### ④-6 D5~D7 — 편향이 D7에 몰려 있었다
착수 전 문항별로 세어 보니 **24건이 고르지 않게 분포**했다.

| 영역 | 오염 문항 | 건수 |
|---|---|---|
| D5 운영자금·ESG보증 | `5_1`만 | 3 |
| D6 사업정리·폐업세무 | `6_1`·`6_4` | 3 |
| **D7 SNS·생성형AI** | **5문항 전부** | **18** |

`5_2`~`5_5`·`6_2`·`6_3`·`6_5` **8문항은 실제로 깨끗했다** — 자금·세무·폐업은 업종 무관이라는
기존 기록이 맞았다. 이 8문항은 **본문을 건드리지 않고 `guide`만 삽입**했다
(원문을 다시 타이핑하면 전사 오류가 날 수 있어 앵커 삽입 방식을 썼다).

| 문항 | 수정 전 | 수정 후 (기본) |
|---|---|---|
| `5_1` | 가계·**매장** 계좌 분리 | 가계·**사업** 계좌 분리 |
| `7_2` | **메뉴판** 폰트 수정, 신**메뉴** 홍보 포스터 | 안내물 문구 수정, 신규 홍보 포스터 |
| `7_3` | 리뷰 QR 스탠드를 **테이블**에 부착 | 접수대에 부착 |
| `7_5` | **매장 외경이나 흐릿한 고기 사진** | 사업장 외경이나 흐릿한 사진 |

그룹별로 실제 접점에 맞춰 분화했다 — 리뷰 QR 위치가 `접수대`(beauty) / `계산대`(retail) /
`상담실`(edu) / `상담 후 요청 문자 템플릿`(pro)으로 갈린다.

⚠ **`시그니처`도 외식 유래어로 보고 정리했다.** 기존 그룹 문구에 `시그니처 시술`·`시그니처 강좌`가
남아 있었는데 어색하다 → `대표 시술`·`대표 강좌`·`주력 상품`·`주력 서비스`로 바꿨다(7건).

⚠ **`매장`은 그대로 뒀다.** beauty·retail·edu_service에는 자연스러운 표현이며 이미 승인된 결정이다.
다만 **`pro_service`의 `3_1`에 `오프라인 매장 상담`이 남아 있어 `사무소 상담`으로 고쳤다** —
"pro_service만 중립어를 유지한다"는 결정과 정면으로 어긋나는 한 곳이었다.
D1~D4는 건드리지 않는다는 제약의 예외이며, **단어 하나만 바꿨다.**

### ⑤ 검증 (D1 9/9 · D2 14/14 · D3 15/15 · D4 15/15 · **D5~D7 16/16**)
| 항목 | 결과 |
|---|---|
| **food 전 35문항** | label·question·scale·ai_trigger **원문과 완전 동일** ✓ |
| 비외식 4그룹 외식용어 | **전 35문항 0건** (착수 전 D5~D7만 24건) ✓ |
| guide | 5그룹 × 35문항 = **175건** 전수 · 렌더링 확인 ✓ |
| scale·ai_trigger | 5단계 전수 · `warning` 코드 전 그룹·전 문항 **불변** ✓ |
| **calcScores** | 1~5점 전부 수정 전과 **동일** (점수 로직 무변경) ✓ |
| 문항 수 | **35 불변** ✓ |
| `7_1`·`7_5` 오버라이드 | 보존 확인 (그룹 경계 사고 회귀 검사) ✓ |
| DOMAINS | `label`·`weight` 전 그룹 불변 · food `domains` 객체 동일 ✓ |
| `INDUSTRY_GROUP_MAP` | 무변경 ✓ |
| food AI 프롬프트 | 무변경 ✓ |
| `detectCrossWarnings` 폴백 | 무인자·undefined·오타 그룹 = 원문 ✓ |

### ⑥ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818j` (D1 `f` · D2 `g` · D3 `h` · D4 `i` · D5~D7 `j`)

### ⑦ 남은 이슈
1. ✅ **DiagMicro 35문항 중립화 완료.** D1~D7 전 영역, 5개 그룹 전부 외식용어 0건이며
   food는 원문과 완전 동일하다. **`guide` 175건이 새로 생겼다.**
   ⚠ 남은 근본 한계는 **제조업 전용 그룹이 없다**는 것이다 — `mfg_parts`·`wholesale`·`construction`·
   `logistics`·`export_sme`가 전부 `retail` 그룹을 쓴다. 소매 문구(진열대·계산대·창고)가
   부품가공·건설에 완전히 맞지는 않는다. **그룹 신설은 별도 작업이다**
2. ✅ **`DOMAINS.desc` 그룹 분기 — D3 단계에서 완료** (④-3 참조)
3. **⚠ 죽은 필드 3종의 처리 방침 결정 필요.**
   - `ai_trigger.warning_msg` (그룹별로 작성돼 있으나 미사용)
   - base `ACTION_PLAN_7DAY`의 `desc`·`tool`·`output` (내용이 풍부한데 미사용)
   - `ACTION_PLAN_7DAY_BY_GROUP`은 **`title`만 있고 `desc`가 아예 없다** —
     위 `desc`를 살리기로 하면 **140개 항목에 desc를 새로 써야 한다**(별도 단계 규모)
4. **`ai_trigger.warning_msg`는 소비처가 0건이다.** `buildPromptSummary`는 `item.label`만 쓰고
   `detectCrossWarnings`는 자체 msg를 쓴다. 패턴 일관성을 위해 채워 두었으나
   **활용할지(취약 문항의 경고 문구로 프롬프트에 주입) 제거할지 결정이 필요하다**
4. **`getSchema()`가 오버라이드 없는 그룹에 기본 `ITEMS` 객체 참조를 그대로 반환한다**
   (`var finalItems = ITEMS`). 현재 호출부가 변형하지 않아 문제없으나, 방어적으로 사본을
   반환하는 편이 안전하다

---

## 최근 수정 이력 (2026-09-03) — 창업 초기(STARTUP) 도메인 매핑 오류 수정

### ① 문제 — 빈 막대는 증상이고 진짜 문제는 점수 오집계였다
[wizard.js `calcDomainScores()`](js/wizard.js)의 `isStartup` 분기에서
```js
finance         ← _s1_, _s2_
differentiation ← (없음)
```
STARTUP은 **4영역**인데 도메인은 **5개**다. `differentiation`이 항상 빈 배열이 되어 막대가 비었고,
더 중요하게는 **`s1`(사업 검증) 점수가 `finance`(자금)로 집계**되어 재무 점수가 왜곡됐다.
창업 초기 라벨에서 `differentiation`은 **'사업 검증도'**다 — 라벨과 소스가 어긋나 있었다.

### ② 수정 전 STARTUP 영역 실제 내용 확인
| 영역 | 문항 | 성격 |
|---|---|---|
| s1 사업 검증 및 계획 수립 | 고객 검증(파일럿·첫 계약) / 사업계획서 | **사업 검증** |
| s2 현금 생존력 (Runway) | 런웨이 개월 수 / BEP 산출·달성 시점 | **자금** |
| s3 초기 고객 확보·매출 파이프라인 | 확정 계약 / 30~90일 잠재 고객 | **고객 확보** |
| s4 핵심 역량 및 운영 준비도 | 즉시 제공 가능 여부 / 파트너·인력 확보 | **운영 준비** |

→ 제안된 매핑이 **정확하다**. 그대로 적용했다.
```js
differentiation ← _s1_                          // 사업 검증도
finance         ← _s2_                          // 자금·사업계획
bm              ← _s3_                          // 고객 확보력
hr              ← _s4_                          // 운영 준비도
future          ← diag-industry-container_      // 업종 대응력
```
⚠ **한 가지 어긋남을 남겨 둔다**: `finance` 라벨이 `'자금·사업계획'`인데 사업계획서(`s1_2`)는 `s1`에 있어 `differentiation`으로 간다. 라벨을 `'현금 생존력'`으로 바꾸는 편이 정확하나 **이번 지시 범위(소스 매핑)를 벗어나므로 건드리지 않았다.**

### ③ 영향 확인 (4곳)
**1. 레이더차트** — 5축 모두 값이 생긴다. 수정 전 `differentiation` 축은 0으로 찍혔고 점수 바에 `미입력`이 떴다. 수정 후 5축·점수 바 5건·해설 카드 5건 모두 정상.

**2. PatternDB — 더 나빠지지 않고 오히려 개선된다.**
`domainScores.differentiation?.avg || 3` 구조라 수정 전에는 **기본값 3**으로 대체됐다. 수정 후 실제 값이 전달된다. (PatternDB의 나머지 결함 — micro/사회적경제 키 불일치, `diagData.industry` 공백 — 은 그대로다)

**3. `consultingType` — 판정이 달라질 수 있다.**
`classifyConsultingType()`은 `d.avg > 0`으로 거르므로 수정 전 `differentiation`은 **순위에서 아예 빠졌다**(오분류가 아니라 무시). 다만 `finance`가 s1+s2 혼합값이었으므로 **최약 도메인 판정과 `overallAvg`가 왜곡**돼 있었다. 수정 후 `finance`가 순수 런웨이 점수가 되어 자금 위험이 제대로 최약으로 잡힌다. 이 값은 `data.consultingType`으로 **AI 프롬프트에 전달**되므로 조언 방향이 바뀐다 — 의도한 교정이다.

**4. AI 프롬프트** — `domainScores` 자체는 프롬프트에 직접 들어가지 않고 `consultingType`을 거쳐 반영된다. 위 3번과 동일.

### ④ ⚠ HistoryTracker — 과거 기록과의 불일치 (처리하지 않음, 제안만)
`_flattenDomains()`가 `v.avg || 0`으로 저장하므로 **과거 스냅샷의 `differentiation`은 0**이고 `finance`는 s1+s2 혼합값이다.
`renderCompare()`는 `finance·hr·bm·differentiation` 4개를 과거와 델타 비교한다.

→ 창업 초기 기업이 **재진단하면**:
- `differentiation`: 과거 0 → 현재 실제값 ⇒ **허위 급상승(▲)** 표시
- `finance`: 혼합값 → s2 단독 ⇒ 실제 개선과 무관한 변동

**영향 범위는 좁다** — 창업 초기 모드로 **이전에 진단한 적이 있고**(같은 회사명) 다시 진단하는 경우에만 발생한다. 일반·소상공인·사회적경제 경로는 무관하다.

**제안(임의 처리하지 않음)**: 스냅샷에 `schemaVersion`을 추가하고, `renderCompare()`가 버전이 다른 과거 스냅샷과는 **도메인 델타 비교를 건너뛰고 안내 문구만 표시**하는 방식이 안전하다. 과거 데이터를 소급 변환하는 것은 원본 훼손이라 권하지 않는다. **결정 후 별도 작업으로 진행할 사안이다.**

### ⑤ 검증 (16/16 통과)
`s1=5 · s2=1 · s3=3 · s4=4 · 업종=2`로 서로 다른 값을 넣어 어느 도메인으로 가는지 식별했다.

| 항목 | 결과 |
|---|---|
| ① 5개 도메인 전부 산출 | `finance 1.0 · hr 4.0 · bm 3.0 · future 2.0 · differentiation 5.0` — **0점 도메인 없음** ✓ |
| ② 사업 검증도 = s1 반영 | `differentiation = 5.0` ✓ / `finance = 1.0`(s2 단독) ✓ / **구 매핑이면 finance가 3.0**이었다 ✓ |
| ③ 일반 경로 회귀 | sme 전항목 3점 → 5도메인 **전부 3.0** · 라벨 불변 · micro는 기존대로 `_calcMicroDomainScores` 사용 ✓ |
| 레이더차트 | 5축 전부 0 초과 · 점수 바 5건 · **`미입력` 0건** · 해설 카드 5건 ✓ |
| PatternDB | `differentiation`이 기본값 3이 아닌 실제 값 5.0 전달 ✓ |
| consultingType | 취약 순 `finance(1) < future(2) < bm(3) < hr(4) < differentiation(5)` → `finance_strategy` ✓ |

**`isStartup` 분기만 수정했다.** 일반 경로 매핑·진단 문항은 손대지 않았다.

### ⑥ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818e`
...
### ① `mfg_parts`가 food(외식) 그룹에 매핑되어 있던 문제
[diagnosis-micro.js](js/diagnosis/diagnosis-micro.js)의 `INDUSTRY_GROUP_MAP`에서 **뿌리 제조·부품가공(`mfg_parts`)이 `food`**였다.
`INDUSTRY_WORDING`은 food를 기준(변경 0건)으로 두므로, **제조업 소상공인이 외식업 문구를 35문항 100% 그대로** 받았다.

**`retail`을 택했다** — 제조업 전용 그룹은 만들지 않았다(범위 밖). 5개 그룹 중 가장 덜 부적합한 쪽을 실제 문구를 비교해 골랐다.

| 비교 축 | `retail` | `pro_service` | 판정 |
|---|---|---|---|
| 원가 구조(1_3) | 상품원가 + 인건비 | **외주비·소모품비** + 인건비 | retail — 제조는 재료비+노무비. pro_service는 **재료비 개념 자체가 없어 원가 진단이 성립하지 않는다** |
| ACM(1_4) | 매입 원가 + 처리·진열 노동시간 | 직접 원가 + **상담** 노동시간 | retail |
| 판로(3_1) | 스마트스토어 + **B2B 도매 납품** | 크몽·숨고 + **B2B 자문 계약** | retail — 부품가공은 B2B 납품이 본업 |
| 자동화(4_2) | 바코드·**자동 재고 집계** | 예약 문자·계약서·서류 정리 | retail |
| 동선(2_5) | **창고** · 진열대 · 계산대 | 파일실 · 상담실 · 접수대 | retail — 자재창고 개념 대응 |

⚠ **완전한 해결이 아니다.** retail 전환 후에도 `메뉴·테이블·조리·주방`이 남는다(그룹 분화 커버리지 13/35). 근본 해결은 제조업 전용 그룹 신설이며 남은 이슈로 둔다.
⚠ 다른 업종 매핑은 건드리지 않았다. **`food_mfg`(식품제조)도 `food`로 남아 있으며 부적합하다** — 식품제조는 B2B 납품·OEM 중심인데 "테이블 회전율"을 받는다. 이번 지시 범위가 `mfg_parts`뿐이라 수정하지 않았고 남은 이슈로 기록한다.

### ② micro 경로에서 `isStartupMode`가 검사되지 않던 문제
`loadDiagnosisUI()`의 분기가
```
if (isSocial) … else if (isMicro) … else { if (isStartupMode) … }
```
구조여서 **창업 초기 검사가 마지막 `else` 안에만** 있었다.
→ 개업 1년 미만 소상공인이 STARTUP(8문항) 대신 **실적 기반 DiagMicro(35+16=51문항)**를 받았다. 매출 이력이 없는데 "최근 3개월 재방문율", "테이블 회전율"을 묻는 상태였다.

**수정**: `isStartupMode`를 `isMicro`보다 **먼저** 구하고 `isMicro`에서 창업 초기를 제외한다.
```js
...
```
micro + 창업 초기가 마지막 `else`로 흘러 STARTUP이 렌더링된다. **분기 구조 자체는 그대로 두고 조건만 좁혔다.**
- `isMicro`가 함수 전체에서 일관된 의미를 갖게 되어 **탭 라벨(`isMicro ? '소상공인 7대 분야' : isStartupMode ? '창업 초기 진단'`)도 자동으로 맞는다**
- `_activeContainers`·`DIAG_CONTAINERS`는 `else` 분기가 이미 `diag-common-container`를 등록하므로 **추가 수정 불필요**

⚠ **`isSocial`(사회적경제 3유형)은 건드리지 않았다.** 조사에서 "창업 초기 배제가 의도인지 확인 불가"로 나왔고, 확인되지 않은 것을 임의로 바꾸지 않는다.

### ③ 검증 (지시된 5개 항목 + 렌더 상태)
| 항목 | 결과 |
|---|---|
| ① micro + `mfg_parts` | `getGroup` = **retail** · food 대비 13문항 변경 적용 ✓ |
| ② micro + 창업 초기 | **STARTUP 8 + 업종 16 = 24문항** · 진행률 `0 / 24` · 탭 라벨 `🚀 창업 초기 진단 (8문항)` · 활성 컨테이너 `diag-common` + `diag-industry` ✓ |
| ③ micro + 일반 (회귀) | DiagMicro **51문항** · 탭 라벨 `🏪 소상공인 7대 분야 (35문항)` ✓ |
| ④ sme + 창업 초기 (회귀) | STARTUP **24문항** 그대로 ✓ |
| ⑤ 사회적경제 3유형 (회귀) | 사회적기업·협동조합·소셜벤처 각 **56문항** 그대로 ✓ |
| 추가 | sme 일반 **36문항** 불변 · 타 업종 그룹 매핑 7건 불변 ✓ |

**문항 내용은 하나도 수정하지 않았다.** 매핑 1줄과 분기 조건 1줄만 고쳤다.

### ④ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818c`

### ⑤ 이번에 수정하지 않은 문제 (조사에서 확인된 것)
심각도 순. 각 항목의 영향 범위와 근거는 조사 결과 참조.
1. **`bizmodel/` 12개 모듈 192문항이 전혀 렌더링되지 않는다** (`TAB_ORDER`에 bizmodel 없음, `renderDiagModule('diag-bizmodel-container')` 호출 0건). 그 결과 **CROSS_RULES 34개 중 21개가 `collect()`의 BM 프록시(공통 진단 도메인 평균 복사)로 발동**한다 — 사용자가 답한 적 없는 값이다. 192개 업종×BM 조합 중 경고가 뜨는 조합은 18개(9.4%). ★★★ **"BM 진단을 되살릴 것인가, CROSS_RULES에서 BM 축을 걷어낼 것인가" 방향 결정이 선행되어야 한다**
2. **DiagMicro 그룹 분화 커버리지 13~14/35** — `pro_service`로 전환해도 요식업 단어 39건 잔존. 제조업 전용 그룹 부재. ★★☆
3. **DiagCommon 20문항 중 8개가 오프라인 매장/요식업 전제** (재작성 5: `2_1` 로컬플랫폼 NAP · `3_2` 오픈·마감/위생검수 · `3_3` 매장·CCTV·POS · `4_3` POS·배달플랫폼 · `5_2` 식재료 로스 / 문구만 3: `1_3`·`3_1`·`3_4`). sme 일반기업 전원 영향 ★★☆
4. **`food_mfg`(식품제조)가 food(외식) 그룹** — B2B 납품·OEM 중심인데 외식 문구를 받는다 ★★☆
5. **DiagSme 18문항이 어떤 경로에서도 렌더링되지 않는다.** `diag-sme-container`는 **git 전체 이력에서 index.html에 존재한 적이 없다** — 끊긴 것이 아니라 처음부터 연결되지 않았다. 살리려면 문항 재작성이 선행되어야 한다(프로세스 마이닝·ACM 산식·n8n 등 난이도 편향이 크다) ★☆☆
6. PatternDB 고정값 노출 (기존 기록) ★☆☆
7. `common.js`·`diagnosis-micro.js`·`diagnosis-sme.js`에 `module.exports`가 없어 Node 테스트에서 `require()`가 빈 객체를 반환한다 ☆

---

## 최근 수정 이력 (2026-09-03) — PPT 다운로드 신설 (PDF는 유지·병행)

**리포트를 발표·제출용 `.pptx`로 내려받는 기능을 신설했다.** 기존 PDF 저장은 그대로 두고 병행한다.

### ① `js/ppt-export.js` 신설 — 브라우저 생성
- **PptxGenJS를 CDN에서 `defer`로 로드**하고 클라이언트에서 생성한다. ⚠ 서버(`api/`)에서 만들지 않는다 — Vercel 함수 부하와 배포 용량이 늘어난다
- 라이브러리 미로드 시 **버튼을 `disabled`로 두고 `title`로 안내**한다. `window load` 후 `initButton()`이 상태를 갱신한다. 로드 실패로 페이지가 깨지지 않는다
- 파일명 `{조직명}_{진단유형}_{YYYYMMDD}.pptx` (조직명 없으면 `BizNavi`, 파일명 금지문자 제거)

### ② 밝은 테마 — 화면(다크)과 분리
발표·인쇄·제출용이므로 어두운 배경은 부적합하다.
흰 배경 `FFFFFF` / 본문 `333333` / 제목 `1A2340`(진한 남색) / 강조 `8B6914`
경고 등급 **CRITICAL `C0392B`(빨강) · HIGH `D35400`(주황) · MEDIUM `7F8C8D`(회색)**

⚠ **폰트는 `맑은 고딕` 하나만 지정한다.** PptxGenJS는 폰트를 임베딩하지 않고 폴백도 지정할 수 없다. 수신자 대부분이 Windows·PowerPoint 조합이라 이 선택이 안전하다(macOS는 시스템 대체 렌더링 — 감수).

### ③ 유형별 슬라이드 구성 — 섹션 수만큼 만들지 않는다
⚠ 리포트 섹션을 1:1로 옮기면 글자만 빽빽한 못 쓸 PPT가 된다. **슬라이드당 핵심 3~5개로 압축**하고 초과분은 하단에 `※ … 상세는 전체 리포트를 참조하십시오`로 명시한다.

| 유형 | 장수 | 구성 |
|---|---|---|
| **micro** | 8 | 표지 · 한눈에 보기 · 7대 영역(레이더) · 생애주기 · 상권STP·시장 · 영역별 처방(취약 4) · 90일 · 정부지원 |
| **sme** | 12 | 표지 · 한눈에 보기 · 5대 역량(레이더) · SWOT(각 3) · **STP** · **4P** · 핵심전략(4) · KPI(5) · 로드맵 · **실행 체계+90일 통합** · 정부지원 · **리포트에서 확인할 내용** |
| **사회적경제 3유형** | 9 | 표지 · 한눈에 보기(총점·취약Top3·CRITICAL/HIGH) · 8대 영역(레이더) · mission · revenue · profit · org · **AI 실행계획+제도** · 정부지원 |
| **정책자금** | 7 | 표지 · 판정 요약 · 기관별 2장 · 실행 로드맵 · 준비 서류 · 정부지원 (**레이더차트 슬라이드 없음**) |

판단이 갈린 지점:
- **STP·4P는 분리했다.** 한 장에 합치면 7줄이 되어 압축 원칙에 어긋나고, 컨설팅에서 가장 많이 인용되는 부분이라 손실이 크다
- **6가지 시스템+90일은 통합했다.** 실행 체계와 그 일정이므로 함께 보는 편이 낫다
- **유형별 특화 분석·린 캔버스는 슬라이드로 만들지 않았다.** 블록이 5~9개라 압축하면 의미가 사라진다. 대신 **마지막에 '리포트에서 확인할 내용' 슬라이드를 추가해 제외 항목명을 나열**한다 — "상세는 리포트 참조"만 적으면 받는 사람이 무엇을 놓쳤는지 모른다. **sme에만 넣는다**(다른 유형은 제외 항목이 없다)
- **사회적경제의 제도(system) 영역은 AI 계획 슬라이드 하단에 2줄로 합쳤다.** 단독 슬라이드로 만들면 얇다
  ⚠ **AI 계획이 실패하면 그 슬라이드가 생략되어 제도 영역이 통째로 사라진다.** 수미쌍관 위반이므로 `_appendSystemNote()`를 **org 슬라이드 하단으로 옮겨 붙인다.** 어느 쪽이든 8영역이 모두 출력된다

### ④ 레이더차트 — canvas → 이미지
`drawRadarChart()`가 Canvas 2D로 직접 그리므로 `toDataURL()`로 바로 추출된다(SVG 변환 불필요).

⚠ **`toDataURL()`은 빈 캔버스에서도 정상 문자열을 반환하므로 길이로 판별할 수 없다.**
→ `getImageData()`로 **알파 채널이 0이 아닌 픽셀**이 있는지 검사한다. `try/catch`로 tainted canvas도 방어.
비었으면 **영역별 점수 표로 대체**하고 다운로드는 계속 진행한다 — 차트 때문에 전체가 실패하면 안 된다.

⚠ **`index.html`에 `id="radarChart"`가 두 개 있다**(`#diag-reveal` · `#sec-diag`). `getElementById`는 첫 번째만 반환하므로 **`#sec-diag`의 canvas는 그려지지 않는 죽은 요소**다. PPT는 `querySelectorAll`로 전부 훑어 내용이 있는 쪽을 고른다.
정책자금은 `diag-reveal`을 거치지 않아 차트가 없으므로 **슬라이드 자체를 만들지 않는다.**

### ⑤ 텍스트 넘침 처리
PptxGenJS는 자동 축소를 하지 않는다 → **글자 수 상한**(줄 110 / 문단 260 / 제목 42)으로 자르고 `…`을 붙인다. 잘림이 발생한 슬라이드는 하단에 `※ 일부 내용이 요약·생략되었습니다`를 붙여 **잘렸다는 사실을 남긴다.** `_clippedInSlide` 플래그를 슬라이드 생성 시 초기화한다.

### ⑥ 데이터 접근 — `render()`가 AI 결과를 버리고 있었다
`Dashboard.render(data, fd, isDemo)`는 AI 결과를 DOM에 밀어넣고 **모듈에 보관하지 않았다.** PPT가 원본을 필요로 하므로 `_lastData`·`_fundingRoadmap`을 추가했다.

⚠ **이전 회사 데이터가 PPT에 섞이는 것이 최악이다.** 세 진입점이 모두 타 유형 잔존 데이터를 지우고, `App.restart()`가 `Dashboard.resetReport()`를 호출한다.

| 진입점 | 처리 |
|---|---|
| `render()` | `_lastData = data` · `_fundingRoadmap = null` |
| `renderSocial()` | `_lastData = null` · `_fundingRoadmap = null` |
| `renderFunding()` | `_lastData = null` |
| `renderFundingRoadmap('done')` | `_fundingRoadmap = roadmap` (그 외 상태는 `null`) |
| `App.restart()` | `Dashboard.resetReport()` — 전부 초기화 |

**유형 판별에 새 분기를 만들지 않았다.** `Dashboard.reportKind()`가 기존 `_isSocialFd()`·`_orgKind()`·`bizScale`·`purpose`를 그대로 재사용하고, 섹션 라벨·영역 매핑·경고 매핑도 `getReportContext()`가 넘겨준다(`secLabel`·`secDomainIds`·`warnSection`) — PPT가 로직을 복제하지 않는다.

### ⑦ 검증 (Node + PptxGenJS 스텁, 49/49 통과)
| 항목 | 결과 |
|---|---|
| 장수 | micro **8** / sme **12** / 사회적기업·소셜벤처·협동조합 각 **9** / 정책자금 **7** ✓ |
| sme 구성 | STP·4P 분리 · 실행체계+90일 통합 · 제외 항목 안내(특화분석·린캔버스) ✓ |
| **AI 실패** | 사회적경제 8장(계획 슬라이드 생략) · **제도 영역이 org 슬라이드에 살아 있음** ✓ / 정책자금 6장 ✓ |
| 빈 슬라이드 | 전 케이스 **0건** ✓ |
| 레이더차트 | 이미지 삽입 ✓ / **빈 캔버스 → 표 대체 + 다운로드 성공** ✓ / 정책자금 이미지 0 ✓ |
| 파일명 | `{조직명}_{유형}_{YYYYMMDD}.pptx` 3유형 검증 ✓ |
| 라이브러리 미로드 | `isAvailable() false` · 안내 후 중단 · 버튼 `disabled` ✓ |
| **데이터 격리** | sme→사회적경제 전환 시 이전 AI 결과 제거 · `resetReport()` 전부 초기화 ✓ |
| 테마·상한 | 흰 배경 · 등급별 색 · 맑은 고딕 · 잘림 표시 ✓ |
| PDF 유지 | `Dashboard.print()` 버튼 그대로 ✓ |

⚠ 실제 PowerPoint로 열어 레이아웃을 확인하지는 않았다(브라우저 다운로드 필요). 좌표·폰트 크기는 16:9 기준으로 잡았으나 **실사용 확인이 필요하다.**

### ⑧ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818b` (신규 `js/ppt-export.js` 포함, 외부 CDN 제외)

### ⑨ 남은 이슈 — PDF 빈 페이지 (보류)
**재현 조건 미확보로 이번에 손대지 않았다.** 원인 후보 3가지:
1. `#sec-six-systems`가 **micro에서도 표시되는데** `break-before: page`가 걸려 있음([print.css:351](css/print.css#L351))
2. `.print-cover`의 `break-after: page`와 뒤 섹션 `break-before: page`가 **연속** — 2026-05-06 재무 리포트에서 동일 문제를 겪고 `break-after: avoid`로 해결한 전례가 있다
3. `display` 처리된 `.section-card`의 마진 잔존

⚠ **`display:none` 요소는 박스를 생성하지 않으므로 페이지 나눔의 직접 원인이 아니다.** 초기 진단이 이 점을 잘못 짚었다. 추정으로 고치면 오진이 기록에 남으므로, **어느 리포트 유형에서 몇 번째 페이지가 비는지** 확인한 뒤 진행한다.

---

## 최근 수정 이력 (2026-09-02) — 협동조합 전용 진단지 신설 (C1~C8 40문항) · 사회적경제 3유형 완성

**`orgType='cooperative'`가 `DiagSocial`(사회적기업)을 빌려 쓰고 있었다.**
이번 신설로 **사회적경제 3유형(사회적기업 S1~S8 / 소셜벤처 V1~V8 / 협동조합 C1~C8)** 전용 진단이 완성됐다.

### ① 근거 — 협동조합기본법
조합원 자격·출자 / **1인 1표 의결권(출자액 무관)** / 총회·이사회 운영 의무 / 임원 선출·임기 규정 /
**잉여금 배당 제한(법정적립금 우선·이용고 배당 원칙)** / 조합원 교육 의무 /
조합원 외 거래 비중 제한(사회적협동조합) / 결산보고서 제출 의무

⚠ **사회적기업은 '인증'을 통해 얻는 지위, 협동조합은 '설립 신고'로 성립하는 법인 형태다.**
→ C7에 **만료·갱신 경고를 넣지 않는다**(S7·V7과 동일 원칙, 활용도에 집중).

### ② `js/diagnosis/diagnosis-coop.js` 신설 — `DiagCoop`
**C1 조합원 기반 / C2 민주적 운영·거버넌스 / C3 사업 지속가능성 / C4 재정·원가구조 /
C5 회계·법규 준수 / C6 판로·마케팅 / C7 제도 활용 / C8 디지털·AX** — 8영역 × 5문항 = 40문항.
점수 키 접두어 `diag-coop-container_`. `detectCrossWarnings` 14규칙(CRITICAL 2 / HIGH 5 / MEDIUM 7).

**⚠ C4·C6이 S4·S3+S6과 겹치는 것은 정상이다.**
재무 구조(변동비·고정비·공헌이익·BEP)는 법인 형태와 무관하게 같은 것을 봐야 한다 — 협동조합이라고 손익분기점을 다르게 계산하지 않는다. 억지로 다르게 만들면 진단 품질이 떨어진다.
대신 **문항 서술을 협동조합 맥락으로 썼다** — 예: `c4_5` 잉여금 처리는 법정적립금 우선·이용고 배당 원칙을 반영.
**협동조합기본법 고유 축은 C1·C2·C3·C5·C7**이며 차별성은 이쪽에 실었다.

### ③ 구조 재사용 — 새 분기를 만들지 않았다
DiagVenture 때 정리한 구조를 그대로 썼고, **`_calcOrgDomainScores()`와 `collect()`는 수정이 필요 없었다**(모듈의 `KEY_PREFIX`·`DOMAINS`에서 파생하므로). 이것이 그때 정리한 구조의 성과다.

| 지점 | 조치 |
|---|---|
| `wizard.js` `_orgDiagModule()` | `cooperative → DiagCoop` **한 줄 추가** |
| `wizard.js` `DIAG_CONTAINERS` · `_countDiagItems` 호출부 | `diag-coop-container` 추가 (⚠ 빠뜨리면 진행률이 틀린다) |
| `wizard.js` `COOP_DOMAIN_EXPLAIN` + `ORG_DOMAIN_EXPLAIN` | 해설 맵 분기를 **테이블 한 곳**으로 모음 |
| `wizard.js` `ORG_ICON` | 탭 라벨·배너의 아이콘 삼항 분기를 테이블로 통합 |
| `dashboard.js` `_orgKind(fd)` | `_isVentureFd` 이진 분기 → **`'social'|'venture'|'coop'` 단일 함수** |
| `dashboard.js` `SEC_LABEL` | 목차·섹션 제목이 **같은 테이블**을 쓴다(분기 중복 제거) |
| `dashboard.js` `SEC_DOMAINS` · `SOCIAL_WARN_SECTION` | coop 매핑·경고 코드 17개 추가 |
| `ai-engine.js` | 모듈 조회 3분기 확장 |
| `index.html` | `#diag-coop-container` + 스크립트 태그 |

**`ORG_TYPE_PENDING = {}`** — 3유형 모두 전용 모듈을 갖췄으므로 협동조합의 "준비 중" confirm과 배너 안내가 사라졌다.

### ④ 리포트 — 9섹션 구조 유지, 라벨·내용만 조직 형태별
| 섹션 | 사회적기업 | 소셜벤처 | **협동조합** |
|---|---|---|---|
| mission | 미션과 사업이 맞물리나 (S1·S2) | 사회문제를 제대로 정의했나 (V1·V2) | **조합원 실익과 사업이 맞물리나** (C1·C3) |
| revenue | 어디서 돈이 들어오나 (S3·S6) | 자금을 어떻게 조달하나 (V5·V4) | 어디서 돈이 들어오나 (C6·C4) |
| profit | 수익 구조 점검 (S4) | 기술이 무기가 되고 있나 (V3) | **법과 정관을 지키고 있나** (C5) |
| org | 조직이 버틸 수 있나 (S5·S8) | 팀이 버틸 수 있나 (V6·V8) | **조합 운영이 건강한가** (C2·C8) |
| system | 제도를 제대로 쓰고 있나 (S7) | 판별과 제도를 활용하고 있나 (V7) | 제도를 제대로 쓰고 있나 (C7) |

⚠ **당초 지시에는 `profit` 섹션에 대응할 협동조합 영역이 지정되지 않아 빈 곳이 생겼다.**
C5(회계·법규 준수)를 그 자리에 두고 **'법과 정관을 지키고 있나'**로 명명해 8영역이 빠짐없이 대응된다.
협동조합에서 법정 의무 이행은 **지원사업 결격과 직결**되므로 독립 섹션 가치가 있다.
- 핵심 강조 경고도 분기: 사회적기업 `winning_but_losing` / 소셜벤처 `tech_social_disconnect` / **협동조합 `legal_duty_unmet`**
- ⑦ 제도 섹션은 협동조합 전용 내용 — **"설립 신고로 성립하므로 인증 만료·갱신을 걱정할 필요가 없다"**를 명시하고 사회적협동조합 전환 검토·활용 제도에 집중

### ⑤ 정부지원사업 — 변경 없음
협동조합 매칭이 이미 적절해 **사업을 추가하지 않았다.**
협동조합 활성화 지원사업 포함 ✓ / TIPS·핀테크 제외 ✓(`orgType !== 'social_venture'` 조건에 자연히 포함) / 소셜벤처 전용 2건 제외 ✓

### ⑥ 검증 (Node DOM 스텁, 123/123 통과 — 소셜벤처 69건 포함)
**수미쌍관을 코드로 확인** — `data-domain` 속성 역추적:

| ID | 영역 | 렌더링 섹션 |
|---|---|---|
| C1 | 조합원 기반 | status + mission |
| C2 | 민주적 운영·거버넌스 | status + org |
| C3 | 사업 지속가능성 | status + mission |
| C4 | 재정·원가구조 | status + revenue |
| C5 | 회계·법규 준수 | status + profit |
| C6 | 판로·마케팅 | status + revenue |
| C7 | 제도 활용 | status + system |
| C8 | 디지털·AX | status + org |

**누락 0. 40문항 라벨이 전부 리포트에 등장.**

| 항목 | 결과 |
|---|---|
| 모듈 | 8영역·40문항·weight 1.0 · scale 1~5 전수 · 1점→20 / 3점→60 / 5점→100 ✓ |
| 경고 | 3점 **0건** / 1점 **14건**(CRITICAL 2) ✓ |
| **⚠ 진행률** | 협동조합 40 + 업종 16 = **56** ✓ |
| **⚠ 레이더차트** | 키 `c1~c8` · **0점 영역 없음** · c5 부분 저점 2.8 정확 · 해설 카드 8건 본문 채워짐 ✓ |
| collect | `orgPrompt` = C1~C8 · **S1~S8 미적용** · `orgDiagKeyPrefix` 전달 · 구 필드 병행 ✓ |
| 프롬프트 지침 | 설립 신고 명시 · 1인1표·법정적립금·이용고 배당 포함 · 만료·갱신 문구 0 ✓ |
| ⑦ 섹션 | 만료·갱신 안내 0 · SVI·소셜벤처 판별 등 **타 유형 문구 혼입 0** ✓ |
| **회귀** 3유형 | `social_enterprise`→social / `social_venture`→venture / `cooperative`→coop 컨테이너, **나머지 0문항 · 진행률 전부 56** ✓ |
| **회귀** micro·일반기업·gov-support | 전부 불변 ✓ |
| 구조 | `ORG_TYPE_PENDING` 비어 있음 · `_orgKind` 단일 분기 · dashboard 접두어 하드코딩 0 ✓ |

### ⑦ 캐시버스팅
`index.html` 로컬 `?v=` **51곳** 전부 `20260818a`

### ⑧ 남은 이슈 (갱신)
1. ✅ **협동조합 전용 진단지 — 이번에 완료.** 사회적경제 3유형 전용 진단이 모두 갖춰졌다.
2. **다음 작업 — 구 필드 `socialPrompt`/`socialWarnings` 제거.**
   `orgPrompt`/`orgWarnings`로 대체됐고 회귀 방지를 위해 병행 유지해 왔다.
   **협동조합 작업 완료 후 제거하기로 한 약속이므로 다음 작업에서 정리한다.**
   현재 병행 지점: `wizard.js collect()`(대입) / `dashboard.js _socialWarnings()`(읽기) / `ai-engine.js _buildSocialPrompt`·`_buildSocialWarnBlock`(읽기).
3. **`gov-support`의 `social_economy` 태그가 사회적기업과 협동조합을 함께 묶고 있어,
   사회적기업 인증 기업 전용 사업(사회적기업 육성사업·재정지원사업)이 협동조합에도 상위 매칭된다.**
   실무상 중복 인증이 흔해 완전한 오정보는 아니나, 태그를 `social_enterprise_only` / `cooperative_only` / `social_all`로 세분화하는 것이 정확하다. **회귀 위험이 있어 별도 작업으로 분리.**
4. **`gov-support` `match()`의 동점 처리가 이름 순(`localeCompare`)** — 상위 6개가 사실상 가나다순으로 정해지는 구간이 있다. 2차 정렬 기준 설계 필요.
5. (기존) 사회적경제 AI 결과가 이력 스냅샷에 저장되지 않음 / `claude-analyze-funding.js`의 continuation 미적용 — 변동 없음
...
### ⑩ 후속 — `orgAffinity` 신설 (소셜벤처에서 TIPS가 상위에 들지 못하던 문제)

**증상**: 소셜벤처 · 관심분야 미선택 시 TIPS가 상위 6개에서 잘리고 "클라우드 서비스 바우처"보다 아래로 밀렸다.

**원인 — 태그가 아니라 점수 축의 소실이었다.**
TIPS는 `orgType` 태그가 없어(`'all'` 취급) 조직 형태 가점이 0이고,
관심분야 미선택이면 3점 축이 통째로 죽어 **업종(+3) + 규모(+1) = 4점**만 남는다.
4점 동점 그룹 3개가 이름 순(`localeCompare('ko')`)으로 갈리면서 TIPS가 마지막이 됐다.
```
글로벌 강소기업 육성사업 → 클라우드 서비스 바우처 → TIPS
```
관심분야를 선택하면(R&D·정책자금·창업성장 모두) TIPS는 **5위**로 정상 진입했다.

**⚠ `orgType: 'venture'`를 붙이면 안 된다 — 실측으로 확인했다.**
`match()`의 게이트가 `if (pOrg !== 'all') { if (!isSocial) return -1; }` 이므로
**일반 기업 결과에서 TIPS가 통째로 사라진다.** TIPS는 기술창업 전반이 대상이지 소셜벤처 전용이 아니다.

**해결 — 필드의 역할을 분리했다**

| 필드 | 의미 | 게이트 관여 |
|---|---|---|
| `orgType` | **자격 제한.** 해당 조직 형태만 신청 가능 | **O** (제외 판정) |
| `orgAffinity` | **적합도 가점.** 자격은 열려 있으나 특정 형태에 더 적합 | **X** (가점 전용, +2) |

`orgAffinity: ['venture']`를 **TIPS·핀테크 혁신펀드 2건에만** 부여했다(전 항목에 붙이면 변별력이 사라진다).
```js
// 게이트 블록 바깥. 여기서 return하지 않는다
if (isVenture && (p.orgAffinity || []).indexOf('venture') !== -1) score += 2;
```
⚠ 제외 판정은 계속 `VC_TRACK_IDS`로 한다. 두 목록이 지금은 같은 사업을 가리키지만 **의미가 다르므로 분리해 둔다** — 적합도가 높다고 해서 반드시 다른 형태에 부적합한 것은 아니다. `orgAffinity`를 게이트에 쓰면 같은 문제가 재발한다(코드에 주석으로 명시).

**검증 — 기준선 대조 (10케이스, 회귀 실패 0)**

| 케이스 | 결과 |
|---|---|
| 일반 기업 (R&D / 미선택 / 외식·디지털 / orgType 미지정 / sme) | **5건 전부 불변** ✓ |
| 사회적기업 (미선택 / R&D) | **불변** · TIPS 제외 유지(`VC_TRACK_IDS`) ✓ |
| 협동조합 | **불변** ✓ |
| 소셜벤처 미선택 | `핀테크 6점 4위` · `TIPS 6점 5위` **진입** · 기술보증기금 **1위 유지** ✓ |
| 소셜벤처 R&D | TIPS·핀테크 10점 상위 유지 ✓ |

소셜벤처 전용 진단 스모크도 69/69 그대로 통과.

### ⑨ 남은 이슈
1. ✅ **협동조합 전용 진단지 — 2026-09-02 완료(DiagCoop, C1~C8).** 예상대로 `_orgDiagModule()` 한 줄 추가로 끝났다.
...
3. **`gov-support` `match()`의 동점 처리가 이름 순(`localeCompare`)이라 사용자에게 보여줄 상위 6개가 사실상 가나다순으로 정해지는 구간이 있다.** 특히 관심분야 미선택 시 3점 축이 통째로 죽어 동점 그룹이 커진다. 이번 `orgAffinity`는 소셜벤처만 보정했고 **모든 경로 공통 문제는 그대로다.** 2차 정렬 기준(예: 기관 신뢰도, 지원 규모, 신청 난이도)을 설계해야 한다.
4. (기존) 사회적경제 AI 결과가 이력 스냅샷에 저장되지 않음 / `claude-analyze-funding.js`의 continuation 미적용 — 변동 없음
...
- **DiagMicro에서 `ITEMS`를 직접 참조하면 그룹 오버라이드가 무시된다.** 화면은 미용실인데 AI 프롬프트·경고 문구는 외식으로 나가는 사고가 `buildPromptSummary`·`detectCrossWarnings` 두 곳에서 실제로 있었다. 문항 내용(label·question·scale)이 필요하면 반드시 **`getSchema(group).items`**를 쓸 것. 키만 필요한 경우(`calcScores`)는 `ITEMS`로 충분하다. **`DOMAINS`는 아직 그룹 인지가 없다** — `desc`가 전 그룹 공통이다
