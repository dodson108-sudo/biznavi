...
- **브랜치**: `main` (단일 브랜치 운영)
- **캐시버스팅**: `index.html`의 로컬 `?v=` **53곳**(외부 CDN 제외). 현재 `20260921b`
...
### sme PPT (2026-09-21 1단계 완료 — 색 체계·표지·본문 띠)
...
- ⚠ **`_smeRows`의 역량 라벨이 화면과 다르다.** PPT는 `재무건전성`·`조직·인력`을
  하드코딩하고 화면(`wizard.js calcDomainScores`)은 `경영재무역량`·`인적자원역량`이다.
  실재하는 불일치이며 **2단계(슬라이드 내용)에서 처리**한다.
- ⚠ **슬라이드 내용·장수는 1단계에서 건드리지 않았다**(sme 11장 그대로).
