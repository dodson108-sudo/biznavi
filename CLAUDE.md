...
- ⚠ **`DOMAINS.label`은 여전히 `점포환경·네이버 플레이스`·`SNS·생성형AI`다.**
  ...
  (`calcScores`가 `domain.label`을 레이더차트·대시보드·AI 프롬프트로 그대로 흘려보낸다).
  **전 그룹 공통으로 바꾸려면 표시 계층까지 함께 설계해야 한다**
- ⚠ **별건 ①: `facility_service` 업종 특화 진단 모듈이 없다.**
  `js/diagnosis/industry/facility_service.js`가 없어 `industryVarMap`에 등록되지 않았고,
  **업종 탭이 비어 있다(진행률 35)**. 사용자가 실사용에서 확인했다.
  [wizard.js:1514](js/wizard.js#L1514)가 탭 버튼을 숨기므로 깨지지는 않으나
  다른 업종이 받는 16문항을 받지 못한다. **모듈 신설이 필요하다**
