...
### ⑨ 남은 단계와 이슈
- **6단계 진행 중 — 기존 4그룹 전용어 정리.** 1단계 `etc` 완료 /
  ✅ 2단계 `beauty` · 3단계 `retail` · 4단계 `pro_service` 완료 /
  5단계 `edu_service`(온라인강의) 남음.
  ⚠ **미용실·소매점 품질이 떨어지면 안 된다** — manufacturing이 food_mfg를 흡수할 때처럼
  본문을 중립화하되 **guide에 원래 업종 예시를 반드시 남길 것**
