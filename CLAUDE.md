...
## 최근 수정 이력 (2026-09-04) — DiagMicro 외식업 편향 제거 (D1~D4 완료, D5~D7 진행 예정)
...
### ⑤ 검증 (D1 9/9 · D2 14/14 · D3 15/15 · D4 15/15)
| 항목 | 결과 |
|---|---|
| food D1~D4 20문항 | label·question·scale·ai_trigger **원문과 완전 동일** ✓ |
| 비외식 4그룹 외식용어 | D1~D4 **0건** ✓ |
| guide | 5그룹 × 20문항 = **100건** 전수 · 렌더링 확인 ✓ |
| scale·ai_trigger | 5단계 전수 · `warning` 코드 전 그룹·전 문항 **불변** ✓ |
| **calcScores** | 1~5점 전부 수정 전과 **동일** (점수 로직 무변경) ✓ |
| 문항 수 | **35 불변** ✓ |
| D5~D7 | 15문항 × 5그룹 **완전 무변경** ✓ |
...
### ⑥ 캐시버스팅
`index.html` 로컬 `?v=` **52곳** 전부 `20260818i` (D1 `f` · D2 `g` · D3 `h` · D4 `i`)

### ⑦ 남은 이슈
1. **D5~D7 15문항 미완.** D5(운영자금·ESG보증)·D6(사업정리·폐업세무)는 CLAUDE.md 기록상
   업종 무관 공통이라 편향이 적을 것으로 보이나 확인이 필요하다. D7(SNS·생성형AI)은 `7_1`·`7_5`에
   기존 오버라이드가 있다
