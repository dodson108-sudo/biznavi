# /fix-js-edit — JS 파일 편집 반복 실패 진단·수정

세션 중 Edit 툴이 "String not found" 또는 유사 오류를 반복하며 JS 파일 수정이 안 될 때.
...
- **수정**: Read 툴로 해당 파일 다시 읽고 실제 라인 내용 확인 후 재시도
...
- **수정**: old_string 앞뒤에 주변 고유 컨텍스트 2~3줄 더 포함
...
**대형 파일 Grep 예시**:
```
Grep: pattern="_calcMicroDomainScores", path="js/wizard.js" → output_mode="content"
→ 라인 번호 확인 후 Read offset=N limit=80 으로 함수 전체만 읽기
```
...
**수정**: 반드시 Read 결과에서 해당 라인을 **그대로 복사**해서 old_string에 사용
