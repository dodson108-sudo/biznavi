---
name: fix-micro
description: 소상공인(bizScale='micro') 전용 기능 버그를 진단·수정한다. micro AI 분석 중 "Failed to fetch"(CDN TTFB 타임아웃), 3차 분석 504, D1~D7 레이더차트 특정 영역 미입력(인덱스 버그), JSON 절단, diag-reveal이 SME 5대 역량으로 잘못 표시 등에 사용.
---

# 소상공인(micro) 모드 오류 진단·수정
...
```powershell
# 로컬 테스트
$env:DART_API_KEY="<DART_API_KEY 값 — .env.local 참조>"
vercel dev
