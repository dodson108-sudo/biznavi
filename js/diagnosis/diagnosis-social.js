...
const DiagSocial = (() => {
  ...
  const ITEMS = {
    ...
    's4_3': {
      label: '사업별 공헌이익',
      question: '사업·서비스별로 얼마가 남는지(공헌이익)를 산출하는가? — ※ 공헌이익 = 판매가 − 변동비(재료비 등). 고정비(임대료·인건비)를 갚는 데 쓰이는 돈이다. 매출이 아니라 이 금액이 클수록 남는 장사다',
      scale: [
    ...
    's4_5': {
      label: '공공 용역 사후 검증',
      question: '공공 용역 종료 후 투입한 사람·시간 대비 얼마가 남았는지(공헌이익) 검증하는가? — ※ 공헌이익 = 판매가 − 변동비(재료비 등). 고정비(임대료·인건비)를 갚는 데 쓰이는 돈이다. 매출이 아니라 이 금액이 클수록 남는 장사다',
      scale: [
    ...
    's7_2': {
      label: 'SVI 측정 이력',
      question: '사회적가치지표(SVI) 측정에 참여한 이력이 있는가? — ※ SVI(Social Value Index) = 한국사회적기업진흥원이 사회적기업의 사회적 성과를 점수로 평가하는 공식 지표',
      scale: [
        { score:1, desc:'사회적가치지표(SVI)가 무엇인지 알지 못함.' },
