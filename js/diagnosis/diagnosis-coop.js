...
const DiagCoop = (() => {
  ...
  const ITEMS = {
    ...
    'c4_3': {
      label: '사업별 공헌이익',
      question: '사업별로 얼마가 남는지(공헌이익)를 산출하는가? — ※ 공헌이익 = 판매가 − 변동비(재료비 등). 고정비(임대료·인건비)를 갚는 데 쓰이는 돈이다. 매출이 아니라 이 금액이 클수록 남는 장사다',
      scale: [
    ...
    'c4_5': {
      label: '잉여금 처리의 적법성',
      question: '잉여금 처리(법정적립금 적립·이용고 배당 등)가 협동조합기본법과 정관에 맞게 이루어지는가? — 출자액 기준 배당이 아니라 법정적립금이 우선이며 이용고 배당이 원칙입니다. ※ 법정적립금 = 법으로 반드시 쌓아 두게 한 잉여금, 이용고 배당 = 출자한 돈이 아니라 조합을 얼마나 이용했는지에 따라 나누는 배당입니다.',
      scale: [
