...
const GovSupport = (() => {
  ...
  const PROGRAMS = [
    ...
    },
    {
      ...
      size: ['micro', 'small'],
      // ⚠ orgType이 아니라 orgAffinity다. TIPS는 기술창업 전반이 대상이므로
      //    자격을 소셜벤처로 제한하면 안 된다(일반 기업 결과에서 사라진다)
      orgAffinity: ['venture'],
    },
    ...
    },
    {
      ...
      purpose: ['fund'],
      industry: ['외식 및 휴게음식업', '서비스업', '유통/물류', '시설관리·경비'],
    ...
    },
    {
      ...
      size: ['micro', 'small'],
      orgAffinity: ['venture'],   // 벤처투자 계열 — 소셜벤처 적합도 가점
    },
    {
      id: 'edu_tech',
      name: '에듀테크 소프트랩 지원',
    ...
    // ── 사회적경제 (2026-08-16 신설) ──────────────────────────────
    // ══ 조직 형태 관련 필드는 두 종류이며 역할이 다르다 ══
    //   orgType     = 자격 제한. 해당 조직 형태만 신청 가능하며 match()의 게이트에서 제외 판정에 쓴다.
    //   orgAffinity = 적합도 가점. 자격은 누구에게나 열려 있으나 특정 조직 형태에 더 적합한 경우.
    //                 ⚠ 게이트에 절대 관여하지 않는다. 가점 전용이다.
    //                    여기에 게이트를 걸면 자격이 있는 기업에서 사업이 사라진다
    //                    (TIPS에 orgType:'venture'를 붙였다면 일반 기업 결과에서 사라졌을 것).
    //
  ...
  function match(d) {
    ...
    const scored = PROGRAMS.map(p => {
      ...
      /* ③ 벤처투자 계열은 일반 사회적기업·협동조합에 부적합하므로 제외한다.
            ...
            ⚠ 이 제외 판정은 VC_TRACK_IDS(게이트용)로 한다. orgAffinity를 쓰지 않는다 —
               두 목록은 현재 같은 사업을 가리키지만 의미가 다르므로 분리해 둔다
               (적합도가 높다고 해서 반드시 다른 형태에 부적합한 것은 아니다) */
      if (isSocial && orgType !== 'social_venture' && VC_TRACK_IDS.indexOf(p.id) !== -1) {
        return { ...p, score: -1, _industryExact: false };
      }

      /* ④ 조직 형태 적합도 가점 (+2) — 게이트가 아니다.
            ⚠ 관심분야를 선택하지 않으면 3점 축이 통째로 죽어 동점 그룹이 커지고
               상위 6개가 사실상 이름 순으로 갈린다. 그때 소셜벤처의 핵심 프로그램인
               TIPS·핀테크가 범용 바우처보다 아래로 밀리던 문제를 이 가점으로 보정한다.
            ⚠ 여기서 return하지 마라. 가점만 주고 흐름을 이어간다 */
