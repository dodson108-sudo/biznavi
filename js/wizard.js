...
const Wizard = (() => {
  ...
  /* DX 탐지 영역 주입
     ⚠ 키를 골라 담지 말고 원본을 통째로 복사한 뒤 areas만 교체한다.
        구 COMMON_DIAGNOSIS는 {title, description, insights}였고 현 DiagCommon은
        {id, label}이라, 화이트리스트로 다시 담으면 label이 탈락해
        renderDiagModule의 `data.label || data.title`이 빈 문자열이 된다
        (_applyIndustryWording이 같은 방식으로 영역 제목을 지우고 있었다). */
  function _injectDxDetect(diagData) {
    return Object.assign({}, diagData, {
  ...
  function loadDiagnosisUI(forceIndustryKey) {
    ...
    if (isSocial) {
      ...
      if (isStartupMode && typeof STARTUP_DIAGNOSIS !== 'undefined') {
        ...
        /* ⚠ 업종 문구 분기는 DiagCommon.getSchema(industryKey) 안에서만 한다.
           여기서 그룹을 계산해 넘기면 안 된다 — DiagCommon의 4그룹 매핑은
           DiagMicro의 10그룹과 달라(logistics: field_service vs trade_logistics)
           호출부가 구분하는 순간 실수가 난다. 업종 키를 그대로 넘긴다. */
        commonDiag = _diagCommonToAreas(DiagCommon, industryKey);
