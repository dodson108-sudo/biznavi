...
[경영 프레임워크 10권 적용 지침]
다음 10가지 경영 프레임워크를 분석 전반에 통합하여 보고서 품질을 높인다:

① 블루오션 전략(김위찬): SWOT 기회 항목에 경쟁 없는 새 시장(ERRC 관점)을 최소 1개 포함. "제거·감소·증가·창조" 관점을 포지셔닝에 반영.
② 경쟁우위(포터): 5 Forces 결과를 SWOT 위협·기회에 직접 연결. 핵심전략에 원가우위 또는 차별화 방향을 명시.
③ 좋은 전략 나쁜 전략(루멜트): keyStrategies 각 항목은 '문제 진단 → 기본 방침 → 구체 행동' 3요소를 포함. 모호한 비전 나열 금지, 집중 전략만 제시.
④ OKR(존 도어): KPI는 측정 가능한 핵심결과(Key Result) 형태로 작성. 분기별 체크포인트를 timeline에 반영.
⑤ 린 스타트업(에릭 리스): 로드맵 1단계는 MVP 관점으로 가장 빠르게 검증 가능한 실행부터 시작. Build-Measure-Learn 사이클을 태스크에 반영.
⑥ 제로 투 원(피터 틸): SWOT 강점에 '독점적 우위' 관점 포함. 경쟁사와 단순 비교가 아닌 기업만의 비밀(Secret)을 포지셔닝에 반영.
⑦ 하이 아웃풋 매니지먼트(앤디 그로브): 핵심전략 owner/timeline 설정 시 레버리지가 가장 높은 활동(최소 자원·최대 산출)을 우선 배치.
⑧ 좋은 기업을 넘어 위대한 기업으로(짐 콜린스): 헤지호그 컨셉(열정·최고·경제적 엔진의 교집합)을 전략 방향에 활용. 플라이휠 관점에서 초기 성공이 다음 성공을 가속하는 구조를 로드맵에 반영.
⑨ 무기가 되는 스토리 7단계(도널드 밀러): 4P 촉진전략·STP 포지셔닝에 StoryBrand 7단계 반영 — ❶고객이 원하는 것 ❷외부·내부·철학적 문제 ❸가이드(브랜드)의 공감+권위 ❹명확한 계획 ❺행동 촉구 ❻실패 회피 ❼성공 비전. 촉진 메시지는 '고객이 주인공, 브랜드는 가이드' 구조로 작성.
⑩ 무기가 되는 6가지 시스템(도널드 밀러): 로드맵·핵심전략에 비즈니스 6대 시스템(리더십·마케팅·판매·제품·운영·재무)을 균형 있게 반영. 취약한 시스템을 우선 강화하는 순서로 로드맵 구성.
