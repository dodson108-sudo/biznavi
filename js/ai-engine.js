...

{
  "sixSystems": [
    ...
    },
    {
      ...
      "icon": "📋",
      "status": "취약|보통|강점",
      "issue": "폐업·전환 시 세무 타임라인과 희망리턴패키지 활용 가능성을 진단 점수 기반으로 2~3문장 구체 서술",
      "actions": ["폐업 세무 타임라인 숙지 (사업자 등록 말소 → 부가세 신고 25일 이내)", "희망리턴패키지 신청 자격 확인 (점포철거비·재취업·재창업 지원)", "업종 전환 시 세금 영향 사전 검토 (세무사 무료 상담 연결)"],
      "resource": "소상공인진흥공단 희망리턴패키지 / 국세청 홈택스 폐업 신고 가이드"
    },
    {
      "name": "D7. SNS·생성형AI",
      "icon": "🤖",
      "status": "취약|보통|강점",
      "issue": "SNS 콘텐츠 생산 주기와 ChatGPT 등 AI 도구 활용 현황, 플레이스 CTR(클릭율) 개선 여지를 진단 점수 기반으로 2~3문장 구체 서술",
      "actions": ["ChatGPT 무료 플랜으로 메뉴 홍보 카피 3가지 생성 — 이번 주 시작", "인스타그램 릴스 1편/주 루틴 설정 (15초 메뉴·매장 소개)", "네이버 플레이스 CTR 개선 — 사진·영업시간·메뉴 업데이트 완성"],
  ...
  "plan90days": [
    {
      "month": "1개월차",
      "theme": "생존 위협 제거 — 당장 이번 주 실행",
      "goal": "이 달 말까지 달성할 구체적이고 측정 가능한 목표 1가지",
      "actions": ["이번 주 무료 도구로 당장 실행 (구체 도구명·방법 포함)", "2주차 실행 (비용 최소화 — 10만원 이하)", "3~4주차 완성"],
      "expectedResult": "행동 완료 시 무엇이 달라지는지 (수치 포함)",
      "govSupport": "이 시기 신청 가능한 정부지원사업 (소상공인진흥공단·지자체 포함 — 사업명 + 마감월)"
    },
    {
      "month": "2개월차",
      "theme": "수익성 회복 — 구조 개선",
...
D1~D4·kpi·roadmap·lifecycleStage는 이미 2차에서 작성됨. 이 응답에는 D5~D7 3개 + plan90days 3개월만 작성. govSupport는 소상공인진흥공단·지자체 실제 사업명 포함.`;
  ...
  const INDUSTRY_CLOSURE_CAUSES = {
    ...
    facility_service: { finance: '최저임금 인상에 따른 인건비 압박·계약 단가 동결', hr: '현장 인력 이직·결원 발생', bm: '대형 업체와의 입찰 경쟁·계약 갱신 실패' },
...
건설업에서 하자 한 건이 수개월치 이익을 지웁니다. 인력 리스크는 재무 리스크와 직결됩니다.`);
    ...
    // IT·소프트웨어 × SaaS·구독 모델
    /* ⚠ 'it'을 단독 대안으로 두면 'facility_service'의 facil-it-y가 걸려
       시설관리·경비 업체에 IT·SaaS 인과사슬이 나간다(2026-09-10 실측).
       'it' 대안을 뺀다 — 'IT/소프트웨어' 라벨은 '소프트웨어'가 이미 잡는다 */
    if (/소프트웨어|테크|개발|knowledge_it/.test(ind)) {
...
물류업에서 원가 관리와 운영 최적화는 같은 문제의 두 면입니다.`);
    ...
    /* 시설관리·경비 × 정기 도급 계약 구조
       ⚠ 이 업종은 인건비가 원가의 대부분이라 다른 업종과 원가 구조가 근본적으로 다르다.
          'it' 오매칭으로 IT 인과사슬이 나가던 것을 2026-09-10에 바로잡고 전용 분기를 뒀다 */
    if (/시설관리|경비|청소|방역|facility_service/.test(ind)) {
      if (isLow(fin)) {
        risks.push(`[구조적 위기 — 실질 인건비 미파악 × 도급 단가 고정]
시설관리·경비의 수익 공식: 도급 단가 - (배치 인원 × 시급 × 근무시간 + 4대보험·퇴직충당금·연차수당) = 마진
${co}의 재무건전성은 ${fmt(fin)}입니다.
→ 기본급만으로 원가를 계산하면 남는 것처럼 보이지만, 사업주 부담분을 더하면 적자인 현장이 나옵니다.
→ 도급 단가는 계약 기간 중 고정인데 최저임금은 매년 오릅니다. 단가 조정 조항이 없으면 계약 자체가 손실원이 됩니다.
이 업종에서 원가 오차는 인원 수만큼 배로 커집니다 — 먼저 실질 인건비 단가부터 확정해야 합니다.`);
      }
      if (isLow(hr)) {
        risks.push(`[실행 위험 — 현장 인력 이직 × 결원의 계약 위반 연쇄]
정기 도급은 '약속한 인원이 약속한 시간에 현장에 있는 것' 자체가 납품물입니다.
조직·인력(${fmt(hr)}) 취약은 단순 채용 문제로 끝나지 않습니다:
→ 이직 → 결원 → 현장 공백 → 발주처 민원 → 계약 해지 또는 갱신 실패
→ 급하게 메우려 단가 높은 인력을 투입하면 그 계약의 마진이 사라집니다.
대기 인력 확보와 이직률 관리는 인사 과제가 아니라 계약 유지 과제입니다.`);
      }
      if (isLow(bmS) && isLow(dif)) {
        risks.push(`[경쟁 위기 — 계약처 편중 × 가격 외 차별화 부재]
고객·매출(${fmt(bmS)})과 차별화(${fmt(dif)})가 동시에 취약하면 입찰이 가격 경쟁으로만 흐릅니다.
→ 대형 업체와 단가로 맞붙으면 규모의 경제에서 이길 수 없습니다.
→ 한 발주처 비중이 크면 단가 인하 요구를 거절할 수 없습니다.
관리 실적·보유 자격·품질 편차 통제 실적을 근거로 제시하는 것이 유일한 가격 외 경쟁 수단입니다.`);
      }
    }
...
검색: "${d.industryKey || d.industry || ''} 소상공인 정부지원사업 2025"`;
  ...
  function buildPrompt2(d, r1) {
    if (d.bizScale === 'micro') return _buildPrompt2Micro(d, r1);
...
- 기관에 [⚠ 재확인 필요] 경고가 붙어 있으면, 그 기관에 대한 실행 계획을 세우기 전에
  해당 확인 과제를 priority의 1번으로 배치하라. 확인이 끝나지 않은 상태에서
  그 기관 신청을 전제한 계획을 세우지 마라.
